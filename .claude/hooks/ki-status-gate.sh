...
while IFS= read -r line; do
  case "$line" in
    ...
    # ★★★2026-09-04 訂正:切分單位原本是 `### `,而【這份檔的條目是 `##`】⇒ 兩個問題同時存在:
    #   ①`###` 子段(訂正段/證據段)被當成新條目 ⇒ 假陽性 16 筆
    #   ②★★`##` 新條目【根本沒被獨立檢查】—— 它的文字被併進前一個 `###` 段的 body
    #      ⇒ 前面那段若有狀態欄,新條目就靜默過關 = ★★★閘檔頭自己警告的「母體縮小⇒閘變綠」
    #   ⇒ 改成以 `## ` 切,`###` 子段【併入所屬條目的 body】(狀態欄寫在子段裡也算)
    *) sect="$sect
$line" ;;
  esac
done < <(tr -d '\r' < "$KI")
