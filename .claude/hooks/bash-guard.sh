...
if printf '%s' "$_cmd" | grep -qE 'git[[:space:]]+(commit|add|merge|rebase|mv)'; then
  ...
  if [ -f "$_lk" ]; then
   ...
   ★★★④【刪之前先取證】：cat .git/COMMIT_EDITMSG —— 那裡躺著【那次夭折的 commit 訊息全文】，
        ...
        ★★★而它的【限】（同日第一次實用就浮現）：EDITMSG ＝ 那個 session
        【最近一次 commit】的訊息 —— 而那次 commit 可能【成功了】
        （血證：15:29:55 的 EDITMSG 對應的 commit ac0294cf7 好端端在 log 裡，
         死掉的是它【之後】的某個 git 呼叫）
        ⇒ ★它給的是【嫌疑人】不是【現行犯】：指認到 session，不指認到那一次操作。
