...
if [ -f "$LIST_F" ]; then
  ...
  printf '%s' "$OUT" | grep -aE "\[FAIL\]|Assertion failed" | sed 's/^ERROR: *//; s/^ *//'     | LC_ALL=C sort | LC_ALL=C uniq -c     | awk '{c=$1; $1=""; gsub(/[0-9]+/,"N"); gsub(/task=[^ |]+/,"task=T"); sub(/^ /,""); print c" "$0}' > "$_TMP_HL_NOW"
