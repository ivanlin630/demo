...
#   ★★做法：【分段前景跑】——進度可見、孤兒較少、被打斷只損失一段。
#   ★★★而這一條是【止血】，不是修法（2026-09-08 晚訂正）：
#     真正卡住的地方是 `bed-triage-sweep.sh` 的 `o="$( ... )"` 【讀端】——
#     現場：`$( )` 那層 bash ★沒有任何子進程（powershell/godot 都已結束）而讀端回不來
#     ⇒ 管道沒有 EOF，而 EOF 沒來只有一個原因：【還有別的東西握著寫端】。
#     ★★外層 timeout 包的是 powershell，而它已經結束 ⇒ 對這個狀態沒有作用。
#     ★★★修法（systems 裁）：導到檔案再讀，把「等 EOF」這件事整個拿掉。
#     ⇒ 在那之前，【分段前景跑不會讓卡住的那一次跑完】—— chunk1 正常是運氣不是機制。
