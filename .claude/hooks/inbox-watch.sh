...
# ── ★★★信箱歸檔觸發（systems 2026-09-23 接線；★這是「①誰觸發」的【第三個】答案）─────
#   前兩個答案都錯過一次，而錯法相同：**選了一個不會按需要的頻率發生的事件**。
#     v1 答「人」   ⇒ 人沒跑 ⇒ 從 2026-08-27 建好起一次都沒被觸發過（長到 1997 封）
#     v2 答「SessionStart」⇒ ★六個角色的 session 是【持久】的，幾乎不重啟
#        ⇒ 實測 2026-09-23：上次歸檔後 20 小時內熱目錄 187 → 635，而 merge gate 上限是 600
#        ⇒ ★★觸發點的頻率【低於】信箱的成長率 ⇒ 它必然會紅，而且沒有人能在 20 小時內清掉
#   ★★★v3 答「watcher 每次 arm」＝ 這裡：持久 session 的 Monitor 每 ≤30 分鐘到期重 arm 一次
#     ⇒ 這是本專案裡【唯一】以分鐘為級距、而且每個角色都會發生的事件。
#   ★本體自己還有 THRESHOLD/MIN_GAP_H 兩道節流（六個角色不會互相踩 index.lock），
#     ★★所以這裡無條件呼叫是安全的：它絕大多數時候只是數個數就 exit 0（幾十毫秒）。
#   ★★★兩條紀律照舊：只警告絕不阻擋、fail-open —— 所以整行吞掉錯誤、永不影響 arm。
bash .claude/hooks/mailbox-archive-trigger.sh 2>/dev/null || true
