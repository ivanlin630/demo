...
#     只因【據點消失】而失效。
# ★★★【把一筆寫了一半的紀錄補完】（systems 裁 2026-09-16）：
#   ★我們已經存了 `task_reason`（**誰**設的），★★卻**沒有存【當時它值多少】**。
#   ⇒ 於是同層換手要判「該不該換」時，**現任那一邊沒有數字** ——
#     ★★★而唯一的替代方案是在 deny 那一刻重算，**那會呼 `gather` ⇒ 觀測改變被觀測物**
#     （本專案第 4 次同族）⇒ **所以存，不重算。**
#   ★`-1.0` ＝【不知道】，**不是 0**：0 是一個合法的 util，而「沒人告訴我」不是。
var task_util: float = -1.0
...
# ★失聯帳本（母→子 派出單位預期聯絡）：每筆 {kind, subject_ref, is_team, dispatched_tick, expected_return_tick, last_known_pos, resolved}。
...
# ★失聯 defensive 反應真 consumer：警覺期限（>current_tick→餵既有 threat_threshold 降=備戰/防衛更易 fire、非新旋鈕）。
var contact_vigilant_until: int = 0
