...
# ★L3 循環貿易：market visit-util 的 staleness 源——{ team_id → { tile_id → last_firsthand_read_tick } }。
# read_market_board 到場 firsthand 讀時 stamp current_tick。未讀過的 known 市集=缺條目→staleness MAX(探索未知)。
# ★零 god-view：這是「我多久沒親眼看該市集」的自我記憶、非市集真實狀態。
var team_market_last_read: Dictionary = {}
