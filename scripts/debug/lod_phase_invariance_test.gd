...
func _run() -> void:
	...
	# ★★★【免疫狀態註記】（systems 立的不變量 2026-09-06）：
	#   下面 C/D/E/F 四條量的是【薪資】，而薪資已被⑦遷到 `CadenceStagger`
	#   ⇒ ★它對【距離分班】免疫 ⇒ ★★把⑧撤掉，這四條【照樣全綠】（實測：撤刀跑 C/D/E/F 全 PASS）
	#   ⇒ ★★★所以【這四條的綠燈對「分班有沒有回來」不含資訊】——
	#      能偵測分班回歸的是上面那三個【沒有 stagger 保護】的系統（collect/vision/fatigue）。
	#   ★不要因為「薪資也綠了」就以為多了一層保障：★★一個被前一張票修好的系統，
	#     正是最差的回歸偵測器 —— 它已經免疫，所以它不會紅。
