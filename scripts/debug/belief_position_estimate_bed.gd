...
#   ★★★【重錨 2026-09-22】上面兩個 fp 屬於【世代 5】。
#     世代 6（觀測雜訊換血統：`observed_speed` 的 randf() ⇒ 決定性雜湊，
#     分支 `feat/deterministic-observation-noise`，邊界 commit 由 merge 時填）之後，
#     ★這兩個值【不可再引用】—— 它們不是錯的，是【另一個世代的】。
#   ★保留不刪的理由：它們是當時那一票的交件證據；刪掉會讓「這裡曾經驗過什麼」消失。
#   ★★而它們【不是活斷言】（只是註解）⇒ 這次重錨不會讓任何閘變紅，
#     ⇒ 所以【也沒有任何機械的東西會提醒下一個人它過期了】—— 這行字就是那個提醒。
