...
# starvation_lockpoint_trace_bed：坐實死隊的確切鎖點
# 直接讀 team state（非經 SpecimenTracer）：瀕死隊(food_days<閾值)每次取樣記錄
# current_task/combat_target/current_option/food_days，抓死亡當下前一筆快照 = 死前鎖點。
...
func _run() -> void:
	...
	for tid in vanished:
		...
		for snap in history[tid]:
			print("    tick=%d task=%s prio=%d reason=%s combat_target=%d option=%s food_days=%.2f pop=%d famine_days=%.1f committed=%s cooldown=%s survival_dispatch_would_succeed=%s" % [
				...
				str(snap["would_survival_dispatch_succeed"])])
