...
func _stakes_count(goals: Array) -> int:
	var n: int = 0
	for g in ["攻擊", "外交", "掠奪", "徵收", "立國"]:
		if g in goals: n += 1
	return n

func _run() -> void:
	print("=== commander directive measure: 統領同發幾個令 ===")
	var personas := [
		{"name": "好戰霸主", "v": {"野心": 0.9, "好戰": 0.9, "義氣": 0.1, "貪婪": 0.5, "求生欲": 0.4}},
		{"name": "貪婪商霸", "v": {"野心": 0.7, "好戰": 0.3, "義氣": 0.2, "貪婪": 0.9, "求生欲": 0.4}},
		{"name": "溫和守成", "v": {"野心": 0.3, "好戰": 0.1, "義氣": 0.7, "貪婪": 0.3, "求生欲": 0.5}},
		{"name": "野心謀略", "v": {"野心": 0.9, "好戰": 0.5, "義氣": 0.3, "貪婪": 0.6, "求生欲": 0.4}},
		{"name": "均衡", "v": {"野心": 0.5, "好戰": 0.5, "義氣": 0.5, "貪婪": 0.5, "求生欲": 0.5}},
	]
	var max_n: int = 0
	for p in personas:
		var goals: Array = _measure_leader(p["name"], p["v"])
		var n: int = _stakes_count(goals)
		max_n = maxi(max_n, n)
		print("  %-10s → f.goals=%s  同發 stakes 數=%d %s" % [
			p["name"], str(goals), n, "⚠矛盾雙令+" if n >= 2 else ""])
	print("\n結論：統領 _update_goals 多閾值並行 → 同發 stakes 最多 %d 個（≥2=矛盾令，需統一成單一姿態 argmax）" % max_n)
