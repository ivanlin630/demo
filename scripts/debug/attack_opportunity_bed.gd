...
func _run() -> void:
	...
	if frows.is_empty():
		...
		# ★★★【這一段的舊說明已經作廃】（2026-09-16，攻擊幣別 final 刀二）：
		#   舊：`loot = clamp(est / ATTACK_LOOT_REF(3.0), 0, 1)` —— ★而那個常數**已刪除**：
		#   它是校準在【舊的 0..3 桶號尺度】上的，而單位對齊後 `est` 是 **coin 當量（破百）**
		#   ⇒ ★★舊式子會整支飽和成 1.0（這就是 72% 撞頂的現場）。
		#   ★★★而我不把這行改成新公式就算了：**本床這一段的判法本來就建在舊尺上**，
		#     要用新尺重讀得重新想一次 ⇒ ★**這裡只誠實標出來，不造一個看起來還能用的說明**。
		print("   ★★★【舊尺說明已作廃】：`loot` 不再是 `clamp(est/3.0)`，而是單調壓縮（相對於自家 reference_wealth）")
		print("      ⇒ ★本段的中位判法建在舊尺上，**新尺下要重新想一次** —— 在那之前不要用這一行下結論。")
