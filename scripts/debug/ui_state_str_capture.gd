...
func _seed_selftest() -> bool:
	...
	# ★★★陽性對照（已跑，2026-09-23）：在第二趟把 `_page_idx` 改成 1 ⇒ 這一格【立刻紅】
	#   ⇒ ★比較器確實會開口；★★而選這個注射是因為它【不需要推進世界】——
	#     原本那個「多推 1440 tick」的注射被一道【推進上限】擋住（世界卡在 120，
	#     2000 frame 零進展），具名候選＝ sim_bridge.gd:56-57「重要事件 → 停止推進」。
	#   ⇒ ★★★教訓：注射若依賴一個【我還沒理解的機制】，它點不著時我分不出
	#     是【守衛不靈】還是【注射沒生效】——而那兩件事的處置相反。
	return true
