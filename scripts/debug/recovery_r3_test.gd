...
# 建 lord(faction leader)+爛地 holding 村(mountain)+更優已知領土(plains own outpost)。
func _mk_lord_relocate() -> Array:
	var state := WorldState.new(); state.world = WorldData.new(); state.world.current_tick = 100000
	...
	var ll := PersonData.new(); ll.id = 11; ll.values = {"義氣": 0.6}; state.persons[11] = ll; lord.leader_id = 11
	...
	var v := TeamData.new(); v.team_id = 9; v.faction_id = 0; v.tile_pos = Vector2i(5,5)
	AnonCohort.add(v.anon_cohorts, "平民", "healthy", 5)
	var vl := PersonData.new(); vl.id = 91; vl.values = {"義氣": 0.7, "野心": 0.2}; state.persons[91] = vl; v.leader_id = 91
