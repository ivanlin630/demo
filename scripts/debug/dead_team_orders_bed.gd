...
# ★★★【只有床走得到的事實】（systems 要求寫進檔頭，2026-09-02）：
#   ★「清死隊自己那格」【不夠】—— `_sync_board` 的 relay 會把同一張單散成 relayed 副本，
#     掛在【別的活市集】上。★★而兩份票（systems 的與我的）裡都沒有這一條：
#     我們都預設「死隊的 board entry」只在一個地方。
#   ⇒ ★★★抓到它的是下面那條【互斥斷言】（A 自己的單不得被誤清），不是想出來的。
#   ⇒ ★下一個改這條路的人會先想到「清死隊的單」，★★而不會想到「單有副本」——所以寫在這裡。
