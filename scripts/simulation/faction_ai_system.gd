...
func _decide_unified(state: WorldState, team: TeamData, src: String = "unknown") -> void:
	...
	# ★★★取樣條件放寬（systems 2026-09-12 裁：那四個條件裡「剛好輪到決策」證明**不需要**）：
	#   ★上一版要求【迎戰姿態 ＋ 目標是它 ＋ 同格】三者同時在**決策當下**成立 ⇒ 母體 ＝ 1
	#   ⇒ ★★而床量到「碰到面那一 tick 有跑決策 ＝ 27／27」⇒ **那個 1 是【儀器的 1】**：
	#     真實順序多半是**決策先發生、同格後發生**（人是走過去之前就想好的）。
	#   ⇒ ★★★新條件 ＝ **決策當下，同一格上有【別家】的隊**（不限它是不是我的迎戰目標）
	#     ★而「是不是我的迎戰目標」改成**記在欄位裡**，不再當取樣門檻。
	if Probe.enabled:
		var _fo: TeamData = null
		var _fo_is_my_target: bool = false
		for _oid in state.teams_on_tile(team.tile_pos):
			if _oid == team.team_id: continue
			var _cand: TeamData = state.teams.get(_oid)
			if _cand == null or _cand.tile_pos != team.tile_pos: continue
			if _cand.parent_team_id == team.team_id or team.parent_team_id == _cand.team_id: continue   # 自家子隊不算對峙
			if _cand.faction_id != -1 and _cand.faction_id == team.faction_id: continue                 # 同派系不算對峙
			_fo = _cand
			_fo_is_my_target = (team.prosperity_target_id == _oid)
			if _fo_is_my_target: break   # ★★優先記【自己的迎戰目標】，否則記同格的任一別家隊
		if _fo != null:
			...
			Probe.bump_sample("faceoff", {"tick": state.world.current_tick, "team": team.team_id,
				...
				"is_my_defend_target": _fo_is_my_target, "my_task": team.current_task,
	...
	for e in ranked:
		...
		# ★★★A 案戳記（2026-09-01）：tracer 以前自己呼 to_task 算「可不可派」，
		#   而那條路會寫 state（gather）⇒ 觀測改世界。現在【由這裡順手回報】：
		#   ★td 已經算好 ⇒ 零重算、零額外呼叫；★★specimen-gated ⇒ 非 specimen 隊 no-op。
		SpecimenTracer.mark_dispatch_verdict(state, team, opt,
			(td.get("target", Vector2i(-1, -1)) == Vector2i(-1, -1))
				and td.get("task", TeamData.TASK_IDLE) != TeamData.TASK_FLEE)
...
func _decide_subteam(state: WorldState, sub: TeamData, merge_queue: Array) -> void:
	...
	for e in ranked:
		...
		#   而那條路會寫 state（gather）⇒ 觀測改世界。現在【由這裡順手回報】：
		#   ★td 已經算好 ⇒ 零重算、零額外呼叫；★★specimen-gated ⇒ 非 specimen 隊 no-op。
		SpecimenTracer.mark_dispatch_verdict(state, sub, opt,
			(td.get("target", Vector2i(-1, -1)) == Vector2i(-1, -1))
				and td.get("task", TeamData.TASK_IDLE) != TeamData.TASK_FLEE)
		if td.get("delegate", false):
...
# tick 末單點：路由遺財（守恆）+ erase。★「中途 erase 不安全」那句已被實測訂正（見上）
...
func _evaluate_independent_infrastructure(state: WorldState, team: TeamData) -> void:
	# ★★★infra path 的歸宿（systems 派 2026-08-26）：`_resolve_build_facility` 的最大回空類是
	#   `defer_infra`（「交給 infra path 就地建」），而 `build_ok = 0` ⇒ ★它交過去的路沒有在建。
	#   ★分母＝進入本函式的次數；★★六個 guard + `_pick_facility` 回空 + 兩條施工路 = 全部歸宿，
	#     逐日加總必須 == 分母。
...
# home-seeking 靶：撿現成優先（settlement S1b）→ fallback 鄰格新建空農地。無 → (-1,-1)。
# ★S1b 目標池擴充：先掃 belief-known 無主(owner==-1)既有 outpost（level>0=已建 infra，撿現成修繕
#   << 新建工期→優先）；再 fallback 原鄰7格空農地新建。到達後 establish_crude_camp（空地）或既有
...
func _find_unowned_farmable_tile(state: WorldState, team: TeamData) -> Vector2i:
	# ① 撿現成：belief-known 無主既有 outpost（最近者）。known 為 discovered set，讀 live owner=既有慣例。
