...
func _resolve_aid_request(state: WorldState, beggar_id: int, target_id: int) -> Dictionary:
	...
	if give_score < 0.3 and give <= 0.0:
		...
		_npc_ai.write_memory(beggar_leader, "rejected_aid", target_id,
			state.world.current_tick, 0.5)
		# ★★★TTL＝`FailureMemory.AID_REFUSED_TTL_TICKS`（＝3 天，值與拆之前相同）。
		#   ★原本借 `BeliefSystem.BELIEF_STALE_TICKS`，而那條線正在被物理化成 per-observer 的函式
		#   ⇒ **沒有「一個數字」可借了**；不拆的話，動位置線會【靜默改掉】這裡的記憶長度。
		...
		FailureMemory.record(state, beggar, "乞食", str(target_id),
			FailureMemory.AID_REFUSED_TTL_TICKS, "aid_refused")
...
func _absorber_accepts(state: WorldState, absorber_id: int, joiner_id: int) -> bool:
	...
	if Probe.enabled:   # ★measurer L3 tap(2026-08-21 C-3 → 2026-08-25 重量令)：估算器輸入(joiner 眼中 host rep) vs 真決定因子(feed_ok) vs 結果，逐筆
