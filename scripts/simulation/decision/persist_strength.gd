...
static func _value(state: WorldState, team: TeamData) -> float:
	...
	# ★沉沒成本（往回看，已投入越多越黏）proxy（Slice 1 決策層自算：committed 時間佔比；Slice 2 補真 construction-tick progress）。
	...
	var progress: float = clampf(float(elapsed) / maxf(horizon, 1.0), 0.0, 1.0)
