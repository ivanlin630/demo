...
const NO_FAILURE_FEEDBACK: Dictionary = {
	...
	# ★★★偵查（2026-09-16 新 option，攻擊幣別＋偵查進秤票）：★**三條裡第 ② 條不成立**——
	#   ①有執行步驟會失敗：**成立**（走到 belief_pos 卻撲空／目標已移走或已滅）
	#   ②失敗當下偵測得到：★**不成立** —— 偵查的「成功」是【看到】，而看不到與【還沒到】
	#     在同一個 tick 上分不出來；★★現在沒有任何東西在記「這一趟有沒有看到東西」。
	#   ③重試會對同一目標重複：**成立**（belief 沒更新 ⇒ 下一 cadence 還是它最肥）
	#   ⇒ ★★★所以它【不是】「不需要」，是**②那一條先要有一個到達判定** ——
	#     而那是偵查自己的下一張票，不是本票。
	"偵查": "TODO:%s ── ★②不成立：缺【到達且看到了嗎】的判定；①③成立（撲空／同目標重撞）" % TODO_TICKET,
	...
	# ★★★④b 登記動詞兩個新 option（2026-09-11）—— ★兩個都是【判準不成立】，而理由不同：
	#   ★★而兩條都寫成可被證偽的形狀（補上那個記號之後，這一格就要改判）。
	"收留": "①不成立: 沒有會失敗的執行步驟——決策選中就在同一行登記完成（faction_ai_system:3375 _seek.work_outpost 直接賦值）；對象走了／已登記走 continue 試次佳，不是做不成",
	"求居": "②不成立: 沒有【被拒絕】的事件——領主不選收留不留任何記號，抄到門口沒被登記也不留記號 ⇒ 偵測不到。★補上 miss 記號時這格要改判（同「領取」）",
