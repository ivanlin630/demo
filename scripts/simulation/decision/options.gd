...
static var REGISTRY: Dictionary = {
	...
	"掠奪": {
		...
		# ★★★【這一格改完，世界【大幅】變了 —— 而我先前說它「結構上必為 null」是錯的】：
		#   ★我當時的推論：世界裡的掠奪**現在**全從 `rank_survival` 出來，而那條路不讀 `consistency_coeff`
		#     ⇒ 所以改這一格碰不到它。★★**①②為真，推論為假** ——
		#     **因為改動可以讓掠奪【從別的地方出來】，而那是我沒問的問題。**
		#   ★★★實測（10 天窗／seed 1337／同儀器；before `192c10757` → after `babbaed5c`）：
		#     掠奪真的被派出去 **5 → 28**；**「有 coeff 也贏」0 → 3**（＝掠奪**第一次**在統一 rank 上贏過 argmax）；
		#     而掠奪的 task print **整條換掉**：before 全是 `[Survival]`、**after 21 筆全是 `[SoloAI]`**。
		#   ★**誠實限**：兩輪世界整個分岔（非 perf 差異 11197 行）⇒ **28 那個數不可直接歸因**；
		#     **可引用的是 `won_anyway` 0 → 3**（同一條路、同一個定義）。
		#   ★★**而「這一格有沒有副作用」仍然【沒有被證明沒有】** —— 單 seed 單窗，方向可看、量級不可引用。
		# ★★★【交件標籤·定稿】（systems 2026-09-16，接 blueprint 裁 A：語意優先）：
		#   **「對照存在，而【全世界平均】不是它的判準 —— 逐隊分化才是。」**
		#   ★先前那條驗收條件（「改完 coeff 分布不得比改之前低」）**已作廢**：
		#     ★★它優化錯目標 —— `coeff` 是**逐隊**算的，而**餓的隊與不餓的隊要相反的東西**，
		#       ★★★**全世界平均把它們抵銷掉** ⇒ 「不餓的隊 coeff 掉下來」**正是我們要的**。
		#   ★而這一列本身的代價是量過的（實測、可引用）：
		#     `alignment` 淨降 ≈ **−0.096**（生存 +0.30×平均急迫 0.060 vs 尊重 −0.25×平均急迫 0.454）
		#     ⇒ coeff 質量從 0.6–0.9 桶掉到 0.4–0.6 桶。★★**那是【已知且被接受】的代價，不是未發現的副作用。**
		#   ★★★**而這張表是行和 1.0 的歸一化表 ⇒ 沒有「只改一格」這回事**：
		#     **提高一格必然壓低另一格，而被壓的那一格可能才是有人在用的。**（本次壓的是尊重，
		#     而尊重是急迫主層第二大 ⇒ **那正是代價的來源**。）
