...
static func _emit_teams(state: WorldState, buf: PackedStringArray) -> void:
	...
	for tid in ids:
		...
		buf.append("T|%d|pop=%d|minor=%d|prisoner=%d|task=%s|prio=%d|reason=%s|prev=%s|pos=%s|mt=%s|fac=%d|parent=%d|combat=%d|social=%d|opt=%s|unrest=%d|famine=%s|rung=%d|phase=%s|res=%s|tags=%s|anon=%s|subs=%s|rep=%s|intent=%s|goals=%s" % [
			...
			_vec(t.tile_pos), _vec(t.move_target), t.faction_id, t.parent_team_id, t.combat_target, t.social_target,
...
static func _emit_tiles(state: WorldState, buf: PackedStringArray) -> void:
	...
	for tid in ids:
		...
		if t.outpost_level <= 0 and t.construction_team_id == -1:
			continue   # 純野格無 outpost/施工 → 不入 fingerprint（decisions 只讀 outpost/施工格；resource_cap 靜態地理非行為結果）
		buf.append("H|%d|owner=%d|lvl=%d|farm=%s|ctid=%d|cleft=%d|store=%s" % [
			tid, t.outpost_owner, t.outpost_level,
