...
func execute(state: WorldState, team: TeamData) -> Array:
	...
	if not state.factions.has(fid):
		state.clear_team_faction(team, WorldState.LEAVE_DEFECT_FACTION_MISSING)   # S11：faction 已不存在防禦路徑，走 chokepoint（語意等同 =-1，無懸空可修）
		return []
	var f = state.factions[fid]
	state.clear_team_faction(team, WorldState.LEAVE_DEFECT_EVENT)   # 脫離 faction（雙向同步）
