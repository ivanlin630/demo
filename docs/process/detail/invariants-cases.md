...
- **★★掃近隊兩-channel（blueprint/用戶定 2026-07-18，遠方危險不得隱形）**：perf 靠「掃近隊」bound 的是**直接感知成本**，**禁把掃近隊當唯一 belief 填充源**。awareness 經 belief，belief **兩源填**：①**直接掃近隊**（近隊進 belief）②**情報網**（message/relay/known_reputations 把**遠方**高危險傳進 belief，獨立於直接掃）。∴ scan-nearby 優化**不得**讓遠方強敵隱形（=戰略盲）；遠隊「從沒掃過」仍須能經情報網進 awareness。scan-nearby spec 前 R① 必坐實既有 message/belief relay 真傳得到遠威脅（率/延遲），別假設。**★★兌現（2026-07-20 god-view Slice B，reviewer R① 載重驗出 relay→discovery 原不存在=前置承諾未履行）**：blueprint 裁 (b) **建 relay-discovery**（`message_system:239` relay claim 提及未識隊→連帶 set `team_discovered`+初始 belief entry，含 distorted）→ **discovery 兩-channel 成真**（①直接視野 vision ②relay 聽說）。**範圍收窄**：只求「聽說→discover」最小行為，**率/延遲/失真的完整情報網模型=defer（資訊操控維度另軌）**。∴ awareness 遠識現靠情報網撐（非只 proximity）；決策 gate 的 team_discovered 經 relay 也長。
...
### ★★★★第五型：**排程型（精確時點）** —— 2026-09-05 補（薪資相位實戰產出）

| 型別 | 例 | 降頻後果 | 處置 |
|---|---|---|---|
| **排程型（精確時點）** | `if tick % SALARY_INTERVAL == 0`（發薪、定期徵收） | ★**不是率失真，是【事件在相位縫裡整個不發生】** | ★★**禁用精確 modulo 當 pass 內的閘**；改與 `last_eval_tick` 比較（`CadenceStagger`）＝**到期後的第一個 pass 就發生** |

★**為什麼前四型接不住它**：前四型共同假設「降頻的後果是**率的失真**」，所以處置都叫**補償**；
★★而排程型的後果是**事件根本沒發生** —— **補償一個從未發生的事件，補償係數乘以零還是零。**
★★★**血證（2026-09-05）**：`salary_system.gd:31` 的 `SALARY_INTERVAL=10080`，far pass 閘 `% FAR_ZONE_INTERVAL(600)`
⇒ `10080k % 600 = 480k % 600` ⇒ **payday k=1..4 對 far 隊全部落空**（k=5 才第一次對上）
⇒ **近隊月月領薪、遠隊四個月領一次；而無玩家的世界裡「遠隊」＝全部 ⇒ 一次都沒發過。**
★**而這是【距離依賴的經濟扭曲】，直接違反世界存在性法**（計算跟隨事件密度、不跟隨觀察者）。

★**驗收的操作定義**：**far 與 near 的【事件發生次數】相等** —— ★★**「有 fire」不算過**，
★★★**「變多了」也不算過**（撤掉修法要**變回 0** 而不是**變小**，否則量的不是這件事）。

★**同族判準（拿去掃新 code）**：**裸 `current_tick % INTERVAL`** 長在 **shape:"teams" 且 LOD_BOTH** 的 step 裡，
且 `INTERVAL` **不是 `FAR_ZONE_INTERVAL` 的倍數**。★★**而「剛好是倍數所以安全」是巧合不是設計** ——
**改動 `FAR_ZONE_INTERVAL` 會讓所有這種巧合同時失效，而且是靜默的。**
...
# ★近期五條的【血證／為什麼】（2026-09-02 從 `invariants.md` 搬來；規則本體留在主檔那張表）
...
## ★第三形狀：**驗了【事實】沒驗【詮釋】，而詮釋是從上游照收的**（2026-09-02，systems 自撤）

同一天前兩個形狀是：**①我自己的主張送去被打**（負斷言翻車，流程有效）、**②用一個樣本定規則邊界**（細則 1a 漏了「閘前」）。
★**第三個不一樣**：
```
reviewer 報：「_village_est 有 inline gate-ok 卻不在 constitution_baseline_v2.txt
             ⇒ 官方清單連 legit 那一邊都漏」
★我驗了【事實】：inline 確實有(:2187)、baseline 確實沒有(grep -c = 0)  ✅ 兩個都對
★★我沒驗【詮釋】：「兩者本來就該一致嗎」
★★★而 constitution_gate.gd:6,8 寫著它們是【不同機制】：
     inline gate-ok ＝「這行根本不算一個站點」(不入 current)
     baseline       ＝「這個【被偵測到的】站點被凍結承認」
   ⇒ 被 inline 豁免的東西【本來就不該在 baseline 裡】—— 那是【設計正確】
```
★**放大它的是光環效應**：reviewer 在同一封信裡剛剛翻掉我的負斷言（而且翻對了）
⇒ ★★**「它剛對過，所以更信」** —— 而那正是最該加驗的時候。
★★★**代價**：我把它寫進 `known_issues`、寫進給 blueprint 的信、還為它寫了一支閘（`gateok-reconcile`）到一半。
**閘已刪** —— **錯前提上的守衛比沒有守衛更貴**：它會每天綠著或紅著，而它量的東西根本不是一件事。

⇒ **規則**：**「file:line 坐實事實 ≠ 坐實詮釋」這條要用在【上游給的詮釋】上，不只用在自己的。**
★**操作化**：拿到「A 與 B 對不上 ⇒ 有 bug」這種形狀的報告，**先讀 A 與 B 各自的契約**（誰定義的、規定它們什麼關係），
**再看差集** —— ★★**差集永遠算得出來，而「它們該不該一樣」不在差集裡。**

## ★★★對句：**「沒發生」與「沒觀測到」長得一樣，而它們不是一件事**（2026-09-02，同日雙向各犯一次）

★**兩次的方向剛好相反，所以兩邊都要防**：
```
★systems 方向：把【沒觀測到】讀成【沒發生】
   godview-1a 驗收指定「新舊候選集合差集」當證據 ⇒ 差集 = 0 ⇒ 差點結論「沒效果」
   ★★而真正的關閉在下一格：occupy.scan_kill_tile_unknown = 161
   ⇒ 立成「指標 = 0 三讀法」（沒效果／效果在下一格／母體塌陷）→ `03b_measurer.md`

★★code 方向：把【觀測到沒事】寫成【沒觀測到】
   observed_activity 在【vision 記錄親見的當下】被呼叫 —— ★那一刻你正看著它
   而它對「不打、不建、沒移動、不在自家地」回 ACT_UNKNOWN
   ⇒ ★★★類別錯誤：分類表沒有一格給「觀察到、靜止、無可辨識活動」
   ⇒ 補 ACT_IDLE；「unknown 一律不通過」★放回去沒放寬，而功能自然活
```
★★★**判別法**：問「**這個 0／這個 unknown 是【誰】產生的**」——
**觀察者產生的 ⇒ 它一定看到了什麼，不明是分類表的問題；讀取者產生的 ⇒ 才可能是真的未知。**

## ★★★★同一個 argmax 裡不得混【flat 死常數】與【人格導出值】（systems 立 2026-09-05，「守成」實戰產出）
```
★血證:ambition_ladder.gd:36-41
...
⇒ 人格值多落在 0.4~0.6 ⇒ 致富≈0.35、防衛≈0.30 【都大於 0.25】
⇒ ★★實測:12335 次意圖選擇,【守成 = 0 次】(建國10／征服245／致富7153／防衛4927)
```
★**結構性事實（不是調參問題）**：**一個常數與一群分佈競爭 argmax，結果只有兩種** ——
**它落在分佈上緣（永遠贏）或下緣（永遠輸）**；★★**而「有時候贏」需要它落在分佈中間，
那要嘛靠運氣，要嘛表示它其實也該是導出的。**
★★★**最難看的一格**：那個常數的註解寫 **`# default base`／「default：無 stakes 令」**
⇒ **一個【自稱 default 的分支，實際上永遠不會被選到】** —— **而讀 code 的人會以為那是常見情況。**

★**規則**：
```
①argmax／評分表裡的【每一個 option】要嘛全部是導出值,要嘛全部是常數 —— ★不要混
②★★若真的需要一個「基準線」,它要【由同一組人格導出】(例:守成 = (1-野心)*a + 慎重*b)
   ⇒ ★★★而不是挑一個數字放進去
③★驗收:任何新增/修改評分表的票,要印【該表各 option 的實際勝出次數分布】
   —— 而【0 次】要當成【結構性問題】處理,不是「這輪剛好沒有」
```
★**而這條與「照妖鏡：死常數人格化」是同一族**（統一路線圖序 5），**本條是它在 argmax 場景的具體形式**。

## ★★★★回歸驗收不能只看【已經被修好的那個】—— 修好的東西對「病回來了」**免疫**（systems 立 2026-09-06）
```
★血證(⑧ 驗收③ 的撤刀對照):
   撤掉 ⑧ ⇒ collect／vision／fatigue 相對差 0.0% → ★82.7%(遠半 72.0 vs 近半 416.6)⇒ 鑑別力坐實
   ★★而【薪資的四條判準在撤刀後照樣全綠】
      —— 因為 ⑦ 已經把薪資遷到 CadenceStagger,它【對距離分班免疫】
⇒ ★★★所以【只看薪資的話,這張床會說「沒問題」,而分班已經回來了】
```
★**一般化**：**一個被前一張票修好的系統，正是【最差的回歸偵測器】** ——
它**已經免疫**，所以**它的綠燈不含資訊**。
★★**規則**：回歸驗收的覆蓋要**跨【不同免疫狀態】的系統**，而不是「多挑幾個」湊數：
```
①至少一個【沒有被任何相關修法碰過】的系統 —— 它才是真的偵測器
②★而【被修好的那個】仍然要留在表上:它證明【修法沒有退化】
③★★兩者的角色不同,卷面要分開講 —— 混在一起會讓人以為「全綠 = 都在偵測」
```
★★★**而這條是【事後】才看出來的**：當初要求「至少三個系統」的理由是**覆蓋率**，
**而它真正的價值是【免疫狀態的多樣性】** —— **對的要求，理由當時說錯了。**

## ★★★★兩種記帳方式**不能同時管同一張單**（systems 立 2026-09-06，B-v0 spec 階段 R² 抓到）
```
★兩種形狀:
   ①【從副作用反推】:觀察狀態的 delta,倒推「發生了多少」
     (例:order_system.gd:371-400 `settle_orders` —— pool[res] = now − before,sell 要 avail<0)
   ②【事件權威】:成交事件自己說發生了多少(例:市場撮合)
★★而【混用】的兩種失敗是【對稱】的:
   ①先扣後撮合 ⇒ 扣的那一刻被反推成【整單成交】(而一件都還沒賣)
   ②先撮合後扣 ⇒ 後續真實成交【完全偵測不到】
⇒ ★★★兩種都【不會報錯】,而且【方向相反】—— 所以「看起來合理」救不了它
```
★**規則**：同一張單**只能有一個記帳權威**；若引入事件權威（撮合／escrow），
**必須把該單【明確排除】在 delta 反推之外** —— ★★**而那要寫進 spec，不是「應該沒事」。**
★★★**識別法**：問「**這張單的數字，有沒有第二個地方也會動它？**」——
**有 ⇒ 兩者必須有一個是【唯讀的】。**

## ★★★★等價性測試**需要一個「不等價」對照**（implementer 揭 2026-09-06，②modulo 遷移產出）
```
★形狀:證明「新舊行為相同」的測試,若【只有相同這一半】,
   ⇒ ★★它證明的是【我沒改東西】,而不是【我的改動是等價的】
   —— 而【什麼都沒改】也會讓它全綠
⇒ ★★★所以要配一個【故意讓前提不成立】的對照:
   血證:把外層 cadence 改成【不整除】(500 vs 1440)⇒ 舊制 fire 1 次／新制 fire 10 次
   ⇒ ★那條紅燈證明【這個測試看得見差異】—— 而綠燈才有意義
```
★**一般化**：**任何「A 與 B 相同」的判準，都要有一個【A 與 B 應該不同】的場景陪它** ——
★★否則你無法分辨「**相同**」與「**這個測試根本量不出差異**」。

## ★★★★算術等價 ≠ 實跑等價（同日、同一票）
```
★「fire 的 tick 集合完全相同」是【算術】
⇒ ★★而它對【我漏看了某個呼叫端】完全不敏感:
   若還有別的路徑會呼到那些函式,集合算對了、【世界照樣可能不同】
⇒ ★★★所以要再加一層【fp 實跑 A/B】(同一支 seeded 床,在兩個 branch 各跑滿窗,比 fp)
★而兩層擋的是不同的東西:
   算術層擋【邏輯寫錯】｜實跑層擋【我對「有哪些呼叫端」的認知是錯的】
⇒ ★★而後者正是今天反覆咬人的那一族(非窮盡的搜尋),所以它【不能只靠算術】
```

### ★★★★而它【當天就真的抓到一次】—— 而更精確的形狀是【起點】不是【有哪些】（同日稍後）
```
★等價性測【印了綠】,而 fp 實跑 A/B 【FAIL】:世界從【第一天】就岔開
★★成因:測試迴圈從 `t = 0` 起跑,而【真正的呼叫端第一次是 t = outer(360)】
   (外層閘是 `tick % 360 == 0`,而 current_tick 在系統跑之前就已遞增)
   ⇒ 舊制在 360 不 fire(360 % 1440 != 0)、新制 fire ⇒ ★★★世界從第一天岔開
⇒ ★而 implementer 自己的定位最精確:【不是我漏看了呼叫端,是我把呼叫端的【起點】記錯了】
```
★**一般化**：**測試對「呼叫端」的模型【本身是一個假設】，而它可以是錯的** ——
⇒ ★★**測試會在一個【不存在的軌跡】上驗證等價，然後印綠**。
⇒ ★★★**而【差一個起點】就足夠** —— 不需要漏掉整個呼叫端。

### ★而它同時證明了「fp 必須不變」這種驗收的第二個價值
```
★本票(②modulo)裁的是【(b) 純到期比較 ⇒ fp 逐位元不變】
⇒ ★★而若當初選了 (a)(加相位偏移 ⇒ fp 本來就會變),
   ★★★這個 bug 會【被吞進「預期內的 fp 變動」裡】—— 完全隱形
⇒ 所以【要求 fp 不變】不只是「不動世界」,它還是【一面照妖鏡】:
   ★任何非預期的行為改動都會撞上它,而【預期會變的驗收沒有這個能力】
```

## ★守衛的輸入母體：**恆空與恆滿是同一個病**（2026-09-06）
```
zero-output-warn  --author 篩不掉任何人(六角色共用 git 身分) ⇒ 母體【恆滿】⇒ 天天喊狼
bash-guard 護欄②  .busy beacon 要人手寫而沒人寫過   ⇒ 母體【恆空】⇒ 一次沒響
⇒ ★一個恆報一個恆不報,而共同形狀是:
   ★★【守衛的判斷依據,來自一個沒被驗證過會不會有內容的來源】
```
★**上線任何守衛前必問**：「這個母體在什麼情況下會是空的／會是全部？」
（＝「指標必須可能失敗」的反面：**必須可能不觸發**。）

### ★★而修「恆空」時最容易做出「恆滿」——符號翻過來的同一個病
```
「起跑寫 beacon、結束刪」⇒ ★而【結束刪】正是【被 kill 時唯一不會執行】的那一步
⇒ 屍體 beacon 永久留著 ⇒ 從【永遠不響】變【永遠亂響】
⇒ ★★正解=【會自己過期的心跳】,不是「用完清理」
   ★★★自己過期 > 清理:清理需要有人活著執行,而失效的場景正是沒人活著
```
★**改契約必須同一顆 commit 改讀取端**：舊契約是「檔內存 deadline epoch」，
只改寫入端 ⇒ 解析失敗成 0 ⇒ **護欄照樣是死的，而卷面上看起來已經修好。**

## ★★預寫的失敗語意：**它對真假紅燈一視同仁地加持說服力**（2026-09-06）
```
腳本印:「上臂拆掉之後世界變了 ⇒ §2 的推導錯了 —— 這才是要停下來的時刻」
★而那個 FAIL 跟 code 無關:patch 根本沒套上(路徑對 MSYS 成立、對 Windows python 不成立)
⇒ ★★正因為那句預寫解讀寫得好,它讓一個【腳本 bug 產生的 FAIL】看起來像【深刻的發現】
```
★**配套兩條，缺一條白做**：
```
①【實驗前提成立】必須在【印出解讀之前】被機械驗證(grep 標記 → ABORT),還原後再驗一次
②★ABORT 訊息【不准跟真紅燈長得像】:真紅燈說「推導錯了」,ABORT 說「這一趟不算數」
   ★★否則只是把一個假紅燈換成另一個假紅燈
```
★★**寫 spec 驗收欄的人曝險最大**：那些「若這格是 0 代表 X」全是**在看到數字之前**寫的
⇒ **每一格驗收都要附「這一格在什麼前提下才算數」，而那個前提要能被機械檢查。**

## ★★★被砍斷的 log 與「跑完但有紅」的 log **文字上不可分**（2026-09-06）
```
merge-gates 註冊表 27 支,而我拿到一份只有 3 支的 log(2 綠 1 紅)
★兩種讀法一樣:(a)跑完了有一支紅  (b)跑到第 3 支被砍
⇒ ★★唯一憑據是【尾巴那行固定會印的總結】(───+「註冊表 N 支｜總時 Ns」)
   ★★★沒有那行 ⇒ 它沒跑完,不管前面印了多少 ✓
```
★**推論**：任何長跑的輸出都該有**一行只有跑完才會出現的收尾**
（measurer 的 `=== DONE ===` 是同一條）——**否則「做完」與「被砍」在卷面上不可分。**

## ★驗「某段是死碼」必須有一個【會動】的對照臂（2026-09-06，⑩ 實證）
```
A：只拆上臂 ⇒ fp 逐位元相同(sha 一字不差) ⇒ 上臂是死碼
B：完整拆   ⇒ fp 變了                      ⇒ 下臂是真的在夾
★只有 A 過【不算數】:「世界沒變」與「patch 根本沒生效」【在數字上不可分】
   —— 而那正是同一個人上一輪真的踩到的那顆(patch 沒套上而實驗照跑)
⇒ ★★真正的判準不是「A 過」,是【A 與 B 一起,而且方向相反】
```

## ★★回歸測試變紅的一種成因：**它一直在保護一個 bug**（2026-09-06）
```
舊測試把 current_tick 釘死在 43200 然後呼叫 200 次
⇒ 舊制 `43200 % 43200 == 0` ⇒ 200 次全 fire ⇒ 測試綠
⇒ ★而它會綠,【只是因為舊機制分不出「同一個 tick 被呼叫 200 次」】
⇒ ★★新制(到期比較)在同一 tick 內冪等 ⇒ 只 fire 一次 ⇒ 測試紅
★★★所以:「舊寫法只是【碰巧】在舊機制下等價」——不是舊測試寫錯,
   是【它的正確性依賴一個缺陷】,而缺陷修好的那天它會變紅、看起來像回歸
```
★**判法**：紅的時候先問「**這條 assert 在舊機制的哪個性質上成立？那個性質還在嗎？**」

## ★★★規則表天生藏得住「這條規則從來沒生效過」（2026-09-06）
```
一條新規則的 regex 少跳脫一個閉括號 ⇒ 無效 regex ⇒ 靜默不匹配任何東西
★而閘不會說「你的規則壞了」,它只是繼續顯示 NEEDS_HUMAN
⇒ ★★而 NEEDS_HUMAN 是這張表的【正常輸出】
⇒ ★★★「錨下錯地方」與「regex 沒編譯過」症狀完全一樣(改三次錨才發現)
```
★**解＝把陽性對照裝進規則表本身**：每條規則自帶【一行必須命中的樣本】，
啟動自檢：編譯失敗 ⇒ 紅；編譯過但樣本不命中 ⇒ 紅。
★★**它在【規則被加壞的那一刻】就叫。**

## ★「跑完了」與「驗過了」是兩件事 —— exit code 只證明前者（2026-09-06 付息）
```
detached 環境裡 bash 找不到 git ⇒ 兩支閘【exit 0 但一個字都沒驗】
⇒ ★若 runner 只看 exit code,它們會印成【綠】
⇒ ★★是 `expect pattern 必須命中` 那一欄把它們攔成 no-verdict
★★★所以每一支閘都要有「它該印出什麼結論」的欄位,而 runner 要看那個,不是看 exit code
```

## ★★★強制「每一格都要有判準」會生出【為了填而填】的判準（implementer 揭 2026-09-06）
```
我的 spec:規則表每條規則自帶【必須命中的樣本】,不命中就紅
★implementer 加的那層:`sample` 留空 ＝【這條沒有自檢】⇒ ★★【不判 FAIL】,
   改成算進「無樣本」那個數字(可見的債)
★★★理由:否則為了過閘,會有人隨便補一個樣本 —— 而【隨便補的樣本比沒有樣本更糟】
```
★**為什麼更糟**：**沒有樣本＝這條沒被驗過，而那是【可見的】；
隨便補的樣本＝它印綠，而它什麼都沒驗** ⇒ ★★**後者把「沒驗」偽裝成「驗過了」。**
> ★★★**通則：要提高覆蓋率，正確做法是【讓沒覆蓋的部分可見】，不是【強制每一格都填】。
> 強制填會把缺口變成假綠，而假綠比缺口難發現。**

★**同一條的反面今天也出現過**：我自己列的驗收 #2b 四條裡有三條與機制無關
（`rate=0` 時恆成立）——**那也是「填滿了格子」而不是「驗到了東西」。**

## ★★★判準要成【對】：讓兩條線互相預測（implementer 設計 2026-09-06，⑩ 5b）
```
前提鏈:`stock >= 0` ⇒ `shortage <= 1.0` ⇒ 上臂永不觸發
量兩條:①資源最小值/負值次數   ②上臂桶命中數
⇒ ★負值非 0 ⇒ 前提破 ⇒ 上臂桶【應該】跟著非 0
⇒ ★★而【只有一條動】⇒ 是【儀器問題】不是【世界問題】
```
★**為什麼這比一條 assert 強**：一條 assert 只能說「這個數字對不對」；
★★**成對的兩條互相預測** ⇒ **一起動＝世界真的變了；只有一條動＝量的人壞了。**
> ★★★**它把「儀器有沒有在說謊」變成一個【可以被同一份資料回答】的問題** ——
> 而其他所有「工具騙人」的處置都得【另外去查工具】。

★**寫驗收時的用法**：不要只問「這一格該是多少」，
**再問一句「如果這一格真的是那樣，【還有哪一格】必須跟著動？」——把那一格也列進去。**

## ★★★預寫解讀的第二個實例：**方向相反，而修法是【對照格】不是註解**（2026-09-06）
```
⑩ A/B:預寫「§2 推導錯了」⇒ ★把【腳本 bug】說成【深刻的發現】(高估發現)
⑨ 床  :預寫「空 ⇒ 儀器沒開」⇒ ★★把【正常的世界事實】說成【儀器故障】(高估故障)
⇒ ★★★兩次都是預寫解讀【在證據到達之前決定了結論】,而方向相反
```
★**打掉它的是對照格**：`coin.flow.n == 0` 而 `bank.writes.n == 658` ⇒ 儀器有開、coin 真的沒動。
★★**所以配套要升級**：**預寫的每一句都要有一個【能把它否證的對照格】** ——
★★★**只把「這個 0 有兩種意思」寫在註解裡不夠：註解不會在跑的時候把你攔下來。**

### ★對照格的形狀：**與被觀測量無關、但共用同一條路徑的計數器**
```
coin.flow.n == 0 且 bank.writes.n == 0 ⇒【儀器沒開】(那條路一次都沒被走過)
coin.flow.n == 0 而 bank.writes.n > 0  ⇒【儀器有開,世界真的沒動】
```
★**它比檢查 `Probe.enabled` 強**：**旗標只證旗標，不證那條路徑真的被走過。**
★★**這與「判準要成對、兩條線互相預測」是同一個手法的兩種用法。**

## ★★★在【前提機制】進來之前量效果 ⇒ 量到的是【母體不存在】不是【沒有效果】（blueprint 2026-09-06）
```
⑩ 讓糧食零價 ⇒ 想問「農隊賣糧收入是不是真的歸零」
★而 ⑨(貨幣創世)才把 coin 打進世界 ⇒ ★★農隊賣糧【到那時才第一次有真買方】
⇒ ★★★在 ⑨ 之前量那個收入 = 量一個還不存在的東西,而它會印成 0
⇒ 而【沒有效果】也印 0 —— 兩者在數字上不可分
```
★**這是「0 的三種意思」的第四種**：**機制存在、儀器也開著，但【被量的那件事還沒有母體】。**
★★**判法**：問「這個量的【上游前提】現在存在嗎？」——不存在 ⇒ **不要量，掛 token 綁那個前提。**

## ★★★把【母體】放進判準本身（implementer 立 2026-09-07，escrow 對帳）
```
audit_escrow 回 {orphan_escrow, orphan_stub, qty_mismatch, ★checked}
★`checked == 0` 時,上面三個 0 【不是「沒有分歧」】,是【沒有東西可比】
⇒ ★★判準要寫成【checked > 0 且三個分歧都 0】,不能只寫【三個分歧都 0】
```
★**這不是「小心 0」，是【把母體放進判準本身】** ——
★★**母體塌陷的時候，判準會【自己變成不成立】，而不是靜靜地變成綠的。**
⇒ **凡是「某某計數應為 0」的判準，都要問一句：【它的母體是誰？把母體也寫進去。】**

## ★★分類的粒度要對齊【處置的粒度】，不是【現象的粒度】（同上）
```
三種分歧分開記,因為處置完全不同:
  orphan_escrow  實貨在市場、存根不見  ⇒ ★貨【永遠沒有人來領】
  orphan_stub    存根說有、市場沒貨    ⇒ ★★賣家【以為自己還有貨在賣】
  qty_mismatch   兩邊都在但數量不同    ⇒ ★★★要決定【信誰】(權威在 tile)
★合成一個「分歧數」會讓三種【處置完全不同】的事變成同一個數字
```
★**判法**：**問「這兩類如果分開，我會做不同的事嗎？」——會 ⇒ 分開記。**

## ★儀器裝好但【沒接電】的第 7 次（2026-09-07）
```
`audit_escrow` 定義在 order_system.gd:424,而【零個呼叫點】
⇒ ★函式存在、閘跑得起來,而那條路【從來沒有被走過】
⇒ ★★而它跟「跑了但沒有分歧」印出來一樣(都是沒有輸出)
```
★**接線的裁定形狀**：**只接在床上 ＝ 只覆蓋床的情境**（正式跑照樣靜默累積）；
**每 tick 叫 ＝ release 路徑白付成本** ⇒ ★★**`Probe.enabled` ＋ 既有 cadence，再加床裡一次硬斷言**
——**一層覆蓋、一層斷言，兩層不重複。**

## ★★★陽性對照【自己也會壞】—— 假紅出現時先查對照，不是先查被測物（2026-09-07，同日兩次）
```
①ensure_ascii:hook 輸出 零產出,而我 grep 中文 ⇒ ★把一支【正常運作】的 hook 判成沒響
②payload 跳脫:測試 JSON 裡的 \" 破壞了 hook 的 _cmd 抽取 ⇒ ★★把一道【好的守衛】判成壞的
⇒ 兩次都是【對照壞了】,而症狀是【被測物看起來壞了】
```
★**而這比一般假紅更難察覺**：★★**對照是你用來判斷真假的那把尺，尺壞了你不會先懷疑尺。**
⇒ ★★★**規矩：對照沒響時，第一步是【證明對照本身能響】**
（例：先餵一個【一定會命中】的最小輸入），**確認尺是好的，再去看被測物。**

### ★★★同型第三次（2026-09-07）：**修法形狀＝【不是放寬，是換成機制斷言，而且要留下會紅的能力】**
```
三個實例(同一天):
  `_test_wild_game_regen`   把【舊機制分不出「同一 tick 呼叫 200 次」】寫進判準
  headless 的 PRODUCE 斷言   是【⑥ 拔掉的那條身分閘】的鏡像
  `owner 得 coin > 0`        是【拆閥前物價下限】的鏡像 —— 它假設【每一次交易都會產生收入】
⇒ ★三次都是:【舊世界的某個性質】被寫進了判準,而那個性質是我們正要拆掉的東西
```
★**而正確修法不是放寬那條 assert，是【拆成兩條】**（implementer 的做法）：
```
新①機制斷言:`owner 收到的 coin == 成交量 × ask`  ←★ask 不為 0 時它【仍然會紅】
新②正數    :`換手 > 0 且 ask == 0`               ←★★證明零價成交【真的發生過】
★★★只做① ⇒「零價路根本沒被走到」也會綠(0 == 0 × 任何數)
   只做② ⇒ 失去對 coin 路的守護
⇒ 兩條一起才把這格釘住
```
...
## ★★★危害被一個【假設】關住，而後來的票把那個假設拆了（2026-09-07）
```
interaction_system.gd:1029 的既有註解:「免費分配跳 affordability(★ocoin/bid div-by-0)」
⇒ ★寫那行的人【知道】bid=0 會除零 —— 而他只在 `free_dist`(仁君賑濟)那條路上繞過它
⇒ ★★因為在【有 clamp 的世界】,bid=0 只會在【賣方主動免費】時出現
⇒ ★★★而 ⑩ 拆閥之後,bid=0 變成【任何深過剩品的常態】—— 那個假設沒了,而【沒有東西知道它們有關】
```
★**這比「忘了改」難防**：**危害被記錄下來了、也被正確處理了 —— 在【當時的前提下】。**
★★**而拆掉前提的那張票，看不到「誰依賴這個前提」。**
⇒ ★★★**處置：拆一個【範圍性質】(例如「價格恆 > 0」)時，要掃【提到那個性質的註解】**
（`grep div-by-0 / 恆 > 0 / 永不 0 / never zero`）—— **那些註解就是依賴清單。**

## ★★★被夾住所以不顯眼的【方向】錯誤（implementer 揭 2026-09-07）
```
套利 proxy 的賣邊:`gain2 = local_value(merchant, res) × qty`
★而【自評值低正是想賣的理由】⇒ 方向是反的 —— 這個錯【一直都在】
★★而有 clamp 的世界裡 local_value 最低 = BASE_PRICE × (1−0.5) ⇒ 永遠 > 0
   ⇒ 方向錯只表現成【排序偏差】,★從來不表現成【排除】
⇒ ★★★⑩ 拆掉下限之後,它從【排序偏差】變成【完全排除】
```
★**而這是「特例變常態」的一個新變體**：
> ★★**這次變成常態的不是【狀態】，是【一個一直錯著、但被夾住所以不顯眼的方向】。**
⇒ ★★★**拆掉一個 clamp／下限之前，要問：【這個下限現在掩蓋著什麼？】**
**——不是「誰依賴這個下限」（那是上一條），是「誰的錯誤【因為它而看不出來】」。**

## ★★★問「合過來會帶什麼」一律用【三點 diff】（implementer 揭 2026-09-07）
```
兩點 `git diff A B`   ⇒ 【這兩棵樹差在哪】—— ★它會把【你自己的改動倒過來顯示】
   實測:13 檔 +372/−254,而 −254 那半是【⑩ 自己的改動】
   ⇒ ★★看起來像「main 會刪掉我的東西」= 假警報
三點 `git diff A...B` ⇒ 【合過來會帶什麼給我】(相對 merge base)
   實測:8 檔 +297/−11 —— 這才是答案
```
★**兩個問題不同，而它們的指令只差一個點。**
★★**而「差一個點」正是這種錯最容易發生的形狀**：**不是不知道，是【打錯的成本看起來很小】。**
⇒ ★★★**凡是要回答「這個 merge 會帶什麼」的場合（包含用它來說服別人去合），一律三點。**

## ★★★規矩寫在【角色名下】，就會被讀成「只有那個角色需要」（implementer 揭 2026-09-07）
```
CLAUDE.md 的【量測員】那一行寫著:
   「留 main dir,`godot --path .worktrees/<slice>` 對 branch code 跑 beds(★禁原地 checkout)」
★而 implementer 讀過那行,卻繞了兩輪才想到把它用在自己身上
⇒ ★★他的自述:【它被我歸檔成「量測員的規矩」,不是「跑 branch code 的辦法」】
⇒ ★★★而那條的內容其實是【工具與被測物可以分離】—— 那對誰都成立
```
★**而我當時給的是錯的辦法**：我說「合 main 拿新 wrapper」——
★★**那要動 branch，而正解【不動任何樹】**：從 main dir 跑、`--path` 指過去，
**wrapper 用 main 的（新的、有 beacon）、code 用 branch 的。**
> ★★★**通則：一條規矩若只寫在某個角色的段落裡，它的【適用範圍】就會被那個標題悄悄縮小。**
> **⇒ 寫規矩時要問：【這條的內容是關於那個角色，還是關於那件事？】——**
> **是關於那件事，就要放在【做那件事的人會看到的地方】，而不是【那個角色的段落】。**

## ★★★儀器要【大聲失敗】，不要【安靜降級】——三種形態（implementer 同日連撞三次，2026-09-07）
```
①★被旗標關掉:計數器掛在 `Probe.bump` 上,而 bump 第一行是 `if not enabled: return`
   ⇒ ★★而【不 arm Probe 的床】正是最可能出問題的那些(會 arm 的通常較新、較守規矩)
   ⇒ ★★★儀器在【最需要它的那些跑】裡恆 0 —— 盲區與它要找的東西【正相關】
②★被噪音淹沒:push_error 每站每 tick 都發 ⇒ 96 站會把 log 淹掉
   ⇒ ★★【把診斷訊息淹成噪音,跟沒有診斷訊息是同一件事】⇒ 只在【第一次】發
③★★★安靜降級:`get_stack()` 只在 `--debug` 下有值,而 `tools/godot.ps1` 【不帶 --debug】
   ⇒ 逐站清單印出 `population @ (no-stack)` —— ★【一個長得像結果的非結果】
```
★**共同修法不是「記得加旗標」**，是 ★★**讓儀器在【拿不到它需要的東西】時，說出【本輪無效】** ——
★★★**而不是輸出一個降級版本**：**降級版本會被下一個人當成結果引用。**

### ★附帶的具體事實（★會咬人，寫下來）
```
`get_stack()` 在沒有 `--debug` 時【回空陣列】,而本庫的 wrapper 預設不帶 --debug
⇒ ★任何經 wrapper 跑的床,用 get_stack() 抓 caller 都會【靜默拿到空的】
⇒ ★★而 `scripts/debug/scale_econ_anon_consumer_trace_bed.gd:5` 的註解描述的正是這種程序
   (臨時 patch anon_tier_system.gd 掛 get_stack)⇒ 下次有人照那個程序做會踩到同一個坑
★★★而若要開 --debug:它【不是 fp-comparable 的跑】—— 錯誤處理與輸出都會變,
   ⇒ 逐站清單那種跑,本來就不該拿去跟 baseline 比逐位元
```

### ★★★而【為了觀測而開的旗標】不是語意中性的（2026-09-07）
```
`--debug` 讓 `get_stack()` 有值 —— ★而它【同時】把 `assert()` 失敗變成【互動式 debugger break】
⇒ ★★那一跑卡在 `Enter "help" for assistance.` 等 stdin,【永遠不會結束】
⇒ ★★★而 main baseline 上本來就有紅 assert ⇒ 這條路對【任何有紅 assert 的床】都走不通
```
★**而症狀是【卡住】不是【報錯】** —— ★★**那正是最難歸因的那一種**（它長得像「跑很久」）。

### ★★而更前面的那個錯是【規格開太細】
```
★我裁「要知道 runtime 執行到【幾處】」= 行粒度 ⇒ 逼出 get_stack ⇒ --debug ⇒ 卡住
★★而決策(分流判準)需要的是【床粒度】:哪一支床受污染,不是哪一行
⇒ ★★★在問「要多細」之前,先問【決策需要多細】—— 多出來的解析度不是免費的,
   它會把你推進一條【為了取得它】而必須付出的代價鏈
```

## ★★★改死碼 ＝ 產生【無法驗證的改動】，比留著更糟（2026-09-07，blueprint 核可入帳）
```
★★★【2026-09-07 當日訂正：血證原文是錯的，而通則仍然成立】
   原本寫：「26 支床…另 16 支的那些行從來沒有跑過」⇒ ★而那 16 支【不是同一類】：
      ★★有一部分寫的是 `DecisionContext.population`（★真欄位，那些 code【本來就對】，根本不在範圍內）
      ★★★另一部分才是【TeamData 但未執行】（實測反例：data_test.gd:26 `team.population = 10`，不在 DIRTY 名單）
   ⇒ ★我原本的「16 支是死碼」與 implementer 的「16 支都是 DecisionContext」【兩句都錯】
      —— 真相是 2×2：TeamData×執行（10 支 DIRTY）／TeamData×未執行／DecisionContext（不在範圍）
   ⇒ ★★而根因是【我們一路都在過濾「屬性名」，從來沒有過濾「接收者的型別」】
血證（訂正後）：【TeamData 且未執行】那一小群的那些行，從來沒有跑過
⇒ ★★若把它們「改成合法寫法」:改壞了【也不會紅】—— 因為它不執行
⇒ ★★★所以「全部改」會產生一批【無法驗證的改動】,而那比【留著一段死碼】更糟
```
★**正解是【刪】，而且刪時要留一句註記**：
...
### ★而它的一般形
> ★★**在動一段 code 之前，先問【它會不會執行】——**
> **因為【驗證能力】跟著執行路徑走：不執行的東西，你改對改錯都得不到回饋。**

### ★★★而這件事的收尾事實：**知識早就在樹裡，而我們的儀器把那句警告算成了病例**
```
`scripts/debug/s6_build_days_truth.gd:51`(★2026-09-01,dc95079a —— 比今天早【六天】):
   # ★★★不能寫 team.population = pop：那顆是【唯讀衍生】(team_data.gd:55-59，setter 是 pass)
⇒ ★有人早就發現、也寫下來了 —— 而它【只是一句本地註解】
⇒ ★★沒有任何機制把【一個本地發現】變成【全庫的守衛】
⇒ ★★★而更刺的是:我用 grep 數病例時,【那句警告本身被算成一筆病例】
```
★**而最終的 2×2（工具量的，不是 grep 數的）**：
```
TeamData × 執行   = 10 支床(DIRTY)          ← 真問題
TeamData × 未執行 = ★【1 站】(data_test.gd:26) ← 而我先前列了六支,六支【全是誤判】:
                     字串字面值 ×3(輸出格式/assert 訊息)、註解 ×1(★就是上面那句)、DecisionContext ×5 站
DecisionContext   = 真欄位,本來就對 —— 一行都不要動
```
