...
> ★★★**2026-09-23 退役公告（讀本檔前必看）**：`inbox-watch.sh` 與 `tg_poll.py` 進站**整支退役**，
> 信箱改成「git handback ＋ 寄件端 `SendMessage` 敲收件人」。
> ★**本檔以下全是歷史案卷，原文不改** —— 它記的是那三支單例 watcher 當年怎麼壞的。
> ★★留著的理由是**那些病不屬於那三支腳本，屬於「單例＋心跳租約」這個形狀**：
> 下一次誰再寫一支常駐守衛，這裡每一條都會再來一次。
> ★★★而看門狗（`watchdog.sh`）**仍在用**，所以這些案例對它**現在就適用**，不是歷史。
