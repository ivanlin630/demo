...
## ★★開跑前驗【commit 真的含那個東西】，不要相信 merge 訊息（2026-09-04）
```
★implementer 開跑前 grep 那顆 commit:SPECIMEN_RUNTIME_N 3 處／_sec_specimen_coverage 2 處／
   `for _l in lines`(16383 修法) 1 處／★★「備戰」在 options.gd 剩 4 處【全是註解】而 REGISTRY = 0
⇒ ★★★merge 訊息是【宣告】,grep 是【證據】—— 而長跑開跑後才發現拿錯 code,代價是整輪
```
★**與既有那條同族**：**commit 訊息含「merge `<sha>`」⇒ 該 sha 必須是 HEAD 祖先**（同樣是把宣告換成可驗事實）。
★★**而這一步只花幾秒，擋掉的是 22 分鐘 ×3** —— **便宜的驗證要放在【昂貴動作之前】，不是之後。**

## ★★★★兩個數字看起來能相減，先查它們【是不是同一條路上的】（2026-09-04，implementer 差點踩進去）
```
★輸出裡 `[Merge]` 只有 7／12／13,而「併入」勝場 80／134／211
⇒ ★★看起來就是「committed 沒送達」的鐵證 —— ★而他去查了 `[Merge]` 的出處:
   它來自 `subteam_system` 的 convoy 併回,★★★【不是 join resolver】
⇒ 兩個數字【不在同一條路上】⇒ 相減【沒有意義】
```
★**而這種陷阱最危險的地方是它【支持你已經相信的假說】** —— 我當時的假說正是「committed ≠ 送達」，
★★**而這個假數字剛好長成它的鐵證** ⇒ ★★★**所以「數字支持我的假說」不是驗證，是【該去查出處】的信號。**

## ★★「儀器沒印」與「值是 0」在輸出上長得一模一樣（2026-09-04，同一輪）
```
★`join.dispatch` tap 確實在(interaction_system:241),★★而 `three_tickets_bed` 【沒有印 join.* 的節】
⇒ seg1 三張輸出裡 join.* key 【一個都沒有】
⇒ ★★★而那是【儀器沒印】不是【值是 0】—— 今天第 N 次同一形狀,而這次擋下它的是【key 一個都沒有】
   （★若床印了一個 join.dispatch=0,就分不出來了）
```

## ★★★★床要證的不是「修好了」，是**它有鑑別力**（2026-09-05，共位必見的驗收）
```
★同一張床兩邊跑:main FAIL=1 ｜ branch FAIL=0
★★而紅的【正是設計來紅的那一格】(G1 森林小 host,dist=0 而 belief_pos=(-1,-1))
★★★而陰性對照 G2(平原大 host)【兩邊都綠】⇒ 證明床【不是在 main 上什麼都紅】
```
★**這一組（陽性 ＋ 陰性 ＋ 兩邊跑）是我們產出過【最強的修法證據形狀】**：
```
①只有 branch 綠 ⇒ 可能是床壞了/床太鬆
②只有 main 紅 ⇒ 可能是床對 main 什麼都紅
★★★兩者【同時】具備,才排除掉「床本身有問題」這個競爭解釋
```
★★**而順手撈到的比主結果還值錢**：**main 上連陰性對照 G2 也是 `moved 0筆/48`、`resolve=0`**
⇒ ★★★**證明「JOIN resolve 只掛移動事件」是 pre-existing，不是這一刀帶進來的** ——
**而那句話若沒有這個對照，就只能用推論說。**

## ★★★★寫「fp 會變」之前，先問【這一刀在那個窗裡會不會 fire】（2026-09-05，我的 spec 寫錯）
```
★我在 spec 寫驗收:「determinism 三跑一致（★fp 會變 —— 這是行為改動）」
⇒ ★★而實測:三跑 byte-identical,★★★且【等於加這一刀之前的 fp】
⇒ 原因:fp 床的窗是 240／1000／2400 tick ＝【不到 2 日】,而這一刀掛在【位置級大事】上
   ⇒ ★那個窗裡它【一次都沒 fire】
```
★**所以「fp 沒變」在這裡【證不了任何事】** —— ★★**它不是【等價證明】也不是【無效證明】，是【母體 0】。**
★★★**而真的有 fire 的證據在別處**：**90 日窗 `mreport.attempt=50 / sent=2 / call=352`。**
⇒ ★**通則**：**fp 類判準要先確認【被測的東西在那個窗裡會發生】** ——
**否則「fp 不變」與「fp 該變而沒變」在輸出上長得一模一樣。**（★★與「走不到目標行的 smoke test」同族。）

## ★★★★「只印前 N 名」會製造【不存在】（2026-09-05，implementer 自揭）
```
★tap 確實存在（`crisis.abs_hunger.team.<id>`,per-team、無 cap）
★★而【卷面只印前 8 名】⇒ 21 隊裡【13 隊在卷上不存在】
⇒ ★★★交叉腳本回 `starved distinct teams = 0`,而真值是 **21**
```
★**所以 top-N 印法是【儀器謊】的一種**：**tap 沒說謊，印法說了。**
⇒ ★★**而它與今天另外三件同族**：**幽靈 counter（印一個不存在的）／整張表消失（截斷）／哨兵撞號** ——
**四者的共同點是【輸出看起來完整】。**
★★★**做法**：**凡是 per-entity 的計數要進分析，卷面就【不能只印前 N】** ——
**要嘛全印，要嘛印【相異數 ＋ 總量】讓下游知道自己看到的是樣本。**

## ★★★★降級要【做進句子】，不是只寫在誠實限（2026-09-05，implementer 的處置）
```
★單 seed 的卷不能講「warring 世界怎樣」⇒ ★★而他【不是】只在 §8 誠實限寫一句
⇒ ★★★他在【同一段】就寫:「pop_end 分布比 peaceful 厚 —— ★而這【不足以】講 warring 世界比較厚」
```
★**為什麼這比寫在誠實限強**：**誠實限在文末，而讀者是【逐段讀、逐段引用】的** ——
★★**被引用的是句子，不是文末那一段** ⇒ ★★★**降級沒有跟著句子走，它就會在引用時掉光。**
★**做法**：**凡是母體不足以支持的推廣，把限制寫在【那一句的同一段】，而誠實限只做總表。**

## ★★★★PowerShell 的 `*>` 重導**預設寫 UTF-16** —— 而檔案看起來完全正常（2026-09-05，implementer 30 秒內自捕）
```
★用 `*>` 導出 ⇒ ★★檔頭 BOM `ff fe` ＝ UTF-16 ⇒ ★★★之後【所有 grep 都會失效】
   —— 而檔案打開來【看起來完全正常】(編輯器會解碼)
★修法:改用 `cmd /c … > file 2>&1`（raw bytes）⇒ 驗法:★前四 bytes 應是內容本身(如 `47 6f 64 6f`)
```
★**而它與今天其他幾件同族**：**幽靈 counter／整張表消失／哨兵撞號／top-N 截斷** ——
★★**共同點是【輸出看起來完整】，而下游的工具卻讀不到東西。**
★★★**做法**：**長跑開跑後【立刻驗前幾個 bytes】** —— **而不是等到要 grep 時才發現，那時已經跑了兩小時。**
...
## ★★★方向註記的由來（2026-09-07，2026-09-18 由 03b_measurer 搬入，字串逐字保留）
...
## ★★★★有 cap 的來源 —— 三條紀律全文（2026-09-18 由 03b_measurer 搬入，逐字保留）
...
## ★★★第四條（長跑卷面檔名帶輪次識別）的血證（2026-09-18 由 03b_measurer 搬入，逐字保留）
