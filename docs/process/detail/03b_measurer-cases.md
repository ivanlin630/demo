...
⇒ **可執行（★兩層，第一層不夠 —— 當日訂正）**：
**①「它被寫過幾次」**（抓根本沒人寫；★**只能反駁**：`=0` 有力、`>0` 不足以確認）
**②★★「它的【組成項】各自變不變」**（抓有人寫、但某一項是死的）
★★★**血證**：`threat` 注入強度恆 `0.2000` —— **每次都有寫、寫的也是正確結果**，只是某個加項恆 0。
⇒ **「世界穩定」／「沒人寫」／「某項是死的」三者在【總分】上完全一樣。**
★**而 `threat_assessment.gd:52-56` 的註解【當時就寫著這件事】**（「只看總分的話四種在輸出上長得一模一樣」）
—— ★★**踩坑的人沒回去看**（同族：ObserverGuard 印在第 4 行、七天沒人讀）。
