...
code: "03b"
status: working
current_ticket: 進行中（★★perf線索包①②已完成寄systems(2026-08-20-measurer-to-systems-perf-clue-12.md,open,待systems裁):baseline現main——①phase細profile(5天force_full_hd+phase_timing warring_states)決定性:near.faction_ai獨占93.1%wall time,★systems原點名4候選(L0階梯檢查/farm生產線/construction tick/labor rebalance)全部<0.15%of wall非真兇;真兇在evaluate_all內部loop1(19%)/unified.rank(17.5%)/leader_unified(12.8%)/gather.market(6.7%);★唯一code-read坐實機制發現=loop1完全忽略_team_ids參數每次全量掃全faction(faction_ai_system.gd:717-729)、near+far雙pass各付一次全量成本;誠實揭露fai_inner累加總和(696.5s)對不上outer桶(191.5s)3.6x未root-cause(loop1雙付只解釋~10%,其餘缺口待implementer查)。②slice歸因bisect(3commit:94e2f826 pre-L0/3d30b3ed post-S2b/現main,同seed同窗10天,嚴格序列非併發)意外結論:49-73團規模窗內三者per-team成本幾乎重合,查無單一slice可歸因顯著加價,mainhead甚至持平或略快;推論『+34%』degradation可能只在更高團數規模(152-242團)顯現、本輪未覆蓋,需100-200團區間同方法論補測才能精確定位。附帶③初步冪次擬合(低N趨勢k≈2.1-2.9疑似O(N²)、樣本少noise大僅方向參考)。數字全落地docs/process/verdicts/perf-clue-package-12.measure.json+4份raw log。3個temp worktree已移除+4個temp tap已revert+2個自建bed已刪,--headless --import乾淨編譯確認。★接著照佇列插隊處理lag份額quantify(12mo大考前必完、labor-v2 combined同床複驗daily_rate非EMA+決策資源軌跡specimen),完成後回perf③④⑤。（原★★農業b最後一輪 CLOSE已完成寄systems(2026-08-19-measurer-to-systems-agrib-final-round-verdict.md,open,待systems裁):warring_states seed1337 3mo(90/90天全跑完exit0無timeout)——①organic facility-vs-cap相關無結構性發展封頂強證據(mint 0%兩組皆同,farming 4.0%vs5.5%小差距)=fixture修完即可不需floor;②popcap分布複核PASS(0 runaway雙向,cap<5=4.6%持平);③team暴增SUPPRESSED(49→152 vs原49→242,day50後趨平)=最強正訊號;★但③另兩軸MIXED非乾淨PASS——perf absolute數字相近(670ms vs793ms)但per-team成本沒改善(4.41ms/team vs3.28ms/team重34%);同對隊反覆數(grep raw SurvivalMergeIn重算)1647行/136組/max81×,反而比原698行/54×都高(方向與churn-fix預期相反)但不確定原輪是否跑滿90天(可能被自身暴增撞timeout腰斬非乾淨對照)+dispatch counter(512)與raw行數(1647)差3.2倍未root-cause,兩者均flag請systems核實/裁決。數字全落地docs/process/verdicts/agriculture-b-final-round.measure.json+raw extract(pair-counts.txt/tickperf.txt)。temp tap(population_system.gd popcap.snapshot/overflow_fire)+自建bed+sidecar已revert/刪,--headless --import乾淨編譯確認。（原★labor-v2 combined re-measure CLOSE已完成寄systems(2026-08-19-measurer-to-systems-laborv2-combined-verdict.md,已consumed):同床(自建labor_v2_combined_remeasure_bed.gd)seed1337 peaceful_economy 6mo controlled對照——churn-fixed baseline(main)=8 starve vs churn-fixed+labor-v2(combined)=28 starve,delta=20/3.5倍(降自舊16×);honest主導/lag-window=0分解不變(兩邊死亡food_flow_avg全≤0);誠實揭露baseline自己2→8落差confound(舊床從未commit過、這輪重刻non-byte-identical、非純churn-fix效果,同bed內3.5×才是最可信數字);headless兩邊0-new確認(★systems已修godot.ps1 timeout-kill race d18ff8fc,本輪長跑stdout完整存活無corruption);②③④churn缺口(team暴增/perf/同對隊反覆)此床天然churn量低、留農業b re-measure覆蓋(同對隊反覆已由churn-fix-gate輪TDD控制床單獨證實非缺口)。數字全落地docs/process/verdicts/labor-v2-combined-remeasure.measure.json。temp tap(resource_system.gd death.starve_detail)+自建bed main+worktree雙邊已revert/刪,--headless --import兩邊乾淨編譯確認。（原★churn-fix gate CLOSE已完成寄systems(2026-08-19-measurer-to-systems-churn-fix-gate-verdict.md,已consumed):核心機制TDD控制床(mergein_join_lifecycle_test.gd)獨立重跑ALL PASS=gate①⑤⑥證實(釋放+join_rejected cooldown結構性斷根churn換皮、timeout/abort_ghost真fire非死碼);constitution PASS(77/0)+headless獨立完整跑0-new=gate⑦;attribution確認churn為main pre-existing非農業b專屬(plain main partial跑已現Team70→Team37重複3次,code-read本fix只碰faction_ai_system.gd)。缺口:②③④(原49→242/793ms農業b高壓規模)未獨立復現,organic 2mo跑兩次都撞godot.ps1 wrapper timeout-kill race(已回報systems、已修d18ff8fc)。數字全落地docs/process/verdicts/mergein-churn-fix.measure.json。
---
