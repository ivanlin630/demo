...
★**理由不是潔癖，是【已經被引用的讀數會失去出處】**。血證（同日）：
`2026-09-09-population-census-90d-warring_states.txt` 先前跑到 `Tick 70000`，
而 blueprint 的故事稽核候選（`勒索 2349 次全妥協`／`Team21 重撞 126 次`／`Envoy 6/152`）
就是從那份半卷讀的。★★後來同名重跑覆寫 ⇒ **那份輸出沒了（measurer 直答「沒了」）**
⇒ **三個已經在流通的數字，從此無法從磁碟回查。**

★★★**這是 `[TREE]` 那條的變形**：`[TREE]` 管的是「結果跑在哪棵樹上」，
這一條管的是「**結果的輸出還在不在**」——**兩者都在問同一件事：這個數字現在還能不能被查證。**
...
| ★★★10 | **比兩份輸出之前，先證明它們是【同一棵樹】** | ★★★血證 2026-09-16：v2→v3 數字對不上，追了兩個假說（RNG 污染／並發）—— **而真因是兩者之間落地了兩個真的 production commit**（`bea3068f1` 走廊降可行性、`2ce8f36a4` (4a)(4b) tap）<br>★**「同 seed 同 config」不蔑涵「同 code」** —— 而這個前提失效時，**它的症狀跟【儀器改變被觀測物】一模一樣**<br>★★**修法不是新工具：閘的橫幅已經有 `[TREE] HEAD=<sha> clean`** ⇒ **量測床的 `[BedSelfCheck]` 同樣要蓋這一行**，而**比對兩份輸出前先比 sha；sha 不同 ⇒ 差異未歸因之前不得讀成【世界變了】** |
...
**原誠實限已解除**：它的任務是「成因未知時防誤讀」，而成因**已判**（QA：症狀非 genuine），
且症狀有**具名票**在追（`k-symptom-A-five-resources-never-candidate`／`k-symptom-B-weapon-util-never-wins`）。
⇒ ★繼續降級所有涉幣結論，等於**為一個已經回答的問題**壓住人口卷等多份卷面。

**改掛一行【方向註記】（★不是降級，是標明偏誤方向）**：
> **「週轉絕對量受 `k-symptom-A/B` 抑制，偏低方向已知。」**

★**用法**：涉幣卷面照常下結論，只是**絕對量**要帶這行。
★★**兩票關掉時，這行註記跟著撤**（★而「撤註記」這件事本身要有人做 ⇒ 已寫進兩票的票面）。
★★★**它與「誠實限」的差別**：
```
誠實限   說「這個數字【不能用】」        ⇒ 只能等
方向註記 說「這個數字【可以用】，而我們知道它往哪邊偏」 ⇒ ★可交付
```
