...
> **★現況檔（開工/完工自更，01 監控用）**：收判決工單開工 → 更 `docs/process/status/04_qa.status.md` frontmatter `status: working` + `current_ticket: <handback檔名>`;判完 → `status: idle`。低成本一行,01(系統) grep 監控。詳 `status/README.md`。
