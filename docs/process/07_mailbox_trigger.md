...
## ★★★停滯警報的處置 ＝ **機械三步，禁「可能在深工」的赦免判斷**（blueprint 立 2026-09-16，用戶抓到）

**血證**：watchdog 對 blueprint 叫了，**他判「大概在深工」把它赦免掉** ⇒ **那一次是真停滯** ⇒ **用戶親自來提醒。**

```
\u2460 **警報即查**：**open 信的【實齡】與【收件人】**（不是憑印象，是掃 mailbox）
\u2061 **任一封 `open` 超過 30 分鐘、且收件人【沒有 consume 動作】** ⇒ ★**直訊廣播立發**
   —— ★★**不做「他可能在深工」的赦免判斷**
\u2062 **廣播後 15 分再哨**；**哨仍紅 ⇒ 推用戶開終端**
```
★**為什麼是機械的**：**誤觸成本 ≈ 0**（多一封被忽略的短訊）／
**誤赦成本 ＝ 用戶親自來罵** ⇒ ★★**不對稱 ⇒ 永遠選誤觸。**
★★★**而「他大概在忙」是一個【看起來體貼】的判斷** —— **它的代價落在別人身上，所以它特別容易被做出來。**

### ★附：查「實齡」時要分清楚三件事（systems 2026-09-16 補）
```
**信的 mtime**（工作樹）／**信進 git 的時間**／**收件人 consume 的時間** —— ★**三者可以差很遠**
⇒ ★★**血證**：2026-09-16 有兩封信被回報「躺了 2 小時」，
  **而它們在 git 裡各只有【一個】commit —— 就是收件人的回信 commit**
  ⇒ **寄件人沒有 commit 自己的信 ⇒ 它只活在工作樹裡，直到收件人 commit 它**
⇒ ★★★**所以報「躺了 N 小時」時要說【你是從哪裡讀到的】** ——
  **否則我們會把「寄件人沒 commit」誤判成「收件人沒收到」。**
```
