...
## ★★★列舉要挑【收斂】的那一軸 —— ★★而「收斂」是指【誰決定它】（2026-08-26 訂正我自己）

**第一版只 grep `Assertion failed` ⇒ 三類 16 行只看到 5。**
**第二版改列舉【正常輸出前綴】，理由是「正常輸出是我們自己寫的、收斂」⇒ ★實測 721 條假陽性。**
★**那句理由是錯的：正常輸出【也】發散**（多數是無前綴的縮排內容行）。

| 問法 | 結果 |
|---|---|
| ❌ 內容決定的軸（錯誤訊息／正常訊息**文字**） | **兩邊都發散** —— 我們每寫一個測試就長出新形式 |
| ✅ ★**引擎決定的軸**（severity 通道 `SCRIPT ERROR`／`ERROR`／`FATAL`） | ★**有限，且不因我們寫新測試而變多** ⇒ 721 → 7 |

★★★**判準一句話：問「這個集合會不會因為我們多做事而長大」——不會的那個才是收斂的。**
