...
# ★2026-09-06 從 `known_issues.md` 搬入的已解決條目（13 個區塊 / 229 行）

> ★`known_issues.md` 的檔頭【從一開始就寫著】「已修項（✅）移這裡」——而它沒有被執行，
> 於是那份檔長到 4292 行、110 個已解決標記混在開放項裡面。
> ★★而混在一起的代價不是「亂」，是【錨 stale】：★★★照著一個指向已修項的門牌去看，
> 會看到「什麼都沒有」，而那看起來像【已經修好了】——兩者長得一樣。
> ★原文逐字保留，沒有改寫。
...
## ★第二批（同日補搬 16 區塊 / 188 行）
> ★第一遍我的區塊切法有 bug：找到 `##` 之後直接跳到下一個 `##`，
> ★★於是【巢狀在裡面的 `###`】從來沒有被當成候選 ⇒ 只搬了 13 個而不是全部。
> ★★★而它【沒有報錯】：行數對得上、斷言全過 —— 少搬的那些看起來就像「本來就沒有」。
