...
## ★★★同源鐵律的延伸：**儀器自己的【排除表】也禁手抄**（blueprint 立 2026-09-10）

```
★「估算器禁手抄物理」防的是【一份手抄的數字，與它宣稱代表的真實脫鉤】。
★★而【儀器自己的排除表】吃同一條：
   一份手抄的「本尺排除：…」，與它宣稱代表的【尺的真實涵蓋範圍】脫鉤。
⇒ 規矩：**「本尺排除」清單禁手抄，由 code 生成。**
```

★**血證 2026-09-10**：`StateFingerprint` 的 `EXCLUDES` 是手抄字串，
**它漏掉整塊 `player_*`** ⇒ 讀那一行的人（systems）**以為玩家狀態在尺裡**，
因此寫了一個**前提錯誤、永遠綠不了**的驗收格。
★★而那一行的存在本身就是 2026-09-01 一次血證的產物 ——
★★★**當初的修法就是「手抄一份排除清單」**，於是同一個病換了一層皮又長回來。

★**而導出之後還有一個坑（同日實測）**：第一版的導出判準寫成
「欄位名有沒有出現在 `state_fingerprint.gd` 裡」⇒ **在註解裡提到它就會讓它消失**
⇒ ★★正確判準是【有沒有真的去讀它（`state.<欄位>`）】——**提及 ≠ 讀取**。
