...
| 3 | `host_rep` 名聲磁鐵（`terms.gd:195`） | 四筆**恆 `0.5`** ⇒ `join_drive` **是常數** ⚠**見下方取樣保留** | 輸入變異性 |
...
### ⚠️ 展品 #3 的取樣保留（**systems 自我修正**，2026-08-21）

**`Probe.bump_sample`（`probe_stats.gd`）是 first-N**：`if arr.size() < cap: append` ——
**滿了就再也不記** ⇒ **所有取樣診斷永遠只看得到最早的 N 筆**。

∴「`host_rep` 恆 0.5 ⇒ 從未差異化」**有兩種可能，目前分不出來**：
- (a) **母體就是 4**（全期只發生 4 次投靠）⇒ 結論成立，但 **n=4 極小**
- (b) **母體更大、4 筆只是最早的樣本** ⇒ ★**「從未差異化」可能是早期取樣的假象**
  （名聲本來就從 0.5 起步、隨時間才分化）

★**我把這顆寫進 §H 當死水展品、還據此叫 implementer 改測試設計 —— 在確認母體之前，那是超前的。**
**已請 measurer 補報母體（`join` 事件總次數 vs 樣本數）。回覆前本列標 `待確認`，不得引用為定論。**
