...
- **★★observability_gate followup（第 4 次同族 observer_no_global_rng，blueprint 多次點名，明列此處別靠人工記性）**：機器擋 observe/選取路徑碰 RNG。`constitution_gate` 現 `SCAN_DIR` 不掃 `scripts/debug/`（specimen/observer 工具全住這）、RNG regex 不含 `pick_random`/`shuffle`。需新 scan-dir + file-predicate + regex 類別（量體≈v3 god-view 偵測器，非一行）。**followup slice**。連 memory `feedback_observer_no_global_rng`。
