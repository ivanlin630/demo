...
## 走查文件第二版（中文欄位名）—— ★**補票 2026-09-17（它之前只活在 blueprint 的記憶裡）**

**用戶原話（2026-09-10）**：「**項目名稱都是代碼 我不容易看懂 有辦法翻成中文嗎 工作量大嗎**」

```
**v1 產物**：`docs/measurements/2026-09-10-c1-walkthrough.txt`（★**存在，58015 bytes／1124 行，我核過**）
**v2 要做的三件**：
  ① **所有代碼名欄位翻成中文人話**
  ② **重埋一個【新位置】的種子錯誤**（★舊位置不算：用戶已經知道那一個）
  ③ **交付時通知用戶來抓錯**
```
★★**這張票的驗收是【人在迴圈中的陽性對照】**：
**用戶抓得到那個種子錯 ⇒ 走查文件真的可讀；抓不到 ⇒ 它只是看起來可讀。**
★★★**而它之前被記成「欠最久」卻【沒有票】** ——
**一件沒有題目的事不會有年資，而它的年資來自一個【不會被稽核的口頭清單】。**
★**補票後它才開始有年資**（systems 裁 2026-09-17：**沒有題目的欠件不算欠件**）。
...
## ★★★共用檔的 merge 會【靕默地洗掉一個決定】（2026-09-16）
