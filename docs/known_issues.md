...
- ~~★compute:`world_state.gd erase_team` O(N)/erase → die-off O(K·N)~~ **✅ 批次化 done（2026-07-02 merged,`erase_teams` 單趟 sweep O(N+K),pointwise CLEAN×3 seed=零行為變,scaling 2.1-3.0× 隨 N 放大）**。**⚠ 但量測誠實揭:現行 N≤100 尺度 erase 佔 die-off tick <1%,「die-off spike」症狀未消——真主導=cadence 共置成本（top spike 全 K=0 cadence tick 1.2-1.6s vs 中位 184us,max/median ~8600×）= per-tick 有界不變量現行最大違反者。本修=尺度保險。→ 新案:cadence spike measure（定位 faction_ai cadence/print I/O/encounter 結算,`dieoff_perf_bed` 常備床沿用）。**
