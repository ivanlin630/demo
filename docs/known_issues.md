...
- **長窗斷鏈修進度（2026-07-03,長窗 6 月+zoom 拆根,藍圖四裁 `chain-rulings-envoy`）**：
  ...
  - **⚠ 無馬 courier 框架缺口**：seeded warring 種子隊 mounts=0 → 信使步行=微速差追移動 target 極慢（timeout 被迫 12 天）。修向:配馬經濟成熟後縮 `FOUNDING_TIMEOUT_*` / courier 速度階（藍圖差異①決策,暫收 (c) 步行慢=believable）。
  - **queue（藍圖已裁待燒）**:②b 飢餓連續下修 raid-for-food readiness（只限搶糧,campaign 維持）+ score 0.30 稍寬（seeded 校「寬度夠生戲+知足者仍蹲」）;②c food<20 硬濾 fold 進 richness score（窮村低分非零分）;③ asm 照受控人力 spec §5/§7 做深（待遇輸入:餵養/苛待/看守/guard-cap）;斷① 打草穀=stakes-to-faction 分界+**「入勢力不換腦」enforce**（invariants 已納,實作待燒）。全修完→**長窗二跑驗複利弧**。
