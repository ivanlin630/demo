...
### 🏚★★★「篩選」的真相：**不是強村活、弱村死，是幾乎所有村都被削到剩一個人**（2026-08-21，三輪翻案後定案）
**因果鏈（measurer 獨立量測 ＋ QA 逐 tick 稽核，`@40ab0ab4` 2026-08-21）**：

1. **存活隊 `effective_food` day60→90 中位 Δ ＝ +10.91、正成長佔比 13／16（81%）**
   ⇒ 表面上像「**擬真篩選：弱村淘汰、強村存活**」。
2. ★★ **但那 81% 正成長的「存活隊」裡，day60 時 11／18 已經萎縮成 `pop=1`**
   ⇒ **不是「強村活、弱村死」，是「幾乎所有村先削到剩 1 人，那 1 人的食物數學才好打平」**
   ⇒ ★**這是【個體存續】，不是【村莊存續】。**
3. **⚠【我的因果推論未證實，可能是相關非因果】**（blueprint／用戶補刀後標注 2026-08-21）：
   我從 **3 隊**（1 forest 掙扎／2 plains 暴衝）就下了「**決定性差異是地格 cap**」。
   ★ **但 11／18 隊萎縮到 `pop=1`，它們不可能全是 forest** ——
   **若 plains 村也萎縮，`cap` 大就毫無解釋力，瓶頸必在【提取端】。**
   `cap` **只對 `team5` 成立**（forest、池小且頂格：**滿 49 ＋ 再生 3 < 需 4 ＝ 真物理**），
   **不足以解釋多數案例**。下表只當「**forest 那一格的證據**」，**不當通則**：
   | 隊 | 地形 | `tile_food_cap` | coin | `effective_food` |
   |---|---|---|---|---|
   | team5 | **forest** | **49（已頂格）** | **705** | **≈2**（knife-edge） |
   | team10 | plains | 268 | 200 | **388** |
   | team11 | plains | 186 | 200 | **457** |
   ⇒ **`coin=705` 買不到糧、`coin=200` 卻食物暴衝** —— **瓶頸是地格承載力，forest 系統性遠低於 plains。**

**⇒ 綜合結論**：**世界的承載力是「一格養一人」等級**；**村莊規模結構性不可持續**。
★ **這也改寫了生育那條線的意義（第三次）**：
**生育修好了（`born` 1→5），但世界養不起新生兒** —— **瓶頸是承載力，不是生育率。**

**★measurer 自己標的解析度限制（照抄，別當更強的結論用）**：
knife-edge 統計 **9／19 隊出現過「1 次高峰後崩落」**，**但那是 daily 採樣** ——
**看不到 QA 描述的 ~800 tick（≈3.3 天）sub-day 週期**
⇒ 該數字只回答「**有沒有大起大落**」，**不回答「持續拉鋸幾次」**。

**★WHAT 已裁（blueprint 2026-08-21，引意圖帳）**：**「一格養一人」不是設計意圖。**
- **野地池只養個位數採集者 ＝ 對的**（**荒涼感 ＝ 無農田之地的正確樣貌**）
- **但【農田 ＝ 獨立產糧線 ＝ 村莊超越地格的出口】**（用戶原案、農業 a 已落地）
＋ recovery arc 用戶拍「**村經濟可持續**」＋「**能苟不能興**」（小＝停滯、村＝**能興**）
⇒ **「帶頭銜的獨居者」＝ 農業出口沒接上生活，不是世界該荒。**

**裁定 ＝ 診斷先行、不是調參**（守「池空 ≠ bug」血訓）→ 已派 measurer 短票**垂死村農田帳**，
**三分流判準先寫死**：(i) 農田**沒建成** ⇒ settlement 執行 funnel（access 斷）／
(ii) 建了但**數學養不起** ⇒ tuning 議程帶用戶／
(iii) **forest 掙扎 ＝ intended 地理張力**（意圖帳 terrain 三態）★**不准用 cap 調平**。

**★粗算紅旗**（systems 查實，**先量再斷**）：`FARM_UNIT_YIELD = 2.0/日`、`FOOD_PER_PERSON_PER_DAY = 0.8/日`
⇒ **一單位農田只養 2.5 人**；**pop5 需 4.0/日 ⇒ L1 連 5 人村都養不起**。
★ 而 `resource_system.gd:14` 的註解**自己就寫著「★校準命門 — measurer 量化食物帳驗淨效應」**。
...
**寫入點清單（implementer 讀出、file:line）**：`decision_context:602` `team.need_urgency = NeedHierarchy.ewma_update(...)`（**非冪等 EWMA**）／`:606` `plan_phase`／`:233` `LaborSystem.ensure_fresh`→`rebalance` 寫 `tile.labor_alloc`＋`labor_eval_next_tick`（**cadence 重排**）／`:243-247` `idle_employ_cached`/`idle_employ_next_tick`／`:546-549` `consolidate/absorb_target_cache`＋`consolidate_eval_next_tick`（§4b 另有 `expand_*` 同族）。
...
`clear_team_faction` 出口中 **4 個無 `Probe.bump`（只 print）**：`faction_ai:5152`（起義自立脫離）/ `:5158`（起義流亡脫離）/ `:5259`（defection path B 投降強鄰 fail→clear）/ `:5262`（defection path C 獨立）。相對地 defect（`reaction_system` → `death.defect_leave`）+ betray（`diplomatic_ai:352` → `g3.betrayal`）出口**有** tap。→ 起義/自立/defection 型 faction-leave **Probe 不可見** = 憲法級觀測盲點（全量暫態可觀測性不變量 [[feedback_full_transient_observability]]）、令 story-audit 見 faction_id→-1 卻無事件留痕（measurer #3 真根）。**cheap 修**（4 出口各加 tap，如 `uprising.secede`/`uprising.exile`/`defection.surrender_fail`/`defection.independent`）+ un-blind 未來 long-game audit。blueprint 排 fix 優先序中。
...
established=0 **不是** 世界死氣的真根=**被標籤騙**。blueprint grep 驗 `is_established` 全引擎只 gate 三處：`faction_ai:248`（戰爭成本−0.3）/ `distortion:114`（唬人招牌選最大 established）/ 宣告效果=國名+公告（:5093）——**零經濟/生產/擴張解鎖=立國是章非槓桿**。因果=**強大→立國、非立國→強大**。∴established=0 是章沒蓋非世界壞；世界死氣真因=**沒人吃飽（famine）+沒人打仗（零戰死）**。#3① 立國 gate 門檻 vs leader-gen mismatch（下方定案）**技術描述仍對但降級 park**（強者上位有獨立價值=未來 arc、非現在解、用戶 b/c 選項撤回）。修正後 fix 序：①famine ②migrant belief 死角 ③零戰死（待 famine 連動：吃飽→readiness 回→打得起來）④立國 park。
