...
★★指路（implementer 2026-09-23，ebbc1e4f6）：那兩欄之一的【離家距離】現在有一份算好的實作——
   `scripts/simulation/player_api_mapper.gd` 的 `_home_distance(state, t)`（hex 距離，家＝`state.own_outpost_tile(team_id)`）。
   ★★而它現在只餵【查詢面的玩家那一隊】 ⇒ **它沒有回答本條**：本條要的是 22 隊的普查，那是量測員的卷。
   ⇒ ★★★要用就【搬出來共用】，不要照抄公式（抄一份＝兩個「離家」可以各自漂）。
