...
### ⏳★★★【未解釋】全掃的床會【godot 真的活著燒滿 604s】，而同一支床單跑 4–6 秒（2026-09-08）

**狀態：未確認** ｜ **回訪：量測窗 下一次全掃（若再現就是可重現線索）**

★這條入帳的理由是【它沒有解釋】—— 而今天已經有【三個】假說在它身上長出來又被打掉。

```
現象：12:56–13:56 六支床每一支都 604–605s + outcome=timeout
對照：同支床、同指令、14:04 / 14:33 / 14:35 / 14:55 各跑 ⇒ 全部 4–6 秒
```

**已被打掉的三個假說（每一個都有實測或 code 證據）**：
```
①資源競爭      ⇒ 否證：同一個 COLLISION-SAMEROLE 簽章下，同一支床 5 秒跑完
②外力殺子進程  ⇒ 否證：godot.ps1:238 `while (-not $proc.HasExited)` —— 子進程一死迴圈就結束，outcome 會是 ok
③死管道卡住      ⇒ 否證：Godot 的 stdout 導到暫存檔（:207-209 $tempOut），不經管道；
                   ★★更強的一層：`Pump-Out`（:239）在迴圈內、在逆時檢查（:248）之前
                   ⇒ 管道若塞住，`$deadline` 那行【永遠跑不到】⇒ 會無限期跑，不是剛好 604s
                   ⇒ 而 `outcome=timeout` 只有 `$proc.Kill()` 那條路寫得出來
                   ⇒ ★★★迴圈當時是【活的】、管道當時【有讀者】
```

★★**不要把它跟【觀察②：godot 已結束而 wrapper 卡住】綁成同一個因**。
兩者的 godot 狀態相反（一個活著、一個已死），
★★★而一旦②被驗證，①會被【順帶】當成解釋掉 —— 而它從頭到尾沒有證據。

下一步：逆時診斷檔已落地（`.sweep-timeout-<bed>-<HHMMSS>.txt`，含床輸出最後 60 行）
⇒ ★若再現，**卡在哪一行**就會直接在手上，不需要再推理。
...
## ✅★★★分片票／互動幀時儀器票 —— **永久關閉（blueprint 裁 2026-09-23，世代 8 內）**

★**狀態：✅已結案（世代 8 內）** ｜ **回訪：觸發事件 —— ★結案謂詞被推翻時【自動重開】**

```
★結案謂詞（機械、可驗）：★★世代 8 上 `[B3-FREEZE]` 兩顆種子都 PASS
  實證：docs/measurements/2026-09-23-P2-P4-P7-gen8-stagger-window.md
    gen=8 seed=1337 over2s_days=0/12 p99=257ms verdict=PASS
    gen=8 seed=42   over2s_days=0/12 p99=267ms verdict=PASS
  對照（世代 7，同一支床同儀器）：over2s_days 7/12 與 5/12｜p99 1161／1052ms
★★為什麼是【關】不是【復活】：它們當初的門票逐字是
  「分片票／儀器票 HALT 轉 defer，門票＝(A) 落地後 B3 【仍紅】」
  ⇒ B3 綠了 ⇒ **門票沒有開** ⇒ 它們要解決的問題已經被裁定 (A) 解決掉了
★★★重開條件（不是「有人想做」，是【謂詞被推翻】）：
  世代 8 之內若有任何 merge 讓 `[B3-FREEZE]` 任一種子變成 FAIL ⇒ ★本條自動重開
  ⇒ 而那時要問的第一句是「切 tick／換執行緒模型」還是「又有一個粒度不對齊」
```

★**這一條刻意留在清單上而不是刪掉**：刪掉的話，下一個遇到凍結的人會**重新發明那兩張票**，
而不知道它們被開過、被 HALT 過、以及**最後是被什麼解決的**。
...
★**用戶終審 2026-09-09：「戰亂停滯＝誠實後果」【認可】** ——
★★**而那不是本條的結案**：blueprint 當場劃線「**接受的是結果，不豁免機制誠實查點**」
⇒ **exp 死門 dump 照跑、查實的凍結閘仍然修**，
★★★**因為修完才知道「停滯」裡面【戰亂真實貢獻佔多少】** ——
...
## 📺 **ticker 洩漏內部負數 ID**：觀眾看到 `Team-1000004`（2026-09-10，用戶親測貼出）

```
用戶貼的 ticker 段：同一個事件出現【兩行】——
   `Team-1000004`（★beast pseudo-team 的內部負數 id）與 `Team26`
⇒ ★觀眾應該只看到一行人話：「Team26 被野豬伏擊」
⇒ ★★`Combat Start` 那類【內部行】應該被人話層【吞掉或改寫】，而不是原樣送到觀眾眼前。
```

★**它與「已滅團宣戰」是【同一段 ticker】揭出來的兩件事，但【不是同一個病】**：
```
已滅團宣戰   ＝ 產生端的問題（一 tick 寬的競態）⇒ is_live_actor 那張票
★本條       ＝ 【呈現層】的問題（內部識別字漏到人話層）⇒ C1 票② 同族
⇒ ★★★分開記，免得「修好了宣戰」被讀成「ticker 也乾淨了」。
```

**狀態：已知未修** ｜ **owner：systems（HOW，呈現層）** ｜ **回訪：觸發事件 —— C1 票②（GUI／呈現層）開工時**
（★blueprint 裁「非急」；★★而它便宜到不值得單獨開票，該在票②順手做掉）
...
## 🔁 **兩個【位置謂詞】可能互為因果**（假說，blueprint 立 2026-09-10；★同日發現的兩個病扣在一起）

```
①`is_resident_static`（faction_ai:600-614）＝ 有 TAG_PRODUCE ＋【此刻站在自家 outpost 上】
②公庫可見性（faction_ai:4607-4611）      ＝【leader 此刻站在自家 outpost 上】才讀得到 vault
⇒ ★兩個都是【當下位置】的謂詞,而它們指向【同一個位置】。
```

★**推測的環（★★而它是假說，不是坐實）**：
```
領袖常在外  ⇒ 公庫隱形（vault={}）⇒ 建造頻繁判「資源不足」⇒ 據點不發展
           ⇒ 沒有值得待著的村子 ⇒ 領袖更常在外
```
★★**而它與【停滯自鎖迴圈】是同型**（兩個機制互為對方的前提），
★★★**但這一條的兩端【都是位置謂詞】** —— 而位置謂詞的特性是：
**它們在任何一次快照上都可能是 0，而【沒有任何狀態是錯的】。**

★**證據現況**：用戶貼的那行是「material 有 5（**公庫 0** ＋私 5）」
⇒ ★★「公庫 0」**可能是真的 0，也可能是沒讀到** —— 而那一行**分不出**（已另記）。

**狀態：未確認（假說已具名，兩端各有一格在量）** ｜ **owner：blueprint（WHAT）** ｜
**回訪：量測窗 —— 居民【身分 vs 位置】分離格
（票 `docs/superpowers/handbacks/2026-09-10-systems-to-measurer-separate-identity-from-position.md`）
★屆時「有身分但不在家」的比例，同時是這一條的第一個數字。**

## 🏘️ ~~**這個世界沒有「村莊」那一層**~~ ⇒ ★**分離格回來了：村莊【有】，是【居民持續不在家】**（2026-09-10 同日訂正）

```
狀態：已知未修（★世界層：村莊有，而【多數生產隊無家可歸】——已進定居設計窗第一題）
回訪：定居設計窗開場時
★★訂正（量測員 resident-identity-vs-position 卷，2026-09-10）：
   村莊（TAG_PRODUCE）**健康成長**：day10 = 2 隊 → day60 = **22 隊**
   而 day60 的分離讀數：**不在家 13 隊 > 在家 9 隊**
⇒ ★★★所以是【(b) 居民不在家】，**不是 (a) 村莊沒出生**。
★★★★**而「不在家」還要再分一次**（blueprint 2026-09-10，★同日第三層訂正）：
   **「不在家」可能只是【在田裡】** —— 這張圖最近目標 2-3 格，
   生產隊在快照時刻落在田間半徑內採集 ＝ **健康村莊的日常**，
   ⇒ ★那是【謂詞太嚴】不是【世界有病】。
   ⇒ **在補上「離家距離 ＋ 當下 task」兩欄之前，不得說「這是決策層的問題」**
     （★我在本條的第一版就寫了那句 —— ★★而它跳過了這一層，已訂正）。
⇒ 下一刀（已派量測員）：幫「不在家的 13 隊」補兩欄，分
   **田間半徑內（≤2-3 格，幹活）＝非問題** vs **遠派（faction 差事／貿易遠行）＝才進決策層查**。
⇒ 本條原標題（「沒有村莊那一層」）**作廢**，留字不刪：
   ★它是「拿一個【位置謂詞】的讀數去描述【存在】」的血證，
   ★★而當時我自己已經寫下「兩種成因分不出」那一段（見下），**分離格證明那個保留是對的**。
★★★**而這一條目本身變成一個教材**：同一個位置謂詞被【連續訂正三次】——
```
①is_resident_static 讀 0-1  ⇒ 我說「這個世界沒有村莊那一層」
②分離格           ⇒ 村莊有（2→22），是【居民不在家】
③blueprint        ⇒ 「不在家」可能只是【在田裡】
⇒ ★通則：**一個位置謂詞的每一次【否定讀數】，都要再問一次「否定的是什麼」**——
  ★★而每問一次，母體就換一次；★★★三次都是【謂詞比世界嚴格】，不是世界壞掉。
```

★而 `DISTRIB_RELIEF_REF_POP` 那一族的 bound【要重讀】：
   母體不是不存在，是**它們不在家** ⇒ ★relief 接 believed 規模這個方向**有對象了**，
   ★★但要先答「領主相信的規模」該不該算不在家的人。
```

## 🏘️（以下為原條目，留字不刪）自家居民母體恆為 0-1（2026-09-10 belief 覆蓋率卷，★問題問錯層才發現的）

```
量測（60 天窗、5 次快照 day10-60，領主 9→13 隊）：
   ★【自家居民】這個群體的母體【恆為 0-1】
...
★**它 bound 住的（比本條本身重要）**：
```
批一④ DISTRIB_RELIEF_REF_POP ／ relief 接 believed 規模
   ⇒ ★blueprint 預註冊的 WHAT 方向（「relief 隨領主相信的聚落規模變」）【目前沒有母體】
   ⇒ ★★不是那個方向錯，是【那個世界裡沒有它要作用的對象】
「地形自給上限 10 人」（同日坐實）
   ⇒ ★★★兩者相扣：大隊【不能靠地活】，而定居居民【又幾乎不產生】
   ⇒ ★★★【降級 2026-09-10】：我原本寫「這個世界是【遊牧的】」——**那句話超出證據**。
     `is_resident_static` 是一個【當下位置】的謂詞（身分 ＋ 此刻站在自家 outpost 上）
     ⇒ 母體 0-1 有兩種成因而卷面分不出：**(a) 村莊沒出生**／**(b) 居民不在家**
     ⇒ ★**在分離格回來之前，正確的說法是「村莊那一層在【這個謂詞】的讀數上是 0-1」**，
       ★★**不是「這個世界是遊牧的」**（blueprint 對用戶說過同一句，也同步降級）
distribute／relief／農田／糧倉 這一整族機制
   ⇒ ★它們的消費者是「居民」，而消費者母體是 0-1
```

★★**方法上的一筆（值得記）**：這一格是**被我票裡「先報母體、再報比例」那條要求逼出來的** ——
★★★**若照原問題只回報「覆蓋率」，答案會是「100%，很好」**，
而**真相是 n=1**。⇒ **「先報母體」不是格式要求，它是【防止一個比例把分母藏起來】的唯一辦法。**

**狀態：未確認（★母體讀數已坐實，而【它量的是哪一件事】待分離 —— 見上方降級段）** ｜ **owner：blueprint（WHAT）** ｜
**回訪：量測窗 —— 「為什麼居民不產生」的成因 dump（★尚未開票，等 blueprint 裁序）**

## 🧱 **地形的自給上限【剛好】是 10 人**：`REGEN_RATE.plains.food = 8.0` ÷ `FOOD_PER_PERSON_PER_DAY = 0.8`（2026-09-09，養活力卷坐實）

```
養活力 predicate（既有定義，decision_context.gd:641-643 GATE-A）：
   regen = REGEN_RATE[terrain]["food"] × tile.harvest_factor ｜ burn = pop × 0.8
實測（measurer，4 種圖尺寸 × 2 seed）：
   pop = 3  ⇒ ★到處都活
   pop = 10 ⇒ ★★剛好卡邊界（plains regen 8.0 == burn 8.0）
   pop = 30 ⇒ ★★★【100% 無解】—— 不是「養得活的地方很遠」，是【整張圖沒有一格養得活它】
```

★**兩個常數的乘積剛好等於 10 人**：`8.0 / 0.8 = 10`（`resource_system.gd:56` 與 `:3`）
⇒ ★★而它們**看起來是各自獨立設的**（一個是地形產出、一個是每人日耗）
⇒ ★★★**這個世界的「一塊地養得起幾個人」是一個【沒有人寫下來的設計上限】，而它是 10。**

★**因此「地理稀缺」在這個世界【存在】，但形狀不是【距離】而是【規模上限】**：
```
上一卷（存在性距離）：到最近有食物的格 p50 0-1 ⇒ 「哪裡都有」
本卷（養活力距離）  ：pop>10 ⇒ 【哪裡都不夠】
⇒ ★★★同一張圖,兩個 predicate,兩個相反的結論 —— 而【問題問的是後者】。
```

★★**它 bound 住的東西（比本條本身重要）**：
```
有大有小 arc   ★規模不是優勢是【負債】：每多一個人就更不可能自給
人口 arc       ★★人口成長在【地形上沒有出路】——長大只能靠 granary 存量/貿易/掠奪
opening_granary_food = 800   ★★★這解釋了它為什麼那麼大:它是【唯一能養大隊的東西】
                             ⇒ 而「被初始參數凍結的閘」那條也連上了
後勤/貿易      ★它們不是「效率選項」,是【大隊存在的前提】
```

★誠實限（照 measurer 卷面）：本卷量的是**這塊地【自己】養不養得活**（raw regen），
**不涵蓋** granary 存量、貿易、掠奪。⇒ ★**它不是說「大隊會餓死」，是說「大隊【不能靠地】活」。**
★★與 `GATE-A 離家 positional 餓 56-61%` **不是同一件事但同方向**：
本卷**更上游**（即使不離家，規模 > 10 就不夠）。

**狀態：已知未修（★刻意 —— 已坐實的世界性質 ＋ 待裁的設計 fork）** ｜ **owner：blueprint（WHAT，拿讀數呈用戶）** ｜
**回訪：觸發事件 —— 用戶對「規模該不該有地形上限／要不要讓地理成簇稀疏化」給出方向時**

## 🌾 **「去哪裡採」不是一個決策**：最近的同地形目標只有 2-3 格（2026-09-09，批二① 順手量到）

```
實測（implementer，批二① 驗收②的分解，三個世界）：
   最遠的「最近同地形目標」＝ 2-3 格；而最小搜尋半徑 14 格（新公式）／舊常數 30
   ⇒ ★上界差 5-7 倍 ⇒ 搜尋半徑【新舊都從來不 binding】（穿透 0/204、0/18、0/42）
```

★**這不是搜尋半徑的問題，是【地形密度】的性質**：任何隊、任何時候、任何地形，**答案都在隔壁**。
⇒ ★★連到三條在飛的線：
```
有大有小／規模經濟   地理不造成差異 ⇒ 【位置不是一種資產】
後勤統一             採集點永遠在隔壁 ⇒ 運輸的存在理由要來自別處（人口/設施密度）,不是距離
批一① 移速逐隊化     ★★★我們剛讓移速差 6 倍,而如果所有目標都在 2-3 格內,那個差異化的舞台很小
```

★★★**而它也解釋了為什麼批二① 是【潛伏的正確】**：機制對（半徑與真速度同源），
但**在目前的地形密度下不咬人** —— 它會在**地形變稀疏**時開始咬，
而那正是「有大有小」arc 想造出來的世界。**⇒ 不 revert：revert 會讓下一個人重做一次，
而且下次他不會知道我們量過。**

★★★**已坐實（2026-09-09 地形密度卷，`docs/process/verdicts/terrain-density-distance.measure.json`）**：

```
到最近【任一 terrain】距離：p50 = 0-1／p90 = 1-2／max = 2-4／★>5 格比例 = 0.00%
   ★★而這在 radius 14 / 16 / 24 / 40（631→4921 格，8 倍面積）【全部同一形狀】
   ⇒ ★★★是【同密度鋪更大】不是【同數量攤更開】
   ⇒ **單純把圖畫大【不會】製造出值得跨越的距離** —— 那條旋鈕是死的。
兩種母體（每格 / 每隊）結果相同 ⇒ 不是「隊剛好都站在好地方」。
```

★**唯一真的稀的是礦**：`p50 = 3／max = 7-8／>5 格比例 1-9%`，
而它稀的原因**不是 mountain 地形**（mountain 佔 20%），是
`world_generator.gd:92-94` 的 **mountain × 12% 機率**才帶 `ore_gold`。

★★★**因此這個世界有【兩種稀缺模型】，而只有一種會長出地理**：
```
terrain 型   RESOURCE_PROFILE 三地形下限【都 > 0】⇒ 食物/材料【哪裡都有】⇒ 不產生距離
機率點狀型   mountain × 12% ⇒ 礦【真的有位置】⇒ ★這是目前唯一「去哪裡」是決策的資源
```
⇒ ★**今天所有距離接線（移速逐隊化／搜尋半徑／convoy）的舞台，目前只存在於礦這條線上。**

**狀態：已知未修（★刻意 —— 它不是缺陷，是【已坐實的世界性質 ＋ 一個待裁的設計 fork】）** ｜
**owner：blueprint（WHAT，拿讀數呈用戶）** ｜
★**（記一個詞彙缺口：`ki-status-gate` 的三選一是 `已知未修／未確認／已知未實裝`，
而【已坐實的世界性質，等設計裁決】沒有對應值 ⇒ 只能借 `已知未修（刻意）`。
★★不在這一輪擴充詞彙——那是 schema 改動，而一次只動一個變因。）**
**回訪：觸發事件 —— 用戶對「要不要讓地理成簇稀疏化」給出方向時**
...
**門檻與世界實際的量差一個數量級以上 ⇒ 結構性不可達**。
★★★**而它是【雙向】的（systems 2026-09-09 補，第二個實例當天出現）**：
```
門檻【低於】世界的量   本條：home_restock_min ≤40 vs opening_granary_food 800  ⇒ 閘永遠不擋
門檻【高於】世界的量   SEEK_TILE_RANGE = 30 vs 全圖 hex 距離上限 28（radius 14）⇒ 閘永遠不擋
```
★**兩個方向的症狀一樣（那一行 `if` 從來沒 fire 過），而看出它的方法也一樣：
先算【世界的可達區間】再看門檻**（界限第八條）。
...
`_evaluate_independent_infrastructure` 的第一道門是 **`faction_ai_system.gd::_evaluate_loop2_teams()` `_find_own_outpost` == -1 就 return（★錨更新 d710c5eee：原 `_evaluate_all_body` 已拆三份，符號改名非語意改變）**，
...
- **★compute top:`faction_ai_system.gd:625 evaluate_all` 忽略 LOD 參數 → O(N²)（2026-07-05 lod_perf_bed 量化坐實）**：`evaluate_all(state, _team_ids)` 的 subset 被 `_` 忽略、`faction_ai_system.gd::_evaluate_loop1_factions()` 對**全 factions×全 member_team_ids** 跑（★錨更新 d710c5eee：原 `_evaluate_all_body` 已拆三份，符號改名非語意改變）（非 near subset）→ faction AI 成本隨總隊數長,LOD 沒 gate。**perf 曲線（seed1337,2月,mean 攤銷）**:21隊 LOD 2994us(334tps)/full-HD 9035us / 41隊 7295us(137tps)/23659us / 107隊 49260us(20tps,max 6.7s)/137747us(7tps,max 7.4s)。**指數~2.0=O(N²) 鐵證;LOD 僅 3× 常數因子（movement/vision LOD-gate 給的,O(N²) 大頭沒 gate）。41隊(warring 自然上限)LOD 已 137tps<240+1s hitch;107隊(強塞 config)兩 regime 全垮**。修=bound faction AI（honor-LOD / 空間分區 / cadence 攤）=獨立 perf arc,規模野心大才值(藍圖裁目標規模,報 `2026-07-05-systems-to-blueprint-lod-perf-data`)。~~`_has_hostile_within` 每隊掃全隊~~ **已修（用 `state.teams_within` 空間索引,此條 stale 劃除）**。
...
- **E-1 弱隊殺不光 / 對攻擊免疫**（高，世界無法收斂）。`armed_anon_ratio=0` 隊 → encounter spawn 0 匿名只 1 named 接戰；normal 遭遇戰**不減隊 pop**（只擊退上場單位）→ 打贏隊 pop 原封不動 → 弱隊無限被刷但殺不死。根因：normal 遭遇戰無「敗方 pop 損耗」機制 + 未上場 unarmed pop 不受傷。修向需設計：敗方 pop 損耗 / 強制 subjugate-or-flee / 武裝率下限。
  ...
  - **分叉解剖（2026-06-19 #3，③ 已挖→證實）**：戰鬥**兩條路徑 explicit by design 分叉**（`ambush:60-66` 註明「Bug9：NPC 不走 encounter」）：
    ...
    - **npc_combat（抽象）**：觸發者 = `interaction:248-260`（NPC 遭遇）、`faction_ai_system.gd::_evaluate_independent_strategy()`（★錨更新 d710c5eee：原 `_evaluate_all_body` 已拆三份，符號改名非語意改變）、`npc_combat_system.gd::_apply_casualties()`（NPC 分支）。`_apply_casualties`(:404)→`wound_random` 打**全 anon pool（無 cap 免疫）**、`_kill_named_npc`(:451)→`on_leader_death`(:456) **有 anon 晉升 fallback**。
...
- **★far 區移速稀釋 10×（世界模型級,2026-07-04 貿易漏斗定罪,裁權=藍圖+系統合裁：修法 HOW 我有,但世界節奏×10=平衡意圖 WHAT+gen 重校=藍圖題,已報 `systems-to-blueprint-lod-carrier`）**：`movement_system.process` `move_tick_acc += TICKS_PER_HOUR` 硬編,但 far 區每 `FAR_ZONE_INTERVAL`(100 tick) 才跑一次 → far 隊 1 hex≈3 天（10× 稀釋）。無玩家世界**全隊=far** → 跨格物流全癱：**envoy 馬鏈 6 月未貫通 + 貿易旅程永不到場同根**（藍圖「一修雙解」假說 ✅ 實測定罪）。姊妹系統（collect/consumption）皆傳 elapsed,唯 movement 不一致；違「大地圖與遭遇戰共用時間尺度」invariant。**修法已驗**：process 收 `elapsed_ticks` 參數（near=NEAR_CADENCE、far=FAR_ZONE_INTERVAL）→ seed1337 6 月成交 6→30、TRADE 到場 0→43(33.9%)——**但世界節奏×10 → pop 172→68(-60%) 塌房=gen 校準全失效** → revert,修須配套節奏重校準（FAR_ZONE_INTERVAL/移速常數/gen 參數一起裁）。diff 見 handback `2026-07-04-trade-loop-ignition`。
  ...
  ★★★★**【2026-09-06 CLOSE：⑧ 已 merge ⇒ 沒有 far 區，就沒有 far 降頻】** —— 本條的載體（`FAR_ZONE_INTERVAL` 分班）**已退場**；★**而「世界節奏×10 需 gen 重校」那個顧慮隨之消滅**（現在全世界同一個 cadence）。★★原文保留：它記錄了「一修雙解」假說**曾被實測定罪又因塌房 revert**的完整過程。
...
★config/peaceful_economy.json 手寫 12 隊:11 隊 ["統領","生產"] ＋ ★1 隊 ["商隊"](id=8)
   而 team8 帶 coin 1000
   ★★★【2026-09-06 訂正:我先前寫「全床唯一帶 coin 的隊」是【錯的】】——
   config 12 隊【每一隊都有 coin】(800×6／300×2／1000×1／200×3,總量 7000)
   ⇒ team8 的 1000 只佔 14%,它是【最多的那一隊】不是【唯一有錢的那一隊】
   ⇒ ★而「1000 三個時間點不變」這個事實【不受影響】,受影響的是我給它的【修辭份量】
...
## ✅★★★★裸 `current_tick % INTERVAL` 在 LOD far pass 下會【週期性漏拍】—— 而它是距離依賴的世界扭曲（2026-09-05，★2026-09-23 結案：前提消失）
★**狀態：✅已結案（2026-09-23，前提消失）**｜~~回訪：觸發事件 —— ⑦票（遷 `CadenceStagger`）落地~~
★★★**systems 自訂（2026-09-23）**：我先前把狀態欄改成「★前提已消失」——
而那**不在 `ki-status` 閘的三選一枚舉裡**（`已知未修`／`未確認`／`已知未實裝`）⇒ **閘當場紅**，
而它是在【500 個 commit 之後】才被一支電池抓到（runner 抬頭自己印著「main 基線量於 2026-09-22」）。
⇒ ★改成標題掛 `✅`（閘對已關條目的既有豁免），**而不是把狀態欄改成閘看得懂但語意錯的值**。
...
```
### ★★★★它要買回的缺口有多大（2026-09-06 量到，blueprint 要求具名進本條）
```
★realtime 1× 播放目標 ＝ 240 tps
★★warring_states(49 隊) 在【⑧ 之前】＝ 46 tps ／【⑧ 之後(投影 1.87×)】≈ 25 tps
⇒ ★★★缺口 ≈ 9.6×
⇒ ★所以本條要買回的【不只是 ⑧ 的 1.9×】,是【通往可玩 realtime 的整段】
   —— 而 ⑧ 只佔其中一小截(1.87/9.6 ≈ 19%)
★★而「跑不動」是【既有的】:⑧ 之前就 46 tps ⇒ ★★★它不進 ⑧ 的帳,⑧ 不為它負責
★誠實限:46 tps 那個數字來自【3 日窗、49 隊】;85 隊 50 tps／99 隊 25 tps 是被砍長跑的殘料
   ⇒ 同向但【不是同一組量測】,而缺口的量級(~10×)不因此改變
```

★**為什麼現在不做**：⑧ 的代價（**★訂正：warring 實測 1.87×，而 2.64× 是 15 隊小世界的上界**）已由 blueprint 裁「買」——**憲法不打折**。
...
```
狀態：已知未修（spec 已鎖、R² CLEAN，等體驗窗收完再 dispatch）
回訪：體驗窗收尾時
...
## 📊 `driver_ledger` 溢出丟棄 277 萬筆 ⇒ ★它的累計數字【不可引用】（2026-09-10）

```
狀態：已知未修
回訪：下一次要引用 driver_ledger 累計量時
量測員回報：④ 累計 settle/convert ＝ 0/0 —— ★而那個 0 是【溢出丟棄】造成的，不是世界的 0。
⇒ ★★任何從 driver_ledger 算出來的【累計量】目前不可信。
⇒ ★★★而它的失效形態是本日反覆出現的那一種：**儀器安靜地降級，而輸出看起來正常**
   （0 與「沒發生」長得一樣）。
處置：★未修。使用前必須先確認它有沒有溢出；
      ★★真要用累計量，改用【不會溢出的通道】（逐日聚合／無條件 print）。
owner=systems（掛號，未開票）
```

## 📏 fp 的【子層級】盲區第一次有數字：`TeamData` 97 欄沒看到被讀（2026-09-10）
...
⇒ ★★★「fp 相同」的重量【又降一級】：它先前的正確讀法是
  「fp 看得到的那部分沒變」，而現在我們知道【看不到的那部分包含會影響世界產出的欄位】。
```

★**它回頭影響今天好幾張票的【證據強度】（★不是結論錯，是強度要標明）**：

```
今天多張票寫過「fp 兩棵樹相同 ⇒【只加入口／只加印，沒改機制】有實證」
（C1 票①／帶因／is_live_actor／群乙）。
⇒ ★那句話的正確版本是：**「沒有改到【fp 看得到的那部分】」**。
⇒ ★★而那些票實際改的東西（查詢面入口、print 參數、謂詞抽名）
   **本來就不太可能落在 TeamData 的那 97 欄裡** ⇒ 結論多半仍然成立，
   ★★★**但【多半】不是【有實證】** —— 措辭要降級，我是寫下那句話的人。
⇒ 處置：往後 fp-identical 的宣稱一律寫成「fp 覆蓋範圍內無變化」，
  ★並附【本次改動有沒有碰到排除清單裡的欄位】那一句。
```

**★不修**：`TeamData` 那 97 欄要不要進尺，是逐欄判斷（很多本來就該排除，
例如 ephemeral 快取／cadence 排程欄）⇒ **本條只記錄【它們現在是具名的】**。owner=systems

## ⏱ fp 計算成本 9.2 ms／次（3 隊小世界）⇒ ★**它不適合當【每 tick 的守衛】**（2026-09-10）

```
狀態：已知未修（★它不是 bug，是【定位】：merge-gate／床級儀器，不得當 per-tick guard）
回訪：下一次有人想把 fp 放進每 tick 時
量測（implementer，欄位擴張 30→103 之後）：9.2 ms／次 ＠ demo（3 隊）
★而它【隨隊數成長】—— warring_states 那種 60-100 隊的規模會更貴。
⇒ ★★fp 的定位：**merge-gate ／ 床 級別的儀器**，不是 per-tick guard。
⇒ ★★★而這一句要跟【可慢不可卡】一起讀：把 9.2ms（且會長大）的東西放進每 tick，
...
★★★第三支（systems 2026-09-11，順手撞到）：`docs/measurements/.construction-duration-source-gate.txt`
   —— 同一個病，而這次它是【**在 merge worktree 裡以未 commit 改動的形態**】出現的：
   7 列全部只有行號變（`faction_ai_system.gd:6520→6577`、`outpost_system.gd:563→565` …），
   判定欄**全是 OK**、呼叫文字**一字沒變**。
⇒ ★所以它不是「兩支」，是**一族**：凡是把【某一行】當身分的 baseline 檔都會漂。
⇒ ★★而第三支的症狀更陰：它不會讓閘變紅，它讓**工作目錄永遠是髒的**
   ⇒ 下一個人 `git status` 看到雜訊 ⇒ ★★★而「髒」正是我自己那條
   『merge 前要確認乾淨』判準的輸入 ⇒ **一支亂咬的閘會讓另一支閘的前提失效**。
⇒ 修法同上（key 換成 file ＋ 正規化 code 文字）；★三支一起改，別一支一支修。

★★★第四支（systems 2026-09-11，同族但**不是行號**）：`headless-regression` 的**清單 baseline**
   —— 它的 key ＝ **斷言訊息全文**，而那段訊息裡**嵌著會漂的世界狀態**。
   血證：唯一的 diff 是 `task=投靠` → `task=外交`（同一條斷言、同一個失敗、數量 3＝baseline 3）
   ⇒ ★**世界沒壞，是那支隊那一刻在做別的事** —— 而它讓整支閘判紅。
   ⇒ ★★而這一格特別值得記：那個 `task` 之所以變，**正是因為這批改動讓被擋住的選項能被選上了**
     —— ★★★**閘把「我們修好的那件事」判成了回歸。**
   已修：正規化多一條 `gsub(/task=[^ |]+/,"task=T")`（原本就已經把數字換成 N，同一個道理）。
⇒ ★通則（四支共有）：**baseline 的 key 不可以含【會隨世界或編輯漂移】的欄位** ——
  行號會漂、訊息裡的世界狀態也會漂，而兩者的症狀一樣：**閘對每一次正常改動都亂咬**。
```

## 🔬 **Probe 開銷隨世界長大而放大（+6.0% @600t → +13.9% @6000t）⇒ 開著儀器量到的牆鐘不可拿去談出貨效能**（implementer 量，2026-09-10）

```
狀態：已知未修
同窗同 config 同 seed，`RC_PROBE=0/1`：
   6000 tick   ON 215.13 s ／ OFF 188.92 s ⇒ **+26.21 s ＝ +13.9%**
    600 tick   ⇒ **+6.0%**
★而它【不改世界】：600 tick ON/OFF 的 fp 相同（34110029e24ea79da457a0212388b796）
  ⇒ 牆鐘差是【儀器差】不是【兩個世界的差】。
★★而歸屬要分兩個尺度，★★★**兩個結論長得一樣而處置相反**：
   【被歸因的那條路上】rank_scored 內的 Probe ＝ 0.152 s ＝ 130.7 s 的 **0.12%** ⇒ 清白
   【全 sim】26.21 s 是所有 Probe 的總和（逐日逐隊 bump／相位字典／各系統 tap）
⇒ ★所以：**相位歸因的結論站得住，而「這個世界跑多快」的絕對數字不可引用**。
★★★而最要記的是【它不是固定百分比】：600t 6.0% → 6000t 13.9%
  ⇒ 用小窗量到的開銷去外推大窗，會低估。
回訪：下一次有人拿牆鐘數字談「玩家會不會卡」時
```

## ✅ ~~**拆除據點沒有清掉個別設施等級**~~ ⇒ ★★★**全文撤回：前提不成立**（2026-09-11）

```
狀態：已撤回
★實測（implementer 床）：拆前 farming=2／weaponsmith=1 ⇒ 拆後 **0／0** ⇒ 重建後 **0／0**
★★而我自己讀了 code 確認：`outpost_system.gd:507-508`
     `for fac_name in FACILITY_DEF: tile.set(FACILITY_DEF[fac_name]["current_level_key"], 0)`
  ⇒ **demolish 當下就歸零所有設施等級**，而它自 **2026-06-12** 就在。
★★★而這條假的條目是怎麼來的：R² 讀的是 `:490-497`，
  而那個歸零迴圈在 **`:507-508`—— 剛好在他讀的那扇窗外面**；
  ★而我【沒有自己驗】就把它寫進待辦、寫進規格、寫進兩封信。
⇒ 教訓（界限第 48 條）：**【X 不存在】這種負斷言，證據範圍必須是【整支函式】，不是一段行號窗**。
```

## ✅ ~~**世界只在【已經發生】之後才喚醒**~~ ⇒ ★★★**撤回：靜態為真、行為上無後果**（implementer 採、systems 立條目 2026-09-12）

```
狀態：已結案（blueprint 裁 2026-09-12）
★★★**撤回的是因果鏈**：「相遇不發事件 ⇒ 那一刻沒人思考 ⇒ 決策晚到」
  血證（大母體）：**碰面那一 tick 有跑決策 124／125**｜碰面後一小時內一次都沒跑 **2／76**
★**沒撤回的**：`WorldEvents.emit` 23 個呼叫點**無一是相遇**（靜態事實仍成立）
  ⇒ ★★**靜態為真、行為上無後果** —— 他們本來就在那一刻思考，只是不是被「相遇」叫醒的。
★過濾器那兩把刀（路過 59%／雙方無可見武裝 29%）**留在庫裡** —— 若哪天要壓尾部，刀是磨好的。
★裸符號全庫掃 `WorldEvents.emit(` ⇒ **23 個呼叫點**（debug 7），production 16 個逐字列出
  ⇒ **沒有一個是「相遇」**；最近的 `npc_combat_system:119 combat_engaged`
  ★★而那是**已經開打之後**。
★喚醒只有一條路：`pending_rethink` 的**唯一寫入點就是 `emit` 自己**。
⇒ ★★★推論（**待 runtime 確認**）：「兩支敵隊面對面而沒人動手」
  **可能根本不需要有人決定不打** —— **那一刻沒有人在思考**，
  而等下一次輪班時，**對方可能已經走了**。
血證（旁證）：面對面 dump 的**母體是 1** —— 四個條件里【剛好輪到決策】把母體殺光。
★**不准直接開藥**：「讓相遇發事件」＝**每次相遇喚醒雙方**
  ⇒ ★★**先量「相遇一天幾次、一個 tick 幾次（p50/p95/max）」**
  ⇒ ★★★若相遇很頻繁 ⇒ 那不是修，是**喚醒風暴**（前例：大世界 p95 是 max 的五倍）
    ⇒ 真正的問題會變成**「哪一種相遇值得喚醒」** —— **那是設計題**。
★影響面不只打架：**送信、求居、貿易、招攬** ——
  所有「要在對的時刻碰到對的人」的事都踩在同一個節律上。
★★★**WHAT 已擺好**（blueprint 2026-09-12）：**【相遇 ＝ 思考時刻】是北極星**，方向不必再裁；
  形狀看「一天幾次」。若相遇海量，喚醒過濾器的**硬條件**：
  ①**只准外觀層便宜謂詞**（可見武裝／規模／逼近）—— **禁讀 tag／意圖**（感知鐵律）
  ②★★**預設醒、具名靜** —— 過濾器是**具名的靜音清單**，不是具名的喚醒清單
    （不變量原句：**沉默的預設要落在安全側**）
  ★systems 加的一條界線：**「敵對與否」可以當【量測的分析欄】，但不得當【過濾器的判準】**
    ⇒ ★★觀察者可以讀 god-view，**世界裡的機制不行**；
    ⇒ ★★★而這一條**很容易被偷渡**：分析欄用久了會變成「那就照這個篩」。
回訪：runtime 兩個數（碰到面那一 tick 有沒跑決策／之後 N tick 內幾次）＋相遇頻率（含外觀層分桶）回來時
```

## ✅ **「迎戰」736 次只打 1 次** ⇒ ★★★**已結案：根在【門】與【秤】兩層的授權，兩層都已 merge**（implementer 量、systems 立條目 2026-09-11）

```
狀態：已知未修
~~before **8／571**｜after **1／736**~~ ★**母體灌大 4.8 倍，已重切**（2026-09-12）
★★真值（193 段迎戰，fp 逐字相同 ⇒ 同一個世界、只換切法）：
  **碰到了但沒開打 60.1%**｜沒有碰到面 **30.6%**｜沒有目標 6.2%｜碰到且開打 **2**
⇒ ★★★**主因在【決策那一側】，不是相遇機制** ⇒ 「相遇＝單一甓頸」在本案**被否證**。
★而這個結論**通過了一次會殺死它的檢驗**：母體改 4.8 倍、最大那一格整格作廃，
  **而排序沒翻、還從 42% vs 22% 拉開成 60% vs 31%**
  ⇒ ★★**穩健性檢驗不是為了推翻，是為了知道結論有多硬**。
★所以「迎戰變多而開打變少」**不矛盾：那是兩條幾乎不相交的路**。
⇒ ★★真正的問題：**那 736 次迎戰，最後都變成什麼了？**
  —— 對峙？散開？還是**什麼都沒有**？
⇒ ★★★**這不是效能題，是戲的題**：一個**擺出姿態然後什麼都沒發生**的世界，
  跟一個**會打起來**的世界，是兩種戲。
★~~附帶：「開打 391 → 81」的來源仍然不明~~
  ⇒ ★★★**作廃（2026-09-15，多 seed 定案）：「開打 391 → 81」本身就是【蝴蝶】**
    三個 seed 的**方向不一致** ⇒ ★**那個「−79%」不是一個現象，是一個樣本**
    （fp 兩趡逐字相同 `e78693df…` ⇒ gen4 三趡可合為一臂 ⇒ 表可讀）
    ⇒ ★★**所以「來源不明」這個問題本身也消失了** —— **沒有一個穩定的現象在等被解釋**。
  ★★★**而【迎戰 736 次只打 1 次】不受影響** —— 它是**同一趡內的比例**，
    不是兩趡之間的差 ⇒ **它不是蝴蝶能碰的東西**。
★★★**已併入【信使手術】同案，不另開票**（blueprint 裁 2026-09-11）：
  三案（信使送達／求居見領主／迎戰接戰）**共用同一台相遇機制**
  —— `interaction_system.gd` 的 pairwise 相遇：送達在 :390-393/:421-425，
  而 `start_combat` **7 個呼叫點裡的 4 個**在 :434/:436/:444/:453。
  ⇒ ★**送信、見面、開打共用同一個【兩支隊真的碰到面】的判定**。
回訪：信使手術那卷出來時（同一份）
```

## ⏱️ **單幀凍結：終線 ＝【單幀 > 2 秒的次數 ＝ 0】**（blueprint 轉達用戶包絡 2026-09-11）

```
狀態：已知未修
★現況（Probe=ON、90 天窗）：**max 26.65 s → 11.86 s**（2.25 倍改善，**問題還在**）；週期性沒變（每小時一次）
★★而【終線不是倍數改善，是一個計數歸零】：**單幀 > 2 秒的次數 ＝ 0**
★★★而那個計數第一次量到了：**307 / 43200**（2026-09-11，90 天窗、Probe=ON）⇒ **終線 0，距離 307**。
次句：兵家位移 —— 舊卷的頭號已不是第一，新頭號是 `loop3.misc`（佔 spike 樣本 86.3%）
  ⇒ ★**一個叫「其他」的桶排第一 ＝ 尺的解析度不夠** ⇒ 下一刀是【切開它】不是優化它。
★而 Probe 開/關對照**尚未完成**（OFF 趟被 contention 污染、已請重跑）
  ⇒ 它只影響「11.86 s 裡有多少是儀器」，**不影響「問題還在」**（儀器只會讓數字更大）。
回訪：`loop3.misc` 切開之後
```

## 🚶 ~~**出發的遠行有四分之三到不了**~~ ⇒ ★★★**23.3% 已降級為【污染基線、不可引用】**（blueprint 裁 2026-09-11）

```
狀態：已知未修　　（★尺那半已修：逐任務判準已裁定並重量；★★世界那半未查，已轉入「信使 2.2%」那一條開刀）
  ★而「信使 2.2%」已另立條目開刀；本條保留是為了記錄**那個被污染的數字曾經被相信過**。
回訪：2026-09-18（信使那張票落地後回頭確認本條的降級標註仍然正確）
★為什麼不可引用：那個數字用的是**一把全域尺**（episode 期間任一 tick `tile_pos == move_target`），
  而**每個任務的【成功】不是同一件事**。已坐實的一格：
  紮營 **0%** ⇒ `faction_ai_system.gd:6297-6300` + `:6442-6447`：
    紮營成功 ＝ **腳下那格能立營**（全函式沒有 `move_target`）⇒ 走到半路就立營 + release
    ⇒ `tile_pos == move_target` **永遠不成立** ⇒ ★**0 是真的 0，而它答的是另一個問題**。
    ★★而【走到哪好就紮哪】本來就是**機會主義設計**（blueprint）—— 世界沒毛病。
★★★**新判準 ＝ 逐任務語意**（blueprint 裁，禁一把全域尺）：
  貿易 ＝ **成交**｜信使 ＝ **送達被讀取**｜求居 ＝ **見到領主**｜紮營 ＝ **立營成立**
  ⇒ 每一項要貼 **file:line：成功是在哪一行被判定的**。
★目標型任務（信使／貿易／求居）的舊逐類數字**可能仍近真**，
  ★★但**不省那次重量** —— blueprint 原句：**「可能仍近真」不是數字**。
重量後的逐類數字才進儀表（【遠行到場率】已升格為世界生命徵象）。

★★★**2026-09-12 結案（blueprint 裁）：信使是健康的，新基線入帳**
  **有旅程的信使送達率：≥3 格 **57.1%**｜1–2 格 **68.4%**（世代 4、長窗）
  ★而【信使 2.2%】彻底作廃 —— 它是 `move_target` 把一趟切成很多段造成的；
  ★★**而真正讓母體出現的是【窗夠長】** —— `closed_far` 在 30 天窗裡恒為 0，
    因為**長旅程還沒走完**。
  ⇒ ★★★**母體小的第三個原因就是這個**（見下）。

### ★★★母體小的三個原因（判準，blueprint 認可 2026-09-12）

```
①**世界很少發生**｜②**我的條件太嚴**｜③**窗太短 —— 事件還沒結束**
⇒ ★**三個原因指向三種完全不同的處置**：**改世界／放寬條件／加長窗**
⇒ ★★而它們在畫面上**都是同一個 0**。
血證（同一天三次）：①紮營 0%（其實是尺）②面對面母體 ＝ 1（★**我選了①，真相是②**）
③信使 `closed_far` ＝ 0（真相是③）
⇒ ★★★**看到小母體，先問「它是哪一種 0」，再決定要不要下結論。**
```

★★★**重量結果（2026-09-11，fp 逐字相同 `4b080cea…` ⇒ 世界一樣、變的只有尺）**：
  **新基準 ＝ 36.8%**（67／182，只含判準已裁定的任務、不含求居）
  ・紮營 **0% → 79.2%**（24 起算／19 立營成立）★尺的 0，已結案
  ・~~信使 **8.9% → 2.2%**~~ ★★★**作廃（2026-09-11 同日）：它是【床的病】** ——
    那支床用 `move_target` 當 episode 的**鍵**，而**追人型任務的 `move_target` 每 tick 依 intel 刷新**
    ⇒ **一趟旅程被切成很多段** ⇒ 分母灌大、成功率稀釋
    ⇒ ★**同一個世界（fp 逐字相同）、兩支床：2.2% vs 50%**
    ⇒ ★★勝負不在世界，在**「一趟」的定義**。
    ⇒ ★★「目標型任務的舊數字可能仍近真」**在這一格是錯的** ——
      而 blueprint 早就寫了：**「可能仍近真」不是數字**。
  ⇒ ★★★**一格變好、一格變壞** ＝ 這次修尺**不是在把數字弄好看**
    （若是，不會有任何一格往下掉）。
⇒ **信使 2.2% 另立條目**：blueprint 預先登記的條件觸發（**它壞 ＝ 所有 belief 都在腐爛**）。
```

### 下面這段是【污染前】的原文（★不刪：它記錄了我們曾經相信什麼）

```
狀態：已量到，未查因
母體：episode **起算時目標距離 ≥ 3 格**的長程任務；★被觀測窗切掉的單獨一類、**不進分母**
窗：warring_states／30 遊戲天／seed 1337／預設 config 未改
  貿易 26.5%｜外交 23.1%｜**信使 8.9%**｜**紮營 0%**｜總計 **87／374 ＝ 23.3%**
⇒ ★**不是某個任務的 bug，是世界的性質**：選對了、也 commit 了，**然後在路上消失**。
⇒ ★★它直接壓在「家的迴路」上 —— 招攬／上門請求／佃農走過去**每一段都是遠行**
  ⇒ **每一段各漏四分之三**。

★查法（寫死，免得變成逐隻抓）：**結構列舉**——
  一趟遠行從 commit 到到場，**中間有幾個地方可以掉出去**
  （任務被換掉／目標消失／隊死了／被威脅打斷／距離重估／走到一半重新決策……），
  每個出口**掛一個計數**，看那 287 趟**是從哪幾個門出去的**。
★★候選（**標待驗，不是結論**）：覓食 episode 中位只有 **0.33 遊戲天**
  ⇒ 每個 cadence 又重新選一次 ⇒ 若長程任務也如此，
  **遠行不是被打斷的，是被【一直重新考慮】磨掉的**。

★★★附一個度量陷阱（同日踩到）：本票 after ＝ 116／655 ＝ 17.7%，
  **比率跌了而到場數從 87 漲到 116（+33%）** —— 分母長大正是那一刀的預期效果
  ⇒ **用總比率判它 ＝ 拿它自己造出來的分母去扣它的分**
...
## 這個世界的據點是蓋出來的還是搶來的？（2026-09-15 開，2026-09-16 答）

★★★**答案（2026-09-16，10 天窗、`warring_states`、seed 1337、從第 0 天起）**：
```
owned_to_owned ＝ **12**（全部 capture） ← ★**搶來的**
construct      ＝ **1**                  ← ★**蓋出來的**
takeover ＝ 3｜init ＝ **37**（起始預置）
```
⇒ ★**大部分據點是【擺好的】，而之後的變動幾乎全是【易主】。**
⇒ ★★**而它蓋得出來**（`construct = 1`，不是 0）—— 只是 1 對 12
  ⇒ ★**我先前那句「我【沒有】斷言這個世界蓋不出據點」得到證實。**
⇒ ★★★**單輪單窗不可引用比率** —— 記【易主遠多於建造】這個方向，**不記 12:1 當結論**。
...
而【擁有】據點的隊 **18 → 24**。
★**所以那 6 支不是蓋來的** —— `own_outpost_tile()` 認的是 `outpost_owner`
⇒ **它們是【易主】**（implementer 掃到的寫入者：佔村 `TASK_OCCUPY` 那一族、以及併入／吸納）。

★★★**而「18 → 24」本身答不了這個問題**：**那是兩個端點的差，不是事件數** ——
淨變化 +6 可以是「6 次易主」，也可以是「10 次易主 ＋ 4 支原擁有者死了」
⇒ ★★**端點差會把 churn 壓平成一個小數字** ⇒ **要的是在改寫 owner 的那幾個點各記一筆**（純計數，便宜）。

★**而這個問題比錢那條線大**：據點是農田、工坊、市集的地基。
★★**而我【沒有】斷言「這個世界蓋不出據點」** —— 要那樣說，**得先證明有人【想蓋】**。


## 征收贏走三分之一的 argmax（2026-09-16）

狀態：未確認
回訪：量測窗 2026-10-06

10 天窗（`warring_states`、seed 1337、從第 0 天起）贏家分布（母體 1048）：
**征收 383（36.5%）**／建設 123／覺食 91／囤貨 73／偵查 72／求和 72。
★**集中在一個 option** ⇒ 這落在預先登記的「集中在一兩個 ⇒ 下一張票的線索」那一支。
★★**而單輪單窗不可引用比率** —— 這裡只記【集中】這個事實，不記 36.5% 這個數字當結論。

## 攻擊：想打 61 次、成 0 次（2026-09-16）

★★★**答案（2026-09-16 晚）：不是手不聽腦，而走廊是原因的一部分**
```
①**(4a)/(4b) 逐事件分類**（2 天窗）：全 option **(4a) 450／(4b) 18**（對帳 ✅）
   ⇒ ★**攻擊 (4b) ＝ 0、偵查 (4b) ＝ 0** ⇒ **它們被擋的【全部】是被更高優先序擋的**
   ⇒ ★★**「手不聽腦」這條懷疑被【否定】。**
   ★★★**而它在【兩個窗】上都成立**（2026-09-16）：
     **10 天窗**：偵查 (4a) **825**／(4b) **0**；攻擊 (4a) **659**／(4b) **0**；
     全 option (4a) **7678**／(4b) **28**（對帳 ✅）
     ⇒ ★**偵查與攻擊被擋的，一次都不是同級或更低。**
   ★**而仍是單 seed** ⇒ **不得說「這個世界不會有 4b」** —— **多 seed 那一輪會答。**
   ★★**而那 28 次 (4b) 屬於【別的 option】** ⇒ **還開著，是哪些未問。**
②**拆走廊的 before/after**（10 天窗）：攻擊【真的被派出去】**0 → 6**
   ★★★**更正（2026-09-16，implementer 兩個獨立證據推翻我）**：
     我原寫「走廊把攻擊【改派】成偵查」—— **不成立**。
     `g3.scout_dispatch` 全窗 ＝ **0**，且 `[Scout] team=…` 那行 print **一次都沒出現**
     ⇒ ★**走廊沒有改派任何東西。**
     ★★**真因**：走廊**嘗試**改派、而 `try_set(SCOUT, PRIO_DISPATCH)` **被更高優先序擋掉**
     ⇒ ★★★**失敗之後【沒有退回攻擊】** ⇒ **那一次 dispatch 整個落空**
       —— **它不是改派，是【呑掉】。**
     ⇒ ★**而拆走廊有效的真正機制**：不確定的目標**根本不進候選**
       ⇒ 被選中的 prey 本來就有把握 ⇒ **不會走進那個呑噬點**。
     ⇒ ★★**而我錯的方式**：**效果（0→6）是真的，而我給它配了一個【沒發生的機制】。**
★**而兩個數來自不同的窗（2 天／10 天）** ⇒ **不得混講。**
```
⇒ **還開著的（不在本票）**：那 **(4b) 18 次屬於【別的 option】** —— **哪些？未問。**


狀態：未確認
回訪：量測窗 2026-09-29（多 seed 那一輪的 priority tap）

同一輪：`攻擊` **真的被派出去 ＝ 0**，而 **no-op ＝ 61，全部「優先序不足」**。
★**要走到 `try_set` 才會有 no-op** ⇒ **那 61 次是【攻擊贏了 argmax】** ⇒ **它有贏**。
⇒ ★★**不是「沒人想打」，是「想打但派不出去」**—— **兩者在「攻擊沒發生」這句話上長得一模一樣。**
★★★**而這【不是攻擊幣別票造成的】**：本票只改 util 的算法，**沒有動優先序**。
**待答**：那 61 次屬於 (4a)【被更高優先序正當擋下】還是 (4b)【同級或更低 ＝ 手不聽腦】。


## `bed-kind` 的母體用【目錄】當代理（2026-09-16）

狀態：已知未修
回訪：第二次出現時，複審日 2026-10-13

`bed-kind-gate.sh` 的母體是 `scripts/debug/*.gd` —— **而那個目錄裡有非床的東西**
（血證：`probe_stats.gd` 是 Probe 的實作，却被要求宣告床種類）。
★**implementer 的處置對**：`kind` 填 `diagnostic` 是【沒有正確選項下的妥協】，
**而「它不是床」寫在註解裡、不靠 `kind` 欄承載**。
★★**而它的危險在於**：**`kind` 欄被迫承載一個它沒有選項的東西**
⇒ ★★★**那是「兩個東西塞進同一個欄位」的前兆** —— **與 `resource_scale`（值 vs 信心）同族**。
**不現在改**（這一族的閘今日已動過兩次，而它目前沒有造成錯誤結論）⇒ **等第二次出現。**


## ✅ fixture 紅了的三種類別（判準，2026-09-16）（已結案：判準本身就是交付物）

**一個修法讓 fixture 紅了 ⇒ 問【它想驗的那件事，現在怎麼了】**：
```
①**意圖成立、而數字是舊尺寫的** ⇒ ★**改 fixture 的【數字】** —— 合法
   （血證：「富屬村」300 coin 在【相對於我】的新尺上不算富）
②**意圖在新制下不再成立** ⇒ ★★**那是【我的 spec 錯】，改 code 不改 fixture**
   （血證：遠距目標被我誤判成零情報而排除）
③★★★**意圖仍成立，而【那件事發生的地點】變了** ⇒ **驗收點搬家**
   ⇒ ★**把測試搬過去，斷言新地點的【等價事實】** —— **不是刪掉，也不是改斷言**
   （血證：兩條測試直接呼叫 `_commit_conquest_attack` 並斷言 `== TASK_SCOUT`，
    而【慵重者面對未驗情報不該直接打】這件事搬到了【候選生成端】）
```
★★**而③最危險的誤處置是【刪掉它】** ——
**因為那個意圖還在，而它就此失去守衛。**
★**這三類是 implementer 在兩天內逐個撞出來的，第③類是他找的（我原本只有兩類）。**


## 【第一天】沒有生產隊（2026-09-15，★標題已更正）

★★★**結案（2026-09-16，第 7 輮 day 1–27 搶救的逐日輸出）**：
`docs/measurements/2026-09-16-run7-day1-27-salvaged-raw.txt`
```
produce_teams：day 1–3 ＝ **0**｜day 4 ＝ **1**｜day 22 起 ＝ **6**
而世代 4 體檢卷（同 `warring_states`、**同 seed 1337**、day 30）＝ **13**
⇒ ★★★**兩份卷接成同一條軌跡**：**0 (d1–3) → 1 (d4) → 6 (d22) → 13 (d30)**
```
⇒ ★**「還沒發生」與「不會發生」就此分開** —— **它會發生，第 4 天就發生了。**
⇒ ★★**而它還在長**：day 22 的 6 支 → day 30 的 13 支（八天內又多 7 支）
  ⇒ ★**不得說「這個世界只會有 6 支」** —— **27 天窗看不出它什麼時候穩下來。**
⇒ ★★★**而我 2026-09-15 那句「三個動詞整個窗裡一次都沒成功」的錯，現在有了硬數字**：
  **那個窗是 1 天，而答案在第 4 天。**
...
implementer 逐域指紋比對順手量到：刪掉 `maintain_coin` 後，goal 筆數 **302 → 250（差 52）**，
而快照時隊數是 **63** ⇒ **只有 52 支隊有過那個 goal**。
★**而 `maintain_*` 是【冪等補齊】的**（`goal_resolver.gd:34-37`：沒有就補上）
⇒ 理論上每支活著的隊都該有五個 maintain goal，63 × 5 ＝ 315，**而實測 302**
⇒ ★★**有隊【從來沒跑過目標生成】**。
★★★**最可能是 `goal_eval_next_tick` 的節流窗還沒到**（新生隊／窗末才出現的隊）
—— ★**而這是推論，不是觀測：沒查那 11 支是誰。**
⇒ **不開票**。★★**而若日後有人做【每支隊都有 X】這種假設，這一格會咬他。**

## 守衛與它要擋的東西住在同一個字面上（2026-09-15，第二次）

狀態：已知未修
回訪：第三次出現時，複審日 2026-10-13

implementer 拆掉一條假判決通道時，**在說明裡寫了那個欄位的字面**
⇒ 閘掃全檔 ⇒ **照樣紅** —— ★**與 confusables 那次同形**：
**守衛與它要擋的東西不可以住在同一個字面上。**
★★**正解不是「註解換個寫法」，是【閘要能分辨「使用」與「談論」】**
（而 merge-gates 本体已經為同一件事付過學費：`:135-138` 的「談論一個字」血證）。
★★★**今天不改閘** —— **等它第三次出現再動它**（不要一直加閘／改閘）。
...
## 徵收贏走三分之一的 argmax（2026-09-16）

狀態：未確認（★**待查，不是票**）
回訪：多 seed 那一輪

implementer 2026-09-16 側報（母體＝`rank.winner_all.__total` ＝ 1048，2 天窗／seed 1337／`warring_states`）：

```
徵收 383（36.5%）｜建設 123｜覓食 91｜囤貨 73｜偵查 72｜求和 72｜外交 38｜survival 36
```

★**大部分贏家確實是「在幹活」**（建設／覓食／囤貨／maintain_*）—— 那一側正常。
★★**但 `徵收` 單一 option 佔 36.5%** ⇒ 落在「集中在一兩個 option ⇒ 值得查」那一支。
★★★**單 seed 單窗 ⇒ 比例不可引用**，這裡記的是**存在性**：它確實很集中。

## 攻擊「想打 61 次、成 0 次」（2026-09-16）

狀態：未確認（★**待查**；★★**不得寫成「攻擊幣別票造成的」—— 本票沒有動優先序**）
回訪：多 seed 那一輪（與 (4a)/(4b) 的 priority tap 同一輪）

implementer 2026-09-16 側報（2 天窗／seed 1337／`warring_states`）：

```
dispatch.攻擊.ok = 0 ｜ dispatch.攻擊.noop = 61（★全部「優先序不足」）
擋它的現任 task：迎戰 27｜投靠 19｜乞食 8｜逃跑 5｜外交 2
```

★**要走到 `try_set` 才會有 no-op** ⇒ **那 61 次是攻擊【贏了 argmax】** ⇒ **意圖存在、執行為 0**。
★★**而【沒人想打】與【想打但派不出去】在「攻擊沒發生」這句話上長得一模一樣** ——
**分開數才看得見。**
★★★**待查的是那 61 次裡有多少是 (4a)【被更高優先序正當擋下】、多少是 (4b)【同層或更低 ＝ 手不聽腦】**
—— 判準要比 `priority`（**不是比 task 名字**：同一個 task 名可以在不同 priority 上被設）。

## ✅ 零情報的排除**實際由 `has_belief` 提供**（2026-09-16）（已結案）



攻擊幣別票曾在 `faction_ai_system.attack_scan` 裡加一支
`if bel.is_empty(): continue`（`attack.excluded.zero_intel`）作為「零情報結構排除」。

```
實測（2 天窗／seed 1337）：attack.excluded.zero_intel = **0**
                          （同輪薄情報 admitted 5211／refused 704 ⇒ 不是「世界沒發生」）
原因：`if not BeliefSystem.has_belief(...)` 的守衛在它【之前】
   ⇒ 連 claim 都沒有的目標**根本走不到**那一行
```

⇒ ★**那支已於 2026-09-16 刪碼**：★★**死碼會讓下一個人以為這一格有人守著**，
而真正守著它的是 `has_belief`。
★★★**而 systems 兩次判錯這一格**（第一次差點說它冗餘、第二次堅持它必要），
**兩次都是因為只查了【想到的那一道閘】（`confident_enough`），而擋住它的是另一道。**

## ★★★共用檔的 merge 會【靜默地洗掉一個決定】（2026-09-16）

狀態：已知未修
回訪：第二次出現時，複審日 2026-10-13

血證：`docs/known_issues.md` 兩個人同時編 ——
implementer 在他的樹把兩條標成 `## ✅ …（已結案）`（**無任何 `狀態：` 行**）；
我在 main 看到它們「缺狀態欄」於是補上 `狀態：已知未修`。
⇒ ★**merge 之後**：一條的 ✅ 被搬到標題尾巴、另一條的 ✅ **整個不見**
⇒ ★★**一個【決定】（他判它結案）被洗成了【另一個決定】，而兩個人都沒發現。**
⇒ ★★★**而它不會紅**：`ki-status` 只問「有沒有合法狀態值」，
  **它答不出「這個狀態是誰判的、有沒有人的判斷被掉了」**。

**而我的歸因也錯了**：我說「閘綠是因為我補的行」——
★**對【現在的 main】成立，對【他那一輪】不成立**（他的樹裡根本沒有我那些行）。
⇒ ★★**而他是 `git show 4b8993bdd:docs/known_issues.md` 查出來的，不是接受我的說法。**


## 床讀【已改名】的 Probe 鍵會安靜地印 0（2026-09-16）

狀態：已知未修
回訪：下一次有人改 Probe 鍵名時，複審日 2026-10-13

★★★**兩人分歧，而我維持【不結案】（2026-09-16）**：
implementer 曾標 ✅（理由：「已改寫法，留這條是為了判準」）。
★**而「留著當判準」正是【未結案】的理由**：
  `✅` 在這份檔裡的意思是**「這個問題已經不存在」**，
  而【床讀改名鍵會印 0】**這個問題仍然存在**（任何一支床都可能再犯）——
  他修的是**那一支床**，而我裁的通則（**讀鍵前先斷言鍵存在**）**還沒實作**。
★★**而這條分歧曾經被一次 merge 靜默地洗掉** —— 見下一條。



血證：床讀 `attack.excluded.no_priced_belief`，而該鍵當天被改名成 `zero_intel`
⇒ 讀不到 ⇒ 印 0 ⇒ ★**看起來像「沒發生」**；★★**而「沒發生」與「讀錯地方」在一個 0 上長得一模一樣。**

⇒ **寫法（不是新閘）**：床分兩種讀法 ——
`_must(k)`：**母體／分母那一類**，這一輪跑過就一定存在 ⇒ **缺 ⇒ 具名紅「不可判」**；
`Probe.counts.get(k, 0)`：**事件計數**，0 是合法答案（例：`dispatch.攻擊.ok` 真的一次都沒成功）。
★★★**而不是每個 0 都該紅** —— 分不清這兩種，就會造出一堆假紅然後開始忽略它們。

## ★間距中位數是 59 不是 60 —— **在判準內，但沒有被解釋**（2026-09-23）

狀態：已知未修（★它不是紅：判準是 60±1，而 59 過了；記這一條是因為**一個沒被解釋的 ±1，下次有人看到 58 會從頭查一遍**）
回訪：下次有人動 `CadenceStagger` 的 `MIN_GAP`，或有人量間距分佈的形狀時，複審日 2026-10-23

```
量到（錯開票 P3，樹 622dcd222）：gap_median=59  gap_min=30  gap_max=119  clamp_ratio=1.64
```

★implementer 的推測是「clamp 把分佈往下拉」——★★**方向反了**：
`next = max(candidate, last + MIN_GAP)` 把 [1,29] 的間距【抬高】成 30 ⇒ **只能把中位數往上推**。

★★★**機制（推導，★兩人各推一次，都【沒有量過】）**：拉低的是**被 clamp 的那一段的【下一段】**。

```
令 φ_k = t_k mod 60（本次的【實際相位】）、o_{k+1} = 下次的 offset
candidate = (C_k + 1)·60 + o_{k+1}
⇒ ★通式：gap = candidate − t_k = 60 + o_{k+1} − φ_k

未被 clamp：t_k 在格子上 ⇒ φ_k = o_k ⇒ gap = 60 + (o_{k+1} − o_k)  ★d 對稱 ⇒ 中位數 60
被 clamp後：t_k 離開格子 ⇒ φ_k ≠ o_k ⇒ ★只要 φ_k > o_{k+1} 就 gap < 60 ⇒ 往下拉
★★而 clamp 之後的相位【不是固定的 30】：t_k = t_{k-1} + 30
   ⇒ φ_k = (φ_{k-1} + 30) mod 60 —— 只有 φ_{k-1}=0 時才等於 30
```

★**訂正紀錄**：systems 第一版寫「gap = 30 + o ⇒ 中位數 ≈ 59.5」——
**那是 φ_k = 30 的【特例】，不是通式**（implementer 推出通式後更正）。
★★而 implementer 的第一版寫「clamp 把分佈往下拉」也是錯的方向（clamp 只能把中位數往【上】推）
⇒ **兩個人各錯一次，錯在不同的地方，而兩次都是靠【把式子寫出來】抓到的**。

★★★**可證偽的預測**：【緊接在一次 clamp 之後】的那些間距應**偏低於 60**，
且偏移量由**該次的實際相位 φ** 決定 —— ★**不是**固定的 [30, 89]。
★要判它需要間距的**順序**與**當次相位**，而床現在只存直方圖 ⇒ **不值得為它多跑一輪**。
★★若哪天要判：床另存「前一段是否被 clamp」與「當次 φ」，不要重新設計間距量測。
★★★**而這整段【沒有被量過】** —— 誰要拿它當前提，**先自己跑一次**。

## ★★★`CadenceStagger._mix` 錯開得**不均勻**（2026-09-22，★量過了，數字在這裡）

狀態：已知未修（★已量、不擋窗 #3 —— 它只擋掉我原本那個 `<20%` 的門檻，擋不掉修法本身）
回訪：下次有人拿 CadenceStagger 錯開任何 pass 時，複審日 2026-10-13
★★★**2026-09-23 這一條到期並已被消費**：裁定 (A)（把每小時那一趟 pass 按隊錯開）
寫進 spec `2026-09-23-stagger-the-hourly-pass-HOW.md` §4d —— 上面那組數字**直接被引用**，
並且在那裡寫死「本票的主張不需要均勻」以及「不得拿桶佔用的 3 倍去判本票紅」。
★條目**仍不結案**：`_mix` 的散佈品質沒有被改，只是這一票不靠它。
...
teams=115 cadence=60 cycles=200 ⇒ 期望每格 1.92
  ...
  ★超過「期望的 3 倍(=6)」的 cycle ＝ **39／200（19.5%）** ⇒ 常態貼著線，不是離群
陽性對照：offset 恆 0 ⇒ max=115 空格=59（★所以上面那個紅不是判準壞掉）
```

**為什麼還是不修**：錯開**有生效**（空格 13／60 vs 沒錯開的 59／60），
它只是**不夠均勻** ⇒ ★而本 slice 的主張已改成「**相位不再被永遠保留**」（1/cadence 推導），
**那個主張不需要均勻**。

★★★**下一個人要知道的**：想改進 `_mix` 的散佈品質 ⇒ **數字已經在上面，不用重量一次**；
而★**改 `_mix` 會動到所有 33 個已錯開的站** ⇒ 那是**世界改變**，要走窗、要世代戳。

## ★★★每 60 tick 有半數隊一起決策 —— **已定位**（2026-09-22）

狀態：已知未修（★真凶已定位 2026-09-22；修法＝`intel_arrived` 只在威脅內容上喚醒，票在 `feat/intel-wake-by-content`）
回訪：該票 merge 後的 B3 絕對尺量測窗（12 天 × 2 seed），複審日 2026-10-06

```gdscript
# scripts/simulation/sim_runner.gd:358   ★★★（main 上的行號；feat 分支上是 374）
if state.world.current_tick % NEAR_CADENCE == 0:     # NEAR_CADENCE = WorldState.TICKS_PER_HOUR = 60
    var all_teams: Array = state.teams.keys()        # ★★全世界的隊，一次進同一個 pass、零錯開
```
★**它不是 `*_next_tick` 排程欄，是【模閘】** ⇒ 先前兩輪只掃排程欄的靜態掃**天生看不到它**。
★★而**我當初搜的是 `% TICKS_PER_HOUR == 0`，code 裡寫的是 `% NEAR_CADENCE`**
   ⇒ **同一顆常數在這裡叫別的名字** ⇒ grep 命中【名字】不命中【意思】。
★★★證據（既有 CSV，四臂獨立同指）：尖峰 tick 中 `tick % 60 == 0` 佔 **100.0%**，
   而全體 tick 的底噪是 **1.67%**；門檻取各自 p99，不是挑的。
★**必要非充分**：17280/60 ＝ 288 個 pass tick，其中只有 174~190 個成為尖峰。
★★**隊的生與滅也 100% 發生在這個 pass 裡**（兩臂皆然）。

★★★**還沒接上的一環**：pass ⇒ 尖峰 已坐實，但**尖峰 ⇒ `>2s` 凍結幀**還沒量。
   **在那一格回來之前不開修法票** —— 否則就是重複今天上午那一次
   （床綠了、世界沒好，因為治的不是那個病）。

---

★以下是**已排除**的部分，留著免得下一個人重查同一批。

```
症狀：尖峰 tick 佔存活隊 **61.5%／62.4%**，★**間距 ≡ 60 tick 的佔比 99.3%／98.9%**
  （60 tick ＝ 一個遊戲小時）
★★**已排除**：那 10 處純加法 cadence（threat／residency／info／subteam／decision／
  labor／infra／consolidate／goal／idle_employ）——
  ⇒ ★**它們的 cadence 值集合只有 {1440, 4320}，最小的也是 60 的 24 倍**
  ⇒ **構造上不可能產生間距 60 的尖峰**
★★★實測佐證：把那 10 處全部改走 `CadenceStagger` 之後，
  **間距 60 的佔比 99.3% → 99.3%（一動也沒動）** ⇒ 排除成立，不是推測。
```

**也已排除（更早的兩次靜態掃）**：`% TICKS_PER_HOUR == 0` 形狀、無錯開的 elapsed 比較 —— **各掃出 0 處**。

★**下一步不是 grep「哪個常數等於 60」**（那是看到答案再去找符合的東西）——
是**逐 tick 到期欄位歸因**：對每個 `*_next_tick` 欄位印它在尖峰 tick 上的到期次數，**讓排名自己出來**。

★★★**教訓（systems 自認）**：這個病的診斷曾經**指著那 10 處**，
而**推翻它只需要把它們的 cadence 值跟 60 比一次** —— 那些值當時就印在我手上的文件裡。
⇒ **「修法有沒有效」與「這個修法治的是不是那個病」是兩句話。**

## ★第一次評估真的會被推遲 —— **但只在【另一種初始化形狀】上**（2026-09-22）

狀態：已知未修（★開著、獨立 —— 兩臂都有、不是本次改出來的 ⇒ 它不解釋 2026-09-22 的存活隊下降）
回訪：下次動 `faction_ai_system.gd` 那兩個 `*_next_tick` 初值時，複審日 2026-10-13
★★★**2026-09-23 這一條到期並已被消費**：裁定 (A) 用的就是**形狀乙**（`if X == 0: X = CadenceStagger.next_tick(...)`）
⇒ 已寫進 spec §4b-1：本票**接受**第一次推遲 [30, 120) tick（一生一次），
但**第一個遊戲日每隊會少一次 pass** 這件事必須寫進 P3 的期望值，
★否則它會以「P3 紅＝頻率被砍」的樣子出現，而歸因會指到錯的地方。
★條目**仍不結案**：`faction_ai_system.gd:1240/1346` 那兩處初值沒有被動。

```
形狀甲（★十處純加法那組）：欄位初值 **0**，閘直接比 `current_tick >= X`
  ⇒ X=0 ⇒ **第一次檢查就到期** ⇒ **第一次沒有被推遲**（★7／7 欄位現場讀出來的，不是印象）
形狀乙（★真身在這裡）：`faction_ai_system.gd:1240`（faction.infra_eval_next_tick）
                      `faction_ai_system.gd:1346`（team.indep_infra_next_tick）
  `if X == 0: X = CadenceStagger.next_tick(...)` ⇒ ★**第一次被推遲 [c/2, 2c)**
```

★**量級**：推遲量 ＝ `c − (current mod c) + offset`，下界被 MIN_GAP clamp 在 c/2
⇒ **上界 ≈ 2c−1**（★**不是 c−1** —— systems 在不變量 #8 初稿寫錯過一次，已訂正）。

★★**還沒證明的**：「最長 ~2c 不重評」會不會**害死一支隊**。要證它需要世界層一格：
**每隊實際相鄰決策間距的 p99 × 該隊是否死亡** —— ★而那顆 tap **目前沒有**。


## ★★★`[HW] mem_free` 量了、印了、**沒有接電**（2026-09-22，systems 自撞）

狀態：已知未實裝
回訪：觸發事件 —— 下一次有背景跑被系統因記憶體壓力殺掉時

```
tools/godot.ps1:226  $freeGB = [math]::Round($osInfo.FreePhysicalMemory/1MB,1)
tools/godot.ps1:228  Write-Output "[HW] … mem_free=${freeGB}GB/${totGB}GB"
⇒ ★每一次跑都拿到了那個數，而【沒有任何比較、沒有任何中止】。
```

★血證：2026-09-22 的 merge 電池跑到 68／72 被系統殺掉（全機 31.89 GB 只剩 3.85 GB）
⇒ 一輪【不可判】，而且沒跑到的 4 支裡正好含兩支世界指紋床。

★★而「我先前設了 2.5 GB 門檻」這句話**是假的**（已撤回）：那個數字全庫裸掃零命中，
只住在三封信裡 ⇒ 它**只改變了一次人的行為**，下一輪沒有東西會再檢查它。

★★★**不要新寫一支 hook**（用戶規矩：先問現有哪一支該接住）——
正確形狀是把**上面那行既有的量測接上電**，而且它要比的是【全機空閒】
不是 Godot 佔用量：★這次死掉時 Godot 只佔 165 MB，機器少的是 28 GB
⇒ 以 Godot 佔用量為基準的門檻**量錯了對象**。

★要不要接電是一個**決定**（會擋到所有人的跑），不是一個修補 ⇒ 等 blueprint／用戶裁。

## ★★★對帳表停在 119 個欄位，而 `DecisionContext` 已經長到 142（2026-09-22，implementer 跑床時撞到）

狀態：已知未修
回訪：觸發事件 —— 下一次有人跑 `c1_info_reconciliation_bed` 或引用那張對帳表時

```
docs/measurements/2026-09-10-c1-info-reconciliation.md
  現況： ## §1 ctx 欄位 × 玩家可讀（119 列，盲格 117）
  實際： 142 列，盲格 139
⇒ ★23 個欄位【從來沒有被那張表審過】，而表上看不出來。
```

★**真因不是有人寫錯，是【重生它的時機沒有人負責】**：
那張表**只有在有人跑那支床的時候才會重生** ⇒ 它在 main 上靜靜地過期，
★★**而過期的時候它顯示為綠**（表還在、格式還對、數字還在，只是少了 23 列）。

★★★同族（2026-09-22 同日）：`walkthrough-v2` 的量測卷落地了、而產生它的 code 沒有。
⇒ **產物與產生器會分開旅行**，而只有產物那一半會留在樹上被引用。

★處置：implementer 發現時把重生的檔 `git checkout --` 還原了、**沒有**帶進他那張票的 diff
（混進去會讓審查多判一件無關的事）——那是對的。這一條是為了讓它不被忘掉。
