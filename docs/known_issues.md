...
- **cost70 判定 = persona-partial-effective**（非「117差3」非「全錯」，keep 不廢）：fresh builder reserve≈100×reserve_factor → affords cost70(門檻 105)⟺ **reserve_factor≥1.05**（貪婪/好戰 leader；measurer 實測 113=factor 1.13 已達）。降門檻 120→105，高-factor 隊搆到、低-factor 卡。**trace 坐實中**（measurer:3 分量+reserve_factor+建成否）。code 註解「117」已訂正（outpost_system:81/87/93）。
