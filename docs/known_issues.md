...
- **長窗斷鏈修進度（2026-07-03,長窗 6 月+zoom 拆根,藍圖四裁 `chain-rulings-envoy`）**：
  ...
  - **★管線序（用戶裁 `gui-after-pipeline`,鎖定）**:①已定管線收完——**收益鏈修（佔村點火前提）→ default 組成/健康 measure → 裁生成參數 → 軌3 二考** + cadence spike 殘餘 + **統一矩陣剩餘全燒**（單寫者 S1/S5/S6/S9/S10/S11/S12+強制閘全立;互動 F-I2/I4/I5/I7;人力 F-M1-7;belief F-B1/B4;例外 F-P 留玩家面 arc）②**觀測 GUI 輕 slice**（事件 ticker/隊伍 inspect/速度控制,畫既有探針資料零新模擬機制;bar=看著狼崛起一條弧追得到;敘事可見提前輕切片非開願景）③願景大塊凍結照舊。G3 Phase D 照排,GUI 薄層可平行。
