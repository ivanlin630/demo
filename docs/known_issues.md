...
★★★**systems 的推導（2026-09-23，★是推導不是量測）**：拉低的是**被 clamp 的那一段的【下一段】**。

```
沒被 clamp：t_k = C·60 + o_k        ⇒ gap = 60 + (o_{k+1} − o_k)，d 對稱 ⇒ 中位數 60
被 clamp後：t_k = last + 30（★落在週期中間，不再是 C·60 + o 的形狀）
           下一次 candidate = (C+1)·60 + o_{k+1}
           ⇒ ★gap = 30 + o_{k+1} ∈ [30, 89] ⇒ 中位數 ≈ 59.5
```

★**可證偽**：「緊接在一次 clamp 之後的那些間距」應該近似均勻分佈在 [30, 89]。
★★要判它需要**間距的順序**，而床現在只存直方圖 ⇒ **不值得為它多跑一輪**。
★★★若哪天要判，做法是床另存一個「前一段是否被 clamp」的旗標，而不是重新設計間距量測。
