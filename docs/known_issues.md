...
- **★systems 建議=defer 到 12mo 大考後**（已報 blueprint 裁）：①1.72% 換「fp intended-change+全故事審+**外交/背叛觸發率腰斬**+可能回退」=收益風險比不成立 ②**★考前改政治頻率會污染大考政治維歸因**（大考正要看結盟/外交/背叛質地）。**correctness 面不變**（每 tick 跑兩次仍是事故）→ 改列 **correctness backlog**、大考後單獨走。
