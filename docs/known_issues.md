...
★**限制（誠實記）**：本條**無法進 `docs/process/defers.tsv`** —— 它的解除條件（「perf 需要時」）
**寫不出可執行的 `met_check`**，而**那張表禁止沒有 met_check 的行**（那是它唯一的鑑別力來源）。
⇒ **所以它只能靠這一條 known_issue 被記住** —— ★★**而那正是 `defer-open` 閘擋不到的那一類**。
