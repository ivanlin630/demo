...
### ⏳★★★深帶找不到施主：整體 33~38% vs 最深帶 0.0~7.2% —— **成因未量**（2026-09-03）

**狀態：未確認** ｜ **回訪：量測窗 — implementer 的 `band × filter` 交叉（每 band 五道拒絕數＋相異 target 集合大小）回來時**

```
整體找得到施主：3559/10750（33.1%）／3437/10176（33.8%）／4354/11582（37.6%）⇒ 世界【有】施主
最深帶（food_days→0）：統一路 7.2%；階梯路 seed1337/42 ＝ 0.0%
```
⇒ **世界有施主，而【餓到最深的那些隊】沒有。**★**band × filter 沒交叉 ⇒ 是被④「對方沒餘糧」還是⑤「走不到」擋的，目前無人知道。**
★★判讀表**已先寫死**（④主導＝分配/產量問題；⑤主導＝可及性模型；③在深帶主導＝資訊門檻只咬最餓那群；①主導＝真根在 discovery 不在乞食）。
★★★**每一列都要附相異 target 集合大小** —— 上一輪就是它跟次數說相反的話。
