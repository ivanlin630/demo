...
## ★observer-neutrality bug：SpecimenTracer re-query 耗 global RNG（2026-08-12、③story-audit 揭、[[feedback_observer_no_global_rng]] 第4 instance）
specimen 手動選不同 team_id → 同 seed 世界分岔（teams130 vs 148、measurer isolation A/B 坐實：9 temp tap 中性、但**選哪組 specimen id 非中性**）。根（systems code-read）=`specimen_tracer.gd:53`「finder→estimate_catch_up→observe_velocity→**randf** = tracer 額外耗 global RNG」某路徑逃逸 `suppress_observe_noise`（:25）→ 不同 specimen 集 re-query 不同團耗不同 RNG。LOD-exempt 路徑已於 `sim_runner:506/518` 移除（前 instance），此為 **tracer re-query 殘留路徑**。fix=tracer re-query 全路徑 suppress randf（or 讀 cached 決策非 re-query）+ 加「specimen id-set 中性」regression（現只鎖 helper on/off 中性、選哪組 id 從沒驗過）。measurer bed 已改回永遠用官方 helper + 檔頭永久危險註記防重蹈。
