...
### ★★SpecimenTracer/SpecimenDumpHelper observer RNG 洩漏 bug（2026-07-28 揭露，影響過去所有 specimen 量測可信度，高優先）
specimen ON 改世界（A/B 坐實 on 動 vs off 凍）= 違「觀測禁擾動世界/禁耗 global RNG」鐵律（`_begin_observe`/`_end_observe` suppression 某處漏包）。**範圍比 latch 大**：影響過去所有 specimen-enabled 量測——measurer 2026-07-25「latch execution-verified」數字（stall%/complete/orig_recall）全 specimen-on **不可信需 clean 重跑**；2026-07-26「build=0+凍結」沒開 specimen、結論站（clarify 佐證）。**已 dispatch implementer 查修**（`2026-07-28-systems-to-implementer-specimen-tracer-rng-leak.md`）：找漏包點 suppress + ★specimen ON==OFF byte-identical 世界一致驗 + observability_gate 機器擋補。餵持守統一觀測中性（measure 前要修好）。連 memory `feedback_observer_no_global_rng`（第 N 次同族）。
