...
- **長窗斷鏈修進度（2026-07-03,長窗 6 月+zoom 拆根,藍圖四裁 `chain-rulings-envoy`）**：
  ...
  - **queue**:斷① spec/plan 出（`raid-continuity-identity-weight`:候選放行+own 減免只給能拍板者+拆 fid 早退;`_evaluate_solo` 全域給成員=後續矩陣格）,**待實作**。收齊→**長窗二跑驗複利弧**（raid→糧→俘→同化→兵力全鏈首閉環,asm 按三帶框驗）。
