...
- **⚠ [中][measure-first] order/trade 迴路 runtime 半 inert — 商隊 runtime 不交易（履約 merge 後揭）**：履約 code 正確但 world_sim 該 run **`g1.arb_attempt=0` + `[Market]成交=0`**（整 run 零交易）→ 履約率仍 0%。**非結算 bug，根因上游**：商隊 `_merchant_trade_target`(faction_ai:1180) 的 `best_arbitrage_order` 從未回非空單 → 沒商隊被 dispatch TASK_TRADE。懷疑（**未驗，別猜** [[feedback_avoid_rabbithole]]）：①商隊沒成形/沒掛 `TAG_MERCHANT`(archetype 派生) ②`received_buy/sell_orders`(team_known order message) 空 — message 沒傳到商隊 or `MERCHANT_MAX_RANGE`(20) 外。
  - **強根因候選（first-pass read-only，待驗非斷言）**：`AmbitionLadder.derive_archetype`(ambition_ladder:21) **純讀 leader values、完全無視 `商隊` tag**。`ARCHETYPE_TRADE` 要 `貪婪(trade) ≥ force(野心/好戰) ≥ settle(義氣/慎重)`，但平手序 `force >= trade and force >= settle` **先返 FORCE**。config 商隊隊（Team1 友軍商隊/Team6 獨立商隊）只設 skills.商業、**values 全默認 0.5 → 三軸平手 → 派 FORCE 非 TRADE**。`#0` 戲劇 archetype（霸主/屠夫/謀士/懦夫）也**無一推高 貪婪** → 全世界幾乎無隊 derive ARCHETYPE_TRADE → `_merchant_trade_target`(faction_ai:1181) 的 arb 分支**永不進** → `arb_attempt=0`、貿易迴路 inert。**驗法**：world_sim/headless 印各隊 `ambition_archetype` 分佈，看是否真零 TRADE。**若成立 = 框架縫**（tag 宣告商隊身份卻不影響 archetype 派生 [[project_framework_seams]]）→ 修：archetype 派生吃 tag（商隊 tag → 偏/鎖 ARCHETYPE_TRADE），或 config 商隊 leader 種高貪婪。**這條釐清前別派腐壞 plan-2**（腐壞造短缺→買單，但商隊仍不履約則經濟照空轉）。**caveat：單一 world_sim run（非確定 [[reference_multi_sanity_unseeded]]），他 run 可能有交易；確認「真never trade」需確定性貿易場景**。後果：#1 經濟閉環 runtime 沒真正活（腐壞/儲限即使造短缺→買單，若商隊仍不履約則經濟照空轉）→ **腐壞 plan-2 dispatch 前宜先釐清此上游**。修需 measure-first：建確定性貿易場景（兩隊互補供需 + 商隊）看 arb_attempt/成交在 code 路徑哪段斷，非長跑猜。
