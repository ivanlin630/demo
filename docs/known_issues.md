...
- **長窗斷鏈修進度（2026-07-03,長窗 6 月+zoom 拆根,藍圖四裁 `chain-rulings-envoy`）**：
  ...
  - **queue → 長窗二跑=三軌驗證（藍圖/用戶裁 `longwindow2-three-tracks`）**:軌1 warring 1337（對照連續,原驗收:複利弧/asm 三帶/GateWait/不 over-war）‖ 軌2 其他 seed×2（泛化,防 1337 過擬合,pattern 一致數值容差寬）‖ **軌3 ★default 自然世界首考**（沙盒 bar 真考場,turtle 時代後首回:戲存在性——狼爬/交易/結盟拒盟/俘虜同化各鏈 fire 過+不崩不凍+tick 曲線;戲稀可、戲 0 不行;軌3 死寂而軌1 活=過擬合 warring 密度=下輪議題）。
