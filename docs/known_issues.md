...
- **P0 加固進度（2026-07-01 merged）**：✅ **tile→teams 索引 done**(co-location O(N²)→O(N)、hostile-within/residency sparse tail 收) + ✅ **team_intel erase-prune done**(top leak 修) + ✅ tick 計時 instrument + scaling bed。**honor-LOD 未觸發**(量到 evaluate_all 誠實 O(N)、索引已足;行為變 measure-gated 沒量到不做)。🔴 **die-off erase O(N) ref-sweep spike = 必收（藍圖 2026-07-02 升級,違「per-tick 早晚期無延遲差」硬不變量）**:大滅團潮 K×O(N) 放大器=大戲時刻(滅團潮)最會爆、直接違反 per-tick 有界不變量(見 invariants 效能域)。**從「另案 backlog」升「長跑滅團潮量到 freeze 前必收」**。修向:erase O(N) ref-sweep 索引化/批次(複用 teams_by_tile pattern or 增量維護)。**驗**:seeded warring harness(已 merged)長跑量 tick-time 早晚期曲線 + 滅團潮 spike。scaling bed sparse+high-movement near-zone 場景待加。
