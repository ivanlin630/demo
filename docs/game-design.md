...
> **★scale-econ 量測線 CLOSED（2026-08-11、measure-first 教科書:5+層 artifact 逐層剝、每層 premature 前接住）：①survival 層 size 不 matter（iii 鎖、確定：confound 清後核心村兩擺法都活、attrition 差全是 overflow-spinoff 邊緣噪）②production 層『集中有無 genuine 生產優勢』答不了 = broken production pipeline（設施建不出→製造不 fire、demand 注入不解、更深斷在 facility-builder dispatch）= size-matter arc production 本體 territory、非乾淨 scale-lever ③★anon-overflow = 反覆中央 deferred 根（擋 relief + builder/manufacturing + labor 三處）。scale-econ 不是獨立 arc、是 size-matter 命題的入口。**
