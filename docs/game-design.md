...
> **★戰爭之路=雙重堵定案(2026-08-14 code-read、settlement 後 next arc 候選)**:12mo 零征服零搶佔真根=(a)intent 從不選征服(survival-mode 守成收斂、經濟修好部分自解)+(b)★capture/flip 機制 encounter-only(佔村翻旗全在 EncounterSystem attacker_win 路、encounter 只 ambush/player 觸發、headless NPC 世界 encounter_active 恒 false=NPC 永遠結構性不可能佔村、同零戰死同源)。**重大含意:經濟修好健康世界照樣零興衰**——戰爭之路要通需雙解鎖:①capture/flip 脫 encounter-only(NPC combat 接管分支)②intent 征服 selectable。與維度二軍力 backlog(combat time-scale wave)同族群、settlement arc 後一起設計。occupy funnel 補 tap 在跑(7 支斷點)。
