...
> **★军民混编/民兵動員 arc = 進行中（用戶 2026-08-12「照路線圖一直做下去」啟）**：**Slice A MERGED `998f5344`**——guard_ratio 離散 tag-gated 死常數→連續人格化（照妖鏡 de-patch）+ `_has_hostile_within` god-view 全除→belief-threat（純軍團也有感知、感知鐵律達）。**Slice B（團型梯度+pool 分數化+guns-vs-butter 動員）= 承重牆 spike 中**：`uses_unified:2394` 綁 TAG binary + ~15 硬 gate → 決策路由須 decouple vs 團型（觸 [[project_framework_seams]] 結構債、可能 Track②A 一角）→ spike 定 decouple 法+effort 後另 spec+R²。R² 異質框外審抓我「5 折 1」自矛盾+承重牆 blast radius = 好框外審。
