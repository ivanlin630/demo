...
## 📍 當前路線圖總覽 → 已搬 `progress.md` §當前路線圖總覽(live 前瞻視圖歸進度檔)。本檔只留願景與規則;arc 記憶見 memory `project_*`。
...
- **商業模型骨幹＝B 市場即地方（用戶定，非 A 追人）**：貨在 outpost/市場 `public_storage`,買方來買 stock（免賣方在場）＝真實市場、穩、可規模化、複用 WS-2b infra。A（追漫遊賣方）＝補釘思維且脆,棄。
  ...
  - **★市場厚度討論（2026-09-02 TG,討論中非定案,等經濟討論一起議）**：①現制=領主佈告欄（只有據點主人能掛原生單,外隊需求只以衰減影本存在）→市場薄、價格發現弱。②候選改向=**到場開放掛單**（人在市場當場可釘委託;**禁遠端掛**=意圖瞬移違資訊法,遠方需求照走資訊網轉貼）→故事更通+撮合更厚。③據點主人特殊性轉為**管理權**（市集稅/定規矩=未來政治層素材+錢 sink）。④防爆閘現成（單到期/影本衰減/板 cap）。⑤看板易主已裁（意圖帳「據點易主」行）：簿=櫃檯跟房子走,單=署名+效期,前朝令自然過期,零特判;「進城撕敵榜」可為未來性格層政治動作非自動物理。
...
### 決策模型接線現況 → 快照搬 `progress.md` §決策模型接線現況(2026-07-06 盤點;現況以 2026-08-21 模型完工清單為準)。**留定義**:接線脊椎=①感知讀 belief ②記憶腳進引擎(經驗=自己的 claim)③情緒腳——同一條「決策模型完成」非各修各的。

### A2 arc 進度(2026-07-09) → 搬 `progress.md` §A2 arc。
...
> ★UI 讀取語意（blueprint 裁 2026-09-22）：畫面永遠呈現【上一顆完整 tick】的世界快照；玩家指令綁 tick 邊界（佇列，下一顆 tick 開頭套用）⇒ 延遲 ≤ 1 tick，屬設計非缺陷。★按鍵當下印「已排入：<動作>」，結果句在下一顆 tick 的畫面上回（拒絕含原因、禁靜默），頁腳常駐「待執行 N 道」；下令不推時間（blueprint 裁 2026-09-23）。【現況 2026-09-22：指令仍直改 live state，佇列化＝待造票，前置於分片】此語意讓 sim 可在幀之間分片而畫面不半更新。
...
## 玩家循環 C1(用戶裁定 2026-09-07,副藍圖 session 討論定案)
...
開局兩模式(第二輪新增):附身=繼承全部記憶(belief 攤開);**創隊=point-buy**(點數換物資/人力/技能/屬性,belief 全空=純霧開局)——
繼承知識 vs 只繼承資源,難度光譜兩端。

記憶模型五類一流四頁:事件=唯一 append-only 流;團/據點/人=狀態頁+事件尾巴視圖;核心=狀態頁無流。
狀態頁必帶 as-of 戳(「身家=殷實(聽說,第28天)」)——認知非真相,不標=介面把舊情報謊稱現況;「要不要重新打聽」本身是玩法。

傳聞污染鐵則:**結論可以被污染,帳不能**——狀態頁摘要值(角色當下判斷)會被傳聞帶跑,原始條目 append-only 永不改寫,
親見那筆永遠在,衝突條目並存遊戲不裁。玩家的信不信不寫回角色 belief(兩條命各走各):
附身低判斷力角色=「你比你的身體聰明」。判斷力 gate:血統標注清晰度吃角色判斷力(來源欄糊掉但條目與時戳永在)——
挑附身對象=挑資訊裝備,point-buy 判斷力屬性的賣點。

收納三原則:索引提供檢索軸永不提供連線;多重掛載(流存一份,頁尾是視圖);顯著度=寫入當下的 fear/stress
(懦夫的記憶裡威脅特別大聲=繼承的記憶帶人格色彩);釘選=唯一玩家標注動詞,不做筆記本。

情緒軸(A4)收口原則:恐慌蔓延等群體情緒**不做傳染機制**——強加=腳本化;它該從既有鏈湧現
(鄰隊潰逃→目擊/聽說→威脅評估升→提早逃),用驗證票確認湧現是否已是事實。

## 貨幣三幕與薪資雙層(用戶裁定 2026-09-07)

礦總量有限,世界的貨幣史分三幕:金潮(礦豐,錢追著經濟跑)→漸緊(挖礦漸不划算+儲量遞減,錢荒零星、囤積成戲)→
錢荒(礦竭+死錢池累積→通縮→「先欠著」的信用內生出場)。金本位不是選擇,是信任真空期的唯一解——
金=信物理,陌生人唯一能用的錢;信用=信人,要等世界長出「認得誰欠誰+違約有後果」才有出生理由。

薪資分兩層:匿名者要的是供養(糧+安全,失守才叛亂);有名字的人有野心,野心是要付錢的——
付不出不是叛亂,是離心(離團/拒晉升/被挖走)。所以無幣村天然封頂於小而穩,
商路或礦把 coin 帶進來才解鎖成長層:壯大要領袖/心腹/軍官=全是記名,記名的燃料是 coin。
例外是恩義:主從之情、血緣可以免薪——但免要求不等於不能給,想賞照樣賞。
叛亂留給真正的暴政:有錢不餵人。
