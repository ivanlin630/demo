...
to: measurer
status: consumed
topic: "[measure] beast-fix(id碰撞+決策洩漏+extinct守衛addendum)→ feat/beast-fix@7fb16350。seed1337/42/4201 真隊無 regression(starve/pop/teams)+真隊 belief 不再含 -1000000 幻影條目。TDD 11/11、headless 0new(baseline 3)、gate 64、determinism byte-identical(3mo ff7878af/addendum後 1mo 548c1b68)。"
---
...
## ★addendum（systems 順手，HEAD 1524d5ed→7fb16350）
systems addendum `2026-07-19-...-addendum-starve-guard`：`_on_team_extinct` 死因分類 Probe 塊加 `team.beast_kind == ""` 守衛（beast 死=狩獵結果非隊死因統計，不計入 extinct.* 計數器）。**冗餘於 loop3 beast-skip**（beast 正常走不到 _on_team_extinct）但防未來別條 extinct 路誤計 = defense-in-depth。**零行為變**：Probe 純觀測（sim 不讀）+ 真隊(beast_kind="")走同分支。determinism 保：1mo 2 跑 byte-identical md5 `548c1b68`。★量測對象＝**7fb16350**（含 addendum）。
