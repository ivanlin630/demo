...
1. **A1 MERGE GO** = correctness groundwork（四象限乾淨/byte-identical/零 regression、無 downside）。premise partial-refute 誠實收。
2. **A2 升佔據率主槓桿**：settle-into-existing 42 既有據點 = **無 geography 限制** + 從未 dispatch = 已坐實真斷點 → 修通 = 最大 occupation lever（正是用戶最初直覺「有初始據點能進入生產」那條路）。
3. A3 照原 target（build noop 獨立坐實）。
4. founding-path 新線索（establish_crude_camp 非 argmax / desperation 門檻 / 鄰 7 格掃）= A2/A3 內加 instrument 診斷、量完再評第三槓桿、不現在擴 scope。
5. 用戶通報 premise-refute（透明）。
序:A1 merge→A2(主)→A3。地基 KEEP。
