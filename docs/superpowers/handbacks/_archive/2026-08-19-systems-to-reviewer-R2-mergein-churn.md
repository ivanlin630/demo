...
spec=`docs/superpowers/specs/2026-08-19-mergein-churn-arrival-pin-HOW.md`。R① 免（前提 measurer probe-pin 坐實 (b)arrival-never）。
## ★審點
1. **前提 fact-check**：JOIN commit→movement→arrival co-locate→`_resolve_join`(interaction:46/206/226)、resolve 走到時運作正常、commit 698 vs resolve ~10 落差=arrival-never 確認？
2. **investigation approach**：T1 runtime-trace pin **i**(movement 不執行)/**ii**(cadence 重評 reset persist 弱=S2b 款)/**iii**(移動 host chase)=measure-first 不假設 sub-cause 合理？
3. **★手不聽腦 root 非補丁**：T2 根修 committed JOIN 真 resolve（persist-to-arrival/movement 修/mobile-host 處理）非繞過補丁。
4. 感知鐵律 trace 讀 own-state 禁耗 global RNG / 5. hand-obeys-brain 家族（12mo 大考#4/S2b corvee cousin）。
gate=churn 消+team 不暴增+perf 回正+committed JOIN 真 resolve。base 現 main。與農業b re-measure/labor-v2 平行。待 R² CLEAN → dispatch。地基 KEEP。
