...
現  tie-break==**平手取最小 tid**（explicit 非靠 dict 序）→ **index 候選集套同一  比較=by construction byte-identical**（reviewer 證 argmin+explicit tie-break over 固定集合與迭代序無關）。

## ★build-once（R² 親驗 terrain immutable=world_generator:67 唯一寫、零 runtime 重寫）
tiles-by-terrain 索引**世界生成完成後建一次全程沿用**（免 per-tick）。
- ①建  索引（world 生成後一次、可 lazy-first-query build-once cache）。
- ②find_nearest_terrain_tile 改**只 iterate 該 terrain 的 tiles**（非全圖）、套同一  tie-break 回同一塊。
