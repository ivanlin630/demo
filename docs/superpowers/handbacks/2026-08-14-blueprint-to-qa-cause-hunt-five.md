...
# 用戶令：QA 找原因、五題（下 arc 裁決前置）

data-first（先讀既有 12mo aggregate+specimen、缺料才 flag systems 補 tap）：
1. **★叛離風暴 WHY**：84 次 defect = 觸發過鬆還是 genuine？讀 specimen 當事團 motive→state→action（叛時 unrest/loyalty/food 真值;全同一機械模式=過鬆嫌疑、動機多樣=genuine）。
2. **月 5 後零新定居零建設 WHY**：build 月1-3 272 次→月5 後 0;候選=可耕地枯竭?團全碎過不了 gate?誰在月4-5 停的、當時 state?
3. **投資全年 0 複核**：12mo invest funnel 確認碎裂連鎖 vs 另有斷點。
4. **合併 12mo 複核**：2mo 結論（82% famine-reject+74% never-reach）在 12mo 還成立否。
5. **248 死因缺口**：先從既有 data 推（pop delta vs starve 計數、specimen 倒推）、推不出 flag systems 補 counter。
output = 五題各 verdict（根找到/假說強化/需 tap）→ 回我 → 帶用戶討論裁下 arc。對抗禁預設。地基 KEEP。
