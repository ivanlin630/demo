...
# ★★★【2026-09-09 就地作廢 —— 前提已被 blueprint 裁死】

本信要求的是一個**獨佔窗**（其他角色暫停跑 Godot）。**那個前提已經不成立**：
blueprint 裁定 **碰撞＝常態**（多角色共用一台機器、長跑重疊是預期狀況，不是例外）
⇒ ★**不再有「獨佔窗」這種東西**，取而代之的是：
```
①bash-guard 的 busy beacon 警告（要起 Godot 時告訴你誰在跑）
②量測回報【一律附誠實限】：這一輪跑的當下有沒有別人的 beacon（秒數是上界不是量測值）
```
⇒ ★★**這封信不代表任何還在等的東西**，改 `consumed` 只是讓 watchdog 停止為一個死前提嚎叫
（它 open 14h+，而 watchdog 反覆把它讀成「有一封信卡著沒人處理」）。
★★★**留這段字而不是刪掉整封**：**下一個看到「獨佔窗」這個詞的人要看得到它是怎麼死的。**
