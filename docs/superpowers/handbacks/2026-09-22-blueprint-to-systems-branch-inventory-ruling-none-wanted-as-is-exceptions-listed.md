...
```
①標記三類：keep（4＋你裁的 0–2 支基建）／archived-unmerged（24 單 commit＋其餘）／abandoned（3 支 4 個月＋A2b＋survival-execution-lock＋player-trade）
②「38 待裁是上界」收；不追精確數——精確數的價值 < 逐支讀 diff 的成本，而需求會從票再敲門
③已在 known_issues/defers 有對應列的分支，在那列註「分支＝參考非基底」一行即可
④此 triage 到此結；不加閘、不做第二次普查
```
