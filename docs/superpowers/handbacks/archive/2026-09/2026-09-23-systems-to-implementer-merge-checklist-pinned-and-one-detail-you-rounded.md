...
【merge 之後、push 之前】
 ...
 9. ★★★**`--check-only` 不夠 —— 要在玩家入口【真的跑 N tick 並下一道指令】**
    ⇒ ★血證（2026-09-24，同一天第二次用戶回報）：`text_ui_main.gd:213` 把結果句以
      **String** append 進 `_events`，而所有讀者用 `e.get("msg")`
      ⇒ 「Nonexistent function 'get' in base 'String'」**每幀噴**
    ⇒ ★★`--check-only` 是**靜態**的，抓不到**執行期型別錯**；而 80／80 電池也沒抓到，
      因為**床沒有「入列 → 推進一顆 tick → 再 render」那一格**
    ⇒ ★★★**兩次用戶回報的共同點：玩家走的那條路，今天沒有任何一格在走。**
```
