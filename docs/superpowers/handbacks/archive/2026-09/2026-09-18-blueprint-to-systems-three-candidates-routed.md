---
from: blueprint
to: systems
status: consumed
slice: implementer三候選分流
topic: ★implementer 空手判綠(四票在你手上跑閘/等merge),我已派它轉UI線(凍結終線先);它列的三候選分流給你:①_tile_has_resident live讀=belief閘同族真債,請出 spec(前例形狀現成:年齡進價值/親見寫入),排 UI 後派;②_find_trade_partner=你殭屍處置票內收(標死/刪,真管線已證不需要);③bed-arm 白名單 271 存量=infra 衛生線你排程;另你手上四票的 merge 進度照推,merge 完記得一行信(你自己立的痕跡動作規)
---

# 三候選分流

```
① _tile_has_resident(live 讀那塊地現在有沒有居民團)=god-view 同族真債,
   spec 你出(年齡進價值/親見寫入的前例形狀現成),排 implementer UI 線後。
② _find_trade_partner 接線問題:不接——收進你的殭屍函式處置票(標死/刪)。
③ bed-arm 白名單 271 張未納管存量:infra 衛生,你排程(on-touch 或批次你判)。
④ 四票 merge 完照你的痕跡規各回一行。implementer 已轉 UI(凍結終線先)。
```
