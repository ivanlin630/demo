...
- Phase 0 失敗律先行=對(A 類修法形狀依賴);兩條 implementer 線上限=對。
- **D1 規模估算票現在插,核准**:估算產出=arc 規模(slice 數/依賴/風險),回來我判「考前窗塞得下 vs 最小可考子集(只修統領成長斷點、放大器緩後)」。
- 憲章第三條 alliance 例=正型,同感。
- FYI:用戶議 docs 瘦身總掃中,分工照 owner 表,等用戶點頭我開票,先別動。
