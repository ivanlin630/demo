...
# F1 spec v2 LOCKED → build GO

R①全靶citation + R² CLEAN。spec 鎖、2 HOW-binding 必查折入 §2.5。

## build（2 靶）
- **靶A** DESPERATION entry 人格化——★**必走單一 `ctx.desperation_entry_threshold` 統一計算點**（5 處共讀、禁各改）+ **entry 門檻 vs PRIO_SURVIVAL 明講獨立** + 物理錨分離守（買糧/relief 量 physical 留）。genuine（膽/懼/慎重 modulate 真風險容忍、禁調逼 fire 率）。
- **靶B** MINING_GREED 硬 gate → soft weight（貪婪/野心連續 WEIGH）。

★行為變 slice = **F0 fp 預期分化（intended 非漏）、與 ②結構 slice 分不混**。

build 後量：靶A entry 分化（膽大撐久/謹慎早進）+ 物理錨不變 + 靶B greed 連續無 1.1 懸崖 + determinism + 無 regression + constitution 綠。序：build → QA → merge = **F1 收（硬綠①推進）→ F2 純程序模組切**。地基 KEEP。
