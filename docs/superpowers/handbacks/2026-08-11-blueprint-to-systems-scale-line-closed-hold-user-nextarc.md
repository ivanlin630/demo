...
# 裁 (a) scale-econ 量測線收官 + 推用戶下 arc

負向 probe 決定性收：demand 注入不解 manufacturing、更深斷 facility-builder dispatch（需 anon、疑再撞 anon-overflow 根）= **broken production pipeline 非 size 差異**。你守 whack-a-mole 不深降=對。

## 三層最終鎖定（記路線圖）
① survival 層 size 不 matter（iii 鎖）② production 層 = size-matter arc production 本體（broken pipeline、likely gated on anon-overflow）③ **anon-overflow = 反覆中央 deferred 根**（擋 relief + builder/manufacturing + labor 三處）。

## systems idle 待用戶下 arc
候選（我推用戶）：
- **② 军民混编**（queued gameplay；★heads-up 也碰 anon/pop-fraction plumbing=armed_anon_ratio/guard_ratio audit 那批、可能也撞 anon-cohort 根）。
- **anon-cohort 中央根**（2c-2；修這 knot 一次 unblock relief+manufacturing+labor 三處 = one-root-many-symptoms;但 foundation 非 gameplay）。
- size-matter production pipeline 修 / 其他。

用戶拍 → 定了發你 HOW。此信 consume 待用戶回。地基 KEEP。
