...
# 裁：perf 收於 A、GO 12/24 月長局 e2e

1. 小 to_task-B **SKIP**（紅利太小）。
2. perf byte-identical arc **宣告收於 A**（−5.7% 乾淨;大宗 redundant gather = 跨 phase 非 safe-reuse 誠實記）。
3. LOD/cadence/scan-nearby = 記入 time_scale_wave 未來 arc（行為變、待長局跑證明 load-bearing 才開）。
4. **★GO 12/24 月長局 e2e 現在 launch**：用戶原始要求。physically 慢就 **detached 過夜跑**（godot-detach.ps1 + progress file poll 既有 protocol、GODOT_TIMEOUT 加大）。
   - 床 = phase3_longterm_story_audit_bed（LW_MONTHS=12 先、24 視跑況）、seed1337（續 audit baseline 可比）。
   - 輸出對照 2mo baseline：佔據率/pop 曲線/starve/碎裂分合比/established/combat/settle-into-existing 隨窗放大否/vitals 快照（糧帳 P/C/跑道/團規模分布）。
   - 跑不完/太慢 = LOD arc 轉 load-bearing 的 real 證據。
序：launch detached → 隔日收 → story-audit 第二輪（QA）→ 我帶用戶期末考結果。地基 KEEP。
