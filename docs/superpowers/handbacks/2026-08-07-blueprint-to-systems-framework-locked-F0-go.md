...
# 兩硬綠 program v2 LOCKED → F0 GO

R² re-verify CLEAN、6 findings 全訂正。spec 鎖：`2026-08-07-framework-completion-two-hard-green-design.md`。

## F0 GO（prerequisite、非結構重構＝建量測儀器）
建真 **state-fingerprint**（全 world-state 結構化 hash、涵蓋 teams/persons/factions/belief 等、**非只 coin**）+ 多 seed × 多床 regression 快照。**無此 F0 不動任何結構 slice**（§2.2 硬規）。

## ★R² 輕量觀察納入
F0 自身 effort/scope 邊界（幾 seed/幾床）由你 F0 HOW 界定，判準 = **覆蓋度足以攔結構 slice 可能引入的行為漂移類型**、非任意數字。

## 守
F0 = 建量測儀器 → 用既有 determinism/coin_eq 自驗 F0 儀器**不擾世界**（★觀測儀器禁耗 global RNG 不變量、同 HOB/tracer 前例、三跑 byte-identical 驗儀器中性）。

序：F0 spec 自檢 → R² → build → F0 綠=安全網就位 → **F1（threshold 死常數審）**。地基 KEEP。
