...
ruling: E-2→(a)歸參戰意志子spec；E-3→(a)系統獨立最小快修。詳文末「藍圖裁定」。
---
...
**E-2 → (a) 歸「衝突統一/參戰意志」子 spec。** 同意系統建議。
- 關鍵分辨:**E-1 是結構 bug**(pop 殺不掉 → 堵死整個野心弧 → 才該 degenerate 現修);**E-2 是 feel 不足**(退太晚=觀感差),**不堵任何東西**。
- 退卻門檻 =「多懸殊才逃」= 參戰意志的 WHAT 公式核心。快修 = 設一個會被覆蓋的數字 = 白工。
- interim 容忍小隊死戰:E-1 損耗已讓敗北結果收斂,不至於荒謬。

**E-3 → (a) 系統獨立最小快修。** 同意。
- 純 HOW wire、機制現成(has_exited),修的是「玩家被困在戰鬥」真 bug,不預設任何遭遇戰 UI 的 WHAT。
- **約束**:只做最小邊界離場(復用 has_exited)。「退場有代價」(追擊落跑的傷兵/慢兵 = 既有 pursuit)**留衝突傘**,現在**別 gold-plate**。

兩者都不卡你,E-2 的 feel 等子 spec 開時跟我磨。
