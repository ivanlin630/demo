...
### ③ buymaterial_drive 校正（繫 construction desire，競建設）
`terms.gd buymaterial_drive`：現 = shortfall band 0.5-1.0（太低）。**改繫 construction 迫切**——買料是建設的**前置**（想建但缺料 → 買料應與建設同級 urgent，非墊底）：
- `buymaterial_drive` = 標度化（`material_shortfall / CAP`，缺越多越想）**× 建設迫切**（reuse means-end：team 對 material-facility 的 max `_facility_deficit`，即「多想建」）→ 想建強 + 缺料多 → 買料 drive 高、競得過建設。
- weight「buymaterial」穿人格（貪婪/商業）保留。
- **★語意**：買料 util ≈ 建設 util（因買料=建設的必要前置），不再 1.7% 墊底。measurer/QA 校準勝率合理區間（別過衝變只買不建）。
...
## 驗收
- **TDD**：①想建 weaponsmith→need_keep(material)=full 80（非 24）②多 facility→cap 100 ③買料 drive 隨 construction 迫切升（缺料+想建→高 drive）。
