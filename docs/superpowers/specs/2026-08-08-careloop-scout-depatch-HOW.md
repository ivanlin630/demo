...
領主**結構知自家 faction 固定據點位置**（LOCKED info-net core spec §結構常識補則「faction 成員天生知自家所有固定據點位置=組織常識、position-only、新據點公告傳播」）。∴ care scout 目標位在 belief 缺時應 fallback `_faction_roster_pos`（faction_ai:4664 static、已 gate-ok 組織常識 accessor）：
```
var vpos = BeliefSystem.best_estimate(state, team.team_id, vid).get("tile_pos", Vector2i(-1,-1))
if vpos == Vector2i(-1, -1):
    vpos = _faction_roster_pos(state, team, vid)   # ★組織常識 fallback：領主知自家村位（position-only）
if vpos == Vector2i(-1, -1):
    return   # 仍無（非自家 faction / factionless target）→ 查不了（保）
```

## ★★§HOW-binding（寫死必守）
1. **belief 優先、roster 僅 fallback**：有 belief 位（含 stale）用 belief（保 fresh-preference）；**僅 belief 缺才** roster fallback（不覆蓋既有 belief-based 行為）。
