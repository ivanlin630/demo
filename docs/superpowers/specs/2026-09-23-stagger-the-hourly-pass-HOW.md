...
| ★P8 | **指派→執行延遲**（R² 2026-09-23 加）：`p100 ≤ 119 tick`（< 2 個 cadence）｜**結束時「已指派但從未執行」的隊 ≤ 最後一小時內被指派的隊數**｜★母體：指派事件數 **> 0**，否則本格**不可判**（不是綠） | 新 tap：loop1 `try_set` 成員時記 tick；該隊下一次 loop2 執行時記差值 | ★指派與執行互相等待 ⇒ 某些隊的延遲**單調增大**、或結束時仍有未執行的指派 ⇒ p100 衝破 119 |
