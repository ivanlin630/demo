...
#### ★★★拆法：批次是【過濾器】不是【替換迴圈來源】（2026-09-23，P5 當場抓到）

```
第一版（錯）：把 loop2／loop3 的迴圈頭改成 `for tid in team_ids`
  ⇒ P5 樁關臂 fp = 2b412247521e21d4aad4a8d3d6718678 ≠ 世代 7 ⇒ ★紅
成因：batch 是【tick 開頭】取的，而 loop1／loop2 期間會【建立／刪除隊】
  loop2 走 `state.teams`          ＝ loop2 開始時的活字典
  loop3 走 `state.teams.keys()`   ＝ ★loop2 跑完【之後】的快照
     （原註解逐字：「keys() 快照 → 滅團可安全 erase」）
  ⇒ ★★兩者的【可見性】與「tick 開頭的那一批」不同
修法（對）：迴圈頭【回到原樣】，錯開改成【過濾】，
  且過濾只在 `pass_stagger_enabled` 為真時生效
  ⇒ ★★★樁關 ＝ 不過濾 ＝ 與今天逐字相同（構造保證，不是靠運氣）
修後 P5：final_fp = 763e9ee9…（＝世代 7）｜traj_fp 同｜rc=0｜348s
```

★**通則**：把「對全體做」改成「對一批做」時，
**替換迭代來源**與**在原來源上過濾**不等價 —— 差別在
**這一 tick 之內被建立／刪除的實體，看不看得到**。
★★而這個差別**只會在指紋上現形**：行為看起來一樣、床也可能全綠。

★★★**P5 在這裡兌現了它的設計目的**：它在其餘六格跑之前就說「你拆錯了」，
⇒ 省掉一輪往【錯開】方向的除錯 —— 而那正是「樁關著紅就別去查錯開」那句話的用處。
...
| ★P8 | **指派→執行延遲**（R² 2026-09-23 加）：`p100 ≤ 119 tick`（< 2 個 cadence）｜★**積壓判準改成「年齡 > 1 個 cadence 的未執行指派 ＝ 0」**（不是「未執行 ＝ 0」——最後一個 cadence 窗內才被指派的隊本來就還沒輪到它的 loop2，釘 N==0 會【恆紅】，而恆紅跟恆綠一樣沒有資訊）｜★母體：指派事件數 **> 0**，否則本格**不可判**（不是綠） | 新 tap：loop1 `try_set` 成員時記 tick；該隊下一次 loop2 執行時記差值 | ★指派與執行互相等待 ⇒ 某些隊的延遲**單調增大**、或結束時仍有未執行的指派 ⇒ p100 衝破 119 |
| ~~P7~~ | ★★★**判準退役（blueprint 裁 2026-09-23，選項乙）**：量到 **+79.2%／+47.6%**（±5% 帶寬）⇒ 而「散相位不該改變總工作量」這個**假設被【已知原因】證偽** —— 本票順手拆掉了【勢力工作被重複 M 次】與【兩圈全世界掃描】。★判準做對了它的事：它偵測到這次改動不是工作量中立。★★不開隔離票（沒有任何決策需要那個歸因）。★★★**模板改**：日後散相位類票的吞吐格＝**「不得【變慢】>5%」**；**世代 8 的吞吐＝新基線** | 既有 perf 床 | ——（已退役） |
...
#### ★★6a-2 P8 實測與它的【範圍限制】（2026-09-23）

```
[PASSSTAG] ★P8：指派事件=131｜延遲 p100=88 tick｜年齡 > cadence 的未執行 ＝ 0（最老 0 tick）
⇒ p100 沒有衝破 119、積壓 0 ⇒ ★本票【不因 P8 blocked】，而這次這句話背後有母體
```

★★**範圍限制（implementer 自己標的，我收進 spec）**：`p8.assigned` 只在 `try_set` **真的設上**時記。
`try_set` 會 no-op（優先序被更高的佔住），而**沒被設上的任務沒有「執行延遲」可言**
⇒ ★**131 不是「每小時 × 每個 member」的總數，是其中【任務真的換了】的那些**。

★★★**兩格都有陽性對照**（而它們在今天之前**從來沒紅過**）：

```
注射 A：指派時刻往前推 200 tick ⇒ p100=288 ✗FAIL，而積壓格仍 PASS ⇒ ★只紅了該紅的那一條
注射 B：只有奇數 tid 才 erase（製造真積壓）⇒ 積壓=10 ✗FAIL
兩次注射跑完都還原，`grep -rn POSCTRL scripts/` ＝ 0 筆
```

★**兩個母體不重疊**（systems 於 `d710c5eee` 自行核過）：
`_assign_member_tasks` 對 `player_commanded_task` 非空的隊 `continue`
⇒ 舊站（玩家指令）攔下的隊**永遠走不到新站** ⇒ 搬 tap 不會重複計、也不會互相蓋掉。
...
★★**而「它應該會自我修正」是一句沒有證據的話** —— reviewer 的原話：
**純靜態讀 code 判斷不了會不會真的卡死**。
⇒ ★★★所以 P8 不是「多一格保險」，是**把唯一無法靜態判定的那件事變成一個會紅的數**。

★**它是計數／結構量**（延遲有沒有上界），不是比率 ⇒ **短窗答得了** ⇒ 與 P1／P3 同批，
由 implementer 跑，不進量測員那一輪。
