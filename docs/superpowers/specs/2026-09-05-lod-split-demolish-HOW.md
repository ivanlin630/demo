...
| 2 | ★★**同一世界內 per-team 的執行次數與距離無關** —— **沿用 ⑦ 的那張床**（`lod_phase_invariance_test.gd`），★★★但判準從「發薪次數」擴大到**至少三個不同系統**（例：`collect`／`interactions`／`vision`）。<br>★★★★**【2026-09-06 補：第三顆相依，implementer 揭】** 那張床的**分組靠 `lod.near/far.byteam` 兩個 tap**，而 **⑧ 之後那兩個 tap 會消失** ⇒ **分組必須改成【由床自己算每隊到錨點的距離】** —— ★否則那張床會從「證明距離無關」**變成「兩組都是同一批」的空綠**（★★而空綠與通過在輸出上長得一樣） |
