...
### ★★★ §6.7 補強：把「必報當下的數」升級成「必報變化」（reviewer 提 2026-09-17，systems 採納＋加一條）

**我自承的限制**：必報欄靠的是**有人會去看那一行**，而那不是機械的。
**reviewer 給的機械形狀（非門檻）**：
```gdscript
# OBSERVED @ <commit>（BED_WORLD=1 BED_DAYS=10 BED_SEED=1337 warring_states）：
#   錨定 = 9.7%／settled 單獨 = 0.59%
# ★這是【觀測到的參考點】，不是門檻 —— 下面的斷言【不會】因為數字變小而紅
```
⇒ 世界級那格**多印一行 delta**：`本次 9.7% vs 戳記 9.7%（Δ 0.0pt）`。
★**差別**：縮水的**方向與幅度自動出現在這次跑的輸出裡** ——
讀的人不必記得去翻三個月前的原始檔，也不必自己判斷「孤立的一個數字算不算小」。
★★**它仍然不是門檻**：delta 再大都不會讓 `_fails` 增加。

## ★systems 加的一條：戳記本身會腐爛，而腐爛的方式有兩種

| 腐爛方式 | 長什麼樣 | 處置 |
|---|---|---|
| ① **沒人更新** | 機制改了、戳記還是舊的 ⇒ ★**這其實是【好的】** —— delta 會變大，**縮水自己跳出來** | 不處理（這正是機制在工作） |
| ② ★★★**順手改成新數** | 每次跑完把戳記改成這次的數 ⇒ **delta 恆為 0** ⇒ **告警關掉而畫面不紅**（恆綠） | ★**戳記那一行必須帶【commit】**，且**改戳記要在同一顆 commit 裡寫理由**；★★delta 那一行**要把戳記的 commit 一起印出來** ⇒ 「戳記是這一輪剛換的」在輸出上看得見 |

★★★**②是這個機制唯一真正的死法**，而它跟我這一輪抓到的「恆真靜音」是同一族
（〈守衛輸入母體沒被驗證〉：**修恆空最容易做出恆滿**）——
