...
★遷移兩顆:
   ...
   ②faction_ai_system.gd:1499 定期徵收,動態 interval
      ★★★【2026-09-05 訂正:上一版寫「只在恰好 600 倍數時 far 隊徵得到」是【錯的】】
      —— faction_ai_system.gd:833 `evaluate_all(state, _team_ids)` 的 team_ids 是【底線參數、根本沒用】,
         而 :863 `for fid in state.factions` 每個 pass 都跑【全部】的盟 ⇒ ★盟【沒有被 near/far 分割】
      ⇒ ★★真病不是距離依賴,是【週期被量化】:
         pass 只發生在 t%60==0 或 t%600==0(而 600 是 60 的倍數)⇒ 觀測集 = { t : t%60==0 }
         ⇒ ★實際 fire 週期 = lcm(60, eff),失真倍率 = 60 / gcd(60, eff)
      ⇒ ★★★而 eff 由 leader 人格算出 ⇒ 【只有「平均人格」(0.5/0.5 ⇒ eff=1800,60 的倍數)準時】,
         其餘慢 15~30 倍(0.2/0.9 ⇒ 實際週期 81420 tick ≈ 56 天,而設計值是 1.9 天)
      ⇒ ★而它【製造了一個假的人格訊號】:看起來像「這位統領不愛徵收」,
         實際上是「他的 eff 剛好不是 60 的倍數」—— ★★偏袒平均、懲罰極端,
         ★★★正好是【人格化想要的反面】
...
| 3 | ~~far/near 次數對照~~ ★★**作廢（問錯了問題）**：盟不分 far/near ⇒ 那個對照**恆等**，**會印綠而什麼都沒驗**。★★★**改判準（implementer 提，我裁採用）**：**逐盟量【真的發了幾次徵收令】 vs 【該盟 leader 人格算出的 `eff` 應該給的次數】（窗/`eff`）** —— ⑦後比值 ≈ 1（每個盟都準時）；⑦前比值 ≈ `gcd(60,eff)/60` 且**隨人格散開**（★那才是這個病的簽名）。★**鑑別力**：撤掉⑦，比值必須**塌回 1/15~1/30 且逐盟不同**，**不是「全部一起變小」** |
