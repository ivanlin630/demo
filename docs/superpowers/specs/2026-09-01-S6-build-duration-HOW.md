---
status: 機制段 R² CLEAN(2026-09-01) ／ ★表值已定案並經 blueprint 正式簽署(2026-09-01)
owner: systems
...
# ★★★§6 表值（★已定案：實測 3.0 遊戲日坐實單位，blueprint 正式簽署）

```
★唯一錨 SETTLE_PERSON_HOURS = 720   （＝ L0_TO_L1_CORVEE_DAYS 3 天 × 24h × pop 假設 10）
```
| 項目 | person_hours | 倍數 | 遊戲日 pop=10 | 現況真值 | 對現況 |
|---|---|---|---|---|---|
| 紮營 | 240 | ⅓ | 1.0 | 240 | 不動 |
| **紮根 L0→L1** | **720** | 錨 | **3.0** | 4320 | ★修 S2 漂移 |
| 農田 | 360 | ½ | 1.5 | 72 | 5.00× |
| 工坊/藥坊 | 720 | ×1 | 3.0 | 168 | 4.29× |
| 馬廄/熔爐/武坊/甲坊 | 1440 | ×2 | 6.0 | 336 | 4.29× |
| 鑄幣 | 2880 | ×4 | 12.0 | 720 | 4.00× |
| 據點 L2 | 2160 | ×3 | 9.0 | 300 | 7.20× |
| 據點 L3 | 4320 | ×6 | 18.0 | 600 | 7.20× |

★**兩次自我驗證（★都是表算完之後才發現對上的，不是調出來的）**：
```
①紮營落在 1.00× —— 而 WHAT 自己標它「不動」
②紮根 3.0 遊戲日 —— 而那個常數的名字就叫 L0_TO_L1_CORVEE_DAYS = 3
```
★★**紮根的處置是【修 S2 漂移】不是變快**：`CORVEE_DAYS * TICKS_PER_DAY` 在舊根下 240 ＝
【24 小時 × 假設 10 人】兩個語意合一，換根只動了時間那半 ⇒ 人力假設被一起放大 6 倍。
★★★**改法必須拆開那兩個語意**（★不得只把 `TICKS_PER_DAY` 換成 `TICKS_PER_HOUR` 了事 ——
