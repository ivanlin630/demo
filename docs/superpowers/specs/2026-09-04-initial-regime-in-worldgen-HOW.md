...
```
★**為什麼 3 獨立**：**它離三方等距**（訂正 2026-09-04：★**我原本用 Euclidean 算，而遊戲用 hex 距離** ——
★★改用 hex 後是 **A／B／獨立各 8，三方等距**；**結論不變而支持數字原本不準**）—— **它就是「流民」那一格**。
★**為什麼 8 獨立**：**商隊 tag** —— ★★**它不是「這一帶的人」，它是路過的人。**
★★★**而這是【刻意蓋過地理】不是漏看**（reviewer 指出）：**`8↔10` 是全資料集最近的一對（hex 距離 2）**，
**而我仍然把它們拆開** —— ★**tag override geography 是有意的，寫在這裡讓下一個人不必重新發現。**
...
# ★★★★★⑦R² 二輪吸收（2026-09-04）
```
①★度量錯了：我用 Euclidean，而★★遊戲用 hex 距離 ⇒ 支持數字訂正（結論不變）
   ⇒ ★★★通則：**算世界裡的距離，要用【世界自己的度量】** —— 數學上方便的那個不算數
②`leader_team_id` 只在 worldgen 寫、無 runtime 重指派 ⇒ ★代理選擇【永久且不自我修正】
③★★連帶範圍【比我寫的寬】：不只政權相關統計 ——
   **`徵收／歸建` 變成新的活候選 ⇒ 整個競爭池改變**
   ⇒ ★★★**任何「哪個 option 贏了 argmax」的既有量測／`known_issues`，只要跑在 `peaceful_economy` 上，都可能受影響**
```
