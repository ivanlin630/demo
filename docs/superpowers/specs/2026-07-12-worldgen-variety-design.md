...
**★位置熵護欄（reviewer 靶A）**：評分**不得是唯一驅動**——防「挑最高分」收斂成新規則熱點（規則從 key-order 換成分數排序＝換湯不換藥）。須注入位置隨機性：**top-N 隨機挑（非純 argmax）**，或評分含強 per-seed 位置噪聲項。有機性由「resource map 已 seeded + 位置熵」共同保，非只靠評分排序。
...
## 結構地板（防「跑不動」，非平衡）— ★局部升全域（reviewer 靶C）
局部幾何（原有）：
- ≥1 勢力、≥ 幾支隊（sim 有東西跑）。
- 據點數 ≤ 硬上限（留空地，見 §2）、不重疊同格、間距守。

**全域結構（新增，擋放野退化角落）**：
- **每勢力 ≥1 可達據點 + 領土連通**（擋「某勢力分到 0 據點/不連通」）。
- **散布覆蓋度下限**（象限/包圍盒分布檢查，擋「據點全擠地圖一角」＝靶A 收斂病的空間版）。
- 獨立隊不全生資源死角（擋「隊生死角空轉」）。

除此全放野：勢力數/據點數/領土/獨立隊多寡/誰獨大 都讓 seed 甩。**放野＝失衡（獨霸/群雄/稀疏 OK），非結構壞死。**
...
- **能跑（全域地板）**：每 seed 結構地板守——sim 不空轉/不 crash、**每勢力≥1可達據點+連通、據點覆蓋度過線（非擠一角）、無死角空轉隊**。
- **★build-outpost 獨立待驗項（reviewer 靶B）**：留空地是**必要非充分**——另立測項「build-outpost 決策路徑在新開局下 **dispatch>0**」（機制存在=faction_ai:3060/outpost_system:295-324，但依 never-fire 教訓須實測會 fire，別把留空地當自動可觀測）。若新開局 build-outpost 罕/不 fire → 回報 blueprint（是另一個 never-fire 待查，非本 slice 保證）。
