...
# §9 ★★★第 0 步回來了 —— **它廢掉了我的 A1，而且理由是【結構的】**

```
修正後：**SCOUT 歸建率 ＝ 1.0000（54／54、29／29）** —— 不是 16.6%／10.6%
   被稀釋的全是 `info_scout`（271／245 次，它們在 `:3940` 就 return，到不了 blanket）
★★★而 implementer 撞到的那個結構事實比數字重要：
   **那條 blanket 是【無條件】的** ⇒ 走到它面前的**每一個**都會被歸建
   ⇒ **`evicted ≡ reached`**
   ⇒ ★**卷面上每一個 ratio 其實都是「抵達後有沒有被【更前面】攔掉」，不是「歸建的傾向」**
```

## §9.1 ★★所以 A1 不能是「那條規則自己的比率」

```
★我原本寫「SCOUT 歸建率下降 ≥ 一半」
⇒ ★★**修法前它恆為 100%（構造上），修法後它的分母被我改掉** ⇒ **前後不可比**
⇒ ★★★**通則（值得記）**：
...
⇒ **A1'（新）**：覓食子隊**真的採集到東西**的量 —— 現成 tap：
   `collect.forage_subteam_task_collected`（★量測員的床已經在印）
   ＋ 對照欄 `collect.l0_forage_ran`
⇒ **門檻**：★**在實作之前，先量它的 baseline**（同窗同種子，現 main）
   —— ★★**我不現在挑一個數**，因為我今天已經挑錯過一次（50% 那個）
⇒ ★★★而**方向**現在就可以寫死：**baseline 已知之後，門檻寫成「顯著上升」且以多 seed 變異為單位**
```

## §9.3 ★仍然保留的三格（它們不受這個結構事實影響）

```
A2【硬閘】`subteam.arrival_decision.{merge,stay}` **兩個分支都要出現過**
   ⇒ ★這一格反而更重要了：**它是「引擎真的在秤」的唯一直接證據**
A3【母體衛生】各型別 arrived 與前置量測同量級
A4【陽性對照】人為把 util 差距拉到極端 ⇒ 兩個方向都要能翻
★★而 **`evicted/reached` 降級成【觀察欄】** —— 它仍然可以印，但**不得當判準**
```
