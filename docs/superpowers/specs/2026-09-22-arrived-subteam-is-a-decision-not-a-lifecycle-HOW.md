...
# §10 ★★★前置量測 #2：**子隊在外時長，是誰在決定它**（WHAT 裁 2026-09-22）

## §10.1 WHAT 的判準（原文照收）

```
量：每次外出的時長 vs「外出時到母隊下一次 subteam 評估的距離」；分佈與相關
判：(a) 時長 ≈ 到下次評估的距離（相關 ≥ 0.7 或時長分佈跟評估相位走）⇒ **節奏驅動**
       ⇒ 病在「評估即歸建／驅逐」那條 blanket 規則（＝本票的根）
    (b) 時長與評估距離無關、跟任務完成（飽／到貨）走 ⇒ **任務驅動** ⇒ 相位改變只是讓評估
       更常落在「已完成」之後 ⇒ 好事，記觀察
    (c) 之間 ⇒ 混合，先修 (a) 的路徑
★兩顆種子；母體＝外出次數（本輪翻倍後 n 夠）
```
★**血證來源**：只動相位（窗 #3）就讓子隊周轉 **×2.07／×1.48**、在外淨數反而變少
⇒ **在外時長被相位推動** ⇒ 它現在**不是任務決定的**。

## §10.2 ★★★systems 補一格：**相關係數在這裡可能【恆真】**

```
★**構造陷阱**：若 blanket 規則**就是**結束外出的那個動作，
   則 **外出時長 ≤ 到下一次評估的距離，是【構造上必然】** ⇒ 相關會很高，
   ⇒ ★★**而它在 (a) 世界與 (c) 世界裡都會很高** ⇒ **這個統計量分不出那兩格**
⇒ ★★★**改用【終止原因】這顆 tap，而不是相關係數**：
   **在外出結束的那一刻，記下是誰結束它的**：
     `blanket_evict`（評估即歸建）｜`task_done`（飽／到貨）｜`recall`（母隊召回）｜`death`｜`other`
   ⇒ (a) ＝ `blanket_evict` 佔多數｜(b) ＝ `task_done` 佔多數｜(c) ＝ 兩者都有量
   ⇒ ★**這是計數不是相關** ⇒ **不受「誰終止誰」的構造關係污染**
★**相關係數照算、照印**（WHAT 要的那一欄不砍），★★**但判決掛在終止原因上**。
★★★**而若 `task_done` 這一類【一次都沒出現】** ⇒ ★**不要判成 (a)** ⇒
   先問**它有沒有被寫進 code**（零呼叫點 ⇒ 是【我量了一個不存在的類別】，不是世界的答案）。
```

## §10.3 序

```
★**排在相位拆解之後**（CPU）；★★這是**將做**，不是已做。
```
