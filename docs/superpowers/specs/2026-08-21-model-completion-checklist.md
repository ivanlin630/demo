...
| # | 件 | 擋考? |
|---|---|---|
| D1 | 領導成長管道斷（established④） | ★**systems 估算：理由已被今日實測推翻**（AT_CAP **0.0%**、`eff_pop_cap` median **15** vs 人口 ≈1.2 named/隊）→ **建議先一次便宜量測（AT_CAP 比例 / 跨門檻 12 隊數 / 統領是否隨時間上移），`AT_CAP>0` 才開 arc**；否則降**非擋考**（harm 未實際發生＝憲章「未知」非「已知壞」）。詳 §D1 估算 |
