...
| `needs.safety > 0.7` | **該隊 named 通過安全門檻的比例**（無 named 時取 leader；★**再無則 0.5**） | **用既有世界訊號，不新增機制**；「當家的人覺得安全」是 anon 處境的合理代理。★**fallback 由 1.0 改 0.5（R² 建議、採納）：缺人管理的隊不該預設「最安全」** |
...
### ★R² 三條建議：全採
1. **安全代理 fallback `1.0 → 0.5`** —— **缺人管理的隊不該預設最安全**（我原案最可疑的就是這點，R² 判得對）。
2. **anon／named 門檻不對稱**（named 兩層都吃、anon 只吃 `f`）：**可接受，但記進 `known_issues`**
   —— **不在本刀對齊**（那會動到現行 named 行為）。
3. ★**`P_ref` 跨多個 peaceful snapshot 取中位數，不用單一快照**
   —— **同 K 值那輪的教訓**：**單一快照的中位數本身就是個抽樣值**，拿它當常數的錨會把抽樣雜訊烙進世界。
