...
owner: systems ｜ 2026-09-09 ｜ player_reachable: no ｜ 狀態：DONE（2026-09-09 merged `b0eadc01`）｜實測 中性 4.20／慢 2.00／快 12.10 tiles/day
...
```
①【腦看得見】——★★★2026-09-09 訂正（implementer 交件時揭）：
   ★原文寫「慢隊的計畫天數【相對舊平版 2.0】變長」——★★【結構性不可能】：
     clamp 上限 MAX_MOVE_TICKS=720 ⇒ tiles_per_day 下限 = 1440/720 = 2.00 = 舊常數本身
     ⇒ 最慢的隊【剛好等於】舊值，沒有任何隊能比舊估更慢。
   ⇒ ★★★那一格【永遠綠不了】，而那比沒有格子更糟：下一個人會以為是自己接錯線。
     （★這是「驗收假設了不存在的開關⇒那格永遠不會紅」的鏡像：永遠不會【綠】。）
   ★訂正後的判準（測【差異化】而不是測 clamp 邊界）：
     慢隊的每格天數 > 中性隊 ＞ 快隊，三者兩兩分得開。
   ★★快隊「相對舊平版變短」照字面成立，保留。
