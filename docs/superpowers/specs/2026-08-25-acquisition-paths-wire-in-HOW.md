...
## §10 ★★★閘③的最終形態：**從「該變」翻轉成「不得變」**（2026-08-25，implementer 推導）

**在「一行動一真值」之下**：
**兩個候選導向【完全相同的 `to_task`】⇒ ★誰贏，世界都一樣 ⇒ `fp` 結構性地不可能變。**

⇒ ★★**閘③在這張票上【不可觸發】，而原因就是裁定本身。**
⇒ ★★★**所以它翻轉了角色**：
| 原本 | ★**現在** |
|---|---|
| ③ ＝ **「證明接上了」的判準** | ★★**③ ＝「一行動一真值成立」的【否證器】** |
| `fp` 不變 ⇒ 紅 | ★★★**`fp` 若變了 ⇒ 紅 —— 那會否證裁定** |

### ★誠實標：**這條判準我改了三次，每次都是被實測逼的**
| 版本 | 內容 | 被什麼推翻 |
|---|---|---|
| v1 | **`fp` 不變 ⇒ 沒被執行** | **實測：執行了（`emitted` 非零）** |
| v2 | **`fp` 不變有三格**（沒執行／有產出不改決策／贏了結果相同） | **裁定：一行動一真值** |
| ★v3 | ★★**`fp` 不得變 —— 變了才是紅** | — |

★**v1→v2 是我補上漏掉的格；★★v2→v3 是【制度改變讓其中一格成為必然】。**
★**兩者不同**：**前者是我沒想周全，後者是世界的規則變了。**

### ★★★而這帶出這張票的【真問題】
**若 facility 分支上兩個候選永遠等價 ⇒ ★means-end 在該分支上對世界【零影響】**（既有候選總是在）。
⇒ ★★**它的價值只能在【既有候選不產生】的情境** ——
★★★**那一格若是 0，這張票就是「機制正確、世界零影響」。**
⇒ ★**驗收必須有一格能回答它有沒有用，否則五閘全綠也只證明了「沒做錯」。**
