...
| 3 | ★**掠奪與攻擊對【同一個目標】** ⇒ 兩者 util **同量級**（★**訂正 2026-09-16：舊版寫「比值落在 `RAID_TAKE_FRACTION` 附近」，而那個判準寫得比它能支撐的精確** —— 兩邊的輸入不同（最弱 prey vs 最富目標）、`need`／`person` 不同、而壓縮函數是非線性的 ⇒ **比值沒有理由等於 `TAKE_FRACTION`**。★★**改成兩條可判的**：**①同量級（比值 ∈ [0.1, 10]）②★方向：對同一目標【掖奪 < 攻擊】**（take 是一個分數）。實測 **0.572** ⇒ 兩條都過。） | 差 4–10 倍 ⇒ 還在兩把秤上（★**這格就是本票的目的**） |
