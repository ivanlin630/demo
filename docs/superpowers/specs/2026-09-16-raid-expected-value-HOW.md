...
- `person` ＝ 好戰／殘忍 ★**只在 eval 的 `person` 項**。
  ★★**訂正 2026-09-16**：舊版寫「在【人格層】調製」是**歧義的** —— 它可以讀成 `weight` 也可以讀成 eval，**而兩個都做了 ⇒ 人格被乘兩次**。
  ⇒ ★★★**`weight("loot")` 必須同一刀歸中性 `1.0`** —— 這是本 code base **逐字寫了四次**的約定（`terms.gd:605-612`：`attack_opportunity`／`intent_fit`／`idle_employ`／`help`／`scout`）。
  ⇒ ★**通則：人格從 `weight` 搬進 eval 時，`weight` 必須同一刀歸中性** —— **搬家做一半＝同一個東西同時活在兩個地方，而那比留在原處更糟。**
  ⇒ ★★與【殘忍進 `take`】**不衝突**：那一層是**世界的物理**（殘忍者真的搶得比較多），**不是偏好**。
...
## ★★★範圍訂正（2026-09-16，**因為切片本身錯了**）

**實測**：`p1` 在 food_est 200／1500／4000／20000 四檔，**掖奪從來沒贏過攻擊**。

```
★根因：blueprint 四個物理詞我只落位兩個半，
  而**沒落位的兩個（立即到手／招報復）剛好都是【攻擊的成本】**
⇒ 我做了掖奪的折扣、把攻擊的折扣留到以後 ⇒ **攻擊嚴格支配掖奪**
⇒ ★★**這不是「先做一半」，是【做一半會產生一個比原狀更錯的中間態】**（舊制至少還會搶）。
```
**★訂正：`effective_loot_rate()` 也要乘進【攻擊】的 loot 項。**
理由是世界的事實：`_loot_resources`（`npc_combat_system.gd:579-586`）**被歼滅與潰逃控地共用**
⇒ **攻擊贏了也只拿 0.3–0.51 倍** ⇒ 舊的 `_loot`（全額 belief 財富）**是高估**。
★★**而「那塊地」不會因此消失**：它由**佔村** option 表達（`options.gd:187-194`、`occupy_drive` 走 `DiscountedFlow`）
⇒ 攻擊 option 再把全額身家算進去 **＝算兩次**。

**★格3 隨之改寫**：舊版「對同一目標 掖奪 < 攻擊」的理由（take 是分數）**在修正後不成立** ⇒
新：**對同一目標、同人格 ⇒ 兩者的 loot 項【逐字相等】**，
util 的差只能來自 `need`／`person`／`weight`；★★**若還有別的差，就是還有一條沒同源的線。**

## ★★★後續：親和度那一格（blueprint 裁 2026-09-16，另一張票）

本票把掖奪搬上秤之後，露出一個**在式子外面**的壓制：
`NeedHierarchy.consistency_coeff` 讀親和度表，而 **掖奪 的生存層只有 0.4、紮營 0.6**
⇒ **餓的隊被系統性導向【紮營】而不是【去搶】**（而紮營不生食物）。
★**blueprint 裁：那一格要改，掖奪的生存層不得低於紮營；具體值從表內語意一致性推。**
★★**我推出的值：`[0.70, 0, 0, 0.25, 0.05]`**（單調帶 0.9 覓食／買糧 > 0.8 乞食 > **0.7 掖奪** > 0.6 紮營；
次層維持舊的 5:1 比例）—— **在另一張票裡做，不在本票**。
