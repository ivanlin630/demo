...
## §8.2 ★「0 比 0」那格【結案】（R² 2026-09-04）—— ★★而理由比我的強
```
★我的論證:底部平手無害(一堆 0 都不會贏)
★★reviewer 的論證更強:能【走到】這個 tie 的前提是【整池沒有任何候選 util > 0】
   (否則 argmax 早就選別的了)
⇒ ★★★那個狀態下選哪個 0-payoff 候選都是【等價的 no-op】—— 不是「選錯了、漏掉更該做的事」
⇒ 所以 clamp 後【不需要】保留「有餘多少」的區分
```
★★**可重用的形式**：**判斷「這個分辨重不重要」，要看【走到那個狀態的前提是什麼】** ——
**若抵達該歧義的前提本身就蘊含「所有選擇等價」，那個分辨就是假問題。**


## §9 ★具名回訪【已答】：段級 `PHASE_TIMING`（2026-09-04，`EXCLUSIVE=yes`）
```
★wall_clock_s 146.5 → 139.2（−5.0%）｜unified.rank 27.95M → 25.21M us（−9.8%）
⇒ ★★我 merge 時明寫的風險（「merge 之後才發現的成本」）【沒有實現】
```
★★★**而方向要講準：不是「導出讓它變快」** ——
```
★原因與計數那半一致:候選池小了 ⇒ rank 要秤的東西少了 ⇒ 省下的比多算的那一次乘法多
⇒ ★★所以正確的說法是【導出改變了世界,而那個世界要秤的東西比較少】
⇒ ★★★不得引用成「導出比較便宜」—— 那會在下一個池子沒變小的情境下騙人
```
★**取樣限制（implementer 自標）**：`[FaiPhase]` 是**尖峰觸發**的取樣、不是每 tick；
兩跑行數 **371 vs 372**（★**幾乎相同 ⇒ 可比**），**但它仍然不是全量。**
