...
- **★感知鐵律**：候選來自 belief（team_discovered/tile belief）；owner=-1 tile 的「空」判定用 belief（過期→抵達發現有人→既有遭遇機制談判/入夥/衝突/離開）。
- occupy 現只掃活 resident（:4992-4999）→ 擴：+掃 belief-known 無主 outpost tile（新分支、margin 對 pop_est=0 必過=撿現成）。
...
- **L0 有**：最低採集（低倍率**單旋鈕** `L0_GATHER_MULT`、物理=無田無具只撿周邊）/ 過夜安全 / 極小快取 / 體驗選址（住著持續觀察地力威脅→餵紮根決策）/ 行軍中繼（既有軍民分型掛）。**L0 無**：設施/農田/糧倉/稅/領土宣稱/居民身分（勞力池從 L1 起=居民/流浪界線）。
