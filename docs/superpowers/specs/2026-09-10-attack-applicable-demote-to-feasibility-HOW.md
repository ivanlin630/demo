...
- **序**：~~排在體驗窗四票之後、不得先 dispatch~~ ⇒ ★★★**停放已解除，現在 dispatch**（blueprint 裁 2026-09-12）
  ★停放理由（體驗窗四票）已清；前置（錯開／十倍／世界甲醒）全到位。
  ★★而推它一把的是一個新證據：**兩支敵隊面對面站著而沒有人動手 ＝ 60.1%**
  ⇒ ★★★**97.37% 那個全域快照，現在有了【現場版】**。
...
| ⑤ | **滅團率**（煞車讀數，護欄③） | ★**若暴走＝殲滅-heavy 債提前到期 ⇒ 回 blueprint，【不得 revert 門】**（門是對的，債在敗北模型）——★處理方式**預先寫死在此表**，免事後臨場判斷 |｜★**前置：母體地板（見下方）**
...
```
★這一格是【煞車讀數】⇒ 它的預設答案是「沒暴走」，
★★而【沒暴走】與【這個窗裡根本沒有滅團】在報表上長得一模一樣。
⇒ 前置：先印【改動前那個窗的滅團次數】。★若它是 0 ⇒ 這一格【不可判】，
   要換一個滅團次數非 0 的窗 —— ★★★不得把 0 讀成「煞車有效」。
★血證 2026-09-10（殭屍窗群甲）：warring_states 自然長跑【10000 tick 滅團事件 ＝ 0 次】
  ⇒ 那一跑 fp 完全相同，而正確讀法【不是】「沒改到」也【不是】「修好了」，
    是【這個窗量不到這件事】。
★★換算：TICKS_PER_DAY = 1440 ⇒ 10000 tick ≒ 6.9 天
  ⇒ ★★★那是【窗長】的事實，**不是「這個世界不會死團」**。
    本 spec 的窗是 30 天，但仍要先印那個數字：【窗夠長】不等於【那個窗裡真的發生了】。
```
...
## ⑨ ★★★2026-09-12 併入：**面對面案折成本票的前後對照**（blueprint 裁）

```
★**不另解剖** —— ★★**修法本身就是判別實驗**：
  開門之後，「**碰到了但沒開打**」的比率**該掉**；
  ★★★**掉剩的殘量 ＝ 雙方【真的】對峙 ＝ 那才是可入帳的戰國日常。**
⇒ 於是一個原本要靠逐例分類才能回答的問題，變成**一個前後對照**。
```

### ★前後兩側都要記的四組（★★before 側不能省 —— 沒有它，殘量無法解讀）

```
①**`攻擊`／`掠奪` 當下 applicable 嗎**：★**三道門各自的開關狀態分開記**（`:332-334`）
   ⇒ ★★這是**本票直接觸的量**，也是**唯一可以單 seed 歸因**的那一格
②雙方各自的 option 排行與 util（前 3 名）★含 `迎戰`／`攻擊`／`掠奪`／`逃跑`
③威脅評估：★**分開記** A 看 B、B 看 A（互相成立才算對峙）
④**有沒有人贏了但 commit 不成**（★手不聽腦的復發檢查）
```

### ★★★歸因的分級（★這一格是我加的，因為 blueprint 自己立的通則會咬到這張票）

```
通則（blueprint 立 2026-09-11）：**單 seed 的前後對照只對【票直接觸的量】有效**。
套到本票：
  ・**攻擊被提名的次數／三道門開關分布** ⇒ ★**直接量 ⇒ 可歸因**
  ・**「碰到了但沒開打」率** ⇒ ★★**一步下游**（提名 → 秤 → commit → 開打）
    ⇒ **可歸因但較弱** ⇒ **要附 before/after 的①才能解讀**
  ・**開打總數／戰爭規模／滅團數** ⇒ ★★★**湧現聚合 ⇒ 單 seed 不可歸因**
    ⇒ **要嘛多 seed，要嘛在交件裡寫「不可歸因」**，★禁止當成本票的成績。
```

### 驗收補兩格

```
⑧★**「碰到了但沒開打」的殘量有【真對峙】的簽名**：
   殘量那一群裡，**雙方 option 都是防守型 ＋ 威脅評估互相成立**（blueprint 的 (a) 判準）
   ⇒ ★★而**現在它可判了** —— 因為①那一格證明**攻擊當下是可選的**。
⑨★★★**禁把缺口包裝成特性**：若開門之後該比率**沒有掉**
   ⇒ **不准**改口說「那就是對峙」。
   ★**訂正我自己原本寫的下一句**（原句：「那代表病不在提名這一層」）——**不夠準**：
   ★★**第四種可能：那一刻根本沒有人在做決策。**血證（2026-09-12）：
     面對面 dump 的**母體是 1** —— 條件是「迎戰 ＋ 目標還活著 ＋ 同格 ＋ **剛好輪到決策**」，
     而前三者成立的 tick 裡**大多數不是 cadence**
   ⇒ ★★★**面對面的那一刻，大部分時候【沒有人在思考】** ⇒ 開門也沒用，
     **因為沒人有機會用到那個提名**。
⑩★**新增（因上）**：每個「碰到了但沒開打」的現場，記**那一 tick 雙方有沒有真的跑決策**。
   ★現成入口：`faction_ai_system.gd:4053` `if WorldEvents.is_pending(...): return true`（事件瞬醒，不等 cadence）
   ⇒ ★★**要查：【相遇】有沒有發出事件給雙方？** ★★★我只讀了 code、**沒有斷定它沒有**，要逐筆資料答。
```
