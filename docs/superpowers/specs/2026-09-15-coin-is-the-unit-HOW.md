...
## ★★★藍圖裁：**A**（錢＝手段、走前置鏈）—— 2026-09-15

`maintain_coin` **拆掉或標死**（標死要寫理由，殘表教訓）。B、C 否決。

### ★而我以為要犧牲的「貪財戲」不用犧牲 —— 它在憲法裡另有位置（已驗）

```gdscript
# discounted_flow.gd:32-38  flow_weight()：「在意哪種明天」
"food":   return 1.0                                  # 活命流：不受人格折扣
"wealth": return 0.5 + float(values.get("貧婪", 0.5))   # ★財流：貧婪愛錢 → 折得少
```
（★`:26` 註解明寫：δ＝耐性／慎重族，**不是貧婪**；用戶 2026-08-21 裁「貧婪≠短視，貧婪調的是在意哪種明天」）
⇒ **貧婪領袖對【產生錢流的選項】天然加權** ⇒ 致富行為從人格×折現湧現，**一個 coin 都不用捿**。
⇒ **接線要求**：前置鏈「先弄到錢」的子選項估值**走折現磚**，貧婪吃 `"wealth"` 權重。

### ★★★而我要撤回自己一句講得太寬的話（驗證時撞到）

我寫過：「這個世界的隊伍，**從來不會因為想要錢而去做任何事**」。
★**太寬。** `trade_valuation.gd:122-135` 的 `_urgency()` 裡：
...
```
⇒ **缺錢已經會改變行為** —— `_urgency` 餵 `ask_price` ⇒ **賣得更急、折扣更深**。
...
⇒ ★★**而這反而是 A 案的強證據**：需求信號**已經存在、已經是真實量**（`estimated_payroll`）——
A 要做的不是造一個新需求，是**把這個現成的需求從【只流進定價】接到【目標選擇】**。

### ★發薪義務＝前置鏈的第一個合法目的（藍圖必含）

記名薪資 coin 現結是用戶定的制度 ⇒ **「發不出薪→想去賺」這條動機鏈必須通**，不只接「買資源」。
★**而它的【需要多少錢】已經有現成來源**：`SalarySystem.estimated_payroll(state, team)` —— **不必新造、不是手抄常數**。
★★**而 `need > 0.0` 那個守衛【承載語意】**（原註明寫：payroll=0 的隊沒有薪資壓力，
coin_urg=0 是誠實而非遺漏，**不得改寫成 `maxf(need, 1.0)`**）⇒ **接前置鏈時不得破它**。

### ★攻擊票的依賴（藍圖必含）：**解除一半**

攻擊票要的是把 belief 估值換算成 **coin 當量**（`...-attack-loot-monotone-compression-HOW.md:121/:142`）
⇒ **不需要 `need_keep(coin)`** ⇒ ★**不等 A**。
★★**但有一個條件**：它的籃子裡含 coin 時必須走**取價函式**（`local_value()` → 1.0），
**不得直接 `BASE_PRICE.get(res, 0.0)`**（那會把 coin 算成 0）。
⇒ ★★★**所以它不依賴 A，但依賴票甲的【取價單一入口】方向** ⇒ **兩票可平行，而攻擊票要在自己那處寫死走函式。**

## ★★★R² 第二輪：A 形狀 CLEAN，而三條必須**寫死**才算完整（2026-09-15）

### 硬約束一：子前置是【取代】不是【附加】

「先弄到錢」必須接在**同一條早退鏈**裡：買不起 ⇒ 那個 slot 回的**不是** `TASK_TRADE` candidate，
**而是「先弄到錢」 candidate** —— ★**取代**，**不是多生一個**。
（與 material／tools 買路同構；`goal_resolver.gd:628-629` 的舊註解已經記著同一個病被騙過一次：
...
### 硬約束二：【買資源缺錢】的量**自己算**，**禁止**借用 `estimated_payroll`

`SalarySystem.estimated_payroll` **只服務薪資這一個合法目的**。
「買 X 缺錢」的需求量是**自己的式子**（`qty_needed × 取價 − 手上 coin`，逐筆算）。

★**理由寫死**：`trade_valuation.gd:127` 的作者（就是我）**已經自己承認**
「誠實限：payroll ≠ 全部的 coin 需求（建設／採購／稅仍看不見）」
⇒ ★★**抽近路去讀它（因為它現成、已驗證）會逬著 `need > 0.0` 那個守衛去對【採購壓力】負責**，
而它現在的語意窄（只答「有沒有薪資壓力」）⇒ ★★★**這時候才會有人想去改 `maxf(need, 1.0)`** ——
...
```
【內層】「先弄到錢」的幾個手段互相比（賣貨／接活／劈掠）
        ⇒ 走折現磚，貧婪吃 `"wealth"` 權重 ⇒ ★**決定【選哪一個手段】**
【外層】這個子前置整體對 argmax 報的 util
        ⇒ ★★**仍然是【繼承來的 payoff】**（`derived_payoff` 那把已驗的尺）
⇒ ★★★**PV 值局部用完即丟，不外流。**
```

★**為什麼要寫死**：`flow_weight()` 回的是**係數**（∈[0.5,1.5]），`option_value()` 的輸出才是值，
而它的單位由 `daily_flow` 決定 ⇒ **沒人驗過它與 `price × qty` 同單位**。
★★**而【詮釋能通】不等於【事實通】** —— 這正是我今天已經抵到一次的同型病（`Σ gap × price`）。
