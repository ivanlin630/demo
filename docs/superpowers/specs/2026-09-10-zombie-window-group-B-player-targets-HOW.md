...
```
### (5) ★★★交件回來，而我的驗收⑤【前提是錯的】—— 撤回並訂正

```
我寫：「fp 會變（清單是 state 的一部分）」。
★實測：fp 完全相同 —— 因為 `player_pending_targets` **根本不在 StateFingerprint 的涵蓋範圍內**
  （`state_fingerprint.gd` 全檔 `player_` 命中 ＝ 0）。
⇒ ★★我犯的具體錯誤：**我把「在 state 裡」讀成「在 fp 裡」** —— 那是兩個集合。
⇒ 訂正後的⑤：**fp【不會變】，而它不變【不構成任何證據】** ——
   ★★★本票的證據是①②③④那四格（清單內容本身），不是 fp。
```

★**而更該記的是我在訂正時查到的東西**：

```
state_fingerprint.gd:16 有一個 `EXCLUDES` 常數，它是「本尺排除：…」那一行的【單一來源】，
而那一行的存在本身就是一次血證的產物（2026-09-01：有人拿 fp 當
「tracer 沒污染世界」的證據，而 fp 對那類是【結構性地瞎的】）。
★★★而現在：**那個 EXCLUDES 字串【是手抄的】，而它沒有列到 `player_*` 那一整塊。**
⇒ 讀那一行的人（我）會以為玩家狀態在尺裡。
⇒ ★**「儀器對某類是瞎的」的修法，當初是【手抄一份排除清單】——
   而手抄的清單不會跟著世界長大**（這正是 implementer 對 `snapshot_dict`
   用 `get_property_list` 而不是手抄清單的同一條理由，只是這次它長在【儀器的自我描述】上）。
⇒ ★★另開一張票：**讓 EXCLUDES 由 code 導出**（見 `2026-09-10-fp-excludes-must-be-derived-HOW.md`）。
```
