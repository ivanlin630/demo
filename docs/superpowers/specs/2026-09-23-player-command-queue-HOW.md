...
★★★**2026-09-24 訂正：上面那個列舉的【邊界選錯了層級】—— 實驗抓到的，不是 R② 漏審。**

```
sim_runner.gd:101-104 在【呼叫 _advance_tick_body 之前】就 return：
  if state.game_over: return "game_over"
  if player_forced_event.action == "choose_heir": return "awaiting_heir"
⇒ ★而消費點在 _advance_tick_body 【裡面】⇒ 這兩條路上佇列【永遠不會被消費】
⇒ ★★血證（票5 第一次跑）：等待繼承人時世界凍結，
  而**唯一能解凍世界的那條指令**（選繼承人）也進了佇列 ⇒ **永久死鎖**
⇒ ★★★通則：**列舉「有幾條路」之前，要先問「這個函式【會不會被呼叫】」** ——
  邊界畫在函式【內】，而真正的控制流在函式【外】就分岔了。
```

**修法（systems 裁 2026-09-24，採 (乙)）**：

```
★那兩個凍結分支裡【也各消費一次】，不動「記【遞增後】的 tick」那條規則
★★而理由比「解死鎖」更寬：**沒有它，凍結世界裡的每一條指令都是【靜默丟棄】**
  —— 而「拒絕禁靜默」（blueprint 裁）在那兩個狀態下同樣適用：
  玩家在等繼承人時按移動，他該看到「你現在得先選繼承人」，不是什麼都沒有。
★★★P3（無後門）的判準跟著改：
  舊「production 路徑上只有【一處】dispatch 呼叫」
  新「**只有 _consume_player_commands() 這一支函式會呼叫 dispatch**」
  ⇒ **消費的【位置】可以有三處，消費的【實作】仍然只有一份。**
```

**★新增 P16（缺陷變對照）**：

```
P16 [凍結狀態不得靜默] 讓世界進入凍結（等待繼承人）⇒ 送一條【非解凍】指令（例：移動）
    ⇒ ★必須收到【拒絕的結果句】，不是沉默
    ⇒ ★★再送解凍指令（選繼承人）⇒ 世界必須真的解凍（下一顆 tick 推得動）
