...
```
- ∴ home productive → restock_need=1.0 → 返家補給 drive 高 → **out-rank 買糧**（買糧 buyfood weight 非商隊 0.3×base0.5-1.0=0.15-0.3；返家補給 survival_pressure weight 1.0×1.0=1.0）→ 選返家 → TASK_RETURN_HOME → 到家被動採（harvest driver 站 tile 就採）→ granary 回填 → 脫餓。**不需 gate 買糧**（返家補給 競贏）。
...
## 驗收
- **TDD**：①plains 家（regen≥burn）+空 granary+food低 → 返家補給 applicable=true、restock_need=1.0 ②forest 家（regen<burn）+空 granary → 返家補給 applicable=false（不變，走 買糧）③granary 滿 → restock_need 照舊（granary_q 主導）④home_food_productive 算式（regen×harvest_factor vs burn）⑤無 home outpost → home_food_productive=false。
