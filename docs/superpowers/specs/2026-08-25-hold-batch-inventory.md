...
| 3 | `watchdog.sh` 頂端：明寫豁免清單 ★**＋ fire 訊息帶分類路徑 `class=X via:Y`**（blueprint 核可 2026-08-25） | 下次加分類不會又被默默吞 ★★**＋ 讓 fire【自述】** |
...
## §6 ★★★`#3` 的追加項為什麼值得單獨記一句
**表面上「fire 訊息帶 `class=X via:Y`」只是加一行。★實際上它改變了診斷的【類別】**：
| 沒有它 | ★**有它** |
|---|---|
| **異常 ⇒ 要重現 ⇒ 而 watchdog 的 fire 是【時間／狀態驅動的自然事件】，未必重現得了** | ★★**一次輸出即足診斷 —— 不需要重現** |

★**這是既有原則的延伸**：**「守衛不要輸出【需要被解讀的狀態】，要輸出【已經處置完的結果】」**
⇒ ★★★**再加一層：輸出還要【自述它從哪條路來】。**

★**而窗口內是唯一會碰那個檔的機會** —— **錯過就要等下一次凍改。**

---

## §7 ★重啟後首批（**blueprint 定，我記在這裡免得散掉**）
| # | 項 | 備註 |
|---|---|---|
| ★1 | **wire-in 故事稽核** | ★★**前置：`specimen` 要【真的產出來】** —— ★**我先前寫「已請 implementer 產」而它根本沒產，QA 對帳抓到** |
| 2 | **`stock-vs-flow-ruler`** | **R²-v2 CLEAN、spec 就緒、未起飛** |
| 3 | **`failure-memory ①` 續作** | ★**判準已在 `spec §25`（連坐折價集合型）** |
| ★4 | **`§8` 世界層量測** | `dispatch_fail.資源不足 33 → ?`（★**同床同 seed，判讀已寫死**） |

## §8 ★★★給 `memory` 積壓再加一條（**第六條**）
| # | 條目 | 血證 |
|---|---|---|
| ★f | ★★**「我已請 X 做 Y」不等於 Y 會發生 —— 而我自己的 `_promise_check` 抓不到這種** | **我在給 QA 的信裡寫「`specimen trace` 我已請 implementer 產」，★而它根本沒產；`_promise_check` 掃「已請 `<role>`」，但它的判準是【有沒有寄信給該角色】—— ★★我寄了很多封，所以漏網** |

★★★**而這正是我當天自己標過的弱綠限制**：
> **「它會把【我後來寄了別的信給他】誤判成【已回應】。」**
⇒ ★**這次是誤判成【已請求】。★★我標了那個限制，然後就被它漏掉了。**
