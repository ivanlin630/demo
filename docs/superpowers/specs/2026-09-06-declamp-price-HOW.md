...
## §4a-0 ★★★★【2026-09-06 裁定明確版：**沒有上游票要開**，⑩ 即刻動工】
```
★先前的順序裁定是:「撞下界 31.8% ⇒ 照分流規則【先治上游再拆】」
⇒ ★★而【歸因站的功能已經完成】:它查出【所謂「上游的病」其實是機制】——
   深過剩 = 經濟真相;而 glut 期間蓋東西 = 【世界吸收過剩的正解】
⇒ ★★★所以【沒有上游票要開】,而【「先治上游」那一站【被歸因站自己解掉了】】
★而【安全網不是那一站,是三格觀察】(§4b 的 7a/7b/7c):任一亮才回頭開【分軌票】
⇒ ★而分軌票是【新票不是⑩】
```
★**這一段要留著的理由**：**「先治上游」曾經是一個真的裁定** ——
★★**刪掉它會讓下一個人以為我們從來沒考慮過那條路**；而**它被解掉的方式（量一格、發現病是機制）本身就是方法**。

## §4b ★★★★【blueprint 裁 (a)：估值與市場價【同用一個真值】，不分軌】＋ 三格驗收
```
★裁定:深過剩時【邊際價值 ≈ 0 是經濟真相】;「東西多到爛,拿去蓋房」= 世界吸收過剩的正解
   ⇒ 而它餵【設施 → 規模紅利】閉環(剛好接 C 那條線)
★★而【為決策層舒適另設一個內部價】= 【util 禁 crank】的同族 ⇒ 不開
★★★而【壞了才開分軌票】(market_price／internal_value)—— 那是【新票不是⑩】
```
| # | 加的驗收格（★「(a) 會不會壞」的**直接讀數**，不是再猜一輪） |
|---|---|
| 7a | ★**`cost_value == 0` 的次數與佔比**（★★拆前基準是 0 次 —— 它是**新出現的狀態**） |
...
✗ 【不用 epsilon】(例 maxf(cost, 0.001))
   ⇒ ★因為 epsilon 【就是一個隱形的內部價】—— 它把成本偷偷墊高到一個【沒有人裁過】的數字
   ⇒ ★★而 blueprint 明文警告過這一格(「別讓它變隱形內部價」),他是對的
✅ 【重新定義】:`cost_value <= 0` ⇒ 該方案視為【最高優先】(正收益 ＋ 零成本)
   ⇒ ★而那【不是「無限大」這個數字】,是【一個語意】:零成本的正收益方案本來就該先做
   ⇒ ★★實作上用 INF 或一個明確的 free 分支都行,★★★但【不得寫成一個可調的常數】
   ⇒ ★tie-break 沿用既有 argmax 的確定性順序(不新增隨機,determinism 不破)
```
