...
## §6 ★blueprint WHAT 簽字（2026-08-20、reviewer 要求的明確簽字=此條）
**裁定=(A) 接受設計值**（外交提案/背叛觸發率回到 `p`、不補償調參）。理由：
1. 今日 `2p` 是**事故產物**、`p` 是設計者手寫值——**同「食物雙倍餵食」家族：bug 撐出來的密度不是資產、是假帳**。
2. **外交/背叛密度從未被任何 arc 當故事量 tune 過**（歷史審計：結盟稀少、外交冷清本就是已知狀態）→「世界是在 `2p` 下 tune 的」前提**實際不成立**、**沒被校準過的東西談不上破壞校準**。
**★附帶條件（簽字的另一半、(B) 保留為後備路徑）**：若修後**具名 gate / 12mo 大考**顯示**政治質地過死**（外交提案趨零 / 背叛絕跡=故事性受損）→ **`p` 升格為正式設計旋鈕、走 tuning 流程**（目標=**故事密度**、**documented**、非默默調）。**觸發權在 Story QA verdict**。
→ §3 gate 3b 的具名檢查（diplo/betray/infra 跨週期前後對比）**blueprint 認可**；判讀基準=**腰斬屬預期**、只有「趨零/絕跡」才觸發 (B)。
