...
## §1b ★★★【2026-09-08 訂正：我 §1 原本那句是錯的】

```
我原寫：「salary_system.gd 裡【沒有任何 unrest 寫入】（只有 :209 一個 Probe 讀）」
★真相：salary_system.gd:210-211
        if budget_ratio < 1.0:
            UnrestBank.add(team, 1, "salary")
⇒ ★★unrest【確實鉤在薪資上】，而且鉤在【同一個條件】上
⇒ ★★★無幣村 ⇒ budget_ratio<1 恆成立 ⇒ 每週 +1 unrest ⇒ **永動**（票面說的是對的）
```

### ★我漏掉它的原因，是我當天已經抓過一次的同一個病
```
我用 grep 'loyalty\|unrest'（★全小寫）
而實際符號是 `UnrestBank`（★大寫 U）⇒ 不命中
——★★而我在同一次調查裡【已經因為 `LoyaltyBank` 大寫吃過一次虧並自己說出來】，
   然後在同一份 spec 裡【對 Unrest 再犯一次】
★★★更難堪的是「重掃」也沒看見它：我加了 `head -5`，而它排第 6
   ⇒ 完整結果是 6 行，我先前報「1 處」
```
⇒ **教訓（已入 memory）**：★負斷言的搜尋**不得帶 `head`、不得帶大小寫假設**；
★★而「我剛剛才犯過這個錯」**不會**讓我下一次自動避開它 —— 只有**機械紀律**會。
...
| ③ | unrest **從薪資摘鉤**，歸**供養失敗** | `salary_system.gd:210-211`（★已坐實有鉤） | ★★**真改動**（訂正見 §1b） |
