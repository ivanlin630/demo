...
- ★★**必查項（R² 抓、非可選）：比對必須覆蓋「所有離開路徑」，不能只掛函式尾端**。
  `_update_goals` 有**兩個 early-return**：①`leader_team == null`（在 `clear()` **之後**）②**玩家 override**（`if not f.player_goal_override.is_empty(): f.goals.append(...); return`）。
  若比較只在尾端做，這兩條路**繞過比較 → 永不蓋戳記**。
  ★**而 ② 這條在今天是被「`_emit_goal` 無條件蓋」掩蓋著的**：玩家 override 路徑**完全不經 `_emit_goal`** → **玩家設定 faction 目標從來不蓋戳記、成員不被喚醒**；一旦改成「只在真變化時蓋」而比較又沒覆蓋它，**玩家下令會完全沒人理**（把既有缺口引爆）。
  **實作形狀（要求）**：`_update_goals` 拆成**薄外殼 + 內層重建**——外殼：**存快照 → 呼內層（內層可任意 early-return）→ 比對 → 蓋戳記**；內層專心重建。**所有離開路徑一律經過外殼的比對**。
  **副產品（正面）**：玩家 override 若改變了 goals，**現在會正確蓋戳記 → 成員真的被喚醒**＝修好一個既有的「玩家下令沒人理」缺口。
...
★**修正（R² 指出、我原本的宣稱過強）**：「反而讓『命令從有變無』正確蓋戳記」**只有在比對覆蓋 early-return 路徑時才成立**（見 §2 必查項）。若比較只掛尾端，這條路**照樣不蓋** → 我原本的宣稱會落空。**已把「比對覆蓋所有離開路徑」升為必查項**，該宣稱才站得住。
（early-return-after-clear 的形狀本身仍值得日後單獨看：領袖隊短暫消失時命令被清空且不重建。）
