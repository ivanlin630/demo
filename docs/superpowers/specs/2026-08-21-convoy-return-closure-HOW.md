...
9. ★**證偽誤殺（硬性）** —— ★**2026-08-21 實測後訂正：我原本的指標有洞。**
   原指標「`stranded(timeout)` 時 porter 距母隊 **≤2 格的比例**」**分不出兩種完全相反的情境**：
   - **(a) 即將抵達**：距離正在收斂，再一兩步就到 ⇒ timeout ＝ **誤殺**
   - **(b) 永恆尾隨**：**距離恆為 1、永遠追不到**（porter_12 那趟就是 **1000 tick 距離恆為 1**）⇒ timeout ＝ **正確收掉**
   ⇒ **同一個「距離 1」，一個該救、一個該殺。單看距離判不出來。**
   **訂正後的指標（三選二即可判）**：
   1. **stranded 前 N tick 的距離序列**：**單調下降 ⇒ (a)**；**持平 ⇒ (b)**
   2. **母隊在該窗內有沒有移動**：**沒動而 porter 靠不近 ⇒ 另有問題**；**一直在動 ⇒ (b)**
   3. **`rehome_n` 在該窗內的增量**：**一直在追新目標 ⇒ (b)**
   ★ **實測結果（warring 30 天、`@f9ad46c3` 2026-08-21）**：`stranded=3`，補上趨勢後
   **0/3 出現單調收斂 ⇒ 【三筆均不支持 T3 誤殺】**（measurer 主動撤回自己「100% ≤2 格支持誤殺」的 v1 初判）：
   - `porter100`：距離 **惡化 1→2**，母隊移動 3 次但方向不收斂 ⇒ **強 (b)**
   - `porter118`：**15 筆距離全為 1**、無例外 ⇒ **教科書級「永恆尾隨」**（同 porter_12 型）
   - `porter164`：★**第三型**——**母隊全程靜止**，porter **曾同格（dist=0）卻退回 dist=1 卡住 160 tick**
     ⇒ **抵達過但 merge 沒接上 ＝ 執行斷**，**不是 chase-timeout 問題**（另案，見下）
   **⇒ gate 9 判 PASS（無誤殺證據）**，★**但 n=3、低信心**，且**對照極乾淨**：peaceful 150 天 **0** vs warring 30 天 **3**。
   **gate 10（防呆絕對上限不得綁到）：`by_abs_cap` 全程 0 ⇒ 滿足。**
