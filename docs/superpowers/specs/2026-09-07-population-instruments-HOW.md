...
## §4c ★順手驗一個設計錨（blueprint 2026-09-07）
```
實測：約 75 天/胎（單隊最大 breed_progress=0.7975 @60 天窗）
設計錨：「健康村 5 適齡成人 ≈ 月產 1 名額」⇒ 30 天/胎
⇒ ★差 2.5 倍
```
★人口卷順手驗**這個錨在真實世界的落點**——★★注意它是【錨 vs 實測】的比對，
不是「哪個對」：錨可能過樂觀，也可能實測世界的 safe/fed 條件本來就比「健康村」差。
⇒ **兩個數字並排 + 各自的前提，讓用戶判。**
