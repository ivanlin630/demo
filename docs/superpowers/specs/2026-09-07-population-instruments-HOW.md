...
## §4b ★★★卷面紀律：**中性條目，不替用戶下判**（blueprint 裁 2026-09-07，用戶終審權保留）

人口卷**必須帶一條中性條目**：
```
warring_states 1000 tick ／ 65 支活隊 ／ ★小孩總數 = 0 ／ erase.batches = 9
```
★**中性的意思是兩個方向都不預設**：
```
不寫「生育壞了」——★資源池空 ≠ bug（既有法）；戰亂沒人 safe ⇒ 沒人生育，故事上成立
不寫「一切正常」——★★「世界需不需要人口循環才有長期故事」是【願景題】，不是我們的判斷
⇒ ★★★給數字 + 給機制（safe>0.7 AND fed>0.7 AND minors<pop*0.2）＋給對照（peaceful config）
   然後【停在那裡】，讓用戶說話
```
★**而這一格的現況是【暫准非終審】**：blueprint 依既有法暫准，token
`zero-children-genuine-or-defect` **照掛不撤**，判準＝peaceful config 對照。

★★**同時成立的儀器結論（與裁決無關）**：
> `erase.minors_lost = 0` 在**空母體**上**不得被讀成「沒有小孩死」**。
> ⇒ 這是 **0 的兩義病**又一例：**0 對「沒發生」與「沒有母體」給同一個判決。**
