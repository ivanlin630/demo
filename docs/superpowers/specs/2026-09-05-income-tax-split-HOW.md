...
## §0-RECONFIRM ★★★★【2026-09-05 撤回 → **重量後確認**】三個「0」**是真的 0**（★保留撤回原文，不刪）
```
★重量結果(measurer,drain 間隔 50 tick,同 seed 同床):
   ①member_tax 總額 = 0.00        ★overflow_hits = 0  ⇒【真 0】
   ②salary_named／salary_anon = 0 ★overflow_hits = 0  ⇒【真 0】
   ③匿名池:★母體【1 → 18】(舊版低估),而【18 筆全是 delta 恰好 = 0 的 no-op】
      (新增的 ZERO-DELTA 計數器 = 18,與 treasury_rows 完全對上)
      ⇒ 真正有金額的 inflow/outflow 仍是 0;水位樣本 979 → 39020,仍全程恆 0.00
★★而 `_ledger_seen` 從 266240 → 1017343 —— ★★★【舊版少看了 75%】,而它當時「看起來正常」
⇒ ★所以撤回是【正確的程序】,即使結論最後沒變:
   ★★那三個數字在被重量之前【沒有資格當證據】,而它們現在有了
⇒ ★★★而【母體 1 → 18】這一格證明撤回不是白做:舊版的 1 【本身就是被吃掉的結果】
```
★**下面的撤回原文保留** —— **它記錄了「我們曾經不知道」，而刪掉它會讓下一個人以為這三個數字一直都是可信的。**

## ~~§0-RETRACT~~【2026-09-05 撤回原文，已由上方 §0-RECONFIRM 解除】本 spec 引用的三個「0」**當時無效，待重量**
```
