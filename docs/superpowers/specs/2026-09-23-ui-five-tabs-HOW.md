...
★②用【逐行計數】比，不得用集合成員測試（∈）
   ...
   ⇒ ★★★理由具體：舊輸出裡有【多條一模一樣的 `────────────────` 分隔線】
     （:693／:706／:718／:741）—— 集合測試下，四條變一條仍然「每一行都還在」
...
★★★而票B 之後【天窗文字要帶理由】（implementer 立 2026-09-23，systems 收）：
  「<欄位名>：未接出（<為什麼>）」例：「糧食跑道（缺趨勢）」「位置與家（查詢面無此欄）」
  ⇒ ★一個沒有理由的天窗，跟【還沒做】與【做不到】分不開
  ⇒ ★★而兩者的下一步完全不同：前者等下一批，後者要先動【查詢面】＝另一張票
```
