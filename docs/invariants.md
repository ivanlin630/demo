...
**enforcement（觀測盲點閘，憲法閘同精神）**：①新增 decision/resource/state 未接 tap → FAIL；②**新 tracer/probe 未 suppress global RNG（specimen=A/B/無 三跑非 byte-identical）→ FAIL**（RNG-中性檢查）；③**specimen 完整性**——runtime churn 床（`tracer_completeness_test`）斷言 timeline gap≤HEARTBEAT_CADENCE + commit-fail entry 現形 → FAIL 擋；static tripwire：生產側 `SpecimenTracer.capture*` call-site baseline＝**6 capture_decision + 2 capture_intent + 2 capture_options**（新決策 commit 點未伴隨 tap→計數失衡示警，弱訊號）。與 `constitution_gate.gd` 同級。**現況=不變量已立、①②機械閘待實作 backlog、③specimen 完整性閘已落地（runtime 床 + tripwire baseline）**。
