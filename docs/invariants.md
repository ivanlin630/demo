...
## 寫 spec 掛上「決策層」前，必先驗那個動作真的經過決策層（2026-08-21 立，同日兩次血證）

**我在同一天被實作打臉兩次，形狀完全一樣：**

| spec 假設 | code 現實 | 後果 |
|---|---|---|
| convoy RETURN 走 `PROGRESSIVE_HOLD_TASKS` 就守得住 | `merge_queue` 走 `TaskArbiter.release()` **直接寫欄位、繞過 `try_set`** | 那一行單獨補 ＝ **與 main 逐字節相同、零效果** |
| 折價會壓低「下輪同 res **掛單 util**」 | **掛單根本沒有 util**：`tick_team_orders` 每 tick 依 surplus/shortfall 直接 `post_order` | 折價只壓得到「跑一趟市場」的決策；`order.abandoned` **290 → 290 一動也沒動** |

★ **共同形狀 ＝ 機械層旁路決策層。**
我寫 spec 時預設「這個動作當然會經過引擎／仲裁／util」，**而它其實是機械層直接執行的**。
症狀都一樣：**掛上去的東西看起來合理、測試也過，但在 live 從來沒有作用點。**

### 規則
1. **spec 要把某機制掛在「決策層」（util／仲裁／hold）之前，先窮盡 grep 目標動作的呼叫點**，
   確認**它真的從那一層出發**。`post_order` 4 個呼點全在 `order_system` 機械層 —— 這是**寫 spec 前十秒就查得到的事實**。
2. **前提寫進 spec 時要標明是「已驗」還是「假設」**。假設就要在 gate 裡放一條**證偽它的量測**，
   別讓它一路滑到實作才被發現。
3. ★★ **同族第三次（2026-08-21 稍後）：「這個機制只有一處實作」也是「X 會經過 Y」的變體。**
   我寫 `monotonic-team-id` spec 時**只點名 `subteam_system.gd` 一份產生器**，
   R② 親 grep 發現**全站 7 份獨立的 `_next_team_id`、全部同款 `max(id)+1`** ⇒ **我的修法只堵 1/7 個出生口**。
   ⇒ **spec 點名某個實作位置之前，先 `grep -c "func <該函式名>"`**——
   **一行指令就能擋掉的錯，不要靠印象。**（**負斷言協議本來就要求這件事**，我當時沒把它用在自己身上。）

   ★★ **同日第四次，而且它證明上面那條還不夠**：person id 我以為 2 個出生口，**實測 4 個**——
   **第 4 個不叫 `_next_person_id`**，它是 inline 的 `p.id = team_id*1000 + 序號`（`game_setup._make_person`），
   **按函式名 grep 永遠抓不到**（而且它還埋了一顆 `static var _member_counters` ＝ **跨 `WorldState` 殘留**）。
   ⇒ **規則升級：要搜的是「被產生的東西」，不是「產生它的函式名」。**
   - 找 id 配發 → grep **`XxxData.new()`** 與 **誰在寫 `.id =`**，不是 grep `_next_*`
   - 找某機制的觸發 → grep **被寫的欄位／被 bump 的 tap**，不是 grep 你以為的函式
   **按函式名搜只抓得到「複製品」，抓不到「另一種寫法」。**
   ★ 而且 7 份獨立實作**本身就是結構性風險**：修法不該是「7 個地方各自改對」，
   而是**收斂成單一分配器** —— 否則 bug 只是從「重用 id」變成「7 個理論上該同步、物理上分開的計數概念」。
4. ★ 這正是 **R①（factcheck 未驗 code 斷言）存在的理由**——我這兩次都判定「小 slice、前提已坐實」而略過 R①，
   **兩次都判錯**。**判準修正：只要前提句型是「X 會經過 Y」而我沒有當場貼出 Y 的呼叫點，就送 R①。**

### 連帶影響：不要用「照抄」估算尚未驗證的族
A1 五族原本估「從五個修法縮成**五處接線**」——**該估算的前提是「五族的失敗都由 util 決定」**。
示範族已證明**至少有一族的動作在機械層** ⇒ **每一族都要先查「它的動作經不經過 util」再談照抄**。

### 跨 merge 的 before/after 對照，必須同 commit 重跑（2026-08-21 血證）

**世界軌跡會被「別的刀」改變。** implementer 一度報「`dispatch 3 → 7`」像是本刀的功勞——
**那是錯的比較**：母刀那組數字是 **merge `FailureMemory` 之前**跑的，世界軌跡本來就不同。
同 commit 同參數重跑後，**兩邊完全一樣、fp 相同、零行為差**。

**規則**：before/after 兩側**必須在同一個 base 上、同 seed 同 config 重跑**。
拿「上一輪留下的數字」當 baseline，會把**別人的改動**記成**自己的功勞**（或**自己的罪過**）。
★ 這是 R6 保鮮期的**更尖銳形式**：不只是「舊數字會過期」，而是
**兩個數字各自都新鮮、但它們的世界已經不是同一個**。

★ 連帶：**當初促成某個修法的證據，可能在世界軌跡改變後不再可重現**。
遇到這種情況要**明寫在帳上**（「原始證據在 `<commit>` 的軌跡下取得，現行軌跡已無法重現」），
不要假裝仍有現行證據，也不要因此就當作問題不存在。

### 以 id 為鍵的量測記錄，必問「同一個 id 會不會有第二次」（2026-08-21 血證，同日第三次）

床把 `_rec` 設成 `Dictionary(porter_id → 單一 trip 記錄)`，dispatch 偵測條件 `not _rec.has(tid)`
⇒ **porter 第二次被派遣時永遠 false ⇒ 第二趟徹底隱形**。
本輪 `dispatch = 7` 但 `porters_tracked` 只有 3 —— **背後藏著 `trips_total = 6`**。
**QA 抓到的 `porter_12 → Team1` 正是這個漏測的受害者**：不只是分類粗，是**那趟根本沒被看見**。

★ **這是同一天第三次同族**：
① specimen 選樣清單**凍結** ② fate 分類器**以「隊伍消失」推論** ③ trip 記錄**以 id 為鍵**。
**共同形狀 ＝ 儀器悄悄少算，而少算出來的數字看起來完全正常。**

**規則**：任何以 id／名稱為鍵的量測容器，**設計時就要回答**：
1. **這個 id 會不會有第二次事件？**（porter 會被再派、team 會再打一場、order 會再掛一次）
2. 若會 → 容器必須是 **`id → Array[事件]`**，開新事件的條件是「**無記錄 or 最新一筆已終局**」，
   **不是「這個 id 沒出現過」**。
3. **報「追蹤了 N 個」時要同時報「共 M 個事件」** —— **N 與 M 不同就是警訊**。

★★ **更尖銳的形式（同日稍後坐實，比上面三條更根本）：`team_id` 會被【重用】。**
`SubteamSystem._next_team_id`（`subteam_system.gd:346-351`）＝ **`max(現存 id) + 1`**
⇒ **最高 id 的隊一死，下一個子隊就撿回同一個號碼**。
而 specimen／量測床／QA 的讀法**都拿 `team_id` 當身分**
⇒ **兩條不同的命被縫成一條假故事**，中間那段空白（該 id 暫時無主）看起來就像「觀測斷了」。

**實測（team 12）**：第一條命 `dispatch@2400 → 結案@4600`；**空白 4600–7300（2700 tick，沒有任何隊持有 id 12）**；
第二條命 `7300` 起又是 `task=運輸 parent=5`。`max_gap = 2740` 與空白區間**完全吻合**。

⇒ **規則升級**：**身分不是 id。**
- 任何跨時間追蹤**都要問「這個 id 在這段期間有沒有換過主人」**；
- 「同一個 id」只能讀成「**同一個號碼**」，**不能讀成「同一支隊」**；
- 修法優先序：**讓 id 永不重用**（單調遞增計數器）＞ 在每個消費端改用複合鍵
  —— 後者是「**記得在每個地方註冊**」那族，**今天已經因為這族栽了三次**。

### 判「正在靠近／正在惡化」要用趨勢，不能用瞬時值（2026-08-21 血證，systems 自己的 gate 設計）

我把 gate 9 的證偽指標定成「`stranded` 當下**距母隊 ≤2 格的比例**」。首批實測 **3/3 ＝ 100%**，
**看起來像「近在咫尺卻被砍」＝誤殺確認**。
...
|---|---|---|
| **即將抵達** | 距離**正在收斂** | **誤殺** |
| **永恆尾隨** | **距離恆為 1、永遠追不到** | **正確收掉** |

補上趨勢後：**0/3 出現單調收斂** ——
一筆**距離惡化**（1→2）、一筆**教科書級持平**（15 筆全為 1）、一筆是**另一種病**（見下）。
**結論從「100% 誤殺」翻成「3 筆均不支持誤殺」。**

**規則**：**任何「距離／差距／落後量」類指標，判「好轉或惡化」必須看序列，不能看單點。**
- 單點小 **≠** 正在靠近（**可能是穩定卡在那裡**）
- 單點大 **≠** 正在惡化（**可能剛起步**）
★ 設計 gate 時就要問：**「這個數字在『正在改善』與『穩定卡住』兩種世界裡，會不會長得一樣？」**
會的話，**那個 gate 判不了它想判的事**。

★ 附帶收穫：趨勢資料**同時撈出一個原本看不見的第三型**——
**母隊全程靜止、porter 曾經同格（dist=0）卻退回 dist=1 卡住 160 tick**
⇒ **抵達過但 merge 沒接上**，是**執行斷**，不是追逐問題。
**單點指標永遠看不到這個**，因為它跟「永恆尾隨」的瞬時值一模一樣。

### ★★兜底會遮蔽它兜住的東西：fire 時必須留下「它蓋掉了什麼」的證據（2026-08-21 血證）

**T3（convoy 逾時轉獨立）本身判定沒錯**——elapsed 確實超過 ETA，**超時是真超時**。
**但 QA 逐 tick 讀 specimen 後發現**：三筆 `stranded` 的 porter **全部**不是「追不上移動中的目標」，
而是**已經走到相鄰格（真六角相鄰，我驗過 `tile_pos→move_target` 的差值全在 `HEX_DIRS` 裡），
然後連續多個判斷週期一步都不走**，直到 timeout 把它們收掉。

★ **T3 正確地兜住了一個更上游的「移動／抵達執行斷」bug，並且把它悄悄消化掉。**
> **如果沒有這三個案例，這個「明明相鄰卻不走最後一步」的缺陷，
> 會被 T3 的【正確行為】完全遮蔽，永遠不會有人發現。**

**規則**：
1. **任何兜底／回收／逾時機制，fire 的時候要一併記下「它當時在兜什麼」** ——
   最少要有：**觸發前一段的關鍵狀態序列**（位置／目標／有沒有在動），**不能只記「我 fire 了」**。
2. **「兜底 fire 了 N 次」不可以單獨當成健康指標** ——
   **它同時是「上游壞了 N 次」的計數**。兩種讀法都要在帳上出現。
3. ★ **驗收一個兜底時，要分開回答兩件事**：
   **(a) 它有沒有正確 fire**（本例：有）／**(b) 它兜住的東西本身是不是 bug**（本例：是）。
   **只答 (a) 就結案 ＝ 把一個缺陷升級成「已驗證的正常行為」。**

★ 這是「**症狀 vs 根因**」與「**補丁閘優先查**」的第三種變體：
前兩種是「機制壓過引擎」「機制繞過引擎」，**這種是「機制正確地清掉了現場」**。

### 同一個物理量不得有兩套獨立模型（2026-08-21 血證，convoy ETA）

**「porter 相鄰卻不走最後一步」**——所有人（measurer 讀快照、QA 讀逐 tick、我讀指標）**都讀成「凍結」**。
**真相：它一直在走，只是【一格要走 144 tick】**，而判斷週期遠短於 144。

★ **真缺陷 ＝ 同一件事有兩套獨立模型**：
- `PathSystem.eta_ticks`（**預估**）＝ `path_cost × BASE / (1−fatigue)` —— **只吃疲勞**
- `MovementSystem._move_cost`（**真實**）＝ `BASE / (隊速×地形×疲勞×超載×車輛)`，**clamp `[BASE/3, BASE×3]`**

**porter 永遠超載**（`BASE_CARRY = 10/人`、`pop=1` 背 30–200）⇒ **每格都吃 MAX clamp ＝ 3×BASE**
⇒ **預估系統性低估 3×**。

★★ **而 T3 的預算 ＝ `3.0 × eta` ＝ `3 × BASE × 格數` ＝ 真實路程時間本身 ⇒ 餘裕恰好為零。**
**那個「恰好」是兩個獨立常數相乘的巧合** —— 沒有人設計過它，也沒有人會在讀 code 時看見它。

**規則**：
1. **同一個物理量（走一格要多久／一次生產多少／一次戰鬥損多少）只能有一個模型。**
   需要「便宜的預估版」時，**預估必須由真實模型導出**（同一函式加參數、或真實模型的封裝），
   **不得另寫一份公式**。
2. **兩份公式一旦並存，它們的分歧會被下游的常數乘法放大或抵消，而且【在檔面上完全看不見】。**
3. **修法一律是「收斂成一個」，不是「讓兩邊各自校準」** ——
   校準會過期，收斂不會。（同族今日已出現六次：specimen 選樣清單／fate 推論／trip 以 id 為鍵／
   七份 `_next_team_id`／四種 person 出生口／本條。）
4. ★**留一個持續可觀測的比值**（如 `eta_vs_actual`）——**讓「兩套模型是否同步」變成可量的東西**，
   而不是修完就忘。

### 聚合相關的第一輪，永遠要過「分層拆開」這一關（2026-08-21 血證，blueprint 要求入帳）

**我從 3 隊**（1 forest 掙扎／2 plains 暴衝）**下了「決定性差異是地格 `cap`」**。
**用戶一句「池不是蠻多的嗎？」** ⇒ 分層拆開後：**萎縮隊 `{plains: 9, forest: 2}`，82% 在 plains**
⇒ **`cap` 假說死透**，真根是**提取 funnel 斷**（**8／11 站在滿池上餓死**）。

**規則**：**任何「A 高 ⇒ B 好」的聚合相關，在下因果之前必須先分層拆開**：
1. **反例層**：**B 差的那些，A 也高嗎？**（本例：**plains 村也萎縮嗎？** 答案是**九隊**）
2. **樣本量**：**n=3 的對照不能當通則**（我用它下了通則）
3. **機制層**：**有沒有一個能解釋「A 高但 B 差」的物理路徑？**（本例：**零被動採集 984 次**）

★ **最省力的破法通常是一句話的反問** —— 用戶那句「池不是蠻多的嗎」比我三輪量測都有效，
因為它**直接問了我沒去看的那一層**（**我一直在看死者，沒看活人；看了地形，沒看提取**）。
