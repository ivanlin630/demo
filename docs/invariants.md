...
| **8** | ★**「只動相位、不動頻率」不等於行為中立** —— 兩個機制競爭同一批目標時，**改變先後順序就改變結果**；且錯開會把每隊的【第一次】評估推遲最多 (cadence−1) tick。★★所以**任何宣稱「零新旋鈕、只換賦值方式」的 slice，驗收不得只守【頻率】，必須守【母體與結果】**（存活數、生滅計數）。★★★血證 2026-09-22：10 處 cadence 改走錯開，頻率不變而**存活隊數兩顆種子同向掉 11–21（第 4~6 天轉負、之後單調擴大）**。★配套讀法：**多種子方向相反 ＝ 洗牌；方向相同 ＝ 訊號** （★★別拿「世界重新洗牌」當整份卷面的解釋）。 | systems 2026-09-22（implementer 逐日軌跡坐實） |
