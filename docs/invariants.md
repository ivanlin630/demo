...
> ★★★**細則 1a：belief 通過 ≠ 內容任取**（藍圖立 2026-09-02）——**belief 閘只授權「要不要評估這個對象」，不授權讀它的 live 值**。
> ⇒ **閘後評估用到的【每一個他隊欄位】都必須是 belief 欄位**（位置走 `belief_pos`、強度走 `best_estimate`），**不是 `other.tile_pos` / `.population` 直讀**。
...
> ★**為什麼擠成一張表**：這五條各自是對的，而**五個標題＝五個入口＝開場讀不完** ⇒ 規則存在但不會被用到。
> ★★**搬走的只有【為什麼】那一半，規則本文一行都沒少** —— 要判事情時讀這裡，要說服人時去讀 cases。
