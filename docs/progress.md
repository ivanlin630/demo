...
5. **★多-tick 動作 latch/承諾 統一機制（churn 家族系統性，blueprint 2026-07-15，經濟 binding 修後）**：flee(重commit never move)/pursuit(重refresh live位)/掛單噪音(重掛)/merchant-target(重pick move_target 震盪 never到)——**全是「多-tick 動作缺 latch→重選震盪 never complete」同一家族**（決策模型 v2「承諾」原則）。逐個修=打地鼠（flee 修了 merchant 又冒）。**該系統性**：共用「多-tick 動作 latch/承諾」機制（pick target→鎖到 arrival/completion/threat-resolve→release，配 timeout=in-flight latch 必 timeout 不變量），全族走它。**經濟 binding 先修**（merchant latch + accessor 統一，解眼前經濟死）；系統性 churn-latch 排此後（同死常數族/雙 resolver 統一框架式）。
