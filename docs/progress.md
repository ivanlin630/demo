...
- **🏛 沙盒 bar arc（(a) 崛起/經濟底）— 連串 measure→fix**：commander-v2 後戰國 seed 揭 default 龜縮（CONQUER=0/established 卡1）。measure-first 逐層挖（別猜）：能人 pop 崩=**飢餓非戰敗** → ①**戰鬥不決勝**(0 擊潰，撤退先於殲滅，吸收掛 `_end_combat` never fire) ②**食物模型沒統一**(成長 surplus gate 讀私產 silo→糧倉/交易糧餵不到成長→非 plains 注定餬口)。
  ...
  - **▶ 轉平行（藍圖裁，各推一沙盒維度）**：
    - **讀 B（活交易經濟）**：覓食=苟活地板（產出 capped 餵不起繁榮）、繁榮須交易（覓食 capped→surplus 靠交易；forest 賣木買糧自然 fire）。非 nerf 覓食補丁，接「覓食限 pop≤15」延伸。= 經濟戲 + G3 貿易戰地基。→ 開 spec。
    - **G3 Phase E（信息域 enforce）**：god-view 2 漏補 + provenance 審計閘 + 背叛折入。= 資訊戲 / 玩家核心地基。spec 已交 `2026-06-29-g3-info-warfare-unified`（Phase E/D 核心、P 降 optional）→ 推 plan。
