...
★★**本節受〈`docs/process/09_exam_gate.md` 長考閘〉約束**（用戶立法 2026-08-21：**半成品禁跑驗收考**）：下列五項**只是清單的一部分**；**驗收考**開考前須產出**完整缺件清單快照**、逐項標「**已修／豁免（＋該科結果無效）**」並**貼進 verdict**。**診斷考不受此閘限制**（長跑本身就是抓缺件的儀器）。
