...
### ✅★★★竣工：`2026-09-23-stagger-the-hourly-pass-HOW.md` —— **凍結線可交玩**（merge `92349afb6`，世代 8）

```
★玩家尺（P2，12 天 × 2 種子）：over2s_days 0／12（兩顆）｜p99 257／267ms ⇒ ★★PASS
  對照（世代 7，同一支床同儀器）：7／12 與 5／12｜p99 1161／1052ms
  ⇒ ★★★**凍結消失了** —— 而這句話的資格來自【那個對照】，不是來自數字好看
★P7 吞吐：+79.2%／+47.6%（±5% 帶寬）⇒ **判準退役**（blueprint 裁，選項乙）：
  「散相位不改總工作量」這個假設被【已知原因】證偽（本票順手拆掉勢力工作 M 次重複
  與兩圈全世界掃描）⇒ ★判準做對了它的事；模板改成「不得【變慢】>5%」，世代 8 ＝新基線
★★P4（2026-09-23 對照補齊）：世代 8 兩顆種子死因／子隊終止＝0；世代 7 同儀器對照
  seed42 量到 `mergein.subteam=2` ⇒ ★★★**儀器有接電、不是恆零** —— 這排除了「量到 0 是因為 tap 沒接上」
  ⇒ ★但判準是【同量級】，而 0 vs 2（12 天）是【母體不足以判方向】
  ⇒ **結論：沒有證據顯示母體衛生變差；也不足以宣稱它沒變**（★兩句都要說）
★★★分片票／互動幀時儀器票 ⇒ **永久關閉**：門票逐字是「(A) 落地後 B3 仍紅」，而 B3 綠了
  ⇒ 結案謂詞「世代 8 上 [B3-FREEZE] 兩種子 PASS」被推翻時【自動重開】
```
...
#### ★★★同一個病剝了三層（2026-09-23，每一層都是【粒度不對齊】）

```
第一層  錯開的單位＝隊；而 faction_ai 是【勢力粒度】
        ⇒ 批次裡有任一成員就把整個勢力的活做一遍 ⇒ M 人勢力每小時做 ~M 次
        ⇒ ★修法：勢力相位（fc6a855b1）｜★★守衛 faction-drive-once 回到 per_hour_max=1
        ⇒ 床 273s → 99s，但 world-fp 仍逾時
第二層  ★「世界變忙了」這條解釋被【量】排除：錯開臂處理的隊-次總量少 7.8%、成本多 1.9 倍
        ⇒ 逐系統成本：faction_ai 佔總增量 102.7%（其餘 13 支合計 −2.7%）
第三層  ★★★_evaluate_all_body 是【三個不同粒度的系統穿同一個名字】：
        loop1 勢力／loop2 `for tid in state.teams`／loop3 `for tid in state.teams.keys()`
        ⇒ 勢力相位之後每小時被呼叫 ~8 次 ⇒ 後兩圈整個世界掃 8 遍
        ⇒ ★修法：按粒度拆三份（loop2/loop3 各自一列、吃 due_teams）
        ⇒ ★★而 registry 上那一列叫 `faction_ai` —— 這次說謊的是【名字】
```

★**驗收多一格 P8**（R² 2026-09-23）：`_assign_tasks` 寫成員隊的 `current_task`，
loop2 `:2102` 真的讀它 ⇒ 拆開後相位早於自己勢力的隊用【上一小時】的指派行動。
★★reviewer：**純靜態讀 code 判斷不了會不會真的卡死** ⇒ P8 ＝ 把它變成一個會紅的數
（指派→執行延遲 p100 ≤ 119 tick；母體 0 判【不可判】不判綠）。

★★★**現況：blocked。我不會在吞吐（P7）沒有答案之前 merge** ——
而理由不只是效能：**世代 8 的邊界一旦開下去，會把缺陷醃進基線**。
...
| 選項 | 舊尺 |
|---|---|
| 覓食／遷移找糧 | `f(food_days)` ── **存量、位置盲**（在自家營地覓食 ＝ 在荒地覓食） |
| 併入（投靠） | ★**只秤 host 名聲，不看 host 有沒有飯** |
| 佔村 | ★**死常數 `1.0 / 0.3`**，不看那村產不產糧 |
| 紮營 | 折現真流（新磚） |

⇒ **「營地 89% 蓋了就丟」是這張表的必然結果，不是 bug。**

### 修之後（同床 `peaceful` seed 1337 / 90d，**measurer 驗收級**）
| 世界驗收 | 結果 |
|---|---|
| ★`outpost.l0_to_l1 > 0`（**二值**） | ★**1**（此前恆 0） |
| ★**outpost 普查** | **main `11→9`、中途新增 0** vs **branch `11→11`、中途新增 1** ⇒ ★**淨 0 vs −2** |
| `camp.abandoned < camp.built` | 24 / 26 ── ⚠️**門檻寫壞（一直滿足、零鑑別力）**；真值**棄置率 89%→92%，未改善** |
| 零採集 < 同床 main | `1123 vs 1133 ＝ −0.9%` ── **邊際綠** |

★**這一刀真正掙到的是前兩條：文明化首次發生 ＋ 止住去文明化。**
**②③ 沒有實質改善 —— 照實記，不多報。**

**其他**：`camp.won_argmax` 12→69、`camp.lost_to` 從「覓食一家獨大」變成備戰／workshop／survival／併入四分天下
⇒ **四端真的在同一把尺上競爭**。
★**新法條測**（測規律不測結果）：`join_flow 好=12.52 → 投靠 ／ 擠=5.33 → 紮營`
—— **兩個場景只差 host 品質，排序跟著翻 ⇒ 秤的是流量，不是人格。**

### ★仍未解、已各自有票
- **投靠四條 headless 紅** ＝ `_inflow_est` **pop ≥ 20 飽和在 16.0/日、人均單調遞減**
  ⇒ **投靠有規模的 host 在食物上必輸自建村** ⇒ ★**「有大有小 arc：CASE B 規模經濟 absent」本尊**，
  標 `known-blocked-by`，**不改 fixture 值、不動 `weight("join")`**（真結論照收）
- **紮根 funnel**：`9 dispatch → 4 start → 1 complete` ⇒ ★**斷點在【工期端】，不在 dispatch**
  （A1 建設族原 scope **證偽**：commit／仲裁端 drop ＝ 0）⇒ 掛 `camp-construction-duration`

---

## 🔧 估算器總審計（2026-08-21～25）——「**信念要追物理的真形狀**」

**用戶立法〈估算器禁手抄物理〉**：估值必須 **(a) 物理同源推導** 或 **(b) 讀自身狀態**；
★**修法形狀 ＝ 改接線，不是改數值**（把 2 改 5 ⇒ 三個月後又爛）。

**血證**：同一個「工期天數」，引擎裡有 **六份獨立公式、三種答案、極端相差 240 倍**
（#3/#4 **高估 24×** vs #5/#6 **低估 10×**）—— ★**沒有一顆是數值調錯，每一顆都是接線錯。**
⇒ `build-eta-single-source`：收斂成 `OutpostSystem.build_eta_days`，
★**分母由施工 cadence 同源推導、禁手抄 24**，並附
`_outpost_tick_runs_in_near_pass()` 讀 registry ＋ 失效時喊救命 ⇒ ★**假設不靜默**。
守衛 `.claude/hooks/estimator-lineage-scan.sh` **規則2 因此轉綠**。

★**新工具**：`estimator-lineage-scan.sh` 把法條**編碼進閘** ——
**寫死字面值的物理量常數要交代血統；RHS 是符號引用（同源推導）自動放行。**
...
## ⚡ 效能 arc（事件比例計算）進度（2026-08-20）

**WHAT**（用戶拍）＝「LOD/重要性」作廢 → **事件比例計算**：計算跟隨**事件密度**、不跟隨任何觀察者。**systems 排序裁定**：零 LOD（拆 near/far）**排最後【★2026-09-05 已解除 → 第⑧票；非待辦】**（它是**花預算**不是賺預算，且 cadence 語意依賴時間包層級制）。

| 里程 | wall/day | 決策次數 | 說明 |
|---|---|---|---|
| main（起點） | 98.0 ms | 2732 | — |
| **A1**（事件匯流排＋瞬醒＋在途不想） | 133.1 ms | 3938 | ★**買反應性的成本**（只加不減，如實報 +35.8%） |
| **A2**（輪詢降頻 ×3） | 110.8 ms | — | ★**證偽「輪詢是浪費」**：`reeval.directive` 佔 92%、×6 反而更差 → 照紅線停手不加倍 |
| **★命令戳記誠實化** | **64.2 ms（−34.5% vs main）** | **925（−66.1%）** | `reeval.directive` **2267→23** |
| **owner→outpost 索引**（B 縮小版） | **−0.9 s/日**（in-situ；全局窗**測不出**） | — | **byte-identical**、影子對照 **43219/0** |

### B 這一刀的兩個方法論產出
- ★**in-situ 開關對照**：全局 wall 雜訊 **±4–8%** > 效果 → **拒絕宣稱全局加速**；改用**同 binary 同軌跡切 shadow**，差值 **+973 ms/日** ＝ 舊掃真實成本（與微觀 per-call 換算 **895 ms/日** 互證）。已立為 invariants 條款。
- ★**失效策略選「epoch + lazy 整表重建」而非增量 patch**：spec §3 點名的「後設 owner 蓋掉迭代序更前者」陷阱**天然免疫**；重建 **≈0.3 次/日**、代價可忽略。**用結構免疫取代持續小心。**
- ★**implementer 找到 spec 沒寫的坑並用它設計測試**：`outpost_level` 跨 0 的站點雖緊接著呼 `set_owner`，但 **`set_owner` 在 owner 沒變時 early-return**（完工只是 level 0→1）→ **不能依賴它失效**；TDD 用此 **discriminating case** 當第②條。


### ★這條線索的形狀（值得記）
1. A2 **失敗得有價值**——它證偽了我的模型（「沒事也每 cadence 想一次＝浪費」），把真兇指出來。
2. 真兇 ＝ `_update_goals` 每輪 **clear-then-rebuild** → `_emit_goal` 的 `if goal not in f.goals` **永遠成立** → **每次重發都像新命令** → 全體成員被喚醒。實測**純重申 88.0%**、**成員喚醒 92.1% 來自純重申**、`directive.stamp 9` vs `restate_no_stamp 1463`。
3. ★**同族第三例**：JOIN churn（同 target 純重申）／`order.replaced`（舊單未清再掛）／本條 ＝ **「重申即喚醒」**。
4. ★**順手修好一個既有缺口**：**玩家 override 那條路完全不經 `_emit_goal`** ⇒ 現況是**玩家下令、成員不被喚醒**（被「無條件蓋」掩蓋著）。R² 抓到「比對必須覆蓋所有離開路徑」，否則本刀會**引爆**它；做對了則順手修好（TDD 有 4 條專守玩家 override）。
5. ★**故事面 gate 回答了「省下的是不是必要思考」**：抽樣 80 筆決策**零 IDLE**、Team0 連 6 輪穩定執行同任務 ＝ **省下的是 churn、不是規劃**。

**gate（合併結果親跑）**：constitution **PASS 75**／directive TDD **10/10**／T0 TDD **回歸 ALL PASS**／det 兩跑 `165399d1…`／headless **0-new**。
**★k≈2.004 仍未被觸碰**：本刀砍的是**常數項最肥的一塊**；打 N² 的是後面的 **B 空間索引**與 **F 隊數收斂**。
...
### 落地
`docs/process/verdicts/exam-12mo-verdict.measure.json`；specimen：`docs/measurements/exam12mo/exam-12mo-{peaceful,warring}.specimen.jsonl`（各 12 隊 strided）→ **QA 故事審進行中**（經濟四科目）。
### 由此開出的工單
1. **★觀察者世界永不凍結**（`game_over` 違憲、blueprint WHAT 已裁）→ spec 已寫、**R② 中**；**下輪長考前置**。
2. exam bed：game_over guard + **no-player**（觀察者長跑不指定 player team）+ prefix 更正 → 已 dispatch。
3. **mint 0% ＝ 設施鏈調查**（跨兩考一致，升為具名 backlog）。
4. `site_thrived=0`（沒有據點升級完工）＝ settlement 發展鏈待查。
...
★★**本節受〈`docs/process/09_exam_gate.md` 長考閘〉約束**（用戶立法 2026-08-21：**半成品禁跑驗收考**）：下列五項**只是清單的一部分**；**驗收考**開考前須產出**完整缺件清單快照**、逐項標「**已修／豁免（＋該科結果無效）**」並**貼進 verdict**。**★診斷考同樣過這張清單、但判準是「對齊審查」**（逐科目問「會不會被已知缺件蓋掉」：會 → 先修或該科標「已知污染、不讀結論」；不會 → 照考）。**本閘只管已知缺件**——未知缺件正是診斷考要找的。
...
0b. **★`work_morale` 基線位移（blueprint 具名列入解讀）**：修 LOD 紅線前，headless 世界的 `work_morale` **恆為 1.0**（只在 reactions 寫入），而它 `gain *= work_morale` 直接乘進採集產出 → 大考將是**第一次有士氣變異的產出**。**產出變異出現 ≠ regression，是世界活了**——解讀時別把它讀成新 bug。
...
- **★★★2026-08-20 帳面註記（blueprint 裁、必讀在前）：以下 accepted cost 數字【量於「零出生世界」】——`reactions`(含生育)在無玩家 headless 全期零執行（見 known_issues〈LOD 紅線〉），人口只出不進 → starve/attrition 量級偏重、【量級待大考重定】。重校方法（選便宜的）：①此處即刻註記（本行）②12mo 大考的 starve/attrition ＝【新的真 accepted cost 紀錄】③**不專門重跑舊 branch 對照**——除非大考數據反過來質疑「接受」這個決定本身，才回頭。另 QA 揭的 GATE-B 歸因（買糧單長期不成交）是同一批死亡的第二個打折理由，兩條要一起看。**
...
- **🏛 框架 arc 後續 goal-run（autonomous，照藍圖）**：① **gate→權重**（merge `b15297a`）貿易去 is_merchant 硬 gate+economic_opp 角色因子 0.3=清藍圖 gate 債第一條（生產隊能 roam-trade 但很少）。② **food 買單側**（merge `a4c4cf8`）缺糧隊發 food buy=食物雙向市集（補 WS-1 只賣）。③ **性別資料+生育需兩性**（merge `e3828d4`）PersonData.sex+anon_female_ratio+balance gate=全男隊內部不繁衍 emergent（④Trait 前置）。各跑 2yr world_sim+回歸全綠+coin_eq/InvariantAudit 0。**⏸ 他域遷入（攻擊/掠奪/徵收/結盟/立國/scout/誘殺/鑄幣）= 未決**：撞協調語意 WHAT（faction-goal 頂層 vs 個體人格驅動 + 主動開戰 feel）→ handback 藍圖求裁（`otherdomain-coordination`），暫緩【★defer token: otherdomain-coordination】。**耦合他域待 ruling**：戰俘（capture 需 combat）、survival 全隊退役+loot/join 還經濟隊、驗證套件 TC2/3/5+S 魂場景（隨域驗）。
...
### 2026-08-21 下半：convoy 鏈的四輪來回 —— ★收穫幾乎全在「儀器」而非「機制」

**機制面**：convoy RETURN 根因修（`merge_queue` 走 `release()` **繞過 `try_set`**）讓歸建 **27.9 → 9.2/1.3 日**、吞吐 ×3。
**但帳目誠實**：`persist.hold` 對 CONVOY **可歸因 ＝ 0** ⇒ **T1 那一行功勞為 0**；
gate 4／gate 8 **只有合成床證據**。T1 inert 的理由後來又被訂正一次——
**不是「結構上碰不到」，是「會搶的那個（survival@80）本來就不受 hold 管」**（先前量到 0 次是**窗口偏差**）。

**★這一輪真正的產出是四個儀器缺陷，全部「看起來完全正常」**：
| # | 缺陷 | 症狀長什麼樣 |
|---|---|---|
| 1 | specimen 選樣清單**在 setup 當下凍結** | 主角（執行期生成的子隊）**一次都沒被錄到**，聚合數字卻全都有 |
| 2 | `_snapshot` **不記「這隊在執行什麼」** | porter 的 entry 跟母隊**長得一模一樣**，QA 拿到也認不出主角 |
| 3 | 量測床 fate 分類器**以「隊伍消失」推論 `merged_home`** | **分不出併進誰**——「乾淨歸建」裡混著「投靠陌生隊」 |
| 4 | 床 trip 記錄**以 `id` 為鍵** | `dispatch = 7` 卻只列 **3 隻**；**第二趟徹底隱形** |

**★★而它們四個之下還有一個共同的更深根因**：**`team_id` 會被重用**
（`_next_team_id = max(現存 id)+1`）⇒ **兩條不同的命被縫成一條假故事**，
中間那段「該 id 無主」的空白**看起來就像觀測斷了**。已立 spec 送 R²（**讓 id 永不重用**）。

**世界行為的新發現（交 blueprint 裁 WHAT）**：**瀕死 porter 投靠陌生隊、貨跟人一起被吸收**
（`food 1.17` → `task 投靠` → `parent_team_id` 改寫）。**第四種結局**，既有三分類都對不上。
全域守恆不破，**但所有權破**。

**★方法論收穫（全升 `invariants`）**：
1. **承諾態只能經仲裁移轉**；診斷序 ＝ **先看 tap 是不是 0，再談門檻**。
2. **specimen 選樣必須血緣封閉**；**檔案存在 ≠ 內容涵蓋**；**驗收關鍵字要挑語料裡真的存在的**。
3. **寫 spec 掛上決策層前，必先驗那個動作真的經過決策層**（同日兩次血證）。
4. **跨 merge 的 before/after 必須同 commit 重跑**（否則把別人的改動記成自己的功勞）。
5. **身分不是 id**；以 id 為鍵的量測容器必問「同一個 id 會不會有第二次／會不會換主人」。

★ **共同形狀**（今天出現五次）：**用「當下枚舉」代替「規則涵蓋」**，
而**過期的枚舉／少算的儀器，看起來跟正常運作一模一樣**。


### 2026-08-21 收尾：七件 merged，四張在飛

**已 merge**：harness 全套（0–9＋O1／O2／**P7 三態誠實表**／**P9 seam-gate SOFT**）／
specimen **血緣封閉**／specimen **覆蓋＋`tile_pos`／`rehome_n`**／**失敗反饋 Phase 0**（A1 形狀源）／
**convoy RETURN 收尾**（27.9 → 9.2/1.3 日、吞吐 ×3）／**`team_id` 永不重用**（七份產生器收斂）／
**`person_id` 永不重用**（**四種**出生口，含刪掉一顆**跨 `WorldState` 殘留的 static**）。

**在飛**：`t3-budget` 同步 → **`breed-anon`**（用戶拍 (a)「匿名也能生」）→ **`subteam-survival-ladder`**（最大一張）；
measurer：warring 床拿 **T3 gate 9** 的第一個真樣本。

**★用戶裁定（blueprint 轉、附授權真檔）**：
- **生育 (a)** ＝ 匿名也能生；**血脈題**（named 子女）＝**王朝 arc 掛點、不現在開**
- **子隊入同一把求生尺 ＝ 完整絕境階梯**（非「瀕死→投靠」單招），依據 ＝ **survival 保序單一源不變量「命運不看 dispatch 路」**
  ★ 查證後**範圍比裁定時更大**：決策真空**不是 convoy 專屬**，**六種在途子隊全早退** ⇒ **一刀六治**
- **「挪用委託資產求生」＝ 湧現允許 ＋ 留帳**（差額 → 母隊 belief → 信任／聲譽）；**不硬禁、不魔法歸還**
- **懸仇 ＝ 轉團體觀感**（現在）；**血仇轉子嗎 ＝ 王朝 arc 升級掛點**；**死目標＝計畫失效升 T0 入 A1 五族**

**★今日方法論總結（全升 `invariants`）**：
承諾態只能經仲裁移轉／specimen 選樣必須血緣封閉／**寫 spec 掛決策層前先驗那個動作真的經過決策層**／
跨 merge 對照必須同 commit 重跑／**身分不是 id**／**窗末未終局 ＝ censored，不是結論**／
★**要搜「被產生的東西」，不是「產生它的函式名」**（按函式名搜只抓得到複製品，抓不到另一種寫法）。

★ **共同形狀（今天出現六次）**：**用「當下枚舉」代替「規則涵蓋」**——
而**過期的枚舉／少算的儀器／半死的機制，看起來都跟正常運作一模一樣**。

---

## [搬自 game-design.md 2026-08-25] A2 arc：faction 權威折入引擎（進度 2026-07-09）
...
## [搬自 game-design.md 2026-08-25] 決策模型接線現況（2026-07-06 盤點）
...
## [搬自 game-design.md 2026-08-25] 大戰略校準（2026-07-16）
...
## [搬自 game-design.md 2026-08-25] 當前路線圖總覽（2026-08-03 起、blueprint owner、live 前瞻視圖）
...
### ★★★★★S2 具名殘差義務 —— **履行（歸因成立）**（2026-08-28）
```
決策次數 per-team-day：+23.7% → ★−5.3%（大幅下降且變號）
訊息送達 per-team-day：+48.7% → ★−9.2%（比值 −10.9%、發出 +1.9% 在 5% 內 ⇒ 完成率與流量同向降）
```
★**歸因到【這條 arc／節律層】成立**（S3 `CadenceStagger` ＋ S4b `T0 接七支` 合力逆轉）；
★★**歸因到【S3 vs S4b 哪一刀】不成立**（before/after 夾著兩刀，要拆需中間點，未派）；
★★★**「S2 的 +23.7% 由根造成」仍是推論** —— **我們證的是【節律層控制得住它】。**
⇒ **blueprint 的「一次為限／兩次即回工」計時結束。**
★**而結案時記一筆【新的反向殘差】**：**送達 −9.2%（比值 −10.9%）／決策 −5.3%** ——
★★**它們往我們想要的方向動，而那不是不記帳的理由**；★★★**送達那條的形狀是「傳播效率變差」不是「發的少所以到的少」，
而 S3／S4b 動的是決策節律不是傳播 —— 不給機制故事，走正常殘差程序。**

### ★時間重錨 arc 現況板（2026-08-28 收線，★下一個 session 從這裡看）
```
★已落地 main：S1b／S2 根旋鈕(TICKS_PER_HOUR 60)／S1c／S3a／S3 七支遷 T3 全走 CadenceStagger
              ／S4a 盤點／S4b T0 接七支+意圖併遷／#3 rung_changed(kind 30→31)
★★已結案：S2 具名殘差義務(歸因成立,一次為限條款零違約)／製造 -7.5%(未歸因【但已刻畫】,掛 R①)
★★★已結案(續)：輪詢退場 ⇒【不退場,升格常設兜底】(blueprint 准)——理由=GOAL 147 筆「此後永無事件喚醒」;
        ★而完備化路徑在此路走完(LADDER 24 筆成因全是 rung 升降,已接 T0)⇒ 零新 kind
★★★在飛：①(已交)輪詢獨特貢獻率 30 日雙床
        ②#3 的 30 日驗收(覆蓋 217 格／rung 變動當 tick INTENT 醒／fp)
        ③⑤欄逐 kind「emit 了但沒人醒」比例（★2 日 smoke 已見分歧,母體小不下結論）
...
## 2026-09-10：體驗窗三票 ＋ 攻擊門結構發現 ＋ 三支機械擋

★**用戶那晚要親手玩**，而當晚抓到的問題把這一天分成兩條線：**玩家看得到嗎**（體驗窗）
與 **世界為什麼不打仗**（攻擊門）。

### ① 體驗窗（blueprint 護欄④的序：效能 → inspect → 帶因）

```
C1 票① agent/REPL 動詞  ★已 merge —— fp 兩棵樹相同 ⇒【只加入口沒改機制】有實證
inspect 深化            ★已 merge —— 五欄 ＋ 覆蓋率閘 ＋ 兩張表
果事件帶因              ★已 merge —— fp【與逐 type 計數】兩樹相同
效能（frame-time）      在量測員手上（★用戶立法【可慢不可卡】＝憲法級：量最壞單 frame 非總時）
C1 票② 走查            spec 已寫、送 R²（★它是【分頁暫定】與【看不看得懂】兩件事的批准閘）
ctx 117 分批端出        spec 已開，B1（生存 36 ＋ 經濟 31）已派
```

★★**資訊完整性對帳**：`decision_context` 119 欄 vs 玩家同名看得到 **2** ⇒ **盲格 117**。
★**同日訂正（覆蓋率閘第五格當場抓到）**：那 **2** 格裡**有一格是偽陽**——
`population` 宣稱 `get_team_details`，而**那支動詞的鍵集裡根本沒有它**
⇒ ★★**真實基線是「已接出 1／盲格 118」**（`food_days` 那格是真的）。
★★★**50% 錯誤率，而兩格在那張表上長得一模一樣** ——
**這就是為什麼追蹤的數字不能是名字比對的差集**（implementer 當時寫的誠實限，當天被自己的閘證實）。
blueprint 裁**全要**（用戶原話「UI 需有所有資訊，否則我會沮喪」本身就是裁定）。
★**而追蹤的數字【不能是 117】**（implementer 自警）：它是名字比對的產物，**改個名字就降**
⇒ 唯一算數的讀數是**覆蓋率表**（`ctx-exposure.tsv` 逐欄具名歸屬 ＋ `ctx-supplement.tsv` 補充母體）。
★★★**兩張表都綠才算完成** —— R² 已證實至少兩項**結構性不在 119 裡**（事件流／關係圖全貌）。

### ② 攻擊門：97.37% 的時間，攻擊【根本不會被提名】

```
量測（warring/1337/30天/380 次全隊快照）：三道門全關 ＝ 97.37%
  faction_directive 1.58%／征服 intent 1.58%／血仇 0.00%；★強→強候選母體 ＝ 0
code（options.gd:332-334）：三道門【全是授權型】（上級令／身分標籤／歷史仇）
  ★★沒有一道是「他弱、我缺、我夠得著」
  ★★★而攻擊 option 掛著完整四 term 效用秤 ⇒ 引擎存在而 97.37% 不被叫到
     ＝【離散硬閘 pre-empt 連續引擎】的教科書形狀
```
blueprint 裁 **WHAT ＝ 應該有圖利型掠奪**；HOW ＝ `applicable` 降級為**可行性檢查**，
該不該打整個交秤，**禁第四張許可證**。spec **R² CLEAN**，依護欄④**停在體驗窗之後**。
★**spec 過程抓到兩個坑**：門不是一道是兩道（`intent_target` 的賦值也 gate 在同一個標籤上）；
R² 再抓第三條（`feud_target_id` 完全沒有 `belief_pos` 守衛）——★★**三處症狀完全相同**：
applicable 率上去了、真實攻擊沒上去 ⇒ ★★★驗收改成**提名數與真的執行分開量**。

### ③ 三支機械擋（★都是同一天的事故逼出來的）

```
role-commit-scope.sh（pre-commit）
  ①blueprint/qa/reviewer 的暫存區出現 scripts/**.gd ⇒ 擋
  ②★裸 git commit 一律擋 —— 正解是 pathspec commit（git 另開暫時 index
    ⇒ 別人 staged 的東西【在型別上】進不來）。★★偵測法實測：GIT_INDEX_FILE
    在 pathspec commit 時指向 next-index-<pid>.lock。
  ★★★裝上去當天【第一個被擋的是 implementer 的裸 commit，而它真的攔下了別人的檔】。
bash-guard.sh ①b 目錄層級 add ⇒ warn（★「收窄到目錄」不是解，目錄仍是容器）
  ★★而補這條時抓到更糟的：舊版「禁 git add -A」的**結尾錨寫錯**
     ⇒【-A 是最後一個 token】的最常見寫法**從上線到今天一次都沒抓到過**。
ctx-coverage-gate.sh（implementer 寫）★腐爛判定照 R² 的【反方向】：
  豁免欄位不得出現在任何玩家可見查詢面（★與 `_dead_equivalents` 查的方向相反，不能照抄）
```

### ④ 規則退役程序（blueprint 立、systems 落 doc）

```
出口只有三個：機械化／前提消滅／併吞。★「好久沒咬」不是出口
  （安靜分不出【威懾生效】與【規則無用】＝恆綠問題的鏡像）
★★出口①附兩條（systems 提、blueprint 核）：必附陽性對照 ＋ ★對照本身要先驗它成立了沒
   （血證：rebase 對照沒真的產生 merge commit ⇒ 看起來像閘沒擋 ⇒ 差點把一條活閘判死）
首輪 3 條 ⇒ 退役 2、★不退役 1（不退役的正是【當天新做的閘】：覆蓋只是子集）
帳面：invariants 212 → 188（上限 190）｜01_architect 227 → 208｜doc-cap 超限 7 → 4
```

### ⑤ 今天新增／升級的判準

```
界限第十條  ★量【對】了但量【錯時刻】（`replace` 的因在扣款後才讀 ⇒ 觸發是 25、印出是 5）
            ★★它不報錯、不不一致，只給讀的人一個【合理但錯誤的因】，
            ★★★而驗「欄位有值」的驗收在這裡是綠的
01_architect ★「不動的東西」清單只承諾【不改】，不承諾【安全】
07_mailbox   ★改了 spec 但沒寄信 ＝ 等於沒改（斷在通知那一步的東西，在 git log 裡看起來完全正常）
04_qa        ★觀眾看到的流【混了兩種來源】（emit_message 的事件 ＋ 裸 print 的除錯行）
             ⇒ 只讀 global_messages 的稽核**結構上**看不到後者
```

### ⑥ 掛著（★下一輪的入口）

```
攻擊門大票     R² CLEAN，等體驗窗做完 ⇒ 屆時驗收②③④⑤要量測員接
is_live_actor  R² 已審（母體 62 → ★68，一整類寫法對裸 grep 系統性隱形），排體驗窗之後
血仇門 0.00%   ★FEUD_ATTACK_MIN=0.5 vs 世界實際 feud 分布【未量】——掛號不下結論
關係圖全貌     補充母體的 todo（`RelationGraph.strongest` 只抽走最強那條）
存威脅快照     inspect 票交付的是「玩家知道自己看不到，以及為什麼」；要真看到需另票
規則退役       節奏＝arc 收官掃 P7 的 📜 列；★首輪已校準，下一輪照走
```

### ⑦ 2026-09-10 下半：★儀器四連（而「fp 相同」這句話從今天起才有重量）

```
①哨兵    player_* 入 fp —— ★理由與直覺相反：不是「玩家影響 sim」，是【sim 不該碰 player_*】
                        ⇒ 兩顆無玩家 seeded 跑若在 player_* 上分岔 ＝ 有系統偷碰 ＝ 當場現形
②頂層導出 「本尺排除：…」從【手抄字串】變【導出值】——★它當初漏掉整塊 player_*
                        ⇒ 讀那一行的人（systems）以為玩家狀態在尺裡，寫了一個永遠綠不了的驗收格
③子層級   同樣的形狀往下一層 —— ★★量到 TeamData 尺內 30／沒看到被讀 97、
                        HexTileData 排除 34（含 apothecary/mint 這種【會影響產出】的設施等級）
④覆蓋擴張 進尺 30 → ★103，設施等級全部進尺，每類【進尺＋具名豁免＝var 總數，差額 0】
          ⇒ 綠：warring_states 43200 tick ＝ 30.0 天、62 隊→破百，同 seed 兩跑 fp 相同 e69e070b…
```

★**這一批的意義一句話**：**「fp 相同」以前是一句空話，現在有重量了**
——而下一張大票（攻擊門）正是需要它的那一張。

### ⑧ 同期的世界發現（★兩個都被訂正過，訂正本身是重點）

```
①「這個世界只長不死」⇒ ★錯：隊伍會消失（23 天 7 次），走的是【合併】不是滅團
   ⇒ ★★而合併是【重組】不是【減法】（成員/資源/金庫全數轉進吸收方）
   ⇒ ★★★「世界沒有減法」反而被坐實 —— 而攻擊門就是那個減法的閥
②「這個世界沒有村莊那一層」⇒ ★★被連續訂正三次：
   村莊有且健康長（2→22）⇒ 是【居民不在家】⇒ 而「不在家」可能只是【在田裡】
   ⇒ **一個位置謂詞的每一次【否定讀數】，都要再問一次「否定的是什麼」**
③效能：spike 中位數 7.78s／max 26.65s、每小時固定、兇手 near.faction_ai（80-95%）、隨隊數惡化
   ⇒ ★用戶的抱怨被坐實而且更糟；blueprint 裁【接受世界改變、錯開思考時刻】
   ⇒ ★★而錯開的工具【已經存在】（CadenceStagger，10+ 處在用）—— 我提新方案時沒先查
```

### ⑨ 今天新增的判準（★界限已搬進 `detail/01_architect-cases.md`，十五 → 二十條）

```
第十四條 【安不安全】不可判 ⇒ 拆成可判的問題再量（★推翻的是【理由】不是【機制】）
第十五條 新哨兵的第一發警報，先驗基準線再信它（★假陽性與真陽性在輸出上長得一樣）
第十六條 【現在是對的】與【被機制保證是對的】是兩件事（★巧合不會在輸出上留下痕跡）
第十七條 偵測器的母體 pattern 會把偵測器自己的零件算進去（★修法是改名不是加例外）
第十八條 ★「fp 相同」的單腿聲明禁止 ⇒ 【fp ＋ 至少一格行為證據】成對（blueprint 立）
第十九條 「這個測法會不會誤判」常常是【前提沒寫死】而不是【測法有問題】
第二十條 ★★一條【風險發生時怎麼辦】的指示若只寫一種成因，會把另一種成因導向錯誤處置
        （★★★血證：我的條款會讓整張地圖被豁免出尺外，而執行者以為自己在遵守規則）
```

---

## 2026-09-11：★「登記動詞」進 main ＋ ★★★一欄餓不餓把整張票的病因翻了

### ① 進 main（`ae339be5f`）

```
內容：卡①糧改走稅路＋卡①(b) tax_rate 接線＋卡③勞力池只收自己人（`07cc09b14`）
     ＋ ④b 三個登記動詞（收留／求居／據點滅→流離）＋ 兩批量測 ＋ try_set 逐筆卷
閘：merged result 跑 55 支 ⇒ **FAIL 只剩 `bed-arm`**（＝ main 既有基線紅，床層債，刻意可見）
★中途擋過兩格，兩格都是【新東西沒登記】：
  ①`收留`／`求居` 兩份失敗反饋表都沒有 ②`_complete_construction` 是新的 `state.teams` 迭代站點
  ⇒ ★★這兩支閘做對了它們該做的事：**新增的東西缺席時，缺席不是靜默的。**
★★★次序裁定：**④b 單張先進**，理由＝兩張一起 ⇒ 新票的 diff 會含別人 8 顆 commit ⇒ 審查對象錯。
```

### ② ★★★病因翻轉（★本日最大的一件事）

```
我派的那一欄（求居被擋的當下，那支隊**餓不餓**）回來之後，結論整個反向：
  ・42 筆拒絕理由 **100% 單一**：「優先序不足」（求居 50 vs 覓食 80）
  ・**41/42 不餓**：`food_days` 中位 **19 天**、最大 **22.87**，只有 1 筆是絕境 1.80
  ・某隊連續 **12 個遊戲日**每次取樣都在覓食，存糧 5.71 → 22.87 → 11.21
★病灶：`scripts/simulation/decision/options.gd:563-565`
    if is_in_set(opt, "survival") ⇒ return TaskArbiter.PRIO_SURVIVAL   # 80
  ⇒ **commit 優先序來自「這個 option 屬於哪一類」，不是「現在多需要它」**
  ⇒ 吃飽 22 天的覓食與絕境 1.8 天的覓食**是同一個 80**。
★★而**秤沒有壞**：`求居` 在 rank 上**贏了 59 次** ——
  斷點是**第二套秤**（committed 之後誰能打斷誰）**不讀需求**
  ⇒ ★★★**引擎每一次都做對了決定，而仲裁層每一次都不認帳（42/42）。**
⇒ 這是「手不聽腦」的**新形狀**：以前是【腦決定了、手沒動】，
  這次是【腦決定了，而**手上一個舊決定還握著麥克風**】。
```

### ③ 兩個人在同一輪各錯一格（★而錯的是同一種錯）

```
★我：指的機制是 `≥PRIO_THREAT(70) 不介入` ⇒ **本窗一次都沒 fire**
  ⇒ 結論相同、經過的分支不同 ⇒ **要改的那一行不同**（事實 vs 詮釋）。
★★blueprint：上一裁「插斷非取代」的前提（旅程開始了、被插斷、被丟掉）**不成立**
  —— 旅程從來沒開始 ⇒ 自己收回，並把 catch-22 那個敘事一併撤。
★★★兩個都不是「講太快」，是**當時沒有那一欄** ——
  而那一欄是**量出來才有的** ⇒ 裁定在證據到之前必然是暫定的，證據到了就翻。
```

### ④ 掛著的（★都有可量的重撿條件，不是「有空再說」）

```
①**「插斷非取代」入庫**：重撿條件寫死 ＝ **這一刀落完後，若那 4 支真到場的隊任務仍被置換**
②**待母體複驗**（blueprint 裁「fixture 驗收＝合法過渡」）：卡①③ 與 ④b 的自然世界那一格，
  ★母體出現（有無家的生產隊）之後回頭補；★★這一格**記在 systems 名下**，不是消失。
③★**陽性對照把現況當乾淨基準**（`failure-feedback-coverage.sh:110-113` `cp` 活的檔）
  ⇒ 世界真的紅時對照必然失準 ⇒ **閘在找到東西的那一刻自廢**，
  ★★★而它自廢的樣子長得像【儀器壞了】不像【世界壞了】⇒ 修法＝乾淨那格改凍結的最小合成 fixture。
④★行號錨第三支（`.construction-duration-source-gate.txt`）⇒ 見 `known_issues.md` 同條，三支一起改。
```
...
## 2026-09-18（末）：★★★merge 閘【62 支全綠】＋ main 基線紅數 1 → 0

**merged**：`feat/bed-arm-seven`（含 C(20) 第三格）—— ★**這是今天第一次跑出【零紅】的完整閘**。

```
bed-arm：母體 409 ＝ helper 119 ＋ 白名單 271 ＋ ★本閘不適用 19 ＋ ★★未涵蓋 0（原 27）
★第三格「本閘不適用」是【推導】不是【宣告】：閘自己掃「這支床讀不讀 Probe」，不進任何名單
  ⇒ ★★哪天那支床開始讀 Probe，它【自動】掉回「需要遷移」⇒ 閘紅 ⇒ 有人被叫醒
★★★陽性對照（implementer 親跑）：給一支「不適用」的床加一行【含 Probe 的註解】
  ⇒ 不適用 19→18、未涵蓋 0→1、閘紅並點名 ⇒ **這一格會紅，不是恆綠**
```

### ★★★為什麼「基線 0」值得單獨記
```
在今天之前：main 基線紅數 ＝ 1（bed-arm），而那個數字【在暫時 merge worktree 裡讀不到】
  ⇒ 每一輪 merge 都印「從未量過」⇒ ★分不出【這一票帶進來的紅】與【main 本來就紅】
今天：①把基線的讀取【接線】修好（相對路徑 → 解析到主工作區）
      ②把 bed-arm 清成綠 ⇒ ★★基線 1 → 0
⇒ ★★★之後任何一次 merge 出現的紅，【由構造保證】是那一票帶進來的
```

### 這一段的兩條判準（都是別人給的，我採納）
1. ★**判準分不出「引用」與「使用」時，讓它往【嚴】的方向分不出** ——
   誤判成「適用」的代價是多遷一支床（≈0）；誤判成「不適用」的代價是**一個盲區**。
2. ★★**`pending` 的 blocker 必須指向【追蹤系統裡真的有的 token】** ——
   否則「還沒判」會變成**最好用的逃生門**。

## 2026-09-18（下半）：★據點知識拆 god-view 進 main ＋ ★★★一場「幻痛」的完整弧

**merged**：`feat/outpost-belief`（地點知識拆 god-view）、`feat/sister-sites-outpost`（兩支姊妹 site）、
`feat/occupy-target-belief`（佔村掃）——★**閘每一輪都只剩 `bed-arm` 這個 main 基線紅。**

### ★★★那場「幻痛」（blueprint 定性，逐字）
```
拆違憲 → 量出驚人跌幅（−87.2%、可見度 1/8）→ 裁新管道（市集自我廣告）
→ ★前提被殺（那支函式 production 零呼叫）→ ★★發現世界早就做對了（team_market_known 三源含 relay）
⇒ 「幻痛確診：整場警報量在死肢上。」
⇒ ★真正的產出＝【對真管線的一次完整體檢】；★★而那段彎路＝我沒做前提檢查
```
★**兩句都要記**：少了後一句，下次還會再繞一次。

### 本段的教訓（都進 memory）
1. ★★★**量一支函式之前先 grep 它的呼叫點** —— 「它的輸出變了」與「世界變了」之間隔著【有沒有人用它】。
2. ★**「別動它」清單是【斷言】不是【豁免】** —— 我把 3 處真違規混進「合法的自家讀取」。
3. ★**met_check 的粒度要對齊病灶的粒度** —— 檔案層的檢查被同檔的另一支函式滿足，害我退役了一條活著的 defer。
4. ★★**同一家族 ≠ 同一句**（合併家族、保留觸發條件）；而**「證據變多」不是「規則要改」**。

## 2026-09-18：★到場點名 19 支進 main ＋ ★★★「靜默變綠」五支被找出來並修掉

**merged**：`feat/bed-roll-call`（main ＝ `7d1ba9065`，60 支閘 59 綠，唯一紅 `bed-arm` ＝ main 基線那一格）。

```
起點：implementer 自報 —— 他的床寫了不存在的欄位，GDScript 執行期錯誤【只中止那一支 func】
      ⇒ 那一格三個斷言一個都沒跑，而末行照樣印「[FAIL] 數 ＝ 0」
      ★「沒有失敗」與「沒有執行」在畫面上一模一樣
範圍：reviewer 換掉問題 —— 不是「哪些床有這個洞」（454／190 支，做不完），
      是「★哪些床的洞會【改變判決】」＝ merge-gates 註冊的 30 支 godot 床
結果：19 支處理完（加點名或釘免疫證據）＋ 4 支本來就有等價守衛 ＋ 剩 4 支
```

### ★證據累積（★reviewer 2026-09-18 裁：**規則本來就是通用寫的，變的是【證據數】不是【規則】** ⇒ 記在這裡，不回頭加長 `03_implementer` 要件③）
```
第三軸（quit() 在死亡點的【裡面】還是【外面】⇒ 決定紅的簽名）：三次實測、兩類、無例外
  外面 ⇒ rc=0、1～8 秒、沒有橫幅（bed_arm／build-duration／ki-anchor）
  裡面 ⇒ rc≠0、掛住 151 秒（merchant／payroll／minor-merge）
⇒ ★操作價值一句話：**看到 rc=0，別以為它跑成功了。**
```

### ★★★三種失效／免疫簽名（每一種都有 ≥2 個實測樣本）
| 形狀 | 死掉時 | 誰接住 |
|---|---|---|
| 判決讀各格產出、預設是**毒值** | 斷言自然紅 | expect（FAIL 數） |
| 格 **inline 在 `_initialize`**、`quit()` 在死亡點之後 | 進程掛住 ⇒ `rc≠0`、timeout | runner 第一道 `RC -ne 0` |
| `quit(_run())`（`quit()` 在**外層**） | 控制權回得去 ⇒ **`rc=0`、秒級、沒有橫幅** | ★**只剩 expect 那一條** |
| ★★★**共用 `_fail` 計數** | **橫幅照印 `ALL PASS`、`rc=0`** | ★**兩道全過 ＝ 靜默變綠**（5 支，已修） |

### 本日教訓（進 memory）
1. ★**陽性對照自己沒打中，讀起來就是「這裡沒有問題」**（注射寫在被呼叫的 helper 裡 ⇒ 中止的是 helper）
   ⇒ **陽性對照有兩層：先證明注射會致死，才用陰性結果下結論。**
2. ★★**「別動它」清單是一個【斷言】，不是一個【豁免】** —— 我把一個 grep 計數貼上「自家」的語意標籤，
   而那 11 處其實是三種東西（自家 6／本票要改的 2／**真違規 3**）。
3. ★★★**寫下限制 ≠ 套回自己的下一句** —— 同日四次，兩次不是我；四次裡三次是**機械守衛或別人**接住的。
4. ★**merge 閘的 main 基線在【唯一判 merge 的地方】讀不到**（相對路徑 ＋ 該檔 gitignored）
   ⇒ 修法是**接線**不是加閘；當天就用它分出「兩格新紅 vs 一格標準紅」。

## 2026-09-17：★過期位置→偵查分池 進 main ＋ ★★★今天被「我自己引用的規則」打倒兩次

**merged**：`feat/stale-pos-recon`（`34bb4aec5`）—— 偵查目標不再因為「位置情報過期」被整個丟掉；
**年齡進【價值】不進【門】**，折扣單調遞減**永不歸零**；★**掠奪／攻擊兩道門逐字未改**（放寬＝隔空作用）。

```
世界尺度：偵查候選評估 137596｜其中位置已過期 80927（★59%，本票之前全部被那行 continue 丟掉）
絕境那格：母體 11｜偵查有候選 10｜候選是過期情報的 3｜★當天真的去偵查 = 0（spec 原文「不要求它贏」）
效能對照：main 側 day10 max=31.97s ＞ branch 24.94s（teams 兩側逐字同 104）⇒ 判 (a) 本票無辜
閘：58 支 57 綠｜唯一紅 bed-arm ＝ main 上本來就紅（兩側同樣 27 張，本票那張床不在其中）
```

### ★★★本日教訓（都進了 memory）

1. ★**我在【引用一條規則當武器的同一票裡】違反它**，同日兩次、兩條都是我 owner 的規則：
   拿感知鐵律擋「放寬攻擊門」的前門，卻從後門 spec 出 god-view（`_tiles_per_day(目標)` 內部讀 live）；
   拿不變量 #6 要求「決定與依據同一次回傳」，卻設計出一個要**讀兩次**的介面。
   ⇒ **引用規則後，立刻把它套回這份 spec 自己的其他段落。**
2. ★★**grep 命中【名字】不命中【意思】**：met_check 一次被**註解**騙、一次被**無關的 probe 名**騙、
   一次寫成**恆假**（缺席式判準，而合法的鄰居永遠在）⇒ **極性當場跑 ＋ 陽性對照 ＋ 正面的治癒訊號**。
3. ★★★**驗收格證明【機制有效】，不證明【機制有規模】**：錨定性六格在
   「碰到世界 0.96%」與「碰到 9.7%」兩個版本上**全都是綠的**。
   ⇒ spec 加「規模必報」——★**不是門檻，是必報欄：擋不住縮水，但讓縮水無法靜默。**
4. ★**被吸收掉的需求不會自己回來敲門**：「等第二個消費者出現再開新軸」的盲點是它假設需求會敲門，
   而真需求常常是**撞牆 → 自己做個次級品 → 安靜下來**（血證：`faction_ai:5884` 的註解自承）。
5. ★★**儀器在它唯一被消費的地方是瞎的**：merge 閘的「main 基線紅數」用相對路徑讀，
   而該檔 gitignored ⇒ **在暫時 merge worktree（唯一判 merge 的地方）永遠讀不到** ⇒ 每輪都說「從未量過」。
   ⇒ 修法是**接線**不是加閘。

## 2026-09-12：★攻擊門進 main ＋ ★★★今天被「尺」打倒三次、而第四次站住了

### ① 進 main（`70d9213f3`）

```
**攻擊 `applicable` 從【授權清單】降為【可行性檢查】**
  ・門開 **2.53% → 75.48%**（母體 7957，★**同一次 gather 同時算新舊兩門 ⇒ 逐字同母體**）
  ・`IDLE 陷阱 0／0`｜`nO`（舊開新關）三筆逐筆具名、全部落在三分類 ⇒ **可證偽判準通過**
★而它是**世代 3 的邊界**（`docs/measurements/_generation-boundary.md`）——
  攻擊從【幾乎不發生】變成【常態】⇒ **所有與衝突相關的舊分布都在邊界的舊側**。
```

### ② ★★★今天的主線其實是【尺】，不是世界

```
①**紮營 0%** ⇒ 其實 **79.2%**（成功判準用**一把全域尺**）
②**信使 2.2%** ⇒ 其實 **50%**（episode 邊界用**會刷新的欄位**當鍵 ⇒ 一趟被切成很多段）
③**迎戰母體 921** ⇒ 其實 **193**（同上，灌大 4.8 倍；「沒有目標 36%」真值 6.2%）
④★**而第四次站住了**：迎戰的**排序**在母體改 4.8 倍、最大格作廢之後**沒翻，還拉開**
  （碰到了但沒開打 42%→**60.1%** ＞ 沒碰到面 22%→**30.6%**）
  ⇒ ★★**穩健性檢驗不是為了推翻，是為了知道結論有多硬**；
  ⇒ ★★★三張被尺打倒的表裡，**被真的測試過的那一張站著出來**。
★救了這四次的是同一樣東西：**fp 逐字相同** ——
...
```
①**任何比率，同句必須寫出【分母怎麼切的】**（episode 的切法是結論的一部分）
②**單 seed 的前後對照只對【票直接觸的量】有效**（blueprint 立）⇒ 歸因分三級
③**翻斷言（禁）vs 換契約（准）**：簽名是**新測試會不會紅**
④**改判準可以，但它必須能紅** —— `nO` 那格就是照這個放行的
⑤**清單對清單**，不是數量對數量（★數量會被「一紅一綠抵消」騙過去）
⑥**「全域 vs 逐隊」是合法的兩份；「同一個問題的兩條路」是重複**
  ⇒ 判法：**拿掉這一行，我會少知道什麼？**
⑦**世代何時開**：某類行為的發生率**跨一個量級**才開；純觀測／效能／修 bug **不開**
```

### ④ 工具側（★都是「工具做了對的事卻報了會被誤讀的話」）

```
①`| tail -N` 包長跑 ⇒ **被殺時輸出是空的** ⇒ ★**那不是「沒有結果」，是工具把結果吃掉了**
  （對照組：implementer 的 `| Out-File` 同樣被殺，**檔裡有半份**）
②分批跑第一版**有效卻照印「註冊表 55 支」** ⇒ 看起來像跑了全部 ⇒ 已改印**【實跑 N 支】**
③`headless` baseline 的 key 含 `task=` 這種**會漂的世界狀態** ⇒ ★**閘把我們修好的東西判成回歸**
④**296 封信在 git 眼中失蹤** ⇒ 根因是 pathspec commit **對未追蹤檔是盲的**（搬移只進了一半）
  ⇒ 已補進 git ＋ 在既有信箱閘加一格（★不是新閘）
```

### ⑤ 在飛的（★狀態，不是承諾）

```
・**相遇量測**（頻率 ＋ 外觀層三桶 ＋ 敵意逐向）—— 跑中，世代 3
・**31 秒那一 tick 的相位樹** —— 唯一值得追尾部的入口
・**信使／相遇解剖**（送達、求居門鈴、迎戰接戰 ＝ 同一台 pairwise 相遇）
・~~帳①戰鬥面多 seed~~ ⇒ ★★★**已定案（2026-09-15）：蝴蝶**
  開打／結束／滅團 **三條方向都不一致** ⇒ ★**「開打 −79%」作廃**（它是一個樣本，不是一個現象）
  而 **勒索 ＝ 真趨勢**（三 seed 全 ↓、相對極差 **0.13 < 0.5**）
  ⇒ ★★正確說法：**「世代 2 → 4 之間，勒索穩定下降」**，
    **不是**「攻擊門讓勒索下降」—— **方向是真的，因果仍未分離**（兩臂之間有兩次 merge）
  ⇒ ★★★**而這張票本身的主張不受影響**：門開率、登記數、util 分布
    都是**票直接觸的量** —— **蝴蝶只吃湧現聚合**。
｜**帳②移動單價**
・★**相遇不發事件**（`WorldEvents.emit` 23 個呼叫點無一是相遇）—— 待 runtime 坐實
  ⇒ ★★若坐實：**世界只在【已經發生】之後才喚醒** ⇒ 比攻擊門大
```

## ✅ 走查文件第二版（中文欄位名）—— **已交付，驗收在用戶手上**（2026-09-17）

★**從 `known_issues.md` 搬過來的理由**：**它不是一個未修的病，是一張【已交付的工單】** ——
而 `ki-status` 閘要求新條目帶【狀態】三選一（已知未修／未確認／已知未實裝），
★★**而這三個都是假的**：它已經做完了。⇒ **強填一個就是在考卷上說謊**（那欄的原意）。
★★★**所以正確的處置是搬家，不是填欄**。

**用戶原話（2026-09-10）**：「**項目名稱都是代碼 我不容易看懂 有辦法翻成中文嗎 工作量大嗎**」

```
**v1**：`docs/measurements/2026-09-10-c1-walkthrough.txt`
**v2**（已交）：`docs/measurements/2026-09-17-c1-walkthrough-v2-clean.txt`（175 行／12020 bytes）
三件：①代碼名欄位翻成中文人話 ②重埋一個【新位置】的種子錯誤 ③交付時通知用戶來抓錯
```
★**驗收（未完成，在用戶手上）**：**用戶抓得到那個種子錯 ⇒ 走查文件真的可讀；
抓不到 ⇒ 它只是【看起來】可讀**。★★**「交付了」不等於「過了」。**
★★★**而它之前被記成「欠最久」卻【沒有票】** ——
**一件沒有題目的事不會有年資，而它的年資來自一個【不會被稽核的口頭清單】。**

## 2026-09-18（下）｜相位樹床 merge ＋ 凍結終線的裁定，而★今天最值錢的兩句都是【量法】的

**merge 第 10 票：`eae0b11a7`**（`feat/phase-tree-net-cost` @`76ce3e4c5`）—— 閘 **64 支 ✓64 ✗0／784s**，判決綁 `HEAD=712e9ad87`。
★兩個歸因要留著：①上一輪 `fp-longwindow` 的 ✗ 是 **wrapper 360s 的 timeout kill**（它已經把 `DONE／FAILS=0` 印完）⇒ **被砍 ≠ 迴歸**，本輪 `GODOT_TIMEOUT=900` 它綠；
②上一輪整輪本來就**不可判**（開跑 `678dc36b5`、結束 `3d305ccab` ＝一輪之內兩棵樹）⇒ 那批紅綠全作廢。
★★而我自己那一格：**上一輪我核了「production 沒動」卻沒核「這顆 sha 是不是新的」** ⇒ gates16 整輪跑在未修的樹上。
**「內容對不對」與「這是不是最新那顆」是兩次不同的檢查，前者綠不代表後者做過。**

### 凍結終線（`*multi` 感知蒐集塊）—— 裁 (丙-2)，而真正的發現是【三個選項踩同一顆地雷】
- implementer 照 §1 先做陽性對照 ⇒ **那支語意閘有鑑別力**；然後**它第一件事就抓到他自己**（同-tick 記憶化把 fp 與逐 tick 行為軌跡都改掉了）。
- 根因他量到了：命中快取 44 次、**其中 65.9% 與當下重算不同**（主凶 `current_task` 27 次）⇒ **引擎在兩次 gather 之間把決策寫下去了**，那 28% 的「重複」其實是**兩個不同時刻**。
- 我裁 **(丙-2)**：每欄一支 `static` 純函式為唯一權威、**`gather` 自己也呼它**（延伸既有的 `pick_recon_target`，不是新造）⇒ 「兩份算式 drift 而不會紅」在構造上不存在。
- ★★★**而我找到的地雷**：`gather(advance=false)` 是否仍是 mutator —— 若是，**任何讓它少跑的修法都會改變世界**，甲乙丙**三個都踩同一顆**。我把它標成**待驗前提**並排了量測（R② 抓到我第 95 行那句**沒有條件化**，條件補在兩行之後 ⇒ 已改成同句帶條件）。

### ★★★今天最值錢的兩句（都是關於【怎麼量】，不是關於【量到什麼】）
```
①【清單保證 vs 構造保證】——照名單數 tap 會量出一個【假的 0】
   血證：known_issues 名單上的 LaborSystem.ensure_fresh 兩個寫入點【全庫沒有任何 tap】
   ⇒ implementer 改成【每次呼叫前後取全世界指紋】＝對清單完整性不敏感
   ⇒ 讀數：1200 tick／seed 1337／65 隊，advance=false 呼叫 95 次、寫世界 0 次 ⇒ 走 A 路
   ★判準句：「我的量法會不會因為【有人漏列了一個點】而變綠？」——會 ⇒ 換掉它
②【換成更強的量法不會免除陽性對照，只是把要對照的東西換成【涵蓋性】】
   ⇒ 我加一格：手動把三個 cadence 欄位各 +1，**指紋必須變**；
     不變 ⇒ 那個 0 的意思是「指紋對 cadence 不敏感」而不是「沒有寫入」
```
★附帶：`known_issues` 那條「`gather` 有寫副作用」的 ② **已過期**（`labor_system.gd:29` 唯讀／`:43` `rebalance` 只在 `advance`／`:46-50` 純算回傳）——**我自己開檔核過三行才訂正**，不是轉抄信。
★★我自己抓掉一格 brittle：格 1-h 原本寫「關閉 ⇒ fp == **釘死的基線字串**」⇒ **任何合法的世界改動都會把床打紅＝噪音**，而噪音的代價是**連旁邊那支真的紅也開始被忽略** ⇒ 改成同一輪內三跑。
★★★implementer 的兩個自抓：**更正自己表格裡一列「當時沒驗」**（拿的是舊版框架的讀數 —— **換框架＝換了母體**，舊讀數是**另一個問題的答案**，而空的那一列跟填好的長得一樣）＋ **`str()` 比物件欄位＝比到實例位址** ⇒ 被報成 44／44 全在變（**比較器在動，不是世界在動**；簽名＝比率 100%／0%）。

### 行政
- `.git/index.lock` 今天撞兩次（08:19:25／08:25:54），**兩次都照四步協定**（0 bytes ＋ 兩次觀察 mtime 不動 ＋ `tasklist` git 進程 ＝ 0）判 orphan 才清除。
  ★成因方向：**六個 session 共用 main dir，而 PreToolUse hook 每次 git 指令都會跑 `git status`／`diff`＝會刷 index＝會取 lock** ⇒ **撞鎖是常態，不是「別人正在 commit」的訊號**。★**不加閘**。
- 新登 defer：`gather-cross-tick-cache-needs-a-data-structure`（(乙) 跨 tick ＋ 髒旗 —— 錢在那裡但要堆／分桶，**本票不偷偷長大**）。

### ★結案（不是排工）：「bed-arm 白名單 271 張存量」
我把它掛在自己的排程上（「先分類再談工作量」）——★**而分類 2026-09-02 就做過了，答案寫在 `bed_arm_gate.gd:16-24`**：
```
白名單 272 → 靜態篩「arm 在 setup 之後」9 張 → ★逐張讀完【0 張真盲】
成因：「arm 在 setup 之後」有兩種相反的意思而在 code 上長得一樣
      ①先 setup 再 arm ＝真盲　②arm 之後又 setup（重置／多世界）＝不盲
```
★**所以那個數字是【未納管存量】，不是【盲區規模】** —— 今天實跑：母體 410 ＝ 已遷移 120 ＋ 白名單 271 ＋ 不適用 19 ＋ 未涵蓋 0，**PASS**。
★★而白名單**自 2026-09-08 起沒有再被編輯過**（`git log` 那個檔）⇒「單向下降」目前是成立的，
且**新床已經被判準③擋住**（不在白名單又不用 helper ⇒ FAIL）⇒ **它只能靠人手動編輯白名單才會長大**。
⇒ ★★★**處置＝結案，不排工、不加閘**。★而我差一點排了一份**已經做完的工** ——
**「我的排程」與「repo 裡的事實」是兩個不同的帳，而前者會過期。**

### ★★★第四條通道：那支「只是去蒐集脈絡」的函式會【抽 global 亂數】
```
gather → ThreatAssessment.score → _approach_score → PathSystem.observe_velocity → randf()
⇒ 少呼一次 gather ＝ 少抽一個亂數 ＝ 之後所有隨機事件整條錯位（★與它回傳什麼值【無關】）
⇒ ★★甲乙丙三個效能形狀【全部】到不了「語意零改變」那一格，而原因不在它們算出什麼
```
★**今天第二次同型**：上一次是「cadence 排程欄」，這次是「亂數抽取」——
**兩次都是「這個函式看起來只是在讀，其實它在動世界」**，而**我和 implementer 的清單上都沒有這兩條**。
★★**WHAT 核准「世界改變一次」**（歸類：「觀測儀器禁耗 global RNG」家族第 4 例；拆掉的是**【被呼叫幾次 ⇒ 世界演化】**這條耦合
＝ what-if／預演／UI 預覽的前置正當性），綁兩條件：①修後**三跑 byte-identical** ②**跨這顆 commit 的單 seed 前後對照不可歸因**。
★★★裁 **(A1) lazy**（把抽取搬到消費點），**否決 (A2) 決定性雜訊**在本票內 —— 它解的「同 tick 兩次觀測不一致」是真問題，
**但那是另一張票；效能票不准偷渡觀測模型的修改**。

### ★我自己的兩格（都記下來）
1. **沒過 R② 就派工**：`§6.6` 是 reviewer CLEAN **之後**才加的，而我在同一封信裡就寫「先做 A1」。
   ★成因不是忘記：**我把它想成「延續已 CLEAN 的那一票」**，而 A1 動的是 `path_system.gd` 的 production 介面＝**新的設計決定**。
   ⇒ 處置：自首、補送 R²、要 implementer 停手、**並明寫「WHAT 的核准仍然有效，要補的是 R² 不是 WHAT」**。
2. **同一份消費者清單，一小時內錯三次**：
```
①implementer「那個亂數純粹是燒掉的」        ⇒ ★寬了一格（threat 路成立、全域不成立）
②我「predict_intercept 是唯一的消費者」      ⇒ ★★窄了一格（還有 estimate_catch_up）
③R² 全庫掃：還有第三個 —— headless_test 的 assert(r.get("speed", 0) > 0, …)
```
★**三次都是同一個機械缺陷**：掃的是 `obs["speed"]` 這種下標讀法，而 **`.get("key", default)` 不長得像它**
⇒ **帶預設值的讀法會從清單裡消失**。★★而測試端的讀者不改，**回歸表上會多一支「預期中的紅」，
它跟「真的紅」在那張表上長得一模一樣**。★★★**「寬一格」與「窄一格」是同一個病**，而今天這一對**都是被別人接住的**。

### ★判準軸（R² 要的那一句，已寫進 spec）
**問「我要驗的那個【變因】，在不在同一棵樹裡？」**
```
變因在樹【內】（旗標／參數／開關）⇒ 同輪比，★禁釘歷史字串（無關的合法改動也會打紅 ＝ 噪音）
變因【就是樹本身】（修法前 vs 後）⇒ ★★必須釘住【另一棵樹】的產物 ＋ 標明 commit
```
⇒ ★**釘歷史字串會「被合法改動打紅」，在語意零改變那一格上【不是缺點、是功能】** ——
世界一旦被 WHAT 核准改變一次，**正確動作是【重錨基線】，不是放寬判準。**

### merge 第 11 票：`e71c0ba71`（凍結終線 A1 —— 觀測不再因【被呼叫幾次】而改變世界）
```
閘 65 支 ✓65 ✗0／790s｜判決綁 HEAD=b30909f31（registry=clean runner=clean code-dirty=0）
判決搬家核過：閘的基底 06dc854ae 與 merge 基底 3e98e6f6c 之間【只動 docs】
```
★**過程中紅過兩支，歸因不同人，兩支都修好才 merge**：
①`cross-run-static`（implementer：兩個診斷 static 沒清）—— 裁**真的清、不進白名單**，
而他**在 `_reset_cross_run` 裡逐個具名**：★**閘不追呼叫鏈是它的誠實限，不是它的 bug**
⇒ **他改的是「讓被檢查的東西長成守衛看得見的形狀」**，不是去改守衛。
②`defer-open`（**我的**：兩個 met_check 錨在【會移動的東西】上，被本票合法推走 ⇒ 誤判成「病消失」）。
★★**而中間還有一輪整批【不可判】** —— 我在它還在跑的時候換了 HEAD（開跑 `87846afe2`、結束 `b30909f31`）
⇒ 那批 62 綠 2 紅**全部作廢**；**那條規矩對我自己一樣適用。**

### ★我今天錯的兩個預測／兩個動作（都記下來）
1. **預測錯**：我說「核准了世界改變一次 ⇒ 任何釘住舊 fp 字串的守衛預期會紅」——**一支都沒紅**。
   錯的方向是**我以為有很多守衛釘著 fp 字串，其實沒有**。
2. ★★★**我自己把錯誤訊息吃掉了兩次**：`git add … 2>/dev/null`／`git merge --ff-only … | tail -1`
   ⇒ 後者只留下 `Updating <old>..<new>`，**那一行看起來就是成功**，我差一點回報「已 merge」。
   ⇒ **通則：傷害在【沒有訊息】那一側；核結果要看【狀態】不要看【那一行輸出】。**
   （修法是行為：merge 後印 `main=$(git rev-parse --short main)` —— **狀態行不會騙人，過程行會。**）

## 2026-09-18（末）｜下半場五票 merged，而★這一天真正的產物是【四種假退役】與【四次分類少一格】

```
eae0b11a7 相位樹淨成本床｜e71c0ba71 凍結終線 A1｜02e8f0950 求居／佔村讀 belief 等級
ff4e44f54 位置 belief 過期線物理化 Slice 1｜94d56ad13 gather 分段量測資產
```

### ★★★一張票【被它自己的規則擋下來】
「掃一次、多段共用」查到最後：走訪 4→3（−25%，★而它只作用在【迴圈開銷】，佔比沒量過）
＋ `score` 呼叫 −9.4%（那三段內部的）⇒ 對 `gather` 的上界 **≈5%**，
代價是改兩條迴圈結構 ＋ 七個 Probe 桶的既有卷面作廢一次。
⇒ 照該票自己的判準表第三列（**不許為了交差去做一個省 3% 的優化**）**結案，修法不做**。
★**那條規則是我為了擋我自己寫的，它第一次生效，擋的就是我自己的票。**
★★而交付不是零：四條通道的量測帳／「同一集合被走四趟」的發現（靠**圈數逐字相同**，不是靠清單）／
`ThreatAssessment.score` 的純度證明（**對世界純、對儀器不純**）／判準表多出來的那一格。

### ★★四種假退役（今天各出現一次，而★★★四種腐爛時顯示的【都是綠】）
```
①【行號範圍】錨   sed -n '242,813p' ⇒ 別人在同檔加了 102 行 ⇒ 範圍滑掉
②【整行文字】錨   錨住一整行 ⇒ 那一行被【合法地搬到別的函式】⇒ 不再命中（★同型犯了兩次）
③【代理量＋任意門檻】「提到『到場點名』的檔 ≥ 30 支」⇒ 新增兩張床就跨過門檻
④【錨在這一票才會出現的符號上】⇒ ★我在 merge【之前】登 defer ⇒ 登記當下就是假退役（自己攔下）
```
★**修法一律是「換成不會因為無關改動而移動的東西」**：函式範圍／性質／我們真的依賴的那個母體。
★★而 ③ 的重錨順便回答了掛很久的「先找對的軸」：**註冊床 35 支裡 8 支缺點名 ⇒ 工作量是 8 支，不是 229 支。**

### ★★四次分類少一格（三次是別人找出來的，第四次是下游說「我判不下去」）
```
①「gather 少呼叫」的第三種原因（RNG 抽取）——我與 implementer 的清單上都沒有
②消費者清單：我「窄一格」、他「寬一格」——★而範圍講寬與講窄是同一個病
③「為什麼這個數字小」——我三個選項【全部預設母體存在】，漏了【它本來就罕見】
④集中度那個軸：我只給了「單一大頭」與「攤平」，★漏了【幾個中等的頭】
   —— 而我 memory 裡就寫著「可比較的量必然是三格」
```
★★★**新的檢查句**：**當對方說「我判不下去」時，先懷疑【選項表少一格】，不要先要求他更果斷。**

### ★sha 紀律：同一格在我們之間【各生效一次】
```
早上：我核了「production 沒動」卻沒核「這顆是不是新的」⇒ 整批閘跑在未修的樹上（13 分鐘白跑）
下午：他一封信裡寫了兩個 sha ＋「merge 順序你排」⇒ 我先 fetch＋rev-parse＋開檔核內容才建樹
      ⇒ ★立規：一封交件信只給【一顆】要被核的 sha
```
★★而「這次沒事」不是規矩有效的證據 —— **這次沒事的原因是我剛好踩過同一個坑。**

### 其他落地
- **A1**（觀測不再因【被呼叫幾次】而改變世界）：WHAT 核准「世界改變一次」，綁三跑 byte-identical ＋ 前後不可歸因。
  ★而它做完之後**效能那條路仍然不通** —— 因為還有**合法的**抽取（`estimate_catch_up` 真的在用那個速度值）。
- **R² 三次不只指出問題，而是自己去找更硬的證據**：讀三支函式的迴圈體（零 `break`）／寫兩支探針實測 parse error ／
  ★★用 diff 獨立核 1-e 的憑證，不只信報告裡的 fp。
- ★**`Callable` 動態呼叫少傳參 ＝ 不報錯、整個 process 卡死到逾時** ⇒「必填、無預設值」的保證**只罩靜態呼叫點**（已登 defer）。
- `index.lock` 今天撞 8 次，**每一次都照四步協定**；★其中一次它**自己消失**（＝活的持有者）⇒ 協定沒有誤刪任何人的鎖。

## 2026-09-22｜復工日：★一個【不是 code】的阻斷、一個我自己造的誤診、以及兩個「機制在 ≠ 現在有」

### merge 第 15 票：`352c711a4`（到場點名④）
```
★「8 支缺點名」實際是 4＋2＋2 —— 而那 2 支【假點名】比「沒有」更糟：
   fp-longwindow 印的是【字面】SECTIONS=1/1；phase-root-conservation 的分母是【跑了幾段】
   ⇒ 崩在第一段也印 1/1，而兩支的 expect 又正好釘那個數
   ⇒ ★★註冊表以為自己在查出席，其實在查【一個不可能不同意的常數】
★修法一致：分母換成 const EXPECT_SECTIONS ＋ 少跑就 push_error
★★★判決由兩半拼起來，而我把血統寫清楚：60 支搬自 implementer 的樹（與本樹之間只動 docs）
   ＋ 5 支【會讀 defers.tsv 的】由我在本樹重跑 —— ★而那五支是 grep 出來的，不是記得的
```

### ★★★我自己造的誤診（而且我已經拿它去要用戶改機器設定）
```
我報：閘全紅是因為「這台機器的 PowerShell 停用指令碼執行」，而那個失敗【rc＝0】
真相：★rc ＝ 1 —— 我那個 0 是【我自己接了 | head 吃掉回傳碼】量出來的
      ★★而阻斷不是「機器壞了」：Claude Code 的【PowerShell 工具】進程 Process scope 本來就是 Bypass；
        是【Bash 工具】裡 spawn 的 powershell 沒有那個 scope ⇒ 阻斷是【誰啟動它】
⇒ ★★★同日第三、第四次同一家族（2>/dev/null ／ | tail -1 ／ | head ×2）
⇒ 而防它的規矩【我今天早上才自己寫進 memory】：核結果要看【狀態行】不看【過程行】
⇒ ★我有那條規矩，卻沒在【診斷一個阻斷】的時候把它套回自己手上那條指令
★★而 Claude Code 的安全閘擋下了我基於這個錯誤診斷要做的 repo 改動 —— 事後看，它擋得更對
```

### ★環境紅成為【第三種顏色】（改既有 runner，不加新閘）
```
0 ＝ 全過｜1 ＝ 判準紅｜★2 ＝ 本輪【不可判】：引擎一次都沒被啟動 ⇒ 綠與紅都不算、不進任何統計
★理由不是「rc 騙人」（那句我撤回了），是【環境失敗與測試失敗都是 rc≠0，而它們要人做的事完全相反】
★★兩個方向【在不同輪次各被驗過一次】：
   陽性＝環境真壞著的三支＋合成成對（「說明裡提到那些字但自己通過」⇒ 仍是 ✓）
   陰性＝implementer 那輪 65 支健康環境、零支被誤判 ⇒ ★正是我早上明寫「今天驗不了」的那一半
```

### ★硬體世代邊界（用戶令）—— 而我先把【兩個軸】分開
```
世代邊界（code 變了）⇒ 連【邏輯量】都不可跨代引用
硬體邊界（機器變了）⇒ ★只有【時間量】不可跨機；fp／圈數／呼叫次數／母體【仍然有效】
⇒ ★★判準一句話：問這個數字是【秒】還是【次】
HW-2 ＝ 2026-09-19 起｜AMD Ryzen 7 5800X3D（8C／16T）；★HW-1 的 CPU 我【留空待補】，沒有編一個
★★★而我加了「哪些【不】受影響」那一節 —— 過度作廢的代價是：下一個人以為什麼都不能引用，於是什麼都重量一次
```

### ★★兩列複審日到期的 defer：我逐項驗過，沒有把日期往後推了事
```
single-writer-leftovers：①反向 roster audit ★【早就做完了】（headless_test:447 有那條不變量）
   ⇒ ★★而我們一直不知道 —— 這就是日期型複審的真實產出
   ②beast_system 的 LOD 升級還沒做（56 行、LOD 字 0 個）⇒ 留著，★判準從【日期】換成【機械】
   ③subteam clear() ⇒ ★★★退役：一個【自己說不清楚要做什麼】的待辦，留著只會每次複審都卡住
otherdomain-coordination：★WHAT 給了機械解除條件，而照著查下去【推翻了它的前提】——
   敗北出路（潰逃俘虜）**已經存在、已接電、已上三顆計數**（npc_combat_system.gd:545-552）
   ⇒ ★★缺的只有【當代世界的一次量測】：非零樣本全部來自 2026-07 ＝ 世代 1 ＝ 跨代不可引用
   ⇒ ★★★所以它等的不是「去建」，是「去量」—— 成本估計整個變了
```
★**而我在這一列上連錯兩次，兩次都是【寫完當場跑極性】抓到的**：
①「production 零呼叫」是錯的（`| head -5` 把 production 那一行截掉了）
②第一版 met_check 命中的是【定義那一行】⇒ 又一次「grep 名字不 grep 呼叫點」，
  ★而這次是我**在同一列裡引用完這條規則之後**犯的。

### ★今天長出來的兩句話
```
①【複審日型】與【解除條件型】是兩種紅，要在列上標型別 ——
  讀 defer-open 紅的人會以為「解除條件已達成」＝「可以退役」，而這種紅的意思是【該回頭看了】
②★★「機制在 ≠ 現在有」—— 接電、計數、print 全都在，而樣本全是舊世代的
```

## 2026-09-22（下）｜merge 第 15／16 票，而★這半天的主線是【我的分類表一直少一格】

```
352c711a4 到場點名④（假點名比沒有更糟）｜f235c08c8 一天的長度改讀單一真值
```

### ★★★那個阻斷的真解：**不用改機器、不用改 repo**
```
病：Bash 工具裡 spawn 的 powershell 沒有 Process-scope Bypass ⇒ godot.ps1 一次都沒被載入
★而我第一版診斷把 rc 說成 0（我自己接了 `| head` 吃掉回傳碼）⇒ 真值 rc＝1
  ⇒ 我對 runner「第一道 RC 判準瞎了」的指控【撤回】
★★線索是 implementer 給的：Claude Code 的【PowerShell 工具】進程本來就是 Bypass
⇒ ★★★真解：在 Bash 環境 `export PSExecutionPolicyPreference=Bypass`
   —— 它不是新增繞過，是【讓 Bash 端拿到 harness 本來就給 PowerShell 端的東西】
⇒ 三條路（改機器安全設定／改 repo 46 處／只從 PowerShell 工具跑）★全部撤回
```
★**而我已經拿那個錯的診斷去要用戶動他機器的安全設定** ⇒ **那是散布，不是「差一點」。**
★★**而 Claude Code 的安全閘擋下了我基於錯誤診斷要做的 repo 改動** —— 事後看，它擋得更對。

### ★環境紅 ＝ 第三種顏色（改既有 runner，不加閘）
```
0 ＝ 全過｜1 ＝ 判準紅｜★2 ＝ 本輪【不可判】：引擎沒被啟動 ⇒ 綠與紅都不算、不進任何統計
★★理由不是「rc 騙人」（撤回），是【兩者都是 rc≠0，而要人做的事完全相反】
★★★兩個方向各兩次證據：陽性＝真壞的三支＋合成成對；陰性＝兩輪健康環境 ENV＝0
   —— 而我早上明寫「陰性今天驗不了」
```

### ★★★這半天我的分類表少了【五次】格
```
①「連潰逃都沒發生」底下住著兩個相反的世界 ⇒ combat_decisive ＝ 0（當代潰逃 92.5%／殲滅 0%）
   ⇒ ★而那是「世代 1 跨代不可引用」在【行為層】的實例
②手抄一天的母體：我寫「至少四處」 ⇒ 實為 25 行／15 檔（我只看了 grep 的前 8 行）
③三類 ⇒ ★四類：窗長本身（range(240*30)）的修法【與前三類相反】——不准改值，只換等值表示
④世代編號：我寫「世代 3」而那個檔【已經寫到世代 5】（我 `head -40` 讀了半份）
   ⇒ ★★而我【沒有順著 WHAT 的假設編出「兩套編號」】—— 沒有兩套，是我讀漏了
⑤2-a 那一格：R² 指出它對【不隨輸入變】的實作是【平凡通過】
   ⇒ ★★★恆真入口躲在【我最信任的那一格】裡 ⇒ 補 2-h 堵掉
```
★**共同成因有兩個**：**`head`／`tail` 截斷**（②④）與**軸只列兩端**（①③⑤）。

### ★兩個實驗，兩個都改變了裁定
```
①`const TPD := 240` → `WorldState.TICKS_PER_DAY` ⇒ **6 FAIL → ALL PASS**
   ⇒ ★那 6 支紅【不是行為 bug】，是床自己的常數在說謊
   ⇒ ★★而它沒有註冊成閘 ⇒ 紅了很久沒人看到（`beds-that-can-red-but-nobody-runs-them` 的第一個受害者）
②L 的定義：`estimate_catch_up` 內部呼 `observed_speed()`（有 `randf()`）
   ⇒ 共用它 ＝ 少抽一次 ＝ 改變世界 ⇒ ★它屬於【計算】那一側 ⇒ L ＝ 44.6% ＜ 57.1%
   ⇒ ★★照 WHAT 預註冊的門檻：那張票【永久結案】
```

### ★預註冊門檻的真正價值（我自己的紀錄）
```
★它沒有讓答案變好看，它讓答案【在被知道之前就有歸屬】
⇒ ★★量出 L_A＝71.2% 的那一刻，如果門檻是量完才訂的，**我會很想把 estimate_catch_up 算成走訪**
```

### 其他
- **潰逃俘虜**：當代世界 `retreat_captured ＝ 9`／母體 `combat_retreat ＝ 160` ⇒ 解除條件成立 ⇒ 那一列**退役**（而「解除」＝變成**合格排程**，不是插隊）。
- **世界改變窗 #2 開**：內容＝觀測雜訊決定性化；★枚舉時我明標**這份枚舉是清單式的、會漏**，而真守衛是**每票自己的 fp 逐字格**。
- **spec 的 `status:` 欄**：162 份裡 **119 份根本沒有那個欄位** ⇒ ★**它從來就不是索引** ⇒ 新增 `specs/_README.md` 明寫「不要拿它當索引」，★★**而不是逐份補齊（那是手工翻，下週再失真一次）**。

### 2026-09-22（三）系統：預註冊門檻、一張結案票的理由死掉、以及一條**假未達成**的 met_check

**① 拋棄樹處置**：`_gt1` 裁可刪（HEAD `f345c1d2c`、未 commit 0 個、**非 main 祖先**，內容經 `352c711a4` 已在 main）。
★**刪之前先打 tag `evidence/2026-09-22-rollcall4-verify-tree`**——因為我在 merge 訊息裡引用過那顆 sha，
刪樹之後它會失去所有 ref ⇒ gc 之後那個引用查不到。★★刪的動作留給 implementer（我有拆掉別人在用的樹的血證）。

**② 預註冊門檻（在數字回來【之前】寫死）**：解鎖的兩塊共用單位 **U ＝ 對 gather 的省法上界**，
**U ≥ 10.0% 開票／< 10.0% 永久結案**。★10.0% 不是現在挑的：上一票 WHAT 定的 L=57.1% 換算過去正好是 10.00%
⇒ 同一種代價類別沿用前例。換算式兩塊都攤開在信裡。已派 implementer **先量規模、不開修法票**
（那兩塊的舊數字 **跨代又跨機**：世代 5⇒6 ＋ HW-1⇒HW-2 ⇒ 沒有分母）。

**③ ★★★我自己那張「永久結案」的理由被推翻了，而我沒有自己裁**：
掃一次多段共用那票是照 WHAT 預註冊門檻結案的，而我當時寫明「裁定不是偏好」，靠的是
「`estimate_catch_up` → `observed_speed()` → `randf()` ⇒ 共用結果＝少抽一次＝改變世界」。
★觀測雜訊決定性化（世代 6 邊界 `3fb2c3201`）落地後逐行核過：`path_system.gd` 剝註解 RNG **零命中**；
讀取路徑每一站（`_visible_this_tick`→`best_estimate`／`belief_pos`／`catch_cost`／`MovementSystem`）**零命中**；
`vision_system.gd` 僅存的兩顆（:148／:183）都在 `_write_tier01`（:138）＝**寫入**路徑，不在這條讀取路徑上。
⇒ ★★分類會從【計算】翻到【走訪】＝過門檻那一側。**已呈 WHAT 裁「永久結案綁那句話還是綁那個理由」**，兩邊都寫。
★★★而我明寫**不用排程把這題繞過去**：②先①後【會】讓它自己消失（②落地⇒①的 L 被拉回不過門檻側），
但那不能當答案——①②不是獨立兩張票，先做誰會改變另一張的分母。

**④ ★★★活捉一條【假未達成】的 met_check（⑨的反極性）**：
`one-pass-merge-worth-revisiting-after-deterministic-noise` 的 met_check 是 `! grep -q 'randf()' path_system.gd`，
而**滿足它的那一票，自己把 `randf()` 寫進了四行註解**（在解釋它換掉了什麼）
⇒ ★**修好它的那一票，正好是把它永久釘在「未達成」的那一票**。已改成剝註解版。
★★並**機械普查**全表 30 條 met_check（抽 pattern ＋ 目標檔，「命中全部落在註解行」就亮）：
掃出 **1 病（已修）＋ 1 個本來就故意比註解的合法列**（`tile-has-resident`）。
★★★**陽性對照**：拿那條已知壞的 pattern 餵自己的稽核器 ⇒ `hits=4 code-line hits=0 -> would flag: YES`。
★誠實限：它抓不到⑨那一極（命中 code 行但語意無關），也跳過抽不出引號 pattern 的列。

**⑤ WHAT 裁回來了（同日）**：永久結案**綁【理由＋世代】，不綁那句話** ⇒ 那一票**自動重開**，
★**門檻（57.1%／10%）一字未動——翻的是【分類的事實前提】，而那個前提本來就標在世代 5 上**。
★★並立**新規矩（我寫進 `defers.tsv` 表頭）**：**以事實為理由的結案，結案書必寫【成立謂詞】；
謂詞被【世代邊界內的 merge】直接推翻 ⇒ 結案自動重開，依原門檻重判；
★★★「我覺得世界變了」不算——沒寫進謂詞的變化【不觸發】**（防它變萬用鑰匙）。
序 ②→① 准。★盤點現存 `status: CLOSED` 的 spec ＝ **1 份**（就是這一份）⇒ 新規矩**沒有回溯債**。

**⑥ ★★★又拆掉一列「綁兩格、而 met_check 只罩得住一格」的 defer**：
`observe_velocity` 那列自己寫著「②與①同票修，一起退役」——★而窗 #2 只做了①（決定性雜訊），
②（「距離越遠估越粗」對 `direction` 從來沒生效）**一行都沒動**（仍回傳 `"direction": actual_velocity` ＝ 真 velocity）。
★★而 met_check 錨在「`path_system.gd` 還有沒有 `randf(`」⇒ **只罩得住①** ⇒ ②跟著被判成「病好了」。
⇒ 已拆成兩列；②的錨第三版釘在**它自己的性質**上（`observe_velocity` 函式本體裡出現 `observation_noise01`）。
★**寫完當場做了兩件事**：①驗極性（FALSE＝未達成，正確）②**跑陽性對照**（往本體注射一行 ⇒ 稽核器看到 TRUE）
—— ★★而第一版錨我**當場自己抓到是假陽性**：awk 區間吃過頭，把 `observation_noise01` 的【定義】也算進函式本體了。
⇒ ★★★通則入表頭：**一列包 N 格時，met_check 必須罩得住【最後才會好的那一格】，否則就要拆列。**
`defer-gate.sh` 現在 **PASS（78 筆）**。

**⑦ ②`estimate_catch_up` 只算一次 ⇒ ★裁定：不做（U₂ ＝ 8.39%／8.15% ＜ 預註冊 10.0%）**
結案書 `docs/superpowers/specs/2026-09-22-estimate-catch-up-once-HOW.md`，**帶【成立謂詞】**（WHAT 今天立的新規矩）：
P1【形狀，可機械檢查】生產碼呼叫點 ＝ **7**（核過，全在 `faction_ai_system.gd`）⇒ 變 8 ⇒ 謂詞假 ⇒ 自動重開；
P2【規模】S×E ＝ 0.170／0.166 ⇒ ★**不可 grep，而我沒有替它編一條會恆綠的 met_check**，
寫明觸發器是【世代邊界時重量】。★數字我重算過不是轉貼（0.587×0.290×0.493＝0.0839 ✔），四個檔逐一開過。
★★判準第三格（≥10／<10／**不可判**）：母體沒塌、D2 沒超構造上界、兩顆種子 ⇒ **可判**。

**⑧ ★★★預註冊順手寫下【構造上界】＝ 免費的母體錯配偵測器（本輪真的救了一次裁定）**
我在派工信裡寫了「D 的上界 1/2」。implementer 第一版量出 **D_all ＝ 0.695 ＞ 0.5**，
★**他自己把它判成【母體錯配】而不是「賺更多」**——成因是分子含全部 7 個呼叫點、而分母（計時）只含 2 個。
★★代價具體：那個錯配把結果高估 **1.4 倍（8.4% → 11.8%）⇒ 直接翻過門檻 ⇒ 會做出相反的裁定**。
⇒ 入帳：**凡派【比值】的量測，同句寫出它的構造上界**——不花錢，而它會在換了母體時自己亮。

**⑨ 兩件不讓它被吸收掉的**
`share-estimate-catch-up-across-all-seven-call-sites`（新列）：把 7 處**全部**共用是更大的形狀，
★**而它的 U【算不出來】不是「比較小」**——E 的分母只涵蓋 2 處 ⇒ 另外 5 處的 T 是空白。
①（一圈形態）已派量，★**但先釘形狀**：世代 6 之後原票 §1 的「禁共用計算結果」**理由也死了**
⇒ 寬形狀／窄形狀的 L 是**兩個數**，而**只有窄形狀的換算式是預註冊過的** ⇒ 這一輪只量窄形狀。
★★並把上一輪的量法血證一起交待：(乙) stub 對照法自己否掉自己（圈數 206119→10422）
⇒ **圈數要當母體衛生標一起印**；掉一個量級 ＝ 那一輪作廢，不是答案。

**⑩ ①一圈形態（窄形狀）⇒ ★裁定：不做（U₁ ＝ 9.44%／9.24% ＜ 10.0%）**
結案書 `docs/superpowers/specs/2026-09-22-one-cycle-shape-HOW.md`，帶 WHAT 今天分出的**兩種成立謂詞**
（形狀謂詞＝可 met_check；規模謂詞＝**不可 grep、不編恆綠檢查**）。
★★★**這是邊界案例（差 0.56 個百分點）⇒ 我沒有只看那個數**：
`U₁ ＝ S × (0.094 ＋ 0.156L)` 對 L 單調遞增 ⇒ 要推過 10.0% 需 L 高 **6.07 個百分點**
＝ 計時被高估 **3.27 秒** ＝【整套樁成本 0.545 秒】的 **6.0 倍** ⇒ **樁的偏差翻不動這個判定**。
★implementer 把樁偏差標成「誠實限」而沒定量——**那是對的，他不判；而定量是判準持有者的活**。
★★他提議的 12 分鐘補量我二度回絕，理由是**方向**：具名更多計算 ⇒ L 變小 ⇒ U₁ 變小 ⇒ 只會更穩；
真正可能翻盤的是**反方向**（樁灌大 T_calc＋T_catch），而那一格已定量否掉。
★★★**兩個方向是兩件事**——別讓「還有一個補量沒做」看起來像「還有不確定性沒處理」。
★附：①②的數字**同源**（同一個 log）⇒ S 是同一次量測的同一個數；
而 implementer 自己寫出同源的壞處（**共用一切誤差**）—— 那正是我去壓力測的原因。

**⑪ ★★★一個我特地【不做】的算術（它看起來很有說服力，而它是錯的）**
已知那 2 處的 T 已占 gather **17.0%／16.6%**，而 D_all ＝ 0.695。
★誘惑：`17.0% × 0.695 ＝ 11.8%` ⇒「過門檻了，直接開修法票」。
★★而 C 是【2 處】的、D 是【7 處】的 ⇒ **不同母體，乘起來沒有意義**——
**這就是上一輪抓到的那個錯配，連數字都是同一個 11.8%**。
⇒ 寫成「下界線索、不得相乘」，並要求**逐呼叫點分列的卷面**（7 行，T 與呼叫次數各一欄）
⇒ ★**讓母體錯配下次自己現形，而不是靠人記得**。

**⑫ 七處共用：WHAT 裁【排量、不排修】，而我訂正了他預註冊式子的一處**
他寫「另外 5 處的耗時」⇒ ★**會漏算**：共用的節省**橫跨全部 7 處**（那 2 處的呼叫也在共用之列）。
⇒ 訂正為 `U₇ ＝ C₇ × D₇`，**C₇ ＝ 全部 7 處的耗時佔 gather**（缺的只是另外 5 處的 T）；
D₇ 照他寫的（同鍵重算 ÷ 全 7 處呼叫），**構造上界 1−1/7 ＝ 0.857** 照收。門檻 10.0% 不變。**已派量。**

**⑬ ★訂正我自己帳上的一筆：那支「在 main 上紅著沒人跑」的床，其實是【綠的】**
`scripts/debug/crisis_override_test.gd` 我實跑：`=== DONE === ALL PASS`（17 格全過，rc=0）。
★我帳上寫它「紅了一段不知道多久」——**那是過期的狀態**（同族：我的狀態帳會過期）。
★★但它確實**沒被註冊**，而它**可以紅** ⇒ 已補進 `docs/process/merge-gates.tsv`。
★★★**註冊前我先做了成對驗證**（而不是「看起來會紅就註冊」）：
```
陽性：反轉 `_ok` 的判準 ⇒ `=== DONE === 16 FAIL`   ⇒ 它【會紅】
陰性：原檔                ⇒ `=== DONE === ALL PASS` ⇒ 它【不會亂紅】
★而 rc 在紅綠兩邊都是 0（`quit()` 不帶碼）⇒ **判準通道只有字串，禁拿 rc 當閘**（裸 FAIL 式）
★★expect 我拿兩條真實輸出各比一次：綠命中、紅不命中 ⇒ **不是恆真**
★★★陽性對照檔放在 `test_*.gd`（**已 gitignore**）⇒ 別的 session 掃不進 commit；跑完即刪
```

**⑭ ★數字往【不可能的方向】動 ＝ 免費的偵測器（今天第二次）**
我先報「註冊表 39 支」，加了一列之後數出 **38** ——★**加了一列不可能變少** ⇒ 當場去對。
成因：兩次用**不同的樣式**數（`[a-z0-9_]+\.gd` vs `scripts/debug/[a-z0-9_]+\.gd`），
而前者會把 `_test.gd`／`_hysteresis_test.gd` 這種**碎片**也數進去 ⇒ 兩個數根本不是同一個量。
★真相：兩種數法**各自都 +1**（40／38），沒有缺陷。★★**而 defer 的 met_check 用的是帶前綴那個 ⇒ 是對的。**
★★★同族：上一輪是「比值超過構造上界」，這一輪是「單調量往反方向走」——
**兩個都是【我先寫下它應該往哪走】才換來的**。

**⑮ hook 抓到我一句裸承諾（★機械守衛接住了我自己 memory 裡那條規則）**
我寫「**已派量**」而同一行沒有票的 exact path ⇒ `📬` hook 當場擋。已補上路徑。
★這正是我 memory 裡「『已請』是宣告不是事實」那條 —— ★★而**接住我的是機械守衛，不是我記得**。

**⑯ ⑦七處共用 ⇒ ★裁定：開票（U₇ ＝ 19.13%／19.09% ≥ 預註冊 10.0%）**
spec `docs/superpowers/specs/2026-09-22-share-catch-up-seven-sites-HOW.md`（97 行），**已送 R²，CLEAN 前不 dispatch**。
形狀：`estimate_catch_up` 內部加 **per-gather memo**，key ＝ `(team_id, target_id, trusted)`
（★`trusted` 進 key：今天 7 處全傳 `true`——我逐點貼過引數——★★而「今天全是 true」不是不變量）。
★★★**方向**：U₇ 是【下界】（`trade_target` 零呼叫 ＋ 重算集中在最貴的三處），
而 U₁／U₂ 是【上界】⇒ **「高於門檻」與「低於門檻」各自站在安全側，但理由相反** —— 這格最容易講反。
★★**毛值 vs 淨值**：19.13% 沒扣快取自己的成本。我算了翻盤所需 ＝ 每次查表得吃 **9.19 µs**
＝ 一次完整呼叫（27.70 µs）的 **33%** ⇒ 不合理，★**但那是估的不是量的** ⇒ 壓成驗收 A2（驗**淨值**）。
★**門檻說的是「開票」，不是「這個修法保證划算」** —— 淨值 < 10% ⇒ 回退結案。

**⑰ ★★★implementer 自報的兩個錯，比那個 19% 值錢**
①**他的樁被 `continue` 跳過**（記帳插在 `... .reachable: continue` 之後）——
★**指出它的不是讀 code，是【未歸因缺口固定為 1233 次】**：
隨機的缺口像雜訊，★★**固定值 ＝「有一條路徑系統性地跳過」的指紋**。
②★★★**他的第一個假設被他自己的對照否掉**（以為是 `_in_gather` 巢狀旗標，改完缺口分毫未變）
⇒ **若沒跑那一輪，他會帶著一個「已修好」的錯解交件，而它長得跟修好了一模一樣。**
③★而他把 C₇ 從【清單保證】換成【構造保證】（計時包在函式內部、涵蓋 100% 呼叫，逐點分列只作歸因，
差額自己現形成「未歸因」那一格）—— **這比我派工時要求的形狀更好。**

**⑱ 今天三個「先寫下它該長什麼樣」換來的免費偵測器（同一族，同一天）**
①比值**超過構造上界**（0.695 > 0.5）⇒ 母體錯配
...
**㉑ `tg_poll` 那條線結了 —— ★而實測是【該做的人】做的**
blueprint 自己 arm 實測，三件全綠（2026-09-22）：
第二次印「♻ 換血接手」＋「✅ ARMED pid=7240」／lock pid `17404 → 7240`／前任印「⛔ 讓位」。
★我上一則寫的是「**我沒有實測**」＋理由（跑它會真的消費用戶頻道的訊息）——
★★而正確的收尾不是我後來偷偷補跑，是**由持有那個頻道的人跑，然後回報**。
⇒ ★★★現在這句可以改寫成：「**已由 blueprint 實測，三件全綠**」——
   **斷言的範圍跟著證據的範圍一起長，不是我先講大再補證據。**

**㉒ ★★★「6 支欠到場點名」是假的 backlog —— 真數是 1，而那 1 支是我今天自己補註冊的**
```
我的判準只認三個【名字】：`到場點名|roll_call|_attendance` ⇒ 數出 6 支
★而其中 5 支本來就有點名，只是叫 `EXPECT_SECTIONS`：
   const EXPECT_SECTIONS: int = 4 ＋ `_sections += 1` ＋ 印 `SECTIONS=%d/%d FAILS=%d`
   ★★而**註冊表的 expect 又把 `4/4` 釘了一次** ⇒ **雙重外部釘死**（原始碼一層、註冊表一層）
   ⇒ 少跑一段 ⇒ 印 `3/4` ⇒ expect 不命中 ⇒ **紅**。★這比我那三個名字強。
⇒ 同族：**grep 命中【名字】不命中【意思】**——★★而這一次它製造的是**假的 backlog（比實際大 6 倍）**，
   而不是假的綠燈。★★★兩種方向都會出現，別只記得其中一種。
⇒ defer 的 met_check 已放寬到認 `EXPECT_SECTIONS|SECTIONS=`。
```
**而那唯一真的一支是 `crisis_override_test.gd` ——今天我自己補註冊的那支。**
★★★**我註冊它之前做的成對驗證（反轉 `_ok` ⇒ 16 FAIL／原檔 ⇒ ALL PASS）
只證明了【判決印得出來】，沒有證明【到場的人是全的】** —— 那是兩件事，而我當時沒分開。
⇒ 已開低優先 handback 給 implementer（**排在 memo 那張票之後，不准為它打斷**），
要的形狀照那 5 支、不發明新的；並附陽性對照（把任一段第一行換 `return` ⇒ 必須印 `N-1/N` ⇒ 紅）。

**㉓ ★★★把「到場點名」這個病普查完：40 支 Godot 床裡，真正還盲的是【1 支】——而它還是我那一支**
```
註冊表 68 列 ＝ Godot 床 40 ＋ shell hook 28
★**先切出 shell hook**：腳本中途死 ⇒ 橫幅根本印不出來 ⇒ 已經是紅的
  ⇒ 【某支 func 中止、程序繼續】是 **GDScript 專有**的病 ⇒ 28 支不在母體內
Godot 床 40 支：
  [A] expect 釘了【會因少跑一段而改變的數字】 ＝ **30**
  [B] 只有固定橫幅、但**附了具名的免疫理由** ＝ **9**
      （「橫幅在 `_run` 內＝true」／「_verdict 毒值預設 15 處」／「格 inline，本地 _test_* func ＝ 0 支」）
  ★★**兩者皆無 ＝ 1 支：`crisis-override`（我今天補註冊的那支）**
⇒ ★★★而這個 1 與我先前用完全不同的方法（grep 點名形態）數出來的 1 **對得上** —— 兩條獨立通道同答案。
```

**㉔ ★★★而我的分類器在這一次普查裡【錯了三次，全是同一種錯】**
```
①`\d+/\d+` 只認半形 `/` ⇒ 漏掉「到場點名 5／5」（全形 U+FF0F）⇒ [B] 假性 36
②`=\s*\d+` 只認半形 `=` ⇒ 漏掉「[FAIL] 數 ＝ 0」（全形 U+FF1D）⇒ `outpost-belief` 被誤判成盲
③把 shell hook 與 Godot 床混在同一個母體裡數（★分類少一格，今天第二次）
⇒ ★**三次的方向一致：都把 backlog 灌大**（36 → 27 → 11 → 2 → 1）
```
★★**而每一次抓到它的，都是【我把桶子裡的東西印出來】而不是只印桶子的大小。**
⇒ ★★★通則入帳：**永遠不要只報一個分類的【數量】而不印它的【成員】** ——
桶子一大就先看裡面，因為**分類器錯的時候，錯法長得跟「問題很多」一模一樣**。
★這條在本專案是老病（`[FAIL]` 式普查、29 支零判準通道那次都是），而今天它在**同一次工作裡連犯三遍**。

**㉕ ⑦七處共用 `estimate_catch_up`（per-gather memo）★MERGED `69c00dbdb` → 合併點 `4c52a6cdd`**
```
★我在【合併結果】上自己重跑全套：68 支 ✓68 ✗0｜375s｜rc=0
  —— implementer 那 68 支是在他 branch 上跑的，而 main 當時已往前 6 顆 ⇒ **不是同一棵樹**
★★數字我自己算過（不轉貼）：A2 淨省 14.26%／15.95%；A3 hit 率 0.6948／0.6989 ＝ D₇ 逐位相同
★★★獨立掃了兩件：①新增行裡所有 `Probe.enabled` 只有 3 處、**全是 tap，零語意**
                  ②【共用同一顆 Dictionary 參照】⇒ 全庫零個消費端就地改回傳值
diff：3 檔、+63 行、零刪除
```
**㉖ ★★★而 A3 抓到的是【我這份 spec 的洞】——形狀是我的老毛病**
```
我寫「正確性完全不依賴清空路徑」＋判準句「被跳過了還會不會錯」
★而那句話**預設了「每一次呼叫都在某一次 gather 裡面」**
⇒ 漏掉第三格：【**不在任何一次 gather 裡面**】⇒ gather 結束後的呼叫仍命中那次 memo
⇒ 過期的 eta／reachable，實測 **6134 次**（1 天窗 18048 − 11914）**不是零頭**
⇒ ★**二分法少一格**：「這一次」／「下一次」，漏掉「不在裡面」
```
★★**而 implementer 沒有照我 spec 那句「差很多 ⇒ 鑰匙錯了」直接改鑰匙** ——
他先問「為什麼兩個數對不上」才看到生命期超界。★★★**照著我寫的結論去修，會修錯地方。**
★A3 設計來抓【鑰匙錯】，這次抓到【母體範圍錯】⇒
**構造交叉驗不知道自己在抓什麼，它只知道兩個數該相等** —— 這就是它的價值。
★★那 6134 次已登 defer，**而登它的理由逐字留著：它【看起來已經解決了】**（卷面漂亮而它沒被碰過）。

**㉗ ★★★一個今天三張票都沒人問過的分母：`gather` ＝ 整場 tick 時間的 47.6%**
```
今天三張票的門檻全是「佔 gather 的百分比」，而那個單位是從更早那張票【繼承】來的
★沒有人回頭問過 gather 是整場的多少 —— 而我自己今天還寫了兩次「這一格最容易被講反」
  ★★**卻沒查過自己在用的分母**
⇒ 用已落地的驗收 log 算（不必重跑）：
   八天逐日 tick 總時 8.2／14.4／18.6／18.8／22.7／28.8／32.8／42.8 ＝ **187.14 秒**
   gather.* 合計 ＝ **89.01 秒** ⇒ ★**47.6%**
```
| | 佔 gather | 佔整場 |
|---|---|---|
| ★我們用的門檻 | 10.00% | **4.76%** |
...
| U₁（已結案） | 9.44% | 4.49% |
| U₇ 毛值 | 19.13% | 9.10% |
| **U₇ 淨值（已 merge）** | **14.26%** | **6.78%** |

★★**`gather` 佔將近一半 ⇒ 它不是角落，是最大的一塊**；而沒收的那 6134 次就坐在旁邊，**且看起來已經解決了**。
★★★**我不建議現在改門檻**（改了就不是預註冊的門檻）——
建議下一張票**直接用「佔整場 tick 時間」當單位**，並把換算率寫進預註冊。
★誠實限：一顆種子／一個 config／8 天窗／Probe 開；不含 setup；
★★而早期天數便宜很多（8.2→42.8 秒，隊數 68→103）⇒ **窗越長 gather 佔比還會往上** ⇒ 47.6% 是這個窗的數，不是常數。

**㉘ ★訂正兩處我自己的數字 ＋ 把本票換算成 WHAT 定的新最終尺**
```
①**47.6% 是【memo 關】那一輪的**（merge 前基準）⇒ **合併後 gather ＝ 43.7%**
②整場口徑收益 ＝ (187.140 − 174.445)/187.140 ＝ **6.78% 的整場 tick 時間**
③>2s 幀數（WHAT 新定的最終尺）：seed1337 **5→2**｜seed77 **1→0**（合計 6→2 於 23040 幀）
④blueprint 訂正我：gather 外那 6134 次在**已 merge 的版本**裡一律重算 ⇒ 是【沒省到】不是【過期答案】
   ⇒ ★我核過 `path_system.gd`（`_catchmemo_active` 為 false 時走 impl）⇒ **他是對的**
   ⇒ 票身補 §9a：§9 描述的是**修法【前】那一版**，免得下一個人讀成「main 上有正確性洞」
```

**㉙ ★★★WHAT 新定的最終尺，我先量了它：方向對，但**母體 n＝6**
```
基準率 ＝ 6/23040 ＝ **0.026%**
★它能回答「有沒有變壞」（門檻天然是 0）
★★它【不能】回答「改善多少」：6→2 與 6→4 在 n=6 上沒有分辨力；
   下一張票若只買到 6→5，這把尺會說「沒動」——而那可能是真的省了 3%
★★★**兩個方向都會在小效果上失靈** ⇒ 已建議寫成兩格：
   ①【硬閘】>2s 幀數不得增加  ②【效果量】用 p99.9 幀時間（母體 ＝ 全部 11520 幀，不是 6 幀）
```

**㉚ ★★★我差點把結論做反 —— 而抓到它的不是我小心**
```
我把 `memo0`／`memo1` 讀成「0＝關、1＝開」⇒ 算出「memo 讓 >2s 幀數變多」
★沒送出去，是因為那份 log 裡 `gather.* 合計 89.013` 與**卷面「memo 關 ＝ 89.013」對不上**
⇒ 檔名是 `MEMO_OFF=` 的值：**`memo1` ＝ 關、`memo0` ＝ 開**（與我猜的相反）
⇒ ★★**抓到它的是【另一份獨立記錄的同一個數】** ——
   今天同一形狀第四次（超過構造上界／往不可能的方向動／缺口是固定值／**檔名與內容對不上**）
⇒ ★★★通則：**不要從【標籤】推內容；標籤與內容是兩條獨立通道，而它們對不上的時候才會說話。**
```

**㉛ 效能票【預註冊模板】寫進 `docs/process/reference-measurement-protocol.md`（★不新建檔，擴充既有那份）**
四件：①單位＝**佔整場 tick 時間**（附換算率，不再用「佔 gather」）②硬閘＝`>2s` 幀數不得增加
③效果量＝**p99 ＋ median**（WHAT 裁）④★**每一格都要印它的尾巴／母體有幾顆**。
★★而第三格換了兩次：`>2s` 幀數 n＝6 ⇒ 換 p99.9 ⇒ **尾巴 12 幀＝同一個病** ⇒ 才落在 p99（尾巴 115）。
★★★**「先問母體再問比率」套在百分位上就是「先問尾巴有幾顆」。**

**㉜ ★★★而我把那一節寫成【未驗】，不是寫成用法**
```
尺的工具已經存在（`dieoff_perf_bed.gd:88-94` 印 median/p90/p99/max）——★而我跑了三次都沒拿到 `[perf]` 那一行：
  ①預設 12 個月 ⇒ 我自己的 timeout（rc=124）
  ②`DIEOFF_MONTHS=1` ⇒ **wrapper 預設 360s 逾時**（`tools/godot.ps1:84`）⇒ **rc=98**
  ③`GODOT_TIMEOUT=1500` 那輪 ⇒ **被 harness 因系統記憶體不足停掉**（非失敗；且我不自行重啟）
⇒ ★rc=98 ＝ wrapper 逾時、rc=124 ＝ 呼叫端逾時 —— **兩個都不是床壞掉**
⇒ ★★所以協議那一節分成【✔ 已驗】與【✘ 還沒驗到】兩欄，並明寫
   「**在有人真的看到 `[perf]` 印出來之前，這一節的『必跑』是尚未可執行的**」
⇒ ★★★**寫成用法，下一個人會以為它跑得起來** —— 而這正是我今天在別人身上抓的那個形狀：
   **驗收假設了一個不存在的開關。**
```
...
**㉟ ★★★一個我【查完決定不加規則】的稽核（而這比加規則難）**
```
起心動念：今天「只認半形」咬了我兩次 ⇒ 想掃全部 met_check 有沒有同樣的病。
★而稽核一跑就 38 筆 —— ★★**幾乎全是檔案路徑裡的 `/`**（`docs/process/verdicts/...`）
⇒ 我的樣式比對到的是【字元】，不是【那個字元的角色】⇒ **今天第四次同一族的假陽性**。
⇒ ★★★而分開「路徑裡的 `/`」與「搜尋樣式裡的 `/`」要解析 shell ⇒ **這個稽核不會便宜地收斂**。
```
★**然後我回頭看那兩個真實案例，發現我連母體都挑錯了**：
```
①`\d+/\d+` 漏掉「到場點名 5／5」（U+FF0F）　②`=\s*\d+` 漏掉「[FAIL] 數 ＝ 0」（U+FF1D）
⇒ ★★**兩個都不是 met_check —— 都是我【當場臨時寫的普查腳本】**
⇒ 病住在「每次重寫的一次性分析腳本」裡，而那**沒有任何閘或規則罩得住**
```
**⇒ 結論：不加規則。** 理由三條：
```
①`defers.tsv` 表頭已經 **143 行**，而「每個開場必讀都單向長大、而失效是靜默的」是我自己記過的病
②真正的母體（一次性腳本）**規則管不到**，加了也只是讓表頭更長
③★★★而**真正每次都抓到它的那個動作已經在 memory 裡**：**把桶子裡的成員印出來，不要只印大小**
   —— 今天四次假陽性，**四次都是這個動作抓到的**，不是任何一條規則抓到的
```
★★所以這一格的產出是【一個沒有被加上去的規則】，而我把「為什麼不加」寫下來——
否則下一個人會重做這個稽核，然後得到同樣的 38 筆。

**㊱ 兩件收尾**
```
①拋棄樹 `A:/GDS/_gt1` **已刪**（implementer 做的，我核過不存在）——
  ★而 `_gt4／_gt5／_gt6` 還在、worktree 共 **49 棵**
  ⇒ ★★**我不動**：他正在跑跨 tick 那輪量測，那幾棵極可能是活的
     （血證：我曾拆掉別人還在用的兩棵樹 ⇒ **「乾淨」對活躍的東西是【時刻】不是【屬性】**）
  ⇒ 等他回報做完再一起處理
②`specs/_README.md` 補了第 ④ 條：「什麼已經落地 ⇒ `landed-slices.tsv`」
  ★★★**並且把它今天之前是壞的這件事寫進去** ——
  「若你在那之前查過它，你得到的是一個錯的答案」
  ⇒ ★這是 `落地≠通知` 的文件版：我修好了那份清單，**而指著它的那份 README 並不知道**。
```

**㊲ ★★★量測 wrapper 現在自己印硬體戳（`tools/godot.ps1`）**
```
★這不是我發明的需求 —— `docs/measurements/_generation-boundary.md` **自己就寫著**：
   「真正的修法是卷面自己印硬體戳記；在那之前，跨機比較一律要人工指認」
   而 **HW-1 那一格到今天還是「未知」，正是因為沒有人戳過**（而我們拒絕編一個）。
⇒ 現在每一次跑都印：
...
★★**實跑驗過，不是只過語法**：PowerShell parser OK ＋ 真的印出來 ＋ 那支床仍 `8／8`、rc=0
★★★**而它改變了每一次跑的輸出** ⇒ 我跑了**全套 68 支**確認沒有閘被新 header 弄紅：**✓68 ✗0（521s）**
★它只管【時間量】：次數／比率／指紋**不受硬體影響**，不得因為這一行而作廢。
★★HW-1 那一格**仍然是「未知」** —— 我掃過所有量測檔與 process 檔，**沒有任何可查證的紀錄** ⇒
   **回溯不了，而我不編一個**；這一行是為了**下一次不要再發生**，不是為了補上一次。
```

**㊳ 立不變量 #7：★「記帳可以閘，語意不可以」（R² 兩輪找到的那顆地雷）**
```
★它是既有那條的**鏡像**：
   既有「觀測者禁耗 global RNG／禁污染 Probe」⇒ 管【觀測者不得影響被觀測物】
   新的 #7 ⇒ 管【**被觀測物不得依賴觀測開關**】
★★危險不在「有人故意」，在**照樣造句**：`decision_context.gd:503／1444` 的
   `if Probe.enabled: _in_gather = …` 就站在 `gather()` 的 entry／exit 旁邊
★★★而它會被【整張驗收表】一起掩護：要讀 tap 的格結構上只能跑 Probe 開
   ⇒ 語意在測試時發生、在 production 不發生 ⇒ 全綠，bug 只在玩家跑法發作
⇒ 修法不是再寫一句規矩，是**讓至少一格驗收跑在 `Probe.enabled = false`**
```
**★★★而我為這一行【付了錢】**：
```
加完那一行，`doc-line-cap.sh` **當場從 ✅ 變 🟡（601 > 600）** —— 三個角色的開場讀量一起超限
★而「warn-only 所以算了」正是**讓上限變成裝飾**的方法（我 memory 裡那條：雜訊會讓原本那支也開始被忽略）
⇒ **同檔省回一行**：把表頭上方兩行 meta 說明併成一行（規則本文一個字沒少）
⇒ **doc-cap 回到 ✅**。★★而我把「新增一行就要同檔省回一行」寫進那一行本身
   —— 下一個要加第 8 條的人，會在**同一個地方**讀到這個代價。
```

**㊴ ★今天第五次「差點開一個假 backlog」——而這次是【先查有沒有現成的閘】擋下來的**
```
起點：`known_issues.md` 5018 行／371 條，而我用名字比對數出「只有 22 條帶回訪條件」
★★而我沒有把那個數字送出去，因為它有兩個一眼可見的毛病：
  ①判準又是**名字比對**（`defer_until|回訪條件|重驗|…`）—— 條目可能用別的說法寫同一件事
  ②規則原文是「**新**條目必帶」⇒ **母體是 2026-09-01 之後的條目**，不是全部 371 條
⇒ ★★★然後我先問了那句該先問的：**「這條規矩有沒有東西在檢查它？」**
   ⇒ **有**：`ki-status` 閘（`.claude/hooks/ki-status-gate.sh`，註冊在 `merge-gates.tsv:54`）
     ——「known_issues 新條目必帶狀態欄」，而它**今天全套 68 支裡是綠的**；
     另有 `ki-anchor` 閘（`merge-gates.tsv:51`）管錨。
⇒ **結論：不加任何東西。** 規矩已經有不靠人記得的那一半。
```
★**今天五次假 backlog 的形狀一模一樣**（25 支欠點名→1／6 支→1／38 筆 met_check→不可判／
「29 支零判準通道」→1／這次 349 條→不存在），而**抓到它們的只有兩個動作**：
**①把桶子裡的成員印出來 ②先問「現有那支守衛為什麼沒接住我」。**

**㊵ ★「情報喚醒改看內容」那一票：merge 卡在【不可判】，而卡住它的不是那一票**
```
merge 電池跑在 merged result（不是分支——★world-fp 兩行只登在 main 的註冊表，
  分支樹上根本沒有它們，上一輪就是這樣漏掉的）
⇒ 68／72 時被系統因【全機記憶體不足】殺掉：綠 67｜紅 1｜環境紅 0｜★收尾判決行 0 個
⇒ ★依 runner 自己的定義，本輪【不可判】——不是綠，不當 merge 判決
⇒ ★★而沒跑到的 4 支裡，正好含那兩支世界指紋床（第 69、70 列）
   ——同一個結果連兩輪，★成因是兩個完全不同的東西（上輪:警報被吞／這輪:沒跑到）⇒ 不互相充數
那 1 紅是我的：一條【解除條件在登記當下就已經成立】的延後裁定列（恆真）⇒ 已刪，成對驗證過
★憲法審三條全過：感知鐵律（score() 五道具名閘＝構造保證）／可觀測性（tap 只增不減）／
  指紋正當性（pending_think 的 clear() 是 consume_and_clear 第一句、在早退之前、無條件）
```

**㊶ ★★★今天兩個方向的同一個病：【只活在一封信裡】**
```
方向甲（我）：我寫「我先前【設】了記憶體門檻 2.5 GB」⇒ 全庫裸掃零命中。
  `godot.ps1:226` 算出 freeGB、`:228` ★只印出來 —— 沒有比較、沒有中止。
  ★它不是沒有效果：收信的人當時真的照做了（量到 4.95 GB 才開跑）
  ⇒ ★★所以它感覺像「已經生效了」，而真相是它【只改變了一次人的行為】，
    下一輪沒有任何東西會再檢查它，★★★而失效是安靜的。
方向乙（implementer 自書）：他早就在信裡撤回過一句錯的註解，★卻從來沒改 code
  ⇒ 撤回只到了信箱，沒有到達【下一個人會讀的地方】。
⇒ 判準句：**「這件事寫在【下一個人會撞到的地方】了嗎？」**
⇒ ★機械形狀（本次做法）：把「紅了怎麼讀」的三格釘進 `merge-gates.tsv` 的【用途】欄，
  因為 runner 紅的時候會把那一欄印在判決行後面 —— 不是寫進信，也不是寫進我的暫存檔。
★附帶：那個量測【量錯了對象】—— 這次死掉時 Godot 只佔 165 MB，機器少的是 28 GB
  ⇒ 以 Godot 佔用量為基準的門檻守不住這件事。要不要接電是【決定】不是修補，已呈報待裁。
```

**㊷ ★★★凍結那條線：從「切迴圈」一路追到「一支系統佔最壞 tick 的一半」**
```
起點：世代 7 的玩家絕對尺【兩顆種子都不達標】（p99 1161／1052ms，>2s 事件 107／59 筆，同向＝訊號）
①分片票（切互動迴圈）⇒ ★R² 判 premise_contradiction：B3 量的是 advance_tick，
  而 spec 自己寫 advance_tick 契約不動 ⇒ 驗收恆真。★★那兩句是我寫的，隔了兩節。
②而更深的一層在倉庫自己的文件裡：現有量法把【tick】當【frame】的代理，
  理由是「12ms 預算不可切割」—— 而這張票要做的就是把它切開
  ⇒ ★★★修法本身摧毀了驗收儀器的成立理由。
③再往下：B3 那 166 筆是【單顆 tick 自己】花 2～4 秒（相鄰間距=1 的有 0／164 對），
  而全庫零執行緒、預算在整顆 tick 跑完後才檢查 ⇒ 2.4 秒的 tick 佔住主執行緒 2.4 秒
  ⇒ ★切迴圈救不了，因為【沒有人能去畫那一幀】。
④WHAT 裁 (A)：把每小時 pass 按隊錯開 ⇒ ★而我要求先量天花板（門檻在看到數字前寫死）
  ⇒ 量出來：★★7 支「必須留整點」的只有 500ms（通過），
    而 `near.faction_ai` 單支 1301.5／1111.1ms（45～51%）—— ★★★擋路的是【一支系統】。
⑤根因：`faction_ai_system.gd:1218` `_evaluate_all_body(state, _team_ids)` ——
  ★底線前綴＝刻意不用，函式體 392 行，跑的是 `for fid in state.factions`（全世界）
  ⇒ 散相位下會【每批重跑整個世界】：不是漏，是重複執行。
⇒ 前置票已派：讓它真的吃 team_ids，而【順序必須等於 state.factions 原順序】——
  ★寫成看得到的構造，不是意圖句；R² 的話：**不要事後靠指紋去發現它**。
```

**㊸ ★今天方法論上真正變硬的三條**
```
①★★★綱要與簽章都會說謊，只有函式體不會
  （`shape` 欄標 teams 而實際 world-scoped；簽章吃 team_ids 而體不看）
  ⇒ 已寫進 02_reviewer.md：分類一定要逐函式體核，且追委派要追到底（轉發存根常有兩層）。
②★回歸柵欄 ≠ 陽性對照：柵欄【今天恆真是合法的】（它抓未來的改變），
  陽性對照【現在】就必須有鑑別力。判準句＝問它「你要抓的是現在的錯還是未來的改變」。
③★★預註冊今天真的發揮作用：門檻與三格判讀寫在看到數字之前
...
### 2026-09-23（傍晚）：量測卷面的 145/148 —— 裁「一份都不動」

```
起點：我掃到 7/148 份 .measure.json 不是合法 JSON ⇒ 派量測員修
★而他修完【順手跑了我沒派的那一步】：verification_gate 全量模式
⇒ 145/148 FAIL —— 136 份歷史檔沒有 is_sim 欄，9 份 is_sim=true 但無 .qa.json
⇒ ★★訂正我自己：我派工那封寫「七份 stale 的紅」，★★★實際是 145 份
   —— 我把剛發現的那一小塊當成了全部
```

**裁定：一份都不動**（不補 is_sim／不挪 `_archive/`／不加日期式 grandfather）

```
①沒有消費者：is_sim 的唯一用途是「sim 量測 ⇒ 要有 QA 故事稽核才准 merge」，
  而這 145 份的 slice 全部早就 merge ⇒ 補了不會有人去消費
②★★補完【紅燈數量一模一樣】：從「缺 is_sim」變成「缺 qa.json」
  —— 量測員那七份正好示範了這一點（修完仍在 FAIL 清單，只是換了層級）
③挪 _archive/ 不做：實查那九份各被 2～6 份文件用路徑引用
  ⇒ ★用搬家安撫一個【掃描範圍】的問題
```
...
```
verification_gate.gd:41-47 把 active 定義成「不在 _archive/」⇒ 而 verdicts/ 只會長大
⇒ ★「active」實際上等於【全歷史】
⇒ ★★「在飛」的定義【已經存在】：scripts/hooks/pre-push:45-48 的 branch-scoped 那幾行
⇒ ★★★全量模式不是「比較嚴格的 branch-scoped」，它在問另一個問題
   （「repo 裡每一份歷史卷面合不合今天的規矩」）—— 而那個問題【沒有人需要答案】
```

落地：`pre-push:50-58` 註解換成數字（免得下一個人重新發現並重新低估）；
...
### 2026-09-23（夜）：玩家指令佇列化 —— 一張差點靜靜消失的票，現在有家了

```
★它原本只活在一份【已被 blueprint 永久關閉】的 spec 的 §1 裡（分片＋邊界快照）
⇒ 門票死了、需求還活著、附在屍體上：不在 known_issues、不在 defers、也不是自己的 spec
⇒ ★★沒有任何機制會提醒任何人它還開著 —— 我是查別的東西時【偶然】翻到的
⇒ ★★★而舊理由（讓分片有前提）已隨凍結被修掉而消失；blueprint 給的新理由是【重播可重現】
```

**目標（blueprint 定，逐字）**：同種子 ＋ 同一串玩家指令（每條帶被套用的 tick 編號）⇒ fp 逐字相同。
不是 tick 內順序、不是存檔／載入、不是多人。三個消費者：重現用戶的種子／把玩家路徑納進 fp 床／bug 回報附指令串。

```
★R①（premise_ok）坐實不決定性是【真實的】：
  tick_step 一次吃 min(60, remaining) 且【無 delta 縮放】⇒ 幀率決定單位時間吃幾個 tick
  ★★而 reviewer 多給的那一格才是關鍵：`_input()` 【完全不看 is_advancing()】
     ⇒ 推進一天 ＝ 1440 tick ≥24 幀的窗口，玩家在批次進行中照樣能下指令
     ⇒ ★★★這不是邊界上的理論競態，是一個【有真實長度的時間窗】
★入口實測是三個不是一個：command_player 38／set_player_input 26 次（24 個 key）／
  refresh_interaction_targets 3 —— ★而舊 spec 的「14 處測試改寫」我【沒有重現】
  ⇒ spec 明文不引用那個數（數字會變，spec 不該藏會變的數）
```

**★★R② 抓到一件我沒問的**：`player_state` 有**第三個寫入者** ——
`sim_runner.gd:458` 每 tick 若玩家格子變了就 `erase("pending_trade_target")`。
今天無害（指令立即生效），**佇列化之後會開出新窗口**（tick 開頭寫、同 tick 稍後被清）。

```
★處置：不改那個清除鉤子（清掉是對的：不能跟剛走開的人交易）
★★改的是「它今天正確、而且是順序決定的，卻沒有人在看它」⇒ 釘成守衛 P8
★★★母體要求寫進判準：那條 move 必須真的讓格子改變，否則鉤子不觸發 ⇒ 恆綠
```

其餘兩條：**消費點釘死在 `_step1_advance_time()` 之後**（差一格、而且兩種選法各自內部一致
⇒ 錯了不會馬上紅）；**遭遇戰分支也消費**（主理由不是技術：一條沒有例外的規則比兩條好，
而不消費會讓佇列累積到結束那刻一次全生效＝被動產生的行為）；**負對照從 ±1 改成跨小時／日邊界**
（那些邊界上本來就有函式在動東西 ⇒ 母體選擇本身帶保證）。

**狀態**：spec `2026-09-23-player-command-queue-HOW.md`（239 行），R① premise_ok／R② CLEAN（正式確認）。
★排在票A／票B 之後；★★已告知實作端**不要為它預先改任何東西**（預留設計的白工最貴）。

### 2026-09-23（深夜）：UI 五分頁 —— 票A 與三格已 merge，而這一輪真正的產出是【守衛的病歷】

```
✅ 票A（框）merge 01b5b4128：五分頁／常駐狀態列／具名天窗／「未分類（N 行）」區塊
✅ 三支恆真格 merge ed128a1e7：武裝數／能力讀數／狀態列 —— 改成【值】而非【字串在不在】
⏳ 票B：第 1 批（資源 → 經濟頁）、第 2 批（生存頁四題 ＋ focused_member）在分支上未驗
⏳ fix/ui-flow-determinism：R② issues 兩條派回中（★在它併進去之前，ui-flow 的綠都可能是運氣）
⏳ 玩家指令佇列化：spec R² CLEAN，排隊（defers token: command-queue-dispatch-after-ui）
```

**★★★這一天在 UI 這條線上抓到的守衛缺陷，全部是同一族的不同長相**

```
①恆真（斷言依附在一段一定會出現的文字上）
   P1-a「頁首之後非空白」被【頁尾的 Tick·Day】撐著 ⇒ 整頁內容拿光仍綠 ⇒ 窄化到【分頁區】
   _test_armed_count_shown／_test_capabilities_shown ⇒ 印出條件在 code 層面無 if（或 if 恆假）
②到期（判準綁在【下一張票要改變的那個量】上）
   P4「天窗數 0 ⇒ 紅」vs 票B 的目標正是歸零   ★我看出來了
   P1-b「舊行必須在【第 1 頁】」vs 票B 就是要把行搬離第 1 頁  ★★我沒看出來，而那條是我寫的
③母體邊界留在原地
   天窗那一格的迴圈 range(1,…) 跳過第 1 頁，而票B 剛給生存頁加了四個宣告欄位
④值比較的恆真角落
   整段被刪 ⇒ 兩邊取值都撈不到 ⇒ -1 == -1 意外相等（★implementer 預判並用注射逼出來）
⑤★★★隨機紅：ui_flow_test 與 ui_state_str_capture 都沒播全域 RNG
   ⇒ 那份「前」是浮動的 ⇒ P1-b 隨機紅（實測 6 跑 2 紅）
   ⇒ ★而【隨機紅比恆綠更糟】：它會被當成雜訊，然後整支註冊表上的閘被降級
```

**★兩件我要記在自己帳上的**

```
①我點名了 P4 的到期日，卻沒看出 P1-b 是同胞 —— ★我檢查了別人的格，沒檢查自己的
②我在裁定裡給了一句【成品字串】「未分類（3 行，皆為跨頁混合：…）」而沒說那個結論要【算出來】
   ⇒ implementer 照著寫 ⇒ 無條件印 ⇒ ★★他自己抓到
   ⇒ ★★★我今天才剛把「預寫判讀語會對真假一視同仁地加持說服力」寫進 02_reviewer.md，
     而我用的方式更直接：我把那句話放進他手裡
③而我在 R² 信裡逐字要求過的陽性對照【沒有被跑】——★不是誰忘了，是【信不是執行單位】
```

**★★流程上落地的（都不是新閘）**

```
02_reviewer.md 新一節：守衛「永遠給同一個答案」的五種長相 ＋ 兩條不靠跑的抓法
  （①讀印出條件有沒有 if ②問它要能紅在哪；★修壞法＝直接刪子句 ⇒ 反方向的空真，正解是【窄化】）
01_architect.md 新一節：預註冊實驗要【同句指定跑的人】
  ★目的不是省機器時間，是讓「誰的數字算數」在看到數字之前就定下來
merge-gates.sh：紅的時候把該支【完整輸出】落檔並印路徑（.gate-fail/，已 gitignore）
  ★血證：ui-flow 那一輪的格名我以為永遠查不到了 —— 而它只是【沒被我這邊存下來】
```

### 2026-09-23（深夜二）：ui-flow 隨機紅 → 根因是【Godot 每行程隨機播種】，而它牽出一張新票

```
症狀：ui-flow 原碼實測 9 跑紅 4（~44%），紅的永遠是 _test_pages_zero_loss 的同一行
根因：★Godot 每個行程開機時全域 RNG 是隨機的（四個未 seed 行程 randf 0.336/0.970/0.761/0.207）
      ⇒ 沒播種的床，每一次執行都是另一條 RNG 流
⇒ ★★而「那份【前】快照」是浮動的 ⇒ 零損失那一格拿浮動基準去比 ⇒ 隨機紅
⇒ ★★★隨機紅【比恆綠更糟】：它會被當成雜訊，然後整支註冊表上的閘被降級
```

**★這一段路上死掉的假說（全部有數字，不是改主意）**

```
①「3 綠 ⇒ 環境」（我）        ⇒ 我自己撤回：5% 失敗率的格三次全綠機率 86%
②「seed 有效 ⇒ 修好了」（他） ⇒ 他自己的陽性對照當場否證（兩顆種子輸出逐字相同）
③「跨格殘留」（他）           ⇒ 單獨跑那一格照樣紅 ⇒ 否證
④「序列殘留讓前 30 格變確定」（我）⇒ 同上 ⇒ 否證
⑤「推進路徑不同」（他）       ⇒ 路徑儀器：紅的那次與綠的逐字相同（frames=2 [60,60]）⇒ 否證
⑥「時序」（他）               ⇒ 純算數忙等 ⇒ 照樣紅 ⇒ 否證
★★★而其中兩次的注射【根本沒動到世界】（目標 1560、實際 120、frames 燒光）
   —— 而那是他早上才踩過的同一個坑（request_advance 漏參數）
```

**★★結構理由（R² 在另一張票上挖出來，我獨立核過）**

```
GameSetup 自己造一個【局部】RandomNumberGenerator（rng.seed = config.seed = 42）
world-gen 全部吃它：game_setup bare=0／world_generator bare=0／person_generator bare=0
⇒ ★建世界【結構上就是決定性的】；腳本的 seed() 只餵 tick 推進時那 72 處 bare randf/randi
⇒ ★★兩條 RNG 流不相交
```

**新票 `unseeded-gate-beds`（R² CLEAN，已排隊、已登掛鉤）**

```
★而它最值錢的不是名單，是【判準被戳破三次】的紀錄：
  v1「有沒有 seed(」⇒ 漏走 helper 建世界的｜v2「建世界 或 推進」⇒ 誤算靜態 gate
  v3「只看推進」⇒ 漏掉不建世界不推進但【直接呼叫模擬函式】的床
⇒ ★★三次都不是名單漏一支，是【產生名單的那個問題問錯了】
⇒ ★★★所以判準改兩層：靜態只用來【縮小】，經驗層（5 跑逐位元）用來【兜底】
   理由：靜態追查是移動標靶（有人日後在 510 行底下加一個 randf，不會有任何東西叫紅）
```

★**註冊表 `ui-flow` 那一列已標**：「這支閘的綠燈每一次都可能是運氣…在解釋出來之前，
不得拿它的綠當任何一張票的通過依據」——★★那一行會被每一個跑電池的人看到，而不是躺在信裡。

## 2026-09-23（傍晚）｜信箱換代：**輪詢全退、改敲門**，而★今天兩個守衛在說謊、兩個判準是我自己的

### 一、信箱第二版（用戶裁）—— `9c80e2b81`

```
收信：什麼都不用掛。別人寫完 handback 用 SendMessage 敲你 ⇒ 你就醒了。
寄信：①Write handback ②★立刻敲收件人。★★沒敲＝沒送到（git 信箱現在沒有人在掃）。
看門狗：★留 —— 它偵測的是「沒有事發生」，而那件事沒有人會來敲你 ⇒ 輪詢是它唯一可能的形狀。
```

**★★★三代 watcher 的共同病，今天才叫得出名字**：

| 代 | 病 |
|---|---|
| Monitor | 沒有 `persistent`、30 分鐘硬到期 ⇒ 每半小時一次【空重掛】 |
| 背景 Bash `role-watch` | 閒置真的安靜，★**但它印完那一行就結束** ⇒ 每收一封信重掛一次（~20 次/小時）|
| ★敲門 | 零 watcher、零重掛、閒置零 token |

**前兩代都是「我去看有沒有人找我」，第三代是「找我的人直接叫我」**
—— 輪詢的成本隨【角色數 × 小時數】長，推播的成本隨【真實信件數】長。
★**而我一開始沒把這件事講清楚**：我只報了「不會產生空 turn」，**沒報每事件重掛一次**，
是用戶從 blueprint 的分析裡看出來的 ⇒ ★★**比較兩個方案時，成本要逐項對齊列，不能只列我贏的那一項。**

### ★★二、新機制當場開了一個洞，而它不是 bug 是【缺件】

git 信箱用**角色**定址、SendMessage 用 **session 名**定址，★中間沒有東西把兩者接起來
⇒ 我手上有角色名，卻不知道要敲誰。★★而 session 名**只有本人看得到**
（`ListAgents` 在自己那邊印「This session is demo-XX」，在別人那邊不印角色）
⇒ ★★★**這張表只能各自登記，沒有人能代填（代填＝猜）** ⇒ 立 `whoami.sh`，六角色當天全部登記完。

### ★★★三、換掉一個機制，要去找【誰在讀它的輸出】

watcher 退役那一刻，**三個消費者同時變成恆定答案，而它們看起來都還在正常運作**：

```
peers.sh 的 ALIVE        ⇒ 六個角色永遠沒有心跳 ⇒ 全員讀成「沒開」
watchdog 的 alive_roles  ⇒ ★blueprint 17:09 收到假警報「systems 沒開」，而 systems 17:00 才回過它的信
handback-inbox 的「⛔ 信箱沒在聽」⇒ 每一輪都叫（★★而恆真的警告會讓人把整段訊息跳過去，連未讀清單一起）
```

判準改成看【終端本身】（`tasklist` 查 claude 進程）。**對照**：新判準列出六個角色，舊判準列出**零**個。

### ★★★★四、一個躺很久的洞，而註解替它編好了理由

```
peers.sh 的 lock 有四欄（wpid sid cpid proto=2），而它只 read 三個
⇒ 第四欄黏在 cpid 後面 ⇒ tasklist 查 "24004	proto=2" 永遠查無
⇒ ★`NO-WATCH`（終端開著但 watcher 掉了）這一格**從來沒有 fire 過一次**
★★而檔頭註解寫著：「舊格式 lock 沒有 claude_pid ⇒ NO-WATCH 永遠不會 fire，這是保守正確的」
⇒ ★★★那個解釋**是錯的**（格式早就有 cpid），而它讓一個真 bug 看起來像【已知限制】
```

★通則：一條分支從沒亮過時，「為什麼它不亮」的解釋**必須是量出來的，不是寫出來的** ——
★★一段講得通的理由會讓人停止追問，而**停止追問正是 bug 需要的全部條件**。

### 五、backlog 清空（★五張票全部派完，沒有票只活在我腦袋裡）

```
順位：票B 全電池（跑中）→ 未加種子的閘床 → 查詢面補「家」→ render 不得寫 state → 玩家指令佇列化
```

- **查詢面補「家」**：blueprint 裁語意＝**本城**（意圖帳 #55⑤ 2026-09-10 已裁，不是新語意）；
  本城欄未落地前用索引取值，★而畫面「（共 N 處）」旁要標【暫代】——**那三個字已寫進 P4 判準**，
  ★★它是分流器：沒有它，讀的人分不出【這就是家】與【這只是先湊一個】，
  ★★★而「先湊一個」如果看起來像定論，就沒有人會回來把它換掉。
- **render 不得寫 state**：★reviewer 抓到我原判準的漏洞 —— P1／P2 **都直呼 `_build_state_str()`、
  繞過 `_refresh()`** ⇒ 把寫入【搬進 `_refresh()`】就能兩格全綠而病還在
  ⇒ 補 P1b（直接呼 `node._refresh()` 兩次）。★★通則：**判準要打在【使用者真的會走的那條路】上，
  不是打在我方便呼叫的那個函式上。** ★P2 從具名字串改綁 `P1B_EXCLUDE.is_empty()`（票B 未 merge，綁名字會漂）。
- **未加種子的閘床**：★**我在派工信裡訂正了我自己上一封的排序** —— 我寫過「應在票B 驗收之前做完」，
  而票B 電池已在跑 ⇒ 我**不**拿它擋票B（那三張床不是票B 造成的，先前每次 merge 用的都是同一組電池，
  拿它擋票B＝對票B 用一把新的尺），改列為票B 之後第一順位。★★**下游有權知道我上一封那句話已經不作數。**

### 六、尚未做

- `CLAUDE.md` 的信箱那一段**還沒改** —— ★CLAUDE.md 的修改權只認用戶直接授權，**不認轉述**（免授權洗白）。
- 多出一個 session `demo-a1`（六角色已全部登記完且都不是它）—— 已去信問，未回。

## 2026-09-23（夜）→ 09-24｜三張票 merged，而★這一段真正的產物是【第一次跑換到的東西】與一條共通的病

### 一、落地（逐張，每張都是全電池綠才進）

```
96df1dbc4  票3 查詢面補「家」      77/77｜BATTERY_RC=0
a299dd296  票4 render 不得寫 state 77/77｜BATTERY_RC=0
04a9255c8  票5 玩家指令佇列化      78/78｜BATTERY_RC=0
52d9074c1  兩支休眠守衛接電（78→80）★而 main 隨後 80/80 全綠，兩支【真的在卷面上】
```

### ★★★二、第一次跑換到的東西（這就是「凍結＋不盲改」的全部理由）

```
①真死鎖：game_over／awaiting_heir 在 _advance_tick_body 之前 return，而消費點在它裡面
  ⇒ 等待繼承人時世界凍結，★而【唯一能解凍的那條指令】也在佇列裡 ⇒ 永久死鎖
  ⇒ ★★修法不只解鎖：凍結世界裡的每一條指令本來都是【靜默丟棄】，
    而「拒絕禁靜默」沒有寫「凍結時除外」—— **是 spec 把邊界畫在了它管不到的地方**
  ⇒ ★★★通則：**列舉「有幾條路」之前，要先問「這個函式【會不會被呼叫】」**
    —— 我把邊界畫在函式【內】，而真正的控制流在函式【外】就分岔了
②漏了這個專案既有的守衛：`state.teams.has()` 對【待刪除的殭屍隊】也是 true
③一句我的裁定被漏做（入列端擋未知 name）｜④三個守衛自己的缺陷（全由母體地板或對照抓出）
⑤兩個環境事實：`chr()` 是 Godot 3 的；tiles 的鍵是 q*1000+r
```

### ★★★三、一整天只有一條病，而它有很多長相

**「先有了一個解釋，然後只在輸出裡找支持它的部分。」**（implementer 歸納，比我的準）

```
・預寫結論三次（★其中一次【證據就在同一段輸出的上面兩行】）
・靜態掃描器三個獨立 bug【全部往同一方向錯】⇒ 那不是量測，是偏誤：它只會說「沒有問題」
・我宣告「這件事沒有機制」—— 而機制正在 hook 裡執行（同日兩次）
・★★我把閘印出來的正確診斷【越過去】：它說「檔案不存在（路徑錯或樹錯）」，
  而我差一點去修一支沒有壞的閘 —— ★★★正確答案就在眼前，被一個更合理的故事蓋過去
・★一個「沒想到還有別的做法」被包裝成【一條原則】（「只能改產品才驗得到」）
  ⇒ ★★危險在於：**原則不會被重新檢查。**
```

**★解藥只有一個動作，而它有效的理由要寫清楚**：

```
**把變因關掉再量一次。**
★為什麼「再讀一次輸出」沒有用：**再讀一次是同一個解釋器（我）在跑**；
★★而關掉變因之後【世界自己回答，它不知道我的故事是什麼】
⇒ 今天做對的三次都是這個動作：把 PQ 行註解掉重跑／在 main 上開臨時 worktree 跑同一支對照／
  把棘輪上限降到 29 看它會不會紅
```

### ★★四、判準層面的四條收穫（都從真實失敗長出來）

```
①**數字對 ≠ 東西在**（第三次）：三張票的註冊表【都是 77 列而集合不同】
  ⇒ 我立的「merge 後行數 ≥ 兩邊」會被 77 滿足**而那時已經少了一支閘**
  ⇒ ★判準改【指名】不是【數數】；★★而它的偽裝比算錯更好：**兩邊的數字本來就相等**
②**基準值與造成它的改動必須【原子】一起落地**：把新 fp 基準先放 main（沒有 PQ 行）
  ⇒ **等於讓一個守衛先去要求一件世界還沒做到的事**
③**一格好的守衛，在它紅的時候要讓人分得出「產品錯了」還是「我問錯了」**
  ⇒ 做法：**斷言失敗時要連【它看到的東西】一起印**
④**比較的兩邊必須能各自獨立地改變**（第七種恆真）：我用「它們是同一個常數所以不會漂開」
  當安慰去要一行斷言 —— ★而那句話**逐字就是它恆真的證明**
```

### 五、收尾狀態

```
★兩支休眠守衛接電完成（`fp-excludes-derived` 改成【棘輪】：盲區只准變小；`player-fp-sentinel`）
  ⇒ ★★而 `player-fp-sentinel` 接電前那一輪的意義比「綠」大：**它完全不知道票5 存在**，
    卻報出「排除清單裡不再有任何 player_*」⇒ 票5 那四個欄位真的進了 fp
    —— **不是靠實作端自己說，是一支第三方的床說的**（「全量暫態可觀測性」第一次被第三方驗到）
待辦（已登 defers）：`world-fp-ctrl` 改成相對斷言（控制組本來就該是相對的）／
  `.construction-duration-source-gate.txt` 是產生物卻被 git 追蹤（★危害不是髒，是噪音蓋住真的髒）
待 blueprint：打聽的【指令層】今天在 code 裡沒有落點（P13b 量出來的，已寄意圖帳）
```

---

## ★★★六、玩家可觸面這一輪（票B → #4）＋ 打聽 v1 的鏈（2026-09-24／25，systems 記）

### (甲) 已 merge 且玩家看得到的

```
票B 查詢面補「家」（家在哪／離家多遠，多據點標【暫代】）
render-no-write（畫面路徑只讀不寫）＋ 之後的 stale-render 修
指令佇列（入列回音／消費點回結果句／頁腳常駐「待執行 N 道」）
招募「收了錢什麼都沒做」三重缺陷修（proxy 判、未讀的回傳、無條件成功句）
游標懸停真值 ＋ X 鍵 ＋ 待辦去重
★玩家事件流接世界事件匯流排（#4）—— ★★**而他隊那一半刻意沒放行**
```

### ★★(乙) #4 沒放行的那一半（這是**玩家看得到的差別**，不是內部細節）

```
原過濾器第②條走 `BeliefSystem.has_belief()`，而它答的是
「我對那支隊有沒有【任何】claim」⇒ **不是「我知不知道【這件事】發生了」**
⇒ 一筆 30 天前的舊情報，會讓玩家【即時】看到那支隊今天的領袖死訊。
★★★god-view 從一扇 belief 形狀的門漏出來，而漏的形狀最難發現：**它有 belief 撐著**。
⇒ 玩家現在有的：自家隊 ＋ 同勢力自家人通道（★而後者今天休眠：預設開局 `faction_id = -1`）。
⇒ 解除條件登 `release-other-team-events-in-the-feed`（staleness gate 或 per-event 感知）。
```

### (丙) 打聽 v1：鏈走完了，在 `feat/inquiry-v1`

```
blueprint 裁（情報必進 belief／代價＝對方同意）→ HOW spec → R② CLEAN → 派工。
★整票的形狀：**不新造秤、不另開表** ——
  「對方同不同意」＝ 既有 `_decide_exchange_mode` 回不回 `"silent"`（它吃的正是被問方對問話方的評價）
  情報寫入 ＝ 既有 `_exchange_intel` → `record_claim`（**整票只准一個寫入點**）
★★而今天的現況是【兩半都沒有】：`resolve_inquiry` 全程零寫入，UI 只印「情報獲取」四個字。
```

### ★★★(丁) 這一輪的三條教訓（都是我自己犯的）

```
①★**commit 訊息先寫、後 staged ⇒ 訊息與內容不符**：一顆說「81 封 status flip」而落地 424 檔
  （目錄 pathspec 把未追蹤檔一起掃進去），更正那顆說「12 封」而刪了 424 檔。兩顆都已 push。
  ⇒ 紀律：**訊息寫在 staged 之後**，並拿 `git diff --cached --numstat | wc -l` 對一次檔數。
②★★**我裁了一個我沒問過推導的數**（「13 處該刪」）⇒ 其中 7 處是回音印點，刪了會踩我自己立的地板。
③★★★**整塊新守衛被插進 `printf` 的格式字串裡**（那支的格式字串嵌了真換行 ⇒ 檔案上是兩行）
  ⇒ `bash -n` 綠、語法合法、**它是死的**；症狀是「本該幾秒回的自檢跑了 120 秒」。
  ⇒ 通則：**插行式編輯的錨必須是一個完整的語法單位**，而 shell 裡最常見的不完整單位
    就是**嵌真換行的引號字串**。
```
...
```
`merge-gates.sh` 落 `docs/measurements/.battery/<run-id>.txt`：
  run-id／開跑與結束 HEAD／範圍／基線對照／BATTERY_RC ＋ 逐格 id+秒+結果 ＋ carry 段。
★動機是一個具體缺口：runner **刻意不 dump 通過那幾支的正文**
  ⇒ `ui_flow_test.gd` P19「請把負對照地板抬到現值」只在【通過】時印 ⇒ 以前寄不到任何人手上。
★★carry 今天的母體是空的（地板正好等於現值 15／2／7）⇒ 恆空母體與沒接電長得一樣
  ⇒ 配 `--selfcheck`（含負對照：普通正文不得被搬進來）。
