...
- **🏛 沙盒 bar arc（(a) 崛起/經濟底）— 連串 measure→fix**：commander-v2 後戰國 seed 揭 default 龜縮（CONQUER=0/established 卡1）。measure-first 逐層挖（別猜）：能人 pop 崩=**飢餓非戰敗** → ①**戰鬥不決勝**(0 擊潰，撤退先於殲滅，吸收掛 `_end_combat` never fire) ②**食物模型沒統一**(成長 surplus gate 讀私產 silo→糧倉/交易糧餵不到成長→非 plains 注定餬口)。
  ...
  - **✅ (a) 機制鏈完成（三源全活）**：累積（食物統一讀A + 失能-capture 吸收同化）+ founding（獨立戰略層）+ 征服（commander-v2 + P1）。**⚠ 活世界 established 1→多/蓄意征服 volume 未在混亂 warring seed 顯現**（attrition 熔爐+噪音 confound）；機制 unit/deterministic 證，大規模 emergence = 平衡層（attrition/founding rate tune），非機制缺。待藍圖裁 (a) 收尾驗收（機制就位 vs 活世界 emergence）。handback `indep-layer-done-a-mechanism-complete`。
  - **⏭ 下一經濟 arc（讀 B）**：覓食=苟活地板（產出 capped 餵不起繁榮）、繁榮須交易（forest 賣木買糧→買糧自然 fire）。非 nerf 補丁，接「覓食限 pop≤15」延伸。序 (a) 鏈後/平行。
  - **平行不阻塞**：G3 Phase E（信息域 keystone）、rung2→3（攀爬第二段獨立）、bed 變體（econ_bed.json）。3+1 對稱不變量骨架（決策✓/信息 G3/所有權 Pattern B/凡位置必有上位路徑）已納 invariants。
